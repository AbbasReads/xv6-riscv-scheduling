
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
_entry:
        # set up a stack for C.
        # stack0 is declared in start.c,
        # with a 4096-byte stack per CPU.
        # sp = stack0 + ((hartid + 1) * 4096)
        la sp, stack0
    80000000:	00008117          	auipc	sp,0x8
    80000004:	86010113          	addi	sp,sp,-1952 # 80007860 <stack0>
        li a0, 1024*4
    80000008:	6505                	lui	a0,0x1
        csrr a1, mhartid
    8000000a:	f14025f3          	csrr	a1,mhartid
        addi a1, a1, 1
    8000000e:	0585                	addi	a1,a1,1
        mul a0, a0, a1
    80000010:	02b50533          	mul	a0,a0,a1
        add sp, sp, a0
    80000014:	912a                	add	sp,sp,a0
        # jump to start() in start.c
        call start
    80000016:	04a000ef          	jal	80000060 <start>

000000008000001a <spin>:
spin:
        j spin
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
}

// ask each hart to generate timer interrupts.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
#define MIE_STIE (1L << 5)  // supervisor timer
static inline uint64
r_mie()
{
  uint64 x;
  asm volatile("csrr %0, mie" : "=r" (x) );
    80000022:	304027f3          	csrr	a5,mie
  // enable supervisor-mode timer interrupts.
  w_mie(r_mie() | MIE_STIE);
    80000026:	0207e793          	ori	a5,a5,32
}

static inline void 
w_mie(uint64 x)
{
  asm volatile("csrw mie, %0" : : "r" (x));
    8000002a:	30479073          	csrw	mie,a5
static inline uint64
r_menvcfg()
{
  uint64 x;
  // asm volatile("csrr %0, menvcfg" : "=r" (x) );
  asm volatile("csrr %0, 0x30a" : "=r" (x) );
    8000002e:	30a027f3          	csrr	a5,0x30a
  
  // enable the sstc extension (i.e. stimecmp).
  w_menvcfg(r_menvcfg() | (1L << 63)); 
    80000032:	577d                	li	a4,-1
    80000034:	177e                	slli	a4,a4,0x3f
    80000036:	8fd9                	or	a5,a5,a4

static inline void 
w_menvcfg(uint64 x)
{
  // asm volatile("csrw menvcfg, %0" : : "r" (x));
  asm volatile("csrw 0x30a, %0" : : "r" (x));
    80000038:	30a79073          	csrw	0x30a,a5

static inline uint64
r_mcounteren()
{
  uint64 x;
  asm volatile("csrr %0, mcounteren" : "=r" (x) );
    8000003c:	306027f3          	csrr	a5,mcounteren
  
  // allow supervisor to use stimecmp and time.
  w_mcounteren(r_mcounteren() | 2);
    80000040:	0027e793          	ori	a5,a5,2
  asm volatile("csrw mcounteren, %0" : : "r" (x));
    80000044:	30679073          	csrw	mcounteren,a5
// machine-mode cycle counter
static inline uint64
r_time()
{
  uint64 x;
  asm volatile("csrr %0, time" : "=r" (x) );
    80000048:	c01027f3          	rdtime	a5
  
  // ask for the very first timer interrupt.
  w_stimecmp(r_time() + 1000000);
    8000004c:	000f4737          	lui	a4,0xf4
    80000050:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    80000054:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    80000056:	14d79073          	csrw	stimecmp,a5
}
    8000005a:	6422                	ld	s0,8(sp)
    8000005c:	0141                	addi	sp,sp,16
    8000005e:	8082                	ret

0000000080000060 <start>:
{
    80000060:	1141                	addi	sp,sp,-16
    80000062:	e406                	sd	ra,8(sp)
    80000064:	e022                	sd	s0,0(sp)
    80000066:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000068:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000006c:	7779                	lui	a4,0xffffe
    8000006e:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffdd897>
    80000072:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    80000074:	6705                	lui	a4,0x1
    80000076:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    8000007a:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    8000007c:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    80000080:	00001797          	auipc	a5,0x1
    80000084:	dbc78793          	addi	a5,a5,-580 # 80000e3c <main>
    80000088:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    8000008c:	4781                	li	a5,0
    8000008e:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    80000092:	67c1                	lui	a5,0x10
    80000094:	17fd                	addi	a5,a5,-1 # ffff <_entry-0x7fff0001>
    80000096:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    8000009a:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    8000009e:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE);
    800000a2:	2207e793          	ori	a5,a5,544
  asm volatile("csrw sie, %0" : : "r" (x));
    800000a6:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000aa:	57fd                	li	a5,-1
    800000ac:	83a9                	srli	a5,a5,0xa
    800000ae:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000b2:	47bd                	li	a5,15
    800000b4:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000b8:	f65ff0ef          	jal	8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000bc:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000c0:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000c2:	823e                	mv	tp,a5
  asm volatile("mret");
    800000c4:	30200073          	mret
}
    800000c8:	60a2                	ld	ra,8(sp)
    800000ca:	6402                	ld	s0,0(sp)
    800000cc:	0141                	addi	sp,sp,16
    800000ce:	8082                	ret

00000000800000d0 <consolewrite>:
// user write() system calls to the console go here.
// uses sleep() and UART interrupts.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    800000d0:	7119                	addi	sp,sp,-128
    800000d2:	fc86                	sd	ra,120(sp)
    800000d4:	f8a2                	sd	s0,112(sp)
    800000d6:	f4a6                	sd	s1,104(sp)
    800000d8:	0100                	addi	s0,sp,128
  char buf[32]; // move batches from user space to uart.
  int i = 0;

  while(i < n){
    800000da:	06c05a63          	blez	a2,8000014e <consolewrite+0x7e>
    800000de:	f0ca                	sd	s2,96(sp)
    800000e0:	ecce                	sd	s3,88(sp)
    800000e2:	e8d2                	sd	s4,80(sp)
    800000e4:	e4d6                	sd	s5,72(sp)
    800000e6:	e0da                	sd	s6,64(sp)
    800000e8:	fc5e                	sd	s7,56(sp)
    800000ea:	f862                	sd	s8,48(sp)
    800000ec:	f466                	sd	s9,40(sp)
    800000ee:	8aaa                	mv	s5,a0
    800000f0:	8b2e                	mv	s6,a1
    800000f2:	8a32                	mv	s4,a2
  int i = 0;
    800000f4:	4481                	li	s1,0
    int nn = sizeof(buf);
    if(nn > n - i)
    800000f6:	02000c13          	li	s8,32
    800000fa:	02000c93          	li	s9,32
      nn = n - i;
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    800000fe:	5bfd                	li	s7,-1
    80000100:	a035                	j	8000012c <consolewrite+0x5c>
    if(nn > n - i)
    80000102:	0009099b          	sext.w	s3,s2
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    80000106:	86ce                	mv	a3,s3
    80000108:	01648633          	add	a2,s1,s6
    8000010c:	85d6                	mv	a1,s5
    8000010e:	f8040513          	addi	a0,s0,-128
    80000112:	178020ef          	jal	8000228a <either_copyin>
    80000116:	03750e63          	beq	a0,s7,80000152 <consolewrite+0x82>
      break;
    uartwrite(buf, nn);
    8000011a:	85ce                	mv	a1,s3
    8000011c:	f8040513          	addi	a0,s0,-128
    80000120:	778000ef          	jal	80000898 <uartwrite>
    i += nn;
    80000124:	009904bb          	addw	s1,s2,s1
  while(i < n){
    80000128:	0144da63          	bge	s1,s4,8000013c <consolewrite+0x6c>
    if(nn > n - i)
    8000012c:	409a093b          	subw	s2,s4,s1
    80000130:	0009079b          	sext.w	a5,s2
    80000134:	fcfc57e3          	bge	s8,a5,80000102 <consolewrite+0x32>
    80000138:	8966                	mv	s2,s9
    8000013a:	b7e1                	j	80000102 <consolewrite+0x32>
    8000013c:	7906                	ld	s2,96(sp)
    8000013e:	69e6                	ld	s3,88(sp)
    80000140:	6a46                	ld	s4,80(sp)
    80000142:	6aa6                	ld	s5,72(sp)
    80000144:	6b06                	ld	s6,64(sp)
    80000146:	7be2                	ld	s7,56(sp)
    80000148:	7c42                	ld	s8,48(sp)
    8000014a:	7ca2                	ld	s9,40(sp)
    8000014c:	a819                	j	80000162 <consolewrite+0x92>
  int i = 0;
    8000014e:	4481                	li	s1,0
    80000150:	a809                	j	80000162 <consolewrite+0x92>
    80000152:	7906                	ld	s2,96(sp)
    80000154:	69e6                	ld	s3,88(sp)
    80000156:	6a46                	ld	s4,80(sp)
    80000158:	6aa6                	ld	s5,72(sp)
    8000015a:	6b06                	ld	s6,64(sp)
    8000015c:	7be2                	ld	s7,56(sp)
    8000015e:	7c42                	ld	s8,48(sp)
    80000160:	7ca2                	ld	s9,40(sp)
  }

  return i;
}
    80000162:	8526                	mv	a0,s1
    80000164:	70e6                	ld	ra,120(sp)
    80000166:	7446                	ld	s0,112(sp)
    80000168:	74a6                	ld	s1,104(sp)
    8000016a:	6109                	addi	sp,sp,128
    8000016c:	8082                	ret

000000008000016e <consoleread>:
// user_dst indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    8000016e:	711d                	addi	sp,sp,-96
    80000170:	ec86                	sd	ra,88(sp)
    80000172:	e8a2                	sd	s0,80(sp)
    80000174:	e4a6                	sd	s1,72(sp)
    80000176:	e0ca                	sd	s2,64(sp)
    80000178:	fc4e                	sd	s3,56(sp)
    8000017a:	f852                	sd	s4,48(sp)
    8000017c:	f456                	sd	s5,40(sp)
    8000017e:	f05a                	sd	s6,32(sp)
    80000180:	1080                	addi	s0,sp,96
    80000182:	8aaa                	mv	s5,a0
    80000184:	8a2e                	mv	s4,a1
    80000186:	89b2                	mv	s3,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b1b          	sext.w	s6,a2
  acquire(&cons.lock);
    8000018c:	0000f517          	auipc	a0,0xf
    80000190:	6d450513          	addi	a0,a0,1748 # 8000f860 <cons>
    80000194:	23b000ef          	jal	80000bce <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    80000198:	0000f497          	auipc	s1,0xf
    8000019c:	6c848493          	addi	s1,s1,1736 # 8000f860 <cons>
      if(killed(myproc())){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a0:	0000f917          	auipc	s2,0xf
    800001a4:	75890913          	addi	s2,s2,1880 # 8000f8f8 <cons+0x98>
  while(n > 0){
    800001a8:	0b305d63          	blez	s3,80000262 <consoleread+0xf4>
    while(cons.r == cons.w){
    800001ac:	0984a783          	lw	a5,152(s1)
    800001b0:	09c4a703          	lw	a4,156(s1)
    800001b4:	0af71263          	bne	a4,a5,80000258 <consoleread+0xea>
      if(killed(myproc())){
    800001b8:	716010ef          	jal	800018ce <myproc>
    800001bc:	761010ef          	jal	8000211c <killed>
    800001c0:	e12d                	bnez	a0,80000222 <consoleread+0xb4>
      sleep(&cons.r, &cons.lock);
    800001c2:	85a6                	mv	a1,s1
    800001c4:	854a                	mv	a0,s2
    800001c6:	51f010ef          	jal	80001ee4 <sleep>
    while(cons.r == cons.w){
    800001ca:	0984a783          	lw	a5,152(s1)
    800001ce:	09c4a703          	lw	a4,156(s1)
    800001d2:	fef703e3          	beq	a4,a5,800001b8 <consoleread+0x4a>
    800001d6:	ec5e                	sd	s7,24(sp)
    }

    c = cons.buf[cons.r++ % INPUT_BUF_SIZE];
    800001d8:	0000f717          	auipc	a4,0xf
    800001dc:	68870713          	addi	a4,a4,1672 # 8000f860 <cons>
    800001e0:	0017869b          	addiw	a3,a5,1
    800001e4:	08d72c23          	sw	a3,152(a4)
    800001e8:	07f7f693          	andi	a3,a5,127
    800001ec:	9736                	add	a4,a4,a3
    800001ee:	01874703          	lbu	a4,24(a4)
    800001f2:	00070b9b          	sext.w	s7,a4

    if(c == C('D')){  // end-of-file
    800001f6:	4691                	li	a3,4
    800001f8:	04db8663          	beq	s7,a3,80000244 <consoleread+0xd6>
      }
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    800001fc:	fae407a3          	sb	a4,-81(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000200:	4685                	li	a3,1
    80000202:	faf40613          	addi	a2,s0,-81
    80000206:	85d2                	mv	a1,s4
    80000208:	8556                	mv	a0,s5
    8000020a:	036020ef          	jal	80002240 <either_copyout>
    8000020e:	57fd                	li	a5,-1
    80000210:	04f50863          	beq	a0,a5,80000260 <consoleread+0xf2>
      break;

    dst++;
    80000214:	0a05                	addi	s4,s4,1
    --n;
    80000216:	39fd                	addiw	s3,s3,-1

    if(c == '\n'){
    80000218:	47a9                	li	a5,10
    8000021a:	04fb8d63          	beq	s7,a5,80000274 <consoleread+0x106>
    8000021e:	6be2                	ld	s7,24(sp)
    80000220:	b761                	j	800001a8 <consoleread+0x3a>
        release(&cons.lock);
    80000222:	0000f517          	auipc	a0,0xf
    80000226:	63e50513          	addi	a0,a0,1598 # 8000f860 <cons>
    8000022a:	23d000ef          	jal	80000c66 <release>
        return -1;
    8000022e:	557d                	li	a0,-1
    }
  }
  release(&cons.lock);

  return target - n;
}
    80000230:	60e6                	ld	ra,88(sp)
    80000232:	6446                	ld	s0,80(sp)
    80000234:	64a6                	ld	s1,72(sp)
    80000236:	6906                	ld	s2,64(sp)
    80000238:	79e2                	ld	s3,56(sp)
    8000023a:	7a42                	ld	s4,48(sp)
    8000023c:	7aa2                	ld	s5,40(sp)
    8000023e:	7b02                	ld	s6,32(sp)
    80000240:	6125                	addi	sp,sp,96
    80000242:	8082                	ret
      if(n < target){
    80000244:	0009871b          	sext.w	a4,s3
    80000248:	01677a63          	bgeu	a4,s6,8000025c <consoleread+0xee>
        cons.r--;
    8000024c:	0000f717          	auipc	a4,0xf
    80000250:	6af72623          	sw	a5,1708(a4) # 8000f8f8 <cons+0x98>
    80000254:	6be2                	ld	s7,24(sp)
    80000256:	a031                	j	80000262 <consoleread+0xf4>
    80000258:	ec5e                	sd	s7,24(sp)
    8000025a:	bfbd                	j	800001d8 <consoleread+0x6a>
    8000025c:	6be2                	ld	s7,24(sp)
    8000025e:	a011                	j	80000262 <consoleread+0xf4>
    80000260:	6be2                	ld	s7,24(sp)
  release(&cons.lock);
    80000262:	0000f517          	auipc	a0,0xf
    80000266:	5fe50513          	addi	a0,a0,1534 # 8000f860 <cons>
    8000026a:	1fd000ef          	jal	80000c66 <release>
  return target - n;
    8000026e:	413b053b          	subw	a0,s6,s3
    80000272:	bf7d                	j	80000230 <consoleread+0xc2>
    80000274:	6be2                	ld	s7,24(sp)
    80000276:	b7f5                	j	80000262 <consoleread+0xf4>

0000000080000278 <consputc>:
{
    80000278:	1141                	addi	sp,sp,-16
    8000027a:	e406                	sd	ra,8(sp)
    8000027c:	e022                	sd	s0,0(sp)
    8000027e:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000280:	10000793          	li	a5,256
    80000284:	00f50863          	beq	a0,a5,80000294 <consputc+0x1c>
    uartputc_sync(c);
    80000288:	6a4000ef          	jal	8000092c <uartputc_sync>
}
    8000028c:	60a2                	ld	ra,8(sp)
    8000028e:	6402                	ld	s0,0(sp)
    80000290:	0141                	addi	sp,sp,16
    80000292:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    80000294:	4521                	li	a0,8
    80000296:	696000ef          	jal	8000092c <uartputc_sync>
    8000029a:	02000513          	li	a0,32
    8000029e:	68e000ef          	jal	8000092c <uartputc_sync>
    800002a2:	4521                	li	a0,8
    800002a4:	688000ef          	jal	8000092c <uartputc_sync>
    800002a8:	b7d5                	j	8000028c <consputc+0x14>

00000000800002aa <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002aa:	1101                	addi	sp,sp,-32
    800002ac:	ec06                	sd	ra,24(sp)
    800002ae:	e822                	sd	s0,16(sp)
    800002b0:	e426                	sd	s1,8(sp)
    800002b2:	1000                	addi	s0,sp,32
    800002b4:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002b6:	0000f517          	auipc	a0,0xf
    800002ba:	5aa50513          	addi	a0,a0,1450 # 8000f860 <cons>
    800002be:	111000ef          	jal	80000bce <acquire>

  switch(c){
    800002c2:	47d5                	li	a5,21
    800002c4:	08f48f63          	beq	s1,a5,80000362 <consoleintr+0xb8>
    800002c8:	0297c563          	blt	a5,s1,800002f2 <consoleintr+0x48>
    800002cc:	47a1                	li	a5,8
    800002ce:	0ef48463          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    800002d2:	47c1                	li	a5,16
    800002d4:	10f49563          	bne	s1,a5,800003de <consoleintr+0x134>
  case C('P'):  // Print process list.
    procdump();
    800002d8:	7fd010ef          	jal	800022d4 <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002dc:	0000f517          	auipc	a0,0xf
    800002e0:	58450513          	addi	a0,a0,1412 # 8000f860 <cons>
    800002e4:	183000ef          	jal	80000c66 <release>
}
    800002e8:	60e2                	ld	ra,24(sp)
    800002ea:	6442                	ld	s0,16(sp)
    800002ec:	64a2                	ld	s1,8(sp)
    800002ee:	6105                	addi	sp,sp,32
    800002f0:	8082                	ret
  switch(c){
    800002f2:	07f00793          	li	a5,127
    800002f6:	0cf48063          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800002fa:	0000f717          	auipc	a4,0xf
    800002fe:	56670713          	addi	a4,a4,1382 # 8000f860 <cons>
    80000302:	0a072783          	lw	a5,160(a4)
    80000306:	09872703          	lw	a4,152(a4)
    8000030a:	9f99                	subw	a5,a5,a4
    8000030c:	07f00713          	li	a4,127
    80000310:	fcf766e3          	bltu	a4,a5,800002dc <consoleintr+0x32>
      c = (c == '\r') ? '\n' : c;
    80000314:	47b5                	li	a5,13
    80000316:	0cf48763          	beq	s1,a5,800003e4 <consoleintr+0x13a>
      consputc(c);
    8000031a:	8526                	mv	a0,s1
    8000031c:	f5dff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    80000320:	0000f797          	auipc	a5,0xf
    80000324:	54078793          	addi	a5,a5,1344 # 8000f860 <cons>
    80000328:	0a07a683          	lw	a3,160(a5)
    8000032c:	0016871b          	addiw	a4,a3,1
    80000330:	0007061b          	sext.w	a2,a4
    80000334:	0ae7a023          	sw	a4,160(a5)
    80000338:	07f6f693          	andi	a3,a3,127
    8000033c:	97b6                	add	a5,a5,a3
    8000033e:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e-cons.r == INPUT_BUF_SIZE){
    80000342:	47a9                	li	a5,10
    80000344:	0cf48563          	beq	s1,a5,8000040e <consoleintr+0x164>
    80000348:	4791                	li	a5,4
    8000034a:	0cf48263          	beq	s1,a5,8000040e <consoleintr+0x164>
    8000034e:	0000f797          	auipc	a5,0xf
    80000352:	5aa7a783          	lw	a5,1450(a5) # 8000f8f8 <cons+0x98>
    80000356:	9f1d                	subw	a4,a4,a5
    80000358:	08000793          	li	a5,128
    8000035c:	f8f710e3          	bne	a4,a5,800002dc <consoleintr+0x32>
    80000360:	a07d                	j	8000040e <consoleintr+0x164>
    80000362:	e04a                	sd	s2,0(sp)
    while(cons.e != cons.w &&
    80000364:	0000f717          	auipc	a4,0xf
    80000368:	4fc70713          	addi	a4,a4,1276 # 8000f860 <cons>
    8000036c:	0a072783          	lw	a5,160(a4)
    80000370:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000374:	0000f497          	auipc	s1,0xf
    80000378:	4ec48493          	addi	s1,s1,1260 # 8000f860 <cons>
    while(cons.e != cons.w &&
    8000037c:	4929                	li	s2,10
    8000037e:	02f70863          	beq	a4,a5,800003ae <consoleintr+0x104>
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000382:	37fd                	addiw	a5,a5,-1
    80000384:	07f7f713          	andi	a4,a5,127
    80000388:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    8000038a:	01874703          	lbu	a4,24(a4)
    8000038e:	03270263          	beq	a4,s2,800003b2 <consoleintr+0x108>
      cons.e--;
    80000392:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    80000396:	10000513          	li	a0,256
    8000039a:	edfff0ef          	jal	80000278 <consputc>
    while(cons.e != cons.w &&
    8000039e:	0a04a783          	lw	a5,160(s1)
    800003a2:	09c4a703          	lw	a4,156(s1)
    800003a6:	fcf71ee3          	bne	a4,a5,80000382 <consoleintr+0xd8>
    800003aa:	6902                	ld	s2,0(sp)
    800003ac:	bf05                	j	800002dc <consoleintr+0x32>
    800003ae:	6902                	ld	s2,0(sp)
    800003b0:	b735                	j	800002dc <consoleintr+0x32>
    800003b2:	6902                	ld	s2,0(sp)
    800003b4:	b725                	j	800002dc <consoleintr+0x32>
    if(cons.e != cons.w){
    800003b6:	0000f717          	auipc	a4,0xf
    800003ba:	4aa70713          	addi	a4,a4,1194 # 8000f860 <cons>
    800003be:	0a072783          	lw	a5,160(a4)
    800003c2:	09c72703          	lw	a4,156(a4)
    800003c6:	f0f70be3          	beq	a4,a5,800002dc <consoleintr+0x32>
      cons.e--;
    800003ca:	37fd                	addiw	a5,a5,-1
    800003cc:	0000f717          	auipc	a4,0xf
    800003d0:	52f72a23          	sw	a5,1332(a4) # 8000f900 <cons+0xa0>
      consputc(BACKSPACE);
    800003d4:	10000513          	li	a0,256
    800003d8:	ea1ff0ef          	jal	80000278 <consputc>
    800003dc:	b701                	j	800002dc <consoleintr+0x32>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800003de:	ee048fe3          	beqz	s1,800002dc <consoleintr+0x32>
    800003e2:	bf21                	j	800002fa <consoleintr+0x50>
      consputc(c);
    800003e4:	4529                	li	a0,10
    800003e6:	e93ff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    800003ea:	0000f797          	auipc	a5,0xf
    800003ee:	47678793          	addi	a5,a5,1142 # 8000f860 <cons>
    800003f2:	0a07a703          	lw	a4,160(a5)
    800003f6:	0017069b          	addiw	a3,a4,1
    800003fa:	0006861b          	sext.w	a2,a3
    800003fe:	0ad7a023          	sw	a3,160(a5)
    80000402:	07f77713          	andi	a4,a4,127
    80000406:	97ba                	add	a5,a5,a4
    80000408:	4729                	li	a4,10
    8000040a:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    8000040e:	0000f797          	auipc	a5,0xf
    80000412:	4ec7a723          	sw	a2,1262(a5) # 8000f8fc <cons+0x9c>
        wakeup(&cons.r);
    80000416:	0000f517          	auipc	a0,0xf
    8000041a:	4e250513          	addi	a0,a0,1250 # 8000f8f8 <cons+0x98>
    8000041e:	313010ef          	jal	80001f30 <wakeup>
    80000422:	bd6d                	j	800002dc <consoleintr+0x32>

0000000080000424 <consoleinit>:

void
consoleinit(void)
{
    80000424:	1141                	addi	sp,sp,-16
    80000426:	e406                	sd	ra,8(sp)
    80000428:	e022                	sd	s0,0(sp)
    8000042a:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    8000042c:	00007597          	auipc	a1,0x7
    80000430:	bd458593          	addi	a1,a1,-1068 # 80007000 <etext>
    80000434:	0000f517          	auipc	a0,0xf
    80000438:	42c50513          	addi	a0,a0,1068 # 8000f860 <cons>
    8000043c:	712000ef          	jal	80000b4e <initlock>

  uartinit();
    80000440:	400000ef          	jal	80000840 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000444:	00020797          	auipc	a5,0x20
    80000448:	98c78793          	addi	a5,a5,-1652 # 8001fdd0 <devsw>
    8000044c:	00000717          	auipc	a4,0x0
    80000450:	d2270713          	addi	a4,a4,-734 # 8000016e <consoleread>
    80000454:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    80000456:	00000717          	auipc	a4,0x0
    8000045a:	c7a70713          	addi	a4,a4,-902 # 800000d0 <consolewrite>
    8000045e:	ef98                	sd	a4,24(a5)
}
    80000460:	60a2                	ld	ra,8(sp)
    80000462:	6402                	ld	s0,0(sp)
    80000464:	0141                	addi	sp,sp,16
    80000466:	8082                	ret

0000000080000468 <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(long long xx, int base, int sign)
{
    80000468:	7139                	addi	sp,sp,-64
    8000046a:	fc06                	sd	ra,56(sp)
    8000046c:	f822                	sd	s0,48(sp)
    8000046e:	0080                	addi	s0,sp,64
  char buf[20];
  int i;
  unsigned long long x;

  if(sign && (sign = (xx < 0)))
    80000470:	c219                	beqz	a2,80000476 <printint+0xe>
    80000472:	08054063          	bltz	a0,800004f2 <printint+0x8a>
    x = -xx;
  else
    x = xx;
    80000476:	4881                	li	a7,0
    80000478:	fc840693          	addi	a3,s0,-56

  i = 0;
    8000047c:	4781                	li	a5,0
  do {
    buf[i++] = digits[x % base];
    8000047e:	00007617          	auipc	a2,0x7
    80000482:	29260613          	addi	a2,a2,658 # 80007710 <digits>
    80000486:	883e                	mv	a6,a5
    80000488:	2785                	addiw	a5,a5,1
    8000048a:	02b57733          	remu	a4,a0,a1
    8000048e:	9732                	add	a4,a4,a2
    80000490:	00074703          	lbu	a4,0(a4)
    80000494:	00e68023          	sb	a4,0(a3)
  } while((x /= base) != 0);
    80000498:	872a                	mv	a4,a0
    8000049a:	02b55533          	divu	a0,a0,a1
    8000049e:	0685                	addi	a3,a3,1
    800004a0:	feb773e3          	bgeu	a4,a1,80000486 <printint+0x1e>

  if(sign)
    800004a4:	00088a63          	beqz	a7,800004b8 <printint+0x50>
    buf[i++] = '-';
    800004a8:	1781                	addi	a5,a5,-32
    800004aa:	97a2                	add	a5,a5,s0
    800004ac:	02d00713          	li	a4,45
    800004b0:	fee78423          	sb	a4,-24(a5)
    800004b4:	0028079b          	addiw	a5,a6,2

  while(--i >= 0)
    800004b8:	02f05963          	blez	a5,800004ea <printint+0x82>
    800004bc:	f426                	sd	s1,40(sp)
    800004be:	f04a                	sd	s2,32(sp)
    800004c0:	fc840713          	addi	a4,s0,-56
    800004c4:	00f704b3          	add	s1,a4,a5
    800004c8:	fff70913          	addi	s2,a4,-1
    800004cc:	993e                	add	s2,s2,a5
    800004ce:	37fd                	addiw	a5,a5,-1
    800004d0:	1782                	slli	a5,a5,0x20
    800004d2:	9381                	srli	a5,a5,0x20
    800004d4:	40f90933          	sub	s2,s2,a5
    consputc(buf[i]);
    800004d8:	fff4c503          	lbu	a0,-1(s1)
    800004dc:	d9dff0ef          	jal	80000278 <consputc>
  while(--i >= 0)
    800004e0:	14fd                	addi	s1,s1,-1
    800004e2:	ff249be3          	bne	s1,s2,800004d8 <printint+0x70>
    800004e6:	74a2                	ld	s1,40(sp)
    800004e8:	7902                	ld	s2,32(sp)
}
    800004ea:	70e2                	ld	ra,56(sp)
    800004ec:	7442                	ld	s0,48(sp)
    800004ee:	6121                	addi	sp,sp,64
    800004f0:	8082                	ret
    x = -xx;
    800004f2:	40a00533          	neg	a0,a0
  if(sign && (sign = (xx < 0)))
    800004f6:	4885                	li	a7,1
    x = -xx;
    800004f8:	b741                	j	80000478 <printint+0x10>

00000000800004fa <printf>:
}

// Print to the console.
int
printf(char *fmt, ...)
{
    800004fa:	7131                	addi	sp,sp,-192
    800004fc:	fc86                	sd	ra,120(sp)
    800004fe:	f8a2                	sd	s0,112(sp)
    80000500:	e8d2                	sd	s4,80(sp)
    80000502:	0100                	addi	s0,sp,128
    80000504:	8a2a                	mv	s4,a0
    80000506:	e40c                	sd	a1,8(s0)
    80000508:	e810                	sd	a2,16(s0)
    8000050a:	ec14                	sd	a3,24(s0)
    8000050c:	f018                	sd	a4,32(s0)
    8000050e:	f41c                	sd	a5,40(s0)
    80000510:	03043823          	sd	a6,48(s0)
    80000514:	03143c23          	sd	a7,56(s0)
  va_list ap;
  int i, cx, c0, c1, c2;
  char *s;

  if(panicking == 0)
    80000518:	00007797          	auipc	a5,0x7
    8000051c:	31c7a783          	lw	a5,796(a5) # 80007834 <panicking>
    80000520:	c3a1                	beqz	a5,80000560 <printf+0x66>
    acquire(&pr.lock);

  va_start(ap, fmt);
    80000522:	00840793          	addi	a5,s0,8
    80000526:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    8000052a:	000a4503          	lbu	a0,0(s4)
    8000052e:	28050763          	beqz	a0,800007bc <printf+0x2c2>
    80000532:	f4a6                	sd	s1,104(sp)
    80000534:	f0ca                	sd	s2,96(sp)
    80000536:	ecce                	sd	s3,88(sp)
    80000538:	e4d6                	sd	s5,72(sp)
    8000053a:	e0da                	sd	s6,64(sp)
    8000053c:	f862                	sd	s8,48(sp)
    8000053e:	f466                	sd	s9,40(sp)
    80000540:	f06a                	sd	s10,32(sp)
    80000542:	ec6e                	sd	s11,24(sp)
    80000544:	4981                	li	s3,0
    if(cx != '%'){
    80000546:	02500a93          	li	s5,37
    i++;
    c0 = fmt[i+0] & 0xff;
    c1 = c2 = 0;
    if(c0) c1 = fmt[i+1] & 0xff;
    if(c1) c2 = fmt[i+2] & 0xff;
    if(c0 == 'd'){
    8000054a:	06400b13          	li	s6,100
      printint(va_arg(ap, int), 10, 1);
    } else if(c0 == 'l' && c1 == 'd'){
    8000054e:	06c00c13          	li	s8,108
      printint(va_arg(ap, uint64), 10, 1);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
      printint(va_arg(ap, uint64), 10, 1);
      i += 2;
    } else if(c0 == 'u'){
    80000552:	07500c93          	li	s9,117
      printint(va_arg(ap, uint64), 10, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
      printint(va_arg(ap, uint64), 10, 0);
      i += 2;
    } else if(c0 == 'x'){
    80000556:	07800d13          	li	s10,120
      printint(va_arg(ap, uint64), 16, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
      printint(va_arg(ap, uint64), 16, 0);
      i += 2;
    } else if(c0 == 'p'){
    8000055a:	07000d93          	li	s11,112
    8000055e:	a01d                	j	80000584 <printf+0x8a>
    acquire(&pr.lock);
    80000560:	0000f517          	auipc	a0,0xf
    80000564:	3a850513          	addi	a0,a0,936 # 8000f908 <pr>
    80000568:	666000ef          	jal	80000bce <acquire>
    8000056c:	bf5d                	j	80000522 <printf+0x28>
      consputc(cx);
    8000056e:	d0bff0ef          	jal	80000278 <consputc>
      continue;
    80000572:	84ce                	mv	s1,s3
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    80000574:	0014899b          	addiw	s3,s1,1
    80000578:	013a07b3          	add	a5,s4,s3
    8000057c:	0007c503          	lbu	a0,0(a5)
    80000580:	20050b63          	beqz	a0,80000796 <printf+0x29c>
    if(cx != '%'){
    80000584:	ff5515e3          	bne	a0,s5,8000056e <printf+0x74>
    i++;
    80000588:	0019849b          	addiw	s1,s3,1
    c0 = fmt[i+0] & 0xff;
    8000058c:	009a07b3          	add	a5,s4,s1
    80000590:	0007c903          	lbu	s2,0(a5)
    if(c0) c1 = fmt[i+1] & 0xff;
    80000594:	20090b63          	beqz	s2,800007aa <printf+0x2b0>
    80000598:	0017c783          	lbu	a5,1(a5)
    c1 = c2 = 0;
    8000059c:	86be                	mv	a3,a5
    if(c1) c2 = fmt[i+2] & 0xff;
    8000059e:	c789                	beqz	a5,800005a8 <printf+0xae>
    800005a0:	009a0733          	add	a4,s4,s1
    800005a4:	00274683          	lbu	a3,2(a4)
    if(c0 == 'd'){
    800005a8:	03690963          	beq	s2,s6,800005da <printf+0xe0>
    } else if(c0 == 'l' && c1 == 'd'){
    800005ac:	05890363          	beq	s2,s8,800005f2 <printf+0xf8>
    } else if(c0 == 'u'){
    800005b0:	0d990663          	beq	s2,s9,8000067c <printf+0x182>
    } else if(c0 == 'x'){
    800005b4:	11a90d63          	beq	s2,s10,800006ce <printf+0x1d4>
    } else if(c0 == 'p'){
    800005b8:	15b90663          	beq	s2,s11,80000704 <printf+0x20a>
      printptr(va_arg(ap, uint64));
    } else if(c0 == 'c'){
    800005bc:	06300793          	li	a5,99
    800005c0:	18f90563          	beq	s2,a5,8000074a <printf+0x250>
      consputc(va_arg(ap, uint));
    } else if(c0 == 's'){
    800005c4:	07300793          	li	a5,115
    800005c8:	18f90b63          	beq	s2,a5,8000075e <printf+0x264>
      if((s = va_arg(ap, char*)) == 0)
        s = "(null)";
      for(; *s; s++)
        consputc(*s);
    } else if(c0 == '%'){
    800005cc:	03591b63          	bne	s2,s5,80000602 <printf+0x108>
      consputc('%');
    800005d0:	02500513          	li	a0,37
    800005d4:	ca5ff0ef          	jal	80000278 <consputc>
    800005d8:	bf71                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, int), 10, 1);
    800005da:	f8843783          	ld	a5,-120(s0)
    800005de:	00878713          	addi	a4,a5,8
    800005e2:	f8e43423          	sd	a4,-120(s0)
    800005e6:	4605                	li	a2,1
    800005e8:	45a9                	li	a1,10
    800005ea:	4388                	lw	a0,0(a5)
    800005ec:	e7dff0ef          	jal	80000468 <printint>
    800005f0:	b751                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'd'){
    800005f2:	01678f63          	beq	a5,s6,80000610 <printf+0x116>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    800005f6:	03878b63          	beq	a5,s8,8000062c <printf+0x132>
    } else if(c0 == 'l' && c1 == 'u'){
    800005fa:	09978e63          	beq	a5,s9,80000696 <printf+0x19c>
    } else if(c0 == 'l' && c1 == 'x'){
    800005fe:	0fa78563          	beq	a5,s10,800006e8 <printf+0x1ee>
    } else if(c0 == 0){
      break;
    } else {
      // Print unknown % sequence to draw attention.
      consputc('%');
    80000602:	8556                	mv	a0,s5
    80000604:	c75ff0ef          	jal	80000278 <consputc>
      consputc(c0);
    80000608:	854a                	mv	a0,s2
    8000060a:	c6fff0ef          	jal	80000278 <consputc>
    8000060e:	b79d                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000610:	f8843783          	ld	a5,-120(s0)
    80000614:	00878713          	addi	a4,a5,8
    80000618:	f8e43423          	sd	a4,-120(s0)
    8000061c:	4605                	li	a2,1
    8000061e:	45a9                	li	a1,10
    80000620:	6388                	ld	a0,0(a5)
    80000622:	e47ff0ef          	jal	80000468 <printint>
      i += 1;
    80000626:	0029849b          	addiw	s1,s3,2
    8000062a:	b7a9                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    8000062c:	06400793          	li	a5,100
    80000630:	02f68863          	beq	a3,a5,80000660 <printf+0x166>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
    80000634:	07500793          	li	a5,117
    80000638:	06f68d63          	beq	a3,a5,800006b2 <printf+0x1b8>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
    8000063c:	07800793          	li	a5,120
    80000640:	fcf691e3          	bne	a3,a5,80000602 <printf+0x108>
      printint(va_arg(ap, uint64), 16, 0);
    80000644:	f8843783          	ld	a5,-120(s0)
    80000648:	00878713          	addi	a4,a5,8
    8000064c:	f8e43423          	sd	a4,-120(s0)
    80000650:	4601                	li	a2,0
    80000652:	45c1                	li	a1,16
    80000654:	6388                	ld	a0,0(a5)
    80000656:	e13ff0ef          	jal	80000468 <printint>
      i += 2;
    8000065a:	0039849b          	addiw	s1,s3,3
    8000065e:	bf19                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000660:	f8843783          	ld	a5,-120(s0)
    80000664:	00878713          	addi	a4,a5,8
    80000668:	f8e43423          	sd	a4,-120(s0)
    8000066c:	4605                	li	a2,1
    8000066e:	45a9                	li	a1,10
    80000670:	6388                	ld	a0,0(a5)
    80000672:	df7ff0ef          	jal	80000468 <printint>
      i += 2;
    80000676:	0039849b          	addiw	s1,s3,3
    8000067a:	bded                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 10, 0);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4601                	li	a2,0
    8000068a:	45a9                	li	a1,10
    8000068c:	0007e503          	lwu	a0,0(a5)
    80000690:	dd9ff0ef          	jal	80000468 <printint>
    80000694:	b5c5                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    80000696:	f8843783          	ld	a5,-120(s0)
    8000069a:	00878713          	addi	a4,a5,8
    8000069e:	f8e43423          	sd	a4,-120(s0)
    800006a2:	4601                	li	a2,0
    800006a4:	45a9                	li	a1,10
    800006a6:	6388                	ld	a0,0(a5)
    800006a8:	dc1ff0ef          	jal	80000468 <printint>
      i += 1;
    800006ac:	0029849b          	addiw	s1,s3,2
    800006b0:	b5d1                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    800006b2:	f8843783          	ld	a5,-120(s0)
    800006b6:	00878713          	addi	a4,a5,8
    800006ba:	f8e43423          	sd	a4,-120(s0)
    800006be:	4601                	li	a2,0
    800006c0:	45a9                	li	a1,10
    800006c2:	6388                	ld	a0,0(a5)
    800006c4:	da5ff0ef          	jal	80000468 <printint>
      i += 2;
    800006c8:	0039849b          	addiw	s1,s3,3
    800006cc:	b565                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 16, 0);
    800006ce:	f8843783          	ld	a5,-120(s0)
    800006d2:	00878713          	addi	a4,a5,8
    800006d6:	f8e43423          	sd	a4,-120(s0)
    800006da:	4601                	li	a2,0
    800006dc:	45c1                	li	a1,16
    800006de:	0007e503          	lwu	a0,0(a5)
    800006e2:	d87ff0ef          	jal	80000468 <printint>
    800006e6:	b579                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 16, 0);
    800006e8:	f8843783          	ld	a5,-120(s0)
    800006ec:	00878713          	addi	a4,a5,8
    800006f0:	f8e43423          	sd	a4,-120(s0)
    800006f4:	4601                	li	a2,0
    800006f6:	45c1                	li	a1,16
    800006f8:	6388                	ld	a0,0(a5)
    800006fa:	d6fff0ef          	jal	80000468 <printint>
      i += 1;
    800006fe:	0029849b          	addiw	s1,s3,2
    80000702:	bd8d                	j	80000574 <printf+0x7a>
    80000704:	fc5e                	sd	s7,56(sp)
      printptr(va_arg(ap, uint64));
    80000706:	f8843783          	ld	a5,-120(s0)
    8000070a:	00878713          	addi	a4,a5,8
    8000070e:	f8e43423          	sd	a4,-120(s0)
    80000712:	0007b983          	ld	s3,0(a5)
  consputc('0');
    80000716:	03000513          	li	a0,48
    8000071a:	b5fff0ef          	jal	80000278 <consputc>
  consputc('x');
    8000071e:	07800513          	li	a0,120
    80000722:	b57ff0ef          	jal	80000278 <consputc>
    80000726:	4941                	li	s2,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    80000728:	00007b97          	auipc	s7,0x7
    8000072c:	fe8b8b93          	addi	s7,s7,-24 # 80007710 <digits>
    80000730:	03c9d793          	srli	a5,s3,0x3c
    80000734:	97de                	add	a5,a5,s7
    80000736:	0007c503          	lbu	a0,0(a5)
    8000073a:	b3fff0ef          	jal	80000278 <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    8000073e:	0992                	slli	s3,s3,0x4
    80000740:	397d                	addiw	s2,s2,-1
    80000742:	fe0917e3          	bnez	s2,80000730 <printf+0x236>
    80000746:	7be2                	ld	s7,56(sp)
    80000748:	b535                	j	80000574 <printf+0x7a>
      consputc(va_arg(ap, uint));
    8000074a:	f8843783          	ld	a5,-120(s0)
    8000074e:	00878713          	addi	a4,a5,8
    80000752:	f8e43423          	sd	a4,-120(s0)
    80000756:	4388                	lw	a0,0(a5)
    80000758:	b21ff0ef          	jal	80000278 <consputc>
    8000075c:	bd21                	j	80000574 <printf+0x7a>
      if((s = va_arg(ap, char*)) == 0)
    8000075e:	f8843783          	ld	a5,-120(s0)
    80000762:	00878713          	addi	a4,a5,8
    80000766:	f8e43423          	sd	a4,-120(s0)
    8000076a:	0007b903          	ld	s2,0(a5)
    8000076e:	00090d63          	beqz	s2,80000788 <printf+0x28e>
      for(; *s; s++)
    80000772:	00094503          	lbu	a0,0(s2)
    80000776:	de050fe3          	beqz	a0,80000574 <printf+0x7a>
        consputc(*s);
    8000077a:	affff0ef          	jal	80000278 <consputc>
      for(; *s; s++)
    8000077e:	0905                	addi	s2,s2,1
    80000780:	00094503          	lbu	a0,0(s2)
    80000784:	f97d                	bnez	a0,8000077a <printf+0x280>
    80000786:	b3fd                	j	80000574 <printf+0x7a>
        s = "(null)";
    80000788:	00007917          	auipc	s2,0x7
    8000078c:	88090913          	addi	s2,s2,-1920 # 80007008 <etext+0x8>
      for(; *s; s++)
    80000790:	02800513          	li	a0,40
    80000794:	b7dd                	j	8000077a <printf+0x280>
    80000796:	74a6                	ld	s1,104(sp)
    80000798:	7906                	ld	s2,96(sp)
    8000079a:	69e6                	ld	s3,88(sp)
    8000079c:	6aa6                	ld	s5,72(sp)
    8000079e:	6b06                	ld	s6,64(sp)
    800007a0:	7c42                	ld	s8,48(sp)
    800007a2:	7ca2                	ld	s9,40(sp)
    800007a4:	7d02                	ld	s10,32(sp)
    800007a6:	6de2                	ld	s11,24(sp)
    800007a8:	a811                	j	800007bc <printf+0x2c2>
    800007aa:	74a6                	ld	s1,104(sp)
    800007ac:	7906                	ld	s2,96(sp)
    800007ae:	69e6                	ld	s3,88(sp)
    800007b0:	6aa6                	ld	s5,72(sp)
    800007b2:	6b06                	ld	s6,64(sp)
    800007b4:	7c42                	ld	s8,48(sp)
    800007b6:	7ca2                	ld	s9,40(sp)
    800007b8:	7d02                	ld	s10,32(sp)
    800007ba:	6de2                	ld	s11,24(sp)
    }

  }
  va_end(ap);

  if(panicking == 0)
    800007bc:	00007797          	auipc	a5,0x7
    800007c0:	0787a783          	lw	a5,120(a5) # 80007834 <panicking>
    800007c4:	c799                	beqz	a5,800007d2 <printf+0x2d8>
    release(&pr.lock);

  return 0;
}
    800007c6:	4501                	li	a0,0
    800007c8:	70e6                	ld	ra,120(sp)
    800007ca:	7446                	ld	s0,112(sp)
    800007cc:	6a46                	ld	s4,80(sp)
    800007ce:	6129                	addi	sp,sp,192
    800007d0:	8082                	ret
    release(&pr.lock);
    800007d2:	0000f517          	auipc	a0,0xf
    800007d6:	13650513          	addi	a0,a0,310 # 8000f908 <pr>
    800007da:	48c000ef          	jal	80000c66 <release>
  return 0;
    800007de:	b7e5                	j	800007c6 <printf+0x2cc>

00000000800007e0 <panic>:

void
panic(char *s)
{
    800007e0:	1101                	addi	sp,sp,-32
    800007e2:	ec06                	sd	ra,24(sp)
    800007e4:	e822                	sd	s0,16(sp)
    800007e6:	e426                	sd	s1,8(sp)
    800007e8:	e04a                	sd	s2,0(sp)
    800007ea:	1000                	addi	s0,sp,32
    800007ec:	84aa                	mv	s1,a0
  panicking = 1;
    800007ee:	4905                	li	s2,1
    800007f0:	00007797          	auipc	a5,0x7
    800007f4:	0527a223          	sw	s2,68(a5) # 80007834 <panicking>
  printf("panic: ");
    800007f8:	00007517          	auipc	a0,0x7
    800007fc:	82050513          	addi	a0,a0,-2016 # 80007018 <etext+0x18>
    80000800:	cfbff0ef          	jal	800004fa <printf>
  printf("%s\n", s);
    80000804:	85a6                	mv	a1,s1
    80000806:	00007517          	auipc	a0,0x7
    8000080a:	81a50513          	addi	a0,a0,-2022 # 80007020 <etext+0x20>
    8000080e:	cedff0ef          	jal	800004fa <printf>
  panicked = 1; // freeze uart output from other CPUs
    80000812:	00007797          	auipc	a5,0x7
    80000816:	0127af23          	sw	s2,30(a5) # 80007830 <panicked>
  for(;;)
    8000081a:	a001                	j	8000081a <panic+0x3a>

000000008000081c <printfinit>:
    ;
}

void
printfinit(void)
{
    8000081c:	1141                	addi	sp,sp,-16
    8000081e:	e406                	sd	ra,8(sp)
    80000820:	e022                	sd	s0,0(sp)
    80000822:	0800                	addi	s0,sp,16
  initlock(&pr.lock, "pr");
    80000824:	00007597          	auipc	a1,0x7
    80000828:	80458593          	addi	a1,a1,-2044 # 80007028 <etext+0x28>
    8000082c:	0000f517          	auipc	a0,0xf
    80000830:	0dc50513          	addi	a0,a0,220 # 8000f908 <pr>
    80000834:	31a000ef          	jal	80000b4e <initlock>
}
    80000838:	60a2                	ld	ra,8(sp)
    8000083a:	6402                	ld	s0,0(sp)
    8000083c:	0141                	addi	sp,sp,16
    8000083e:	8082                	ret

0000000080000840 <uartinit>:
extern volatile int panicking; // from printf.c
extern volatile int panicked; // from printf.c

void
uartinit(void)
{
    80000840:	1141                	addi	sp,sp,-16
    80000842:	e406                	sd	ra,8(sp)
    80000844:	e022                	sd	s0,0(sp)
    80000846:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    80000848:	100007b7          	lui	a5,0x10000
    8000084c:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    80000850:	10000737          	lui	a4,0x10000
    80000854:	f8000693          	li	a3,-128
    80000858:	00d701a3          	sb	a3,3(a4) # 10000003 <_entry-0x6ffffffd>

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    8000085c:	468d                	li	a3,3
    8000085e:	10000637          	lui	a2,0x10000
    80000862:	00d60023          	sb	a3,0(a2) # 10000000 <_entry-0x70000000>

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    80000866:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    8000086a:	00d701a3          	sb	a3,3(a4)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    8000086e:	10000737          	lui	a4,0x10000
    80000872:	461d                	li	a2,7
    80000874:	00c70123          	sb	a2,2(a4) # 10000002 <_entry-0x6ffffffe>

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    80000878:	00d780a3          	sb	a3,1(a5)

  initlock(&tx_lock, "uart");
    8000087c:	00006597          	auipc	a1,0x6
    80000880:	7b458593          	addi	a1,a1,1972 # 80007030 <etext+0x30>
    80000884:	0000f517          	auipc	a0,0xf
    80000888:	09c50513          	addi	a0,a0,156 # 8000f920 <tx_lock>
    8000088c:	2c2000ef          	jal	80000b4e <initlock>
}
    80000890:	60a2                	ld	ra,8(sp)
    80000892:	6402                	ld	s0,0(sp)
    80000894:	0141                	addi	sp,sp,16
    80000896:	8082                	ret

0000000080000898 <uartwrite>:
// transmit buf[] to the uart. it blocks if the
// uart is busy, so it cannot be called from
// interrupts, only from write() system calls.
void
uartwrite(char buf[], int n)
{
    80000898:	715d                	addi	sp,sp,-80
    8000089a:	e486                	sd	ra,72(sp)
    8000089c:	e0a2                	sd	s0,64(sp)
    8000089e:	fc26                	sd	s1,56(sp)
    800008a0:	ec56                	sd	s5,24(sp)
    800008a2:	0880                	addi	s0,sp,80
    800008a4:	8aaa                	mv	s5,a0
    800008a6:	84ae                	mv	s1,a1
  acquire(&tx_lock);
    800008a8:	0000f517          	auipc	a0,0xf
    800008ac:	07850513          	addi	a0,a0,120 # 8000f920 <tx_lock>
    800008b0:	31e000ef          	jal	80000bce <acquire>

  int i = 0;
  while(i < n){ 
    800008b4:	06905063          	blez	s1,80000914 <uartwrite+0x7c>
    800008b8:	f84a                	sd	s2,48(sp)
    800008ba:	f44e                	sd	s3,40(sp)
    800008bc:	f052                	sd	s4,32(sp)
    800008be:	e85a                	sd	s6,16(sp)
    800008c0:	e45e                	sd	s7,8(sp)
    800008c2:	8a56                	mv	s4,s5
    800008c4:	9aa6                	add	s5,s5,s1
    while(tx_busy != 0){
    800008c6:	00007497          	auipc	s1,0x7
    800008ca:	f7648493          	addi	s1,s1,-138 # 8000783c <tx_busy>
      // wait for a UART transmit-complete interrupt
      // to set tx_busy to 0.
      sleep(&tx_chan, &tx_lock);
    800008ce:	0000f997          	auipc	s3,0xf
    800008d2:	05298993          	addi	s3,s3,82 # 8000f920 <tx_lock>
    800008d6:	00007917          	auipc	s2,0x7
    800008da:	f6290913          	addi	s2,s2,-158 # 80007838 <tx_chan>
    }   
      
    WriteReg(THR, buf[i]);
    800008de:	10000bb7          	lui	s7,0x10000
    i += 1;
    tx_busy = 1;
    800008e2:	4b05                	li	s6,1
    800008e4:	a005                	j	80000904 <uartwrite+0x6c>
      sleep(&tx_chan, &tx_lock);
    800008e6:	85ce                	mv	a1,s3
    800008e8:	854a                	mv	a0,s2
    800008ea:	5fa010ef          	jal	80001ee4 <sleep>
    while(tx_busy != 0){
    800008ee:	409c                	lw	a5,0(s1)
    800008f0:	fbfd                	bnez	a5,800008e6 <uartwrite+0x4e>
    WriteReg(THR, buf[i]);
    800008f2:	000a4783          	lbu	a5,0(s4)
    800008f6:	00fb8023          	sb	a5,0(s7) # 10000000 <_entry-0x70000000>
    tx_busy = 1;
    800008fa:	0164a023          	sw	s6,0(s1)
  while(i < n){ 
    800008fe:	0a05                	addi	s4,s4,1
    80000900:	015a0563          	beq	s4,s5,8000090a <uartwrite+0x72>
    while(tx_busy != 0){
    80000904:	409c                	lw	a5,0(s1)
    80000906:	f3e5                	bnez	a5,800008e6 <uartwrite+0x4e>
    80000908:	b7ed                	j	800008f2 <uartwrite+0x5a>
    8000090a:	7942                	ld	s2,48(sp)
    8000090c:	79a2                	ld	s3,40(sp)
    8000090e:	7a02                	ld	s4,32(sp)
    80000910:	6b42                	ld	s6,16(sp)
    80000912:	6ba2                	ld	s7,8(sp)
  }

  release(&tx_lock);
    80000914:	0000f517          	auipc	a0,0xf
    80000918:	00c50513          	addi	a0,a0,12 # 8000f920 <tx_lock>
    8000091c:	34a000ef          	jal	80000c66 <release>
}
    80000920:	60a6                	ld	ra,72(sp)
    80000922:	6406                	ld	s0,64(sp)
    80000924:	74e2                	ld	s1,56(sp)
    80000926:	6ae2                	ld	s5,24(sp)
    80000928:	6161                	addi	sp,sp,80
    8000092a:	8082                	ret

000000008000092c <uartputc_sync>:
// interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    8000092c:	1101                	addi	sp,sp,-32
    8000092e:	ec06                	sd	ra,24(sp)
    80000930:	e822                	sd	s0,16(sp)
    80000932:	e426                	sd	s1,8(sp)
    80000934:	1000                	addi	s0,sp,32
    80000936:	84aa                	mv	s1,a0
  if(panicking == 0)
    80000938:	00007797          	auipc	a5,0x7
    8000093c:	efc7a783          	lw	a5,-260(a5) # 80007834 <panicking>
    80000940:	cf95                	beqz	a5,8000097c <uartputc_sync+0x50>
    push_off();

  if(panicked){
    80000942:	00007797          	auipc	a5,0x7
    80000946:	eee7a783          	lw	a5,-274(a5) # 80007830 <panicked>
    8000094a:	ef85                	bnez	a5,80000982 <uartputc_sync+0x56>
    for(;;)
      ;
  }

  // wait for UART to set Transmit Holding Empty in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000094c:	10000737          	lui	a4,0x10000
    80000950:	0715                	addi	a4,a4,5 # 10000005 <_entry-0x6ffffffb>
    80000952:	00074783          	lbu	a5,0(a4)
    80000956:	0207f793          	andi	a5,a5,32
    8000095a:	dfe5                	beqz	a5,80000952 <uartputc_sync+0x26>
    ;
  WriteReg(THR, c);
    8000095c:	0ff4f513          	zext.b	a0,s1
    80000960:	100007b7          	lui	a5,0x10000
    80000964:	00a78023          	sb	a0,0(a5) # 10000000 <_entry-0x70000000>

  if(panicking == 0)
    80000968:	00007797          	auipc	a5,0x7
    8000096c:	ecc7a783          	lw	a5,-308(a5) # 80007834 <panicking>
    80000970:	cb91                	beqz	a5,80000984 <uartputc_sync+0x58>
    pop_off();
}
    80000972:	60e2                	ld	ra,24(sp)
    80000974:	6442                	ld	s0,16(sp)
    80000976:	64a2                	ld	s1,8(sp)
    80000978:	6105                	addi	sp,sp,32
    8000097a:	8082                	ret
    push_off();
    8000097c:	212000ef          	jal	80000b8e <push_off>
    80000980:	b7c9                	j	80000942 <uartputc_sync+0x16>
    for(;;)
    80000982:	a001                	j	80000982 <uartputc_sync+0x56>
    pop_off();
    80000984:	28e000ef          	jal	80000c12 <pop_off>
}
    80000988:	b7ed                	j	80000972 <uartputc_sync+0x46>

000000008000098a <uartgetc>:

// try to read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    8000098a:	1141                	addi	sp,sp,-16
    8000098c:	e422                	sd	s0,8(sp)
    8000098e:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & LSR_RX_READY){
    80000990:	100007b7          	lui	a5,0x10000
    80000994:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    80000996:	0007c783          	lbu	a5,0(a5)
    8000099a:	8b85                	andi	a5,a5,1
    8000099c:	cb81                	beqz	a5,800009ac <uartgetc+0x22>
    // input data is ready.
    return ReadReg(RHR);
    8000099e:	100007b7          	lui	a5,0x10000
    800009a2:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
  } else {
    return -1;
  }
}
    800009a6:	6422                	ld	s0,8(sp)
    800009a8:	0141                	addi	sp,sp,16
    800009aa:	8082                	ret
    return -1;
    800009ac:	557d                	li	a0,-1
    800009ae:	bfe5                	j	800009a6 <uartgetc+0x1c>

00000000800009b0 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from devintr().
void
uartintr(void)
{
    800009b0:	1101                	addi	sp,sp,-32
    800009b2:	ec06                	sd	ra,24(sp)
    800009b4:	e822                	sd	s0,16(sp)
    800009b6:	e426                	sd	s1,8(sp)
    800009b8:	1000                	addi	s0,sp,32
  ReadReg(ISR); // acknowledge the interrupt
    800009ba:	100007b7          	lui	a5,0x10000
    800009be:	0789                	addi	a5,a5,2 # 10000002 <_entry-0x6ffffffe>
    800009c0:	0007c783          	lbu	a5,0(a5)

  acquire(&tx_lock);
    800009c4:	0000f517          	auipc	a0,0xf
    800009c8:	f5c50513          	addi	a0,a0,-164 # 8000f920 <tx_lock>
    800009cc:	202000ef          	jal	80000bce <acquire>
  if(ReadReg(LSR) & LSR_TX_IDLE){
    800009d0:	100007b7          	lui	a5,0x10000
    800009d4:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    800009d6:	0007c783          	lbu	a5,0(a5)
    800009da:	0207f793          	andi	a5,a5,32
    800009de:	eb89                	bnez	a5,800009f0 <uartintr+0x40>
    // UART finished transmitting; wake up sending thread.
    tx_busy = 0;
    wakeup(&tx_chan);
  }
  release(&tx_lock);
    800009e0:	0000f517          	auipc	a0,0xf
    800009e4:	f4050513          	addi	a0,a0,-192 # 8000f920 <tx_lock>
    800009e8:	27e000ef          	jal	80000c66 <release>

  // read and process incoming characters, if any.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009ec:	54fd                	li	s1,-1
    800009ee:	a831                	j	80000a0a <uartintr+0x5a>
    tx_busy = 0;
    800009f0:	00007797          	auipc	a5,0x7
    800009f4:	e407a623          	sw	zero,-436(a5) # 8000783c <tx_busy>
    wakeup(&tx_chan);
    800009f8:	00007517          	auipc	a0,0x7
    800009fc:	e4050513          	addi	a0,a0,-448 # 80007838 <tx_chan>
    80000a00:	530010ef          	jal	80001f30 <wakeup>
    80000a04:	bff1                	j	800009e0 <uartintr+0x30>
      break;
    consoleintr(c);
    80000a06:	8a5ff0ef          	jal	800002aa <consoleintr>
    int c = uartgetc();
    80000a0a:	f81ff0ef          	jal	8000098a <uartgetc>
    if(c == -1)
    80000a0e:	fe951ce3          	bne	a0,s1,80000a06 <uartintr+0x56>
  }
}
    80000a12:	60e2                	ld	ra,24(sp)
    80000a14:	6442                	ld	s0,16(sp)
    80000a16:	64a2                	ld	s1,8(sp)
    80000a18:	6105                	addi	sp,sp,32
    80000a1a:	8082                	ret

0000000080000a1c <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    80000a1c:	1101                	addi	sp,sp,-32
    80000a1e:	ec06                	sd	ra,24(sp)
    80000a20:	e822                	sd	s0,16(sp)
    80000a22:	e426                	sd	s1,8(sp)
    80000a24:	e04a                	sd	s2,0(sp)
    80000a26:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a28:	03451793          	slli	a5,a0,0x34
    80000a2c:	e7a9                	bnez	a5,80000a76 <kfree+0x5a>
    80000a2e:	84aa                	mv	s1,a0
    80000a30:	00020797          	auipc	a5,0x20
    80000a34:	53878793          	addi	a5,a5,1336 # 80020f68 <end>
    80000a38:	02f56f63          	bltu	a0,a5,80000a76 <kfree+0x5a>
    80000a3c:	47c5                	li	a5,17
    80000a3e:	07ee                	slli	a5,a5,0x1b
    80000a40:	02f57b63          	bgeu	a0,a5,80000a76 <kfree+0x5a>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a44:	6605                	lui	a2,0x1
    80000a46:	4585                	li	a1,1
    80000a48:	25a000ef          	jal	80000ca2 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a4c:	0000f917          	auipc	s2,0xf
    80000a50:	eec90913          	addi	s2,s2,-276 # 8000f938 <kmem>
    80000a54:	854a                	mv	a0,s2
    80000a56:	178000ef          	jal	80000bce <acquire>
  r->next = kmem.freelist;
    80000a5a:	01893783          	ld	a5,24(s2)
    80000a5e:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a60:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a64:	854a                	mv	a0,s2
    80000a66:	200000ef          	jal	80000c66 <release>
}
    80000a6a:	60e2                	ld	ra,24(sp)
    80000a6c:	6442                	ld	s0,16(sp)
    80000a6e:	64a2                	ld	s1,8(sp)
    80000a70:	6902                	ld	s2,0(sp)
    80000a72:	6105                	addi	sp,sp,32
    80000a74:	8082                	ret
    panic("kfree");
    80000a76:	00006517          	auipc	a0,0x6
    80000a7a:	5c250513          	addi	a0,a0,1474 # 80007038 <etext+0x38>
    80000a7e:	d63ff0ef          	jal	800007e0 <panic>

0000000080000a82 <freerange>:
{
    80000a82:	7179                	addi	sp,sp,-48
    80000a84:	f406                	sd	ra,40(sp)
    80000a86:	f022                	sd	s0,32(sp)
    80000a88:	ec26                	sd	s1,24(sp)
    80000a8a:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a8c:	6785                	lui	a5,0x1
    80000a8e:	fff78713          	addi	a4,a5,-1 # fff <_entry-0x7ffff001>
    80000a92:	00e504b3          	add	s1,a0,a4
    80000a96:	777d                	lui	a4,0xfffff
    80000a98:	8cf9                	and	s1,s1,a4
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a9a:	94be                	add	s1,s1,a5
    80000a9c:	0295e263          	bltu	a1,s1,80000ac0 <freerange+0x3e>
    80000aa0:	e84a                	sd	s2,16(sp)
    80000aa2:	e44e                	sd	s3,8(sp)
    80000aa4:	e052                	sd	s4,0(sp)
    80000aa6:	892e                	mv	s2,a1
    kfree(p);
    80000aa8:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aaa:	6985                	lui	s3,0x1
    kfree(p);
    80000aac:	01448533          	add	a0,s1,s4
    80000ab0:	f6dff0ef          	jal	80000a1c <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000ab4:	94ce                	add	s1,s1,s3
    80000ab6:	fe997be3          	bgeu	s2,s1,80000aac <freerange+0x2a>
    80000aba:	6942                	ld	s2,16(sp)
    80000abc:	69a2                	ld	s3,8(sp)
    80000abe:	6a02                	ld	s4,0(sp)
}
    80000ac0:	70a2                	ld	ra,40(sp)
    80000ac2:	7402                	ld	s0,32(sp)
    80000ac4:	64e2                	ld	s1,24(sp)
    80000ac6:	6145                	addi	sp,sp,48
    80000ac8:	8082                	ret

0000000080000aca <kinit>:
{
    80000aca:	1141                	addi	sp,sp,-16
    80000acc:	e406                	sd	ra,8(sp)
    80000ace:	e022                	sd	s0,0(sp)
    80000ad0:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ad2:	00006597          	auipc	a1,0x6
    80000ad6:	56e58593          	addi	a1,a1,1390 # 80007040 <etext+0x40>
    80000ada:	0000f517          	auipc	a0,0xf
    80000ade:	e5e50513          	addi	a0,a0,-418 # 8000f938 <kmem>
    80000ae2:	06c000ef          	jal	80000b4e <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ae6:	45c5                	li	a1,17
    80000ae8:	05ee                	slli	a1,a1,0x1b
    80000aea:	00020517          	auipc	a0,0x20
    80000aee:	47e50513          	addi	a0,a0,1150 # 80020f68 <end>
    80000af2:	f91ff0ef          	jal	80000a82 <freerange>
}
    80000af6:	60a2                	ld	ra,8(sp)
    80000af8:	6402                	ld	s0,0(sp)
    80000afa:	0141                	addi	sp,sp,16
    80000afc:	8082                	ret

0000000080000afe <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000afe:	1101                	addi	sp,sp,-32
    80000b00:	ec06                	sd	ra,24(sp)
    80000b02:	e822                	sd	s0,16(sp)
    80000b04:	e426                	sd	s1,8(sp)
    80000b06:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000b08:	0000f497          	auipc	s1,0xf
    80000b0c:	e3048493          	addi	s1,s1,-464 # 8000f938 <kmem>
    80000b10:	8526                	mv	a0,s1
    80000b12:	0bc000ef          	jal	80000bce <acquire>
  r = kmem.freelist;
    80000b16:	6c84                	ld	s1,24(s1)
  if(r)
    80000b18:	c485                	beqz	s1,80000b40 <kalloc+0x42>
    kmem.freelist = r->next;
    80000b1a:	609c                	ld	a5,0(s1)
    80000b1c:	0000f517          	auipc	a0,0xf
    80000b20:	e1c50513          	addi	a0,a0,-484 # 8000f938 <kmem>
    80000b24:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b26:	140000ef          	jal	80000c66 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b2a:	6605                	lui	a2,0x1
    80000b2c:	4595                	li	a1,5
    80000b2e:	8526                	mv	a0,s1
    80000b30:	172000ef          	jal	80000ca2 <memset>
  return (void*)r;
}
    80000b34:	8526                	mv	a0,s1
    80000b36:	60e2                	ld	ra,24(sp)
    80000b38:	6442                	ld	s0,16(sp)
    80000b3a:	64a2                	ld	s1,8(sp)
    80000b3c:	6105                	addi	sp,sp,32
    80000b3e:	8082                	ret
  release(&kmem.lock);
    80000b40:	0000f517          	auipc	a0,0xf
    80000b44:	df850513          	addi	a0,a0,-520 # 8000f938 <kmem>
    80000b48:	11e000ef          	jal	80000c66 <release>
  if(r)
    80000b4c:	b7e5                	j	80000b34 <kalloc+0x36>

0000000080000b4e <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b4e:	1141                	addi	sp,sp,-16
    80000b50:	e422                	sd	s0,8(sp)
    80000b52:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b54:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b56:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b5a:	00053823          	sd	zero,16(a0)
}
    80000b5e:	6422                	ld	s0,8(sp)
    80000b60:	0141                	addi	sp,sp,16
    80000b62:	8082                	ret

0000000080000b64 <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b64:	411c                	lw	a5,0(a0)
    80000b66:	e399                	bnez	a5,80000b6c <holding+0x8>
    80000b68:	4501                	li	a0,0
  return r;
}
    80000b6a:	8082                	ret
{
    80000b6c:	1101                	addi	sp,sp,-32
    80000b6e:	ec06                	sd	ra,24(sp)
    80000b70:	e822                	sd	s0,16(sp)
    80000b72:	e426                	sd	s1,8(sp)
    80000b74:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b76:	6904                	ld	s1,16(a0)
    80000b78:	53b000ef          	jal	800018b2 <mycpu>
    80000b7c:	40a48533          	sub	a0,s1,a0
    80000b80:	00153513          	seqz	a0,a0
}
    80000b84:	60e2                	ld	ra,24(sp)
    80000b86:	6442                	ld	s0,16(sp)
    80000b88:	64a2                	ld	s1,8(sp)
    80000b8a:	6105                	addi	sp,sp,32
    80000b8c:	8082                	ret

0000000080000b8e <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b8e:	1101                	addi	sp,sp,-32
    80000b90:	ec06                	sd	ra,24(sp)
    80000b92:	e822                	sd	s0,16(sp)
    80000b94:	e426                	sd	s1,8(sp)
    80000b96:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000b98:	100024f3          	csrr	s1,sstatus
    80000b9c:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000ba0:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000ba2:	10079073          	csrw	sstatus,a5

  // disable interrupts to prevent an involuntary context
  // switch while using mycpu().
  intr_off();

  if(mycpu()->noff == 0)
    80000ba6:	50d000ef          	jal	800018b2 <mycpu>
    80000baa:	5d3c                	lw	a5,120(a0)
    80000bac:	cb99                	beqz	a5,80000bc2 <push_off+0x34>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bae:	505000ef          	jal	800018b2 <mycpu>
    80000bb2:	5d3c                	lw	a5,120(a0)
    80000bb4:	2785                	addiw	a5,a5,1
    80000bb6:	dd3c                	sw	a5,120(a0)
}
    80000bb8:	60e2                	ld	ra,24(sp)
    80000bba:	6442                	ld	s0,16(sp)
    80000bbc:	64a2                	ld	s1,8(sp)
    80000bbe:	6105                	addi	sp,sp,32
    80000bc0:	8082                	ret
    mycpu()->intena = old;
    80000bc2:	4f1000ef          	jal	800018b2 <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bc6:	8085                	srli	s1,s1,0x1
    80000bc8:	8885                	andi	s1,s1,1
    80000bca:	dd64                	sw	s1,124(a0)
    80000bcc:	b7cd                	j	80000bae <push_off+0x20>

0000000080000bce <acquire>:
{
    80000bce:	1101                	addi	sp,sp,-32
    80000bd0:	ec06                	sd	ra,24(sp)
    80000bd2:	e822                	sd	s0,16(sp)
    80000bd4:	e426                	sd	s1,8(sp)
    80000bd6:	1000                	addi	s0,sp,32
    80000bd8:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bda:	fb5ff0ef          	jal	80000b8e <push_off>
  if(holding(lk))
    80000bde:	8526                	mv	a0,s1
    80000be0:	f85ff0ef          	jal	80000b64 <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be4:	4705                	li	a4,1
  if(holding(lk))
    80000be6:	e105                	bnez	a0,80000c06 <acquire+0x38>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be8:	87ba                	mv	a5,a4
    80000bea:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000bee:	2781                	sext.w	a5,a5
    80000bf0:	ffe5                	bnez	a5,80000be8 <acquire+0x1a>
  __sync_synchronize();
    80000bf2:	0ff0000f          	fence
  lk->cpu = mycpu();
    80000bf6:	4bd000ef          	jal	800018b2 <mycpu>
    80000bfa:	e888                	sd	a0,16(s1)
}
    80000bfc:	60e2                	ld	ra,24(sp)
    80000bfe:	6442                	ld	s0,16(sp)
    80000c00:	64a2                	ld	s1,8(sp)
    80000c02:	6105                	addi	sp,sp,32
    80000c04:	8082                	ret
    panic("acquire");
    80000c06:	00006517          	auipc	a0,0x6
    80000c0a:	44250513          	addi	a0,a0,1090 # 80007048 <etext+0x48>
    80000c0e:	bd3ff0ef          	jal	800007e0 <panic>

0000000080000c12 <pop_off>:

void
pop_off(void)
{
    80000c12:	1141                	addi	sp,sp,-16
    80000c14:	e406                	sd	ra,8(sp)
    80000c16:	e022                	sd	s0,0(sp)
    80000c18:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c1a:	499000ef          	jal	800018b2 <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c1e:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c22:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c24:	e78d                	bnez	a5,80000c4e <pop_off+0x3c>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c26:	5d3c                	lw	a5,120(a0)
    80000c28:	02f05963          	blez	a5,80000c5a <pop_off+0x48>
    panic("pop_off");
  c->noff -= 1;
    80000c2c:	37fd                	addiw	a5,a5,-1
    80000c2e:	0007871b          	sext.w	a4,a5
    80000c32:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c34:	eb09                	bnez	a4,80000c46 <pop_off+0x34>
    80000c36:	5d7c                	lw	a5,124(a0)
    80000c38:	c799                	beqz	a5,80000c46 <pop_off+0x34>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c3a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c3e:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c42:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c46:	60a2                	ld	ra,8(sp)
    80000c48:	6402                	ld	s0,0(sp)
    80000c4a:	0141                	addi	sp,sp,16
    80000c4c:	8082                	ret
    panic("pop_off - interruptible");
    80000c4e:	00006517          	auipc	a0,0x6
    80000c52:	40250513          	addi	a0,a0,1026 # 80007050 <etext+0x50>
    80000c56:	b8bff0ef          	jal	800007e0 <panic>
    panic("pop_off");
    80000c5a:	00006517          	auipc	a0,0x6
    80000c5e:	40e50513          	addi	a0,a0,1038 # 80007068 <etext+0x68>
    80000c62:	b7fff0ef          	jal	800007e0 <panic>

0000000080000c66 <release>:
{
    80000c66:	1101                	addi	sp,sp,-32
    80000c68:	ec06                	sd	ra,24(sp)
    80000c6a:	e822                	sd	s0,16(sp)
    80000c6c:	e426                	sd	s1,8(sp)
    80000c6e:	1000                	addi	s0,sp,32
    80000c70:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000c72:	ef3ff0ef          	jal	80000b64 <holding>
    80000c76:	c105                	beqz	a0,80000c96 <release+0x30>
  lk->cpu = 0;
    80000c78:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000c7c:	0ff0000f          	fence
  __sync_lock_release(&lk->locked);
    80000c80:	0f50000f          	fence	iorw,ow
    80000c84:	0804a02f          	amoswap.w	zero,zero,(s1)
  pop_off();
    80000c88:	f8bff0ef          	jal	80000c12 <pop_off>
}
    80000c8c:	60e2                	ld	ra,24(sp)
    80000c8e:	6442                	ld	s0,16(sp)
    80000c90:	64a2                	ld	s1,8(sp)
    80000c92:	6105                	addi	sp,sp,32
    80000c94:	8082                	ret
    panic("release");
    80000c96:	00006517          	auipc	a0,0x6
    80000c9a:	3da50513          	addi	a0,a0,986 # 80007070 <etext+0x70>
    80000c9e:	b43ff0ef          	jal	800007e0 <panic>

0000000080000ca2 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ca2:	1141                	addi	sp,sp,-16
    80000ca4:	e422                	sd	s0,8(sp)
    80000ca6:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ca8:	ca19                	beqz	a2,80000cbe <memset+0x1c>
    80000caa:	87aa                	mv	a5,a0
    80000cac:	1602                	slli	a2,a2,0x20
    80000cae:	9201                	srli	a2,a2,0x20
    80000cb0:	00a60733          	add	a4,a2,a0
    cdst[i] = c;
    80000cb4:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cb8:	0785                	addi	a5,a5,1
    80000cba:	fee79de3          	bne	a5,a4,80000cb4 <memset+0x12>
  }
  return dst;
}
    80000cbe:	6422                	ld	s0,8(sp)
    80000cc0:	0141                	addi	sp,sp,16
    80000cc2:	8082                	ret

0000000080000cc4 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000cc4:	1141                	addi	sp,sp,-16
    80000cc6:	e422                	sd	s0,8(sp)
    80000cc8:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000cca:	ca05                	beqz	a2,80000cfa <memcmp+0x36>
    80000ccc:	fff6069b          	addiw	a3,a2,-1 # fff <_entry-0x7ffff001>
    80000cd0:	1682                	slli	a3,a3,0x20
    80000cd2:	9281                	srli	a3,a3,0x20
    80000cd4:	0685                	addi	a3,a3,1
    80000cd6:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000cd8:	00054783          	lbu	a5,0(a0)
    80000cdc:	0005c703          	lbu	a4,0(a1)
    80000ce0:	00e79863          	bne	a5,a4,80000cf0 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000ce4:	0505                	addi	a0,a0,1
    80000ce6:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000ce8:	fed518e3          	bne	a0,a3,80000cd8 <memcmp+0x14>
  }

  return 0;
    80000cec:	4501                	li	a0,0
    80000cee:	a019                	j	80000cf4 <memcmp+0x30>
      return *s1 - *s2;
    80000cf0:	40e7853b          	subw	a0,a5,a4
}
    80000cf4:	6422                	ld	s0,8(sp)
    80000cf6:	0141                	addi	sp,sp,16
    80000cf8:	8082                	ret
  return 0;
    80000cfa:	4501                	li	a0,0
    80000cfc:	bfe5                	j	80000cf4 <memcmp+0x30>

0000000080000cfe <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000cfe:	1141                	addi	sp,sp,-16
    80000d00:	e422                	sd	s0,8(sp)
    80000d02:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d04:	c205                	beqz	a2,80000d24 <memmove+0x26>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d06:	02a5e263          	bltu	a1,a0,80000d2a <memmove+0x2c>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d0a:	1602                	slli	a2,a2,0x20
    80000d0c:	9201                	srli	a2,a2,0x20
    80000d0e:	00c587b3          	add	a5,a1,a2
{
    80000d12:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d14:	0585                	addi	a1,a1,1
    80000d16:	0705                	addi	a4,a4,1 # fffffffffffff001 <end+0xffffffff7ffde099>
    80000d18:	fff5c683          	lbu	a3,-1(a1)
    80000d1c:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d20:	feb79ae3          	bne	a5,a1,80000d14 <memmove+0x16>

  return dst;
}
    80000d24:	6422                	ld	s0,8(sp)
    80000d26:	0141                	addi	sp,sp,16
    80000d28:	8082                	ret
  if(s < d && s + n > d){
    80000d2a:	02061693          	slli	a3,a2,0x20
    80000d2e:	9281                	srli	a3,a3,0x20
    80000d30:	00d58733          	add	a4,a1,a3
    80000d34:	fce57be3          	bgeu	a0,a4,80000d0a <memmove+0xc>
    d += n;
    80000d38:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d3a:	fff6079b          	addiw	a5,a2,-1
    80000d3e:	1782                	slli	a5,a5,0x20
    80000d40:	9381                	srli	a5,a5,0x20
    80000d42:	fff7c793          	not	a5,a5
    80000d46:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d48:	177d                	addi	a4,a4,-1
    80000d4a:	16fd                	addi	a3,a3,-1
    80000d4c:	00074603          	lbu	a2,0(a4)
    80000d50:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d54:	fef71ae3          	bne	a4,a5,80000d48 <memmove+0x4a>
    80000d58:	b7f1                	j	80000d24 <memmove+0x26>

0000000080000d5a <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000d5a:	1141                	addi	sp,sp,-16
    80000d5c:	e406                	sd	ra,8(sp)
    80000d5e:	e022                	sd	s0,0(sp)
    80000d60:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000d62:	f9dff0ef          	jal	80000cfe <memmove>
}
    80000d66:	60a2                	ld	ra,8(sp)
    80000d68:	6402                	ld	s0,0(sp)
    80000d6a:	0141                	addi	sp,sp,16
    80000d6c:	8082                	ret

0000000080000d6e <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000d6e:	1141                	addi	sp,sp,-16
    80000d70:	e422                	sd	s0,8(sp)
    80000d72:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000d74:	ce11                	beqz	a2,80000d90 <strncmp+0x22>
    80000d76:	00054783          	lbu	a5,0(a0)
    80000d7a:	cf89                	beqz	a5,80000d94 <strncmp+0x26>
    80000d7c:	0005c703          	lbu	a4,0(a1)
    80000d80:	00f71a63          	bne	a4,a5,80000d94 <strncmp+0x26>
    n--, p++, q++;
    80000d84:	367d                	addiw	a2,a2,-1
    80000d86:	0505                	addi	a0,a0,1
    80000d88:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000d8a:	f675                	bnez	a2,80000d76 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000d8c:	4501                	li	a0,0
    80000d8e:	a801                	j	80000d9e <strncmp+0x30>
    80000d90:	4501                	li	a0,0
    80000d92:	a031                	j	80000d9e <strncmp+0x30>
  return (uchar)*p - (uchar)*q;
    80000d94:	00054503          	lbu	a0,0(a0)
    80000d98:	0005c783          	lbu	a5,0(a1)
    80000d9c:	9d1d                	subw	a0,a0,a5
}
    80000d9e:	6422                	ld	s0,8(sp)
    80000da0:	0141                	addi	sp,sp,16
    80000da2:	8082                	ret

0000000080000da4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000da4:	1141                	addi	sp,sp,-16
    80000da6:	e422                	sd	s0,8(sp)
    80000da8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000daa:	87aa                	mv	a5,a0
    80000dac:	86b2                	mv	a3,a2
    80000dae:	367d                	addiw	a2,a2,-1
    80000db0:	02d05563          	blez	a3,80000dda <strncpy+0x36>
    80000db4:	0785                	addi	a5,a5,1
    80000db6:	0005c703          	lbu	a4,0(a1)
    80000dba:	fee78fa3          	sb	a4,-1(a5)
    80000dbe:	0585                	addi	a1,a1,1
    80000dc0:	f775                	bnez	a4,80000dac <strncpy+0x8>
    ;
  while(n-- > 0)
    80000dc2:	873e                	mv	a4,a5
    80000dc4:	9fb5                	addw	a5,a5,a3
    80000dc6:	37fd                	addiw	a5,a5,-1
    80000dc8:	00c05963          	blez	a2,80000dda <strncpy+0x36>
    *s++ = 0;
    80000dcc:	0705                	addi	a4,a4,1
    80000dce:	fe070fa3          	sb	zero,-1(a4)
  while(n-- > 0)
    80000dd2:	40e786bb          	subw	a3,a5,a4
    80000dd6:	fed04be3          	bgtz	a3,80000dcc <strncpy+0x28>
  return os;
}
    80000dda:	6422                	ld	s0,8(sp)
    80000ddc:	0141                	addi	sp,sp,16
    80000dde:	8082                	ret

0000000080000de0 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000de0:	1141                	addi	sp,sp,-16
    80000de2:	e422                	sd	s0,8(sp)
    80000de4:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000de6:	02c05363          	blez	a2,80000e0c <safestrcpy+0x2c>
    80000dea:	fff6069b          	addiw	a3,a2,-1
    80000dee:	1682                	slli	a3,a3,0x20
    80000df0:	9281                	srli	a3,a3,0x20
    80000df2:	96ae                	add	a3,a3,a1
    80000df4:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000df6:	00d58963          	beq	a1,a3,80000e08 <safestrcpy+0x28>
    80000dfa:	0585                	addi	a1,a1,1
    80000dfc:	0785                	addi	a5,a5,1
    80000dfe:	fff5c703          	lbu	a4,-1(a1)
    80000e02:	fee78fa3          	sb	a4,-1(a5)
    80000e06:	fb65                	bnez	a4,80000df6 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e08:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e0c:	6422                	ld	s0,8(sp)
    80000e0e:	0141                	addi	sp,sp,16
    80000e10:	8082                	ret

0000000080000e12 <strlen>:

int
strlen(const char *s)
{
    80000e12:	1141                	addi	sp,sp,-16
    80000e14:	e422                	sd	s0,8(sp)
    80000e16:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e18:	00054783          	lbu	a5,0(a0)
    80000e1c:	cf91                	beqz	a5,80000e38 <strlen+0x26>
    80000e1e:	0505                	addi	a0,a0,1
    80000e20:	87aa                	mv	a5,a0
    80000e22:	86be                	mv	a3,a5
    80000e24:	0785                	addi	a5,a5,1
    80000e26:	fff7c703          	lbu	a4,-1(a5)
    80000e2a:	ff65                	bnez	a4,80000e22 <strlen+0x10>
    80000e2c:	40a6853b          	subw	a0,a3,a0
    80000e30:	2505                	addiw	a0,a0,1
    ;
  return n;
}
    80000e32:	6422                	ld	s0,8(sp)
    80000e34:	0141                	addi	sp,sp,16
    80000e36:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e38:	4501                	li	a0,0
    80000e3a:	bfe5                	j	80000e32 <strlen+0x20>

0000000080000e3c <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e3c:	1141                	addi	sp,sp,-16
    80000e3e:	e406                	sd	ra,8(sp)
    80000e40:	e022                	sd	s0,0(sp)
    80000e42:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e44:	25f000ef          	jal	800018a2 <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e48:	00007717          	auipc	a4,0x7
    80000e4c:	9f870713          	addi	a4,a4,-1544 # 80007840 <started>
  if(cpuid() == 0){
    80000e50:	c51d                	beqz	a0,80000e7e <main+0x42>
    while(started == 0)
    80000e52:	431c                	lw	a5,0(a4)
    80000e54:	2781                	sext.w	a5,a5
    80000e56:	dff5                	beqz	a5,80000e52 <main+0x16>
      ;
    __sync_synchronize();
    80000e58:	0ff0000f          	fence
    printf("hart %d starting\n", cpuid());
    80000e5c:	247000ef          	jal	800018a2 <cpuid>
    80000e60:	85aa                	mv	a1,a0
    80000e62:	00006517          	auipc	a0,0x6
    80000e66:	23650513          	addi	a0,a0,566 # 80007098 <etext+0x98>
    80000e6a:	e90ff0ef          	jal	800004fa <printf>
    kvminithart();    // turn on paging
    80000e6e:	080000ef          	jal	80000eee <kvminithart>
    trapinithart();   // install kernel trap vector
    80000e72:	594010ef          	jal	80002406 <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000e76:	5a2040ef          	jal	80005418 <plicinithart>
  }

  scheduler();        
    80000e7a:	6d3000ef          	jal	80001d4c <scheduler>
    consoleinit();
    80000e7e:	da6ff0ef          	jal	80000424 <consoleinit>
    printfinit();
    80000e82:	99bff0ef          	jal	8000081c <printfinit>
    printf("\n");
    80000e86:	00006517          	auipc	a0,0x6
    80000e8a:	1f250513          	addi	a0,a0,498 # 80007078 <etext+0x78>
    80000e8e:	e6cff0ef          	jal	800004fa <printf>
    printf("xv6 kernel is booting\n");
    80000e92:	00006517          	auipc	a0,0x6
    80000e96:	1ee50513          	addi	a0,a0,494 # 80007080 <etext+0x80>
    80000e9a:	e60ff0ef          	jal	800004fa <printf>
    printf("\n");
    80000e9e:	00006517          	auipc	a0,0x6
    80000ea2:	1da50513          	addi	a0,a0,474 # 80007078 <etext+0x78>
    80000ea6:	e54ff0ef          	jal	800004fa <printf>
    kinit();         // physical page allocator
    80000eaa:	c21ff0ef          	jal	80000aca <kinit>
    kvminit();       // create kernel page table
    80000eae:	2ca000ef          	jal	80001178 <kvminit>
    kvminithart();   // turn on paging
    80000eb2:	03c000ef          	jal	80000eee <kvminithart>
    procinit();      // process table
    80000eb6:	137000ef          	jal	800017ec <procinit>
    trapinit();      // trap vectors
    80000eba:	528010ef          	jal	800023e2 <trapinit>
    trapinithart();  // install kernel trap vector
    80000ebe:	548010ef          	jal	80002406 <trapinithart>
    plicinit();      // set up interrupt controller
    80000ec2:	53c040ef          	jal	800053fe <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000ec6:	552040ef          	jal	80005418 <plicinithart>
    binit();         // buffer cache
    80000eca:	41b010ef          	jal	80002ae4 <binit>
    iinit();         // inode table
    80000ece:	1a0020ef          	jal	8000306e <iinit>
    fileinit();      // file table
    80000ed2:	092030ef          	jal	80003f64 <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000ed6:	632040ef          	jal	80005508 <virtio_disk_init>
    userinit();      // first user process
    80000eda:	4c7000ef          	jal	80001ba0 <userinit>
    __sync_synchronize();
    80000ede:	0ff0000f          	fence
    started = 1;
    80000ee2:	4785                	li	a5,1
    80000ee4:	00007717          	auipc	a4,0x7
    80000ee8:	94f72e23          	sw	a5,-1700(a4) # 80007840 <started>
    80000eec:	b779                	j	80000e7a <main+0x3e>

0000000080000eee <kvminithart>:

// Switch the current CPU's h/w page table register to
// the kernel's page table, and enable paging.
void
kvminithart()
{
    80000eee:	1141                	addi	sp,sp,-16
    80000ef0:	e422                	sd	s0,8(sp)
    80000ef2:	0800                	addi	s0,sp,16
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000ef4:	12000073          	sfence.vma
  // wait for any previous writes to the page table memory to finish.
  sfence_vma();

  w_satp(MAKE_SATP(kernel_pagetable));
    80000ef8:	00007797          	auipc	a5,0x7
    80000efc:	9507b783          	ld	a5,-1712(a5) # 80007848 <kernel_pagetable>
    80000f00:	83b1                	srli	a5,a5,0xc
    80000f02:	577d                	li	a4,-1
    80000f04:	177e                	slli	a4,a4,0x3f
    80000f06:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000f08:	18079073          	csrw	satp,a5
  asm volatile("sfence.vma zero, zero");
    80000f0c:	12000073          	sfence.vma

  // flush stale entries from the TLB.
  sfence_vma();
}
    80000f10:	6422                	ld	s0,8(sp)
    80000f12:	0141                	addi	sp,sp,16
    80000f14:	8082                	ret

0000000080000f16 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000f16:	7139                	addi	sp,sp,-64
    80000f18:	fc06                	sd	ra,56(sp)
    80000f1a:	f822                	sd	s0,48(sp)
    80000f1c:	f426                	sd	s1,40(sp)
    80000f1e:	f04a                	sd	s2,32(sp)
    80000f20:	ec4e                	sd	s3,24(sp)
    80000f22:	e852                	sd	s4,16(sp)
    80000f24:	e456                	sd	s5,8(sp)
    80000f26:	e05a                	sd	s6,0(sp)
    80000f28:	0080                	addi	s0,sp,64
    80000f2a:	84aa                	mv	s1,a0
    80000f2c:	89ae                	mv	s3,a1
    80000f2e:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000f30:	57fd                	li	a5,-1
    80000f32:	83e9                	srli	a5,a5,0x1a
    80000f34:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000f36:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000f38:	02b7fc63          	bgeu	a5,a1,80000f70 <walk+0x5a>
    panic("walk");
    80000f3c:	00006517          	auipc	a0,0x6
    80000f40:	17450513          	addi	a0,a0,372 # 800070b0 <etext+0xb0>
    80000f44:	89dff0ef          	jal	800007e0 <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80000f48:	060a8263          	beqz	s5,80000fac <walk+0x96>
    80000f4c:	bb3ff0ef          	jal	80000afe <kalloc>
    80000f50:	84aa                	mv	s1,a0
    80000f52:	c139                	beqz	a0,80000f98 <walk+0x82>
        return 0;
      memset(pagetable, 0, PGSIZE);
    80000f54:	6605                	lui	a2,0x1
    80000f56:	4581                	li	a1,0
    80000f58:	d4bff0ef          	jal	80000ca2 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    80000f5c:	00c4d793          	srli	a5,s1,0xc
    80000f60:	07aa                	slli	a5,a5,0xa
    80000f62:	0017e793          	ori	a5,a5,1
    80000f66:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80000f6a:	3a5d                	addiw	s4,s4,-9 # ffffffffffffeff7 <end+0xffffffff7ffde08f>
    80000f6c:	036a0063          	beq	s4,s6,80000f8c <walk+0x76>
    pte_t *pte = &pagetable[PX(level, va)];
    80000f70:	0149d933          	srl	s2,s3,s4
    80000f74:	1ff97913          	andi	s2,s2,511
    80000f78:	090e                	slli	s2,s2,0x3
    80000f7a:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    80000f7c:	00093483          	ld	s1,0(s2)
    80000f80:	0014f793          	andi	a5,s1,1
    80000f84:	d3f1                	beqz	a5,80000f48 <walk+0x32>
      pagetable = (pagetable_t)PTE2PA(*pte);
    80000f86:	80a9                	srli	s1,s1,0xa
    80000f88:	04b2                	slli	s1,s1,0xc
    80000f8a:	b7c5                	j	80000f6a <walk+0x54>
    }
  }
  return &pagetable[PX(0, va)];
    80000f8c:	00c9d513          	srli	a0,s3,0xc
    80000f90:	1ff57513          	andi	a0,a0,511
    80000f94:	050e                	slli	a0,a0,0x3
    80000f96:	9526                	add	a0,a0,s1
}
    80000f98:	70e2                	ld	ra,56(sp)
    80000f9a:	7442                	ld	s0,48(sp)
    80000f9c:	74a2                	ld	s1,40(sp)
    80000f9e:	7902                	ld	s2,32(sp)
    80000fa0:	69e2                	ld	s3,24(sp)
    80000fa2:	6a42                	ld	s4,16(sp)
    80000fa4:	6aa2                	ld	s5,8(sp)
    80000fa6:	6b02                	ld	s6,0(sp)
    80000fa8:	6121                	addi	sp,sp,64
    80000faa:	8082                	ret
        return 0;
    80000fac:	4501                	li	a0,0
    80000fae:	b7ed                	j	80000f98 <walk+0x82>

0000000080000fb0 <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    80000fb0:	57fd                	li	a5,-1
    80000fb2:	83e9                	srli	a5,a5,0x1a
    80000fb4:	00b7f463          	bgeu	a5,a1,80000fbc <walkaddr+0xc>
    return 0;
    80000fb8:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80000fba:	8082                	ret
{
    80000fbc:	1141                	addi	sp,sp,-16
    80000fbe:	e406                	sd	ra,8(sp)
    80000fc0:	e022                	sd	s0,0(sp)
    80000fc2:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    80000fc4:	4601                	li	a2,0
    80000fc6:	f51ff0ef          	jal	80000f16 <walk>
  if(pte == 0)
    80000fca:	c105                	beqz	a0,80000fea <walkaddr+0x3a>
  if((*pte & PTE_V) == 0)
    80000fcc:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80000fce:	0117f693          	andi	a3,a5,17
    80000fd2:	4745                	li	a4,17
    return 0;
    80000fd4:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    80000fd6:	00e68663          	beq	a3,a4,80000fe2 <walkaddr+0x32>
}
    80000fda:	60a2                	ld	ra,8(sp)
    80000fdc:	6402                	ld	s0,0(sp)
    80000fde:	0141                	addi	sp,sp,16
    80000fe0:	8082                	ret
  pa = PTE2PA(*pte);
    80000fe2:	83a9                	srli	a5,a5,0xa
    80000fe4:	00c79513          	slli	a0,a5,0xc
  return pa;
    80000fe8:	bfcd                	j	80000fda <walkaddr+0x2a>
    return 0;
    80000fea:	4501                	li	a0,0
    80000fec:	b7fd                	j	80000fda <walkaddr+0x2a>

0000000080000fee <mappages>:
// va and size MUST be page-aligned.
// Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    80000fee:	715d                	addi	sp,sp,-80
    80000ff0:	e486                	sd	ra,72(sp)
    80000ff2:	e0a2                	sd	s0,64(sp)
    80000ff4:	fc26                	sd	s1,56(sp)
    80000ff6:	f84a                	sd	s2,48(sp)
    80000ff8:	f44e                	sd	s3,40(sp)
    80000ffa:	f052                	sd	s4,32(sp)
    80000ffc:	ec56                	sd	s5,24(sp)
    80000ffe:	e85a                	sd	s6,16(sp)
    80001000:	e45e                	sd	s7,8(sp)
    80001002:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    80001004:	03459793          	slli	a5,a1,0x34
    80001008:	e7a9                	bnez	a5,80001052 <mappages+0x64>
    8000100a:	8aaa                	mv	s5,a0
    8000100c:	8b3a                	mv	s6,a4
    panic("mappages: va not aligned");

  if((size % PGSIZE) != 0)
    8000100e:	03461793          	slli	a5,a2,0x34
    80001012:	e7b1                	bnez	a5,8000105e <mappages+0x70>
    panic("mappages: size not aligned");

  if(size == 0)
    80001014:	ca39                	beqz	a2,8000106a <mappages+0x7c>
    panic("mappages: size");
  
  a = va;
  last = va + size - PGSIZE;
    80001016:	77fd                	lui	a5,0xfffff
    80001018:	963e                	add	a2,a2,a5
    8000101a:	00b609b3          	add	s3,a2,a1
  a = va;
    8000101e:	892e                	mv	s2,a1
    80001020:	40b68a33          	sub	s4,a3,a1
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    80001024:	6b85                	lui	s7,0x1
    80001026:	014904b3          	add	s1,s2,s4
    if((pte = walk(pagetable, a, 1)) == 0)
    8000102a:	4605                	li	a2,1
    8000102c:	85ca                	mv	a1,s2
    8000102e:	8556                	mv	a0,s5
    80001030:	ee7ff0ef          	jal	80000f16 <walk>
    80001034:	c539                	beqz	a0,80001082 <mappages+0x94>
    if(*pte & PTE_V)
    80001036:	611c                	ld	a5,0(a0)
    80001038:	8b85                	andi	a5,a5,1
    8000103a:	ef95                	bnez	a5,80001076 <mappages+0x88>
    *pte = PA2PTE(pa) | perm | PTE_V;
    8000103c:	80b1                	srli	s1,s1,0xc
    8000103e:	04aa                	slli	s1,s1,0xa
    80001040:	0164e4b3          	or	s1,s1,s6
    80001044:	0014e493          	ori	s1,s1,1
    80001048:	e104                	sd	s1,0(a0)
    if(a == last)
    8000104a:	05390863          	beq	s2,s3,8000109a <mappages+0xac>
    a += PGSIZE;
    8000104e:	995e                	add	s2,s2,s7
    if((pte = walk(pagetable, a, 1)) == 0)
    80001050:	bfd9                	j	80001026 <mappages+0x38>
    panic("mappages: va not aligned");
    80001052:	00006517          	auipc	a0,0x6
    80001056:	06650513          	addi	a0,a0,102 # 800070b8 <etext+0xb8>
    8000105a:	f86ff0ef          	jal	800007e0 <panic>
    panic("mappages: size not aligned");
    8000105e:	00006517          	auipc	a0,0x6
    80001062:	07a50513          	addi	a0,a0,122 # 800070d8 <etext+0xd8>
    80001066:	f7aff0ef          	jal	800007e0 <panic>
    panic("mappages: size");
    8000106a:	00006517          	auipc	a0,0x6
    8000106e:	08e50513          	addi	a0,a0,142 # 800070f8 <etext+0xf8>
    80001072:	f6eff0ef          	jal	800007e0 <panic>
      panic("mappages: remap");
    80001076:	00006517          	auipc	a0,0x6
    8000107a:	09250513          	addi	a0,a0,146 # 80007108 <etext+0x108>
    8000107e:	f62ff0ef          	jal	800007e0 <panic>
      return -1;
    80001082:	557d                	li	a0,-1
    pa += PGSIZE;
  }
  return 0;
}
    80001084:	60a6                	ld	ra,72(sp)
    80001086:	6406                	ld	s0,64(sp)
    80001088:	74e2                	ld	s1,56(sp)
    8000108a:	7942                	ld	s2,48(sp)
    8000108c:	79a2                	ld	s3,40(sp)
    8000108e:	7a02                	ld	s4,32(sp)
    80001090:	6ae2                	ld	s5,24(sp)
    80001092:	6b42                	ld	s6,16(sp)
    80001094:	6ba2                	ld	s7,8(sp)
    80001096:	6161                	addi	sp,sp,80
    80001098:	8082                	ret
  return 0;
    8000109a:	4501                	li	a0,0
    8000109c:	b7e5                	j	80001084 <mappages+0x96>

000000008000109e <kvmmap>:
{
    8000109e:	1141                	addi	sp,sp,-16
    800010a0:	e406                	sd	ra,8(sp)
    800010a2:	e022                	sd	s0,0(sp)
    800010a4:	0800                	addi	s0,sp,16
    800010a6:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    800010a8:	86b2                	mv	a3,a2
    800010aa:	863e                	mv	a2,a5
    800010ac:	f43ff0ef          	jal	80000fee <mappages>
    800010b0:	e509                	bnez	a0,800010ba <kvmmap+0x1c>
}
    800010b2:	60a2                	ld	ra,8(sp)
    800010b4:	6402                	ld	s0,0(sp)
    800010b6:	0141                	addi	sp,sp,16
    800010b8:	8082                	ret
    panic("kvmmap");
    800010ba:	00006517          	auipc	a0,0x6
    800010be:	05e50513          	addi	a0,a0,94 # 80007118 <etext+0x118>
    800010c2:	f1eff0ef          	jal	800007e0 <panic>

00000000800010c6 <kvmmake>:
{
    800010c6:	1101                	addi	sp,sp,-32
    800010c8:	ec06                	sd	ra,24(sp)
    800010ca:	e822                	sd	s0,16(sp)
    800010cc:	e426                	sd	s1,8(sp)
    800010ce:	e04a                	sd	s2,0(sp)
    800010d0:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    800010d2:	a2dff0ef          	jal	80000afe <kalloc>
    800010d6:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    800010d8:	6605                	lui	a2,0x1
    800010da:	4581                	li	a1,0
    800010dc:	bc7ff0ef          	jal	80000ca2 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800010e0:	4719                	li	a4,6
    800010e2:	6685                	lui	a3,0x1
    800010e4:	10000637          	lui	a2,0x10000
    800010e8:	100005b7          	lui	a1,0x10000
    800010ec:	8526                	mv	a0,s1
    800010ee:	fb1ff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800010f2:	4719                	li	a4,6
    800010f4:	6685                	lui	a3,0x1
    800010f6:	10001637          	lui	a2,0x10001
    800010fa:	100015b7          	lui	a1,0x10001
    800010fe:	8526                	mv	a0,s1
    80001100:	f9fff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x4000000, PTE_R | PTE_W);
    80001104:	4719                	li	a4,6
    80001106:	040006b7          	lui	a3,0x4000
    8000110a:	0c000637          	lui	a2,0xc000
    8000110e:	0c0005b7          	lui	a1,0xc000
    80001112:	8526                	mv	a0,s1
    80001114:	f8bff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    80001118:	00006917          	auipc	s2,0x6
    8000111c:	ee890913          	addi	s2,s2,-280 # 80007000 <etext>
    80001120:	4729                	li	a4,10
    80001122:	80006697          	auipc	a3,0x80006
    80001126:	ede68693          	addi	a3,a3,-290 # 7000 <_entry-0x7fff9000>
    8000112a:	4605                	li	a2,1
    8000112c:	067e                	slli	a2,a2,0x1f
    8000112e:	85b2                	mv	a1,a2
    80001130:	8526                	mv	a0,s1
    80001132:	f6dff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001136:	46c5                	li	a3,17
    80001138:	06ee                	slli	a3,a3,0x1b
    8000113a:	4719                	li	a4,6
    8000113c:	412686b3          	sub	a3,a3,s2
    80001140:	864a                	mv	a2,s2
    80001142:	85ca                	mv	a1,s2
    80001144:	8526                	mv	a0,s1
    80001146:	f59ff0ef          	jal	8000109e <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    8000114a:	4729                	li	a4,10
    8000114c:	6685                	lui	a3,0x1
    8000114e:	00005617          	auipc	a2,0x5
    80001152:	eb260613          	addi	a2,a2,-334 # 80006000 <_trampoline>
    80001156:	040005b7          	lui	a1,0x4000
    8000115a:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    8000115c:	05b2                	slli	a1,a1,0xc
    8000115e:	8526                	mv	a0,s1
    80001160:	f3fff0ef          	jal	8000109e <kvmmap>
  proc_mapstacks(kpgtbl);
    80001164:	8526                	mv	a0,s1
    80001166:	5ee000ef          	jal	80001754 <proc_mapstacks>
}
    8000116a:	8526                	mv	a0,s1
    8000116c:	60e2                	ld	ra,24(sp)
    8000116e:	6442                	ld	s0,16(sp)
    80001170:	64a2                	ld	s1,8(sp)
    80001172:	6902                	ld	s2,0(sp)
    80001174:	6105                	addi	sp,sp,32
    80001176:	8082                	ret

0000000080001178 <kvminit>:
{
    80001178:	1141                	addi	sp,sp,-16
    8000117a:	e406                	sd	ra,8(sp)
    8000117c:	e022                	sd	s0,0(sp)
    8000117e:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    80001180:	f47ff0ef          	jal	800010c6 <kvmmake>
    80001184:	00006797          	auipc	a5,0x6
    80001188:	6ca7b223          	sd	a0,1732(a5) # 80007848 <kernel_pagetable>
}
    8000118c:	60a2                	ld	ra,8(sp)
    8000118e:	6402                	ld	s0,0(sp)
    80001190:	0141                	addi	sp,sp,16
    80001192:	8082                	ret

0000000080001194 <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    80001194:	1101                	addi	sp,sp,-32
    80001196:	ec06                	sd	ra,24(sp)
    80001198:	e822                	sd	s0,16(sp)
    8000119a:	e426                	sd	s1,8(sp)
    8000119c:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    8000119e:	961ff0ef          	jal	80000afe <kalloc>
    800011a2:	84aa                	mv	s1,a0
  if(pagetable == 0)
    800011a4:	c509                	beqz	a0,800011ae <uvmcreate+0x1a>
    return 0;
  memset(pagetable, 0, PGSIZE);
    800011a6:	6605                	lui	a2,0x1
    800011a8:	4581                	li	a1,0
    800011aa:	af9ff0ef          	jal	80000ca2 <memset>
  return pagetable;
}
    800011ae:	8526                	mv	a0,s1
    800011b0:	60e2                	ld	ra,24(sp)
    800011b2:	6442                	ld	s0,16(sp)
    800011b4:	64a2                	ld	s1,8(sp)
    800011b6:	6105                	addi	sp,sp,32
    800011b8:	8082                	ret

00000000800011ba <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. It's OK if the mappings don't exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    800011ba:	7139                	addi	sp,sp,-64
    800011bc:	fc06                	sd	ra,56(sp)
    800011be:	f822                	sd	s0,48(sp)
    800011c0:	0080                	addi	s0,sp,64
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    800011c2:	03459793          	slli	a5,a1,0x34
    800011c6:	e38d                	bnez	a5,800011e8 <uvmunmap+0x2e>
    800011c8:	f04a                	sd	s2,32(sp)
    800011ca:	ec4e                	sd	s3,24(sp)
    800011cc:	e852                	sd	s4,16(sp)
    800011ce:	e456                	sd	s5,8(sp)
    800011d0:	e05a                	sd	s6,0(sp)
    800011d2:	8a2a                	mv	s4,a0
    800011d4:	892e                	mv	s2,a1
    800011d6:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800011d8:	0632                	slli	a2,a2,0xc
    800011da:	00b609b3          	add	s3,a2,a1
    800011de:	6b05                	lui	s6,0x1
    800011e0:	0535f963          	bgeu	a1,s3,80001232 <uvmunmap+0x78>
    800011e4:	f426                	sd	s1,40(sp)
    800011e6:	a015                	j	8000120a <uvmunmap+0x50>
    800011e8:	f426                	sd	s1,40(sp)
    800011ea:	f04a                	sd	s2,32(sp)
    800011ec:	ec4e                	sd	s3,24(sp)
    800011ee:	e852                	sd	s4,16(sp)
    800011f0:	e456                	sd	s5,8(sp)
    800011f2:	e05a                	sd	s6,0(sp)
    panic("uvmunmap: not aligned");
    800011f4:	00006517          	auipc	a0,0x6
    800011f8:	f2c50513          	addi	a0,a0,-212 # 80007120 <etext+0x120>
    800011fc:	de4ff0ef          	jal	800007e0 <panic>
      continue;
    if(do_free){
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
    80001200:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001204:	995a                	add	s2,s2,s6
    80001206:	03397563          	bgeu	s2,s3,80001230 <uvmunmap+0x76>
    if((pte = walk(pagetable, a, 0)) == 0) // leaf page table entry allocated?
    8000120a:	4601                	li	a2,0
    8000120c:	85ca                	mv	a1,s2
    8000120e:	8552                	mv	a0,s4
    80001210:	d07ff0ef          	jal	80000f16 <walk>
    80001214:	84aa                	mv	s1,a0
    80001216:	d57d                	beqz	a0,80001204 <uvmunmap+0x4a>
    if((*pte & PTE_V) == 0)  // has physical page been allocated?
    80001218:	611c                	ld	a5,0(a0)
    8000121a:	0017f713          	andi	a4,a5,1
    8000121e:	d37d                	beqz	a4,80001204 <uvmunmap+0x4a>
    if(do_free){
    80001220:	fe0a80e3          	beqz	s5,80001200 <uvmunmap+0x46>
      uint64 pa = PTE2PA(*pte);
    80001224:	83a9                	srli	a5,a5,0xa
      kfree((void*)pa);
    80001226:	00c79513          	slli	a0,a5,0xc
    8000122a:	ff2ff0ef          	jal	80000a1c <kfree>
    8000122e:	bfc9                	j	80001200 <uvmunmap+0x46>
    80001230:	74a2                	ld	s1,40(sp)
    80001232:	7902                	ld	s2,32(sp)
    80001234:	69e2                	ld	s3,24(sp)
    80001236:	6a42                	ld	s4,16(sp)
    80001238:	6aa2                	ld	s5,8(sp)
    8000123a:	6b02                	ld	s6,0(sp)
  }
}
    8000123c:	70e2                	ld	ra,56(sp)
    8000123e:	7442                	ld	s0,48(sp)
    80001240:	6121                	addi	sp,sp,64
    80001242:	8082                	ret

0000000080001244 <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    80001244:	1101                	addi	sp,sp,-32
    80001246:	ec06                	sd	ra,24(sp)
    80001248:	e822                	sd	s0,16(sp)
    8000124a:	e426                	sd	s1,8(sp)
    8000124c:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    8000124e:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    80001250:	00b67d63          	bgeu	a2,a1,8000126a <uvmdealloc+0x26>
    80001254:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    80001256:	6785                	lui	a5,0x1
    80001258:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    8000125a:	00f60733          	add	a4,a2,a5
    8000125e:	76fd                	lui	a3,0xfffff
    80001260:	8f75                	and	a4,a4,a3
    80001262:	97ae                	add	a5,a5,a1
    80001264:	8ff5                	and	a5,a5,a3
    80001266:	00f76863          	bltu	a4,a5,80001276 <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    8000126a:	8526                	mv	a0,s1
    8000126c:	60e2                	ld	ra,24(sp)
    8000126e:	6442                	ld	s0,16(sp)
    80001270:	64a2                	ld	s1,8(sp)
    80001272:	6105                	addi	sp,sp,32
    80001274:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    80001276:	8f99                	sub	a5,a5,a4
    80001278:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    8000127a:	4685                	li	a3,1
    8000127c:	0007861b          	sext.w	a2,a5
    80001280:	85ba                	mv	a1,a4
    80001282:	f39ff0ef          	jal	800011ba <uvmunmap>
    80001286:	b7d5                	j	8000126a <uvmdealloc+0x26>

0000000080001288 <uvmalloc>:
  if(newsz < oldsz)
    80001288:	08b66f63          	bltu	a2,a1,80001326 <uvmalloc+0x9e>
{
    8000128c:	7139                	addi	sp,sp,-64
    8000128e:	fc06                	sd	ra,56(sp)
    80001290:	f822                	sd	s0,48(sp)
    80001292:	ec4e                	sd	s3,24(sp)
    80001294:	e852                	sd	s4,16(sp)
    80001296:	e456                	sd	s5,8(sp)
    80001298:	0080                	addi	s0,sp,64
    8000129a:	8aaa                	mv	s5,a0
    8000129c:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    8000129e:	6785                	lui	a5,0x1
    800012a0:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800012a2:	95be                	add	a1,a1,a5
    800012a4:	77fd                	lui	a5,0xfffff
    800012a6:	00f5f9b3          	and	s3,a1,a5
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012aa:	08c9f063          	bgeu	s3,a2,8000132a <uvmalloc+0xa2>
    800012ae:	f426                	sd	s1,40(sp)
    800012b0:	f04a                	sd	s2,32(sp)
    800012b2:	e05a                	sd	s6,0(sp)
    800012b4:	894e                	mv	s2,s3
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012b6:	0126eb13          	ori	s6,a3,18
    mem = kalloc();
    800012ba:	845ff0ef          	jal	80000afe <kalloc>
    800012be:	84aa                	mv	s1,a0
    if(mem == 0){
    800012c0:	c515                	beqz	a0,800012ec <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    800012c2:	6605                	lui	a2,0x1
    800012c4:	4581                	li	a1,0
    800012c6:	9ddff0ef          	jal	80000ca2 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012ca:	875a                	mv	a4,s6
    800012cc:	86a6                	mv	a3,s1
    800012ce:	6605                	lui	a2,0x1
    800012d0:	85ca                	mv	a1,s2
    800012d2:	8556                	mv	a0,s5
    800012d4:	d1bff0ef          	jal	80000fee <mappages>
    800012d8:	e915                	bnez	a0,8000130c <uvmalloc+0x84>
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012da:	6785                	lui	a5,0x1
    800012dc:	993e                	add	s2,s2,a5
    800012de:	fd496ee3          	bltu	s2,s4,800012ba <uvmalloc+0x32>
  return newsz;
    800012e2:	8552                	mv	a0,s4
    800012e4:	74a2                	ld	s1,40(sp)
    800012e6:	7902                	ld	s2,32(sp)
    800012e8:	6b02                	ld	s6,0(sp)
    800012ea:	a811                	j	800012fe <uvmalloc+0x76>
      uvmdealloc(pagetable, a, oldsz);
    800012ec:	864e                	mv	a2,s3
    800012ee:	85ca                	mv	a1,s2
    800012f0:	8556                	mv	a0,s5
    800012f2:	f53ff0ef          	jal	80001244 <uvmdealloc>
      return 0;
    800012f6:	4501                	li	a0,0
    800012f8:	74a2                	ld	s1,40(sp)
    800012fa:	7902                	ld	s2,32(sp)
    800012fc:	6b02                	ld	s6,0(sp)
}
    800012fe:	70e2                	ld	ra,56(sp)
    80001300:	7442                	ld	s0,48(sp)
    80001302:	69e2                	ld	s3,24(sp)
    80001304:	6a42                	ld	s4,16(sp)
    80001306:	6aa2                	ld	s5,8(sp)
    80001308:	6121                	addi	sp,sp,64
    8000130a:	8082                	ret
      kfree(mem);
    8000130c:	8526                	mv	a0,s1
    8000130e:	f0eff0ef          	jal	80000a1c <kfree>
      uvmdealloc(pagetable, a, oldsz);
    80001312:	864e                	mv	a2,s3
    80001314:	85ca                	mv	a1,s2
    80001316:	8556                	mv	a0,s5
    80001318:	f2dff0ef          	jal	80001244 <uvmdealloc>
      return 0;
    8000131c:	4501                	li	a0,0
    8000131e:	74a2                	ld	s1,40(sp)
    80001320:	7902                	ld	s2,32(sp)
    80001322:	6b02                	ld	s6,0(sp)
    80001324:	bfe9                	j	800012fe <uvmalloc+0x76>
    return oldsz;
    80001326:	852e                	mv	a0,a1
}
    80001328:	8082                	ret
  return newsz;
    8000132a:	8532                	mv	a0,a2
    8000132c:	bfc9                	j	800012fe <uvmalloc+0x76>

000000008000132e <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    8000132e:	7179                	addi	sp,sp,-48
    80001330:	f406                	sd	ra,40(sp)
    80001332:	f022                	sd	s0,32(sp)
    80001334:	ec26                	sd	s1,24(sp)
    80001336:	e84a                	sd	s2,16(sp)
    80001338:	e44e                	sd	s3,8(sp)
    8000133a:	e052                	sd	s4,0(sp)
    8000133c:	1800                	addi	s0,sp,48
    8000133e:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    80001340:	84aa                	mv	s1,a0
    80001342:	6905                	lui	s2,0x1
    80001344:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001346:	4985                	li	s3,1
    80001348:	a819                	j	8000135e <freewalk+0x30>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    8000134a:	83a9                	srli	a5,a5,0xa
      freewalk((pagetable_t)child);
    8000134c:	00c79513          	slli	a0,a5,0xc
    80001350:	fdfff0ef          	jal	8000132e <freewalk>
      pagetable[i] = 0;
    80001354:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    80001358:	04a1                	addi	s1,s1,8
    8000135a:	01248f63          	beq	s1,s2,80001378 <freewalk+0x4a>
    pte_t pte = pagetable[i];
    8000135e:	609c                	ld	a5,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001360:	00f7f713          	andi	a4,a5,15
    80001364:	ff3703e3          	beq	a4,s3,8000134a <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001368:	8b85                	andi	a5,a5,1
    8000136a:	d7fd                	beqz	a5,80001358 <freewalk+0x2a>
      panic("freewalk: leaf");
    8000136c:	00006517          	auipc	a0,0x6
    80001370:	dcc50513          	addi	a0,a0,-564 # 80007138 <etext+0x138>
    80001374:	c6cff0ef          	jal	800007e0 <panic>
    }
  }
  kfree((void*)pagetable);
    80001378:	8552                	mv	a0,s4
    8000137a:	ea2ff0ef          	jal	80000a1c <kfree>
}
    8000137e:	70a2                	ld	ra,40(sp)
    80001380:	7402                	ld	s0,32(sp)
    80001382:	64e2                	ld	s1,24(sp)
    80001384:	6942                	ld	s2,16(sp)
    80001386:	69a2                	ld	s3,8(sp)
    80001388:	6a02                	ld	s4,0(sp)
    8000138a:	6145                	addi	sp,sp,48
    8000138c:	8082                	ret

000000008000138e <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    8000138e:	1101                	addi	sp,sp,-32
    80001390:	ec06                	sd	ra,24(sp)
    80001392:	e822                	sd	s0,16(sp)
    80001394:	e426                	sd	s1,8(sp)
    80001396:	1000                	addi	s0,sp,32
    80001398:	84aa                	mv	s1,a0
  if(sz > 0)
    8000139a:	e989                	bnez	a1,800013ac <uvmfree+0x1e>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    8000139c:	8526                	mv	a0,s1
    8000139e:	f91ff0ef          	jal	8000132e <freewalk>
}
    800013a2:	60e2                	ld	ra,24(sp)
    800013a4:	6442                	ld	s0,16(sp)
    800013a6:	64a2                	ld	s1,8(sp)
    800013a8:	6105                	addi	sp,sp,32
    800013aa:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    800013ac:	6785                	lui	a5,0x1
    800013ae:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800013b0:	95be                	add	a1,a1,a5
    800013b2:	4685                	li	a3,1
    800013b4:	00c5d613          	srli	a2,a1,0xc
    800013b8:	4581                	li	a1,0
    800013ba:	e01ff0ef          	jal	800011ba <uvmunmap>
    800013be:	bff9                	j	8000139c <uvmfree+0xe>

00000000800013c0 <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    800013c0:	ce49                	beqz	a2,8000145a <uvmcopy+0x9a>
{
    800013c2:	715d                	addi	sp,sp,-80
    800013c4:	e486                	sd	ra,72(sp)
    800013c6:	e0a2                	sd	s0,64(sp)
    800013c8:	fc26                	sd	s1,56(sp)
    800013ca:	f84a                	sd	s2,48(sp)
    800013cc:	f44e                	sd	s3,40(sp)
    800013ce:	f052                	sd	s4,32(sp)
    800013d0:	ec56                	sd	s5,24(sp)
    800013d2:	e85a                	sd	s6,16(sp)
    800013d4:	e45e                	sd	s7,8(sp)
    800013d6:	0880                	addi	s0,sp,80
    800013d8:	8aaa                	mv	s5,a0
    800013da:	8b2e                	mv	s6,a1
    800013dc:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    800013de:	4481                	li	s1,0
    800013e0:	a029                	j	800013ea <uvmcopy+0x2a>
    800013e2:	6785                	lui	a5,0x1
    800013e4:	94be                	add	s1,s1,a5
    800013e6:	0544fe63          	bgeu	s1,s4,80001442 <uvmcopy+0x82>
    if((pte = walk(old, i, 0)) == 0)
    800013ea:	4601                	li	a2,0
    800013ec:	85a6                	mv	a1,s1
    800013ee:	8556                	mv	a0,s5
    800013f0:	b27ff0ef          	jal	80000f16 <walk>
    800013f4:	d57d                	beqz	a0,800013e2 <uvmcopy+0x22>
      continue;   // page table entry hasn't been allocated
    if((*pte & PTE_V) == 0)
    800013f6:	6118                	ld	a4,0(a0)
    800013f8:	00177793          	andi	a5,a4,1
    800013fc:	d3fd                	beqz	a5,800013e2 <uvmcopy+0x22>
      continue;   // physical page hasn't been allocated
    pa = PTE2PA(*pte);
    800013fe:	00a75593          	srli	a1,a4,0xa
    80001402:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    80001406:	3ff77913          	andi	s2,a4,1023
    if((mem = kalloc()) == 0)
    8000140a:	ef4ff0ef          	jal	80000afe <kalloc>
    8000140e:	89aa                	mv	s3,a0
    80001410:	c105                	beqz	a0,80001430 <uvmcopy+0x70>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    80001412:	6605                	lui	a2,0x1
    80001414:	85de                	mv	a1,s7
    80001416:	8e9ff0ef          	jal	80000cfe <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    8000141a:	874a                	mv	a4,s2
    8000141c:	86ce                	mv	a3,s3
    8000141e:	6605                	lui	a2,0x1
    80001420:	85a6                	mv	a1,s1
    80001422:	855a                	mv	a0,s6
    80001424:	bcbff0ef          	jal	80000fee <mappages>
    80001428:	dd4d                	beqz	a0,800013e2 <uvmcopy+0x22>
      kfree(mem);
    8000142a:	854e                	mv	a0,s3
    8000142c:	df0ff0ef          	jal	80000a1c <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    80001430:	4685                	li	a3,1
    80001432:	00c4d613          	srli	a2,s1,0xc
    80001436:	4581                	li	a1,0
    80001438:	855a                	mv	a0,s6
    8000143a:	d81ff0ef          	jal	800011ba <uvmunmap>
  return -1;
    8000143e:	557d                	li	a0,-1
    80001440:	a011                	j	80001444 <uvmcopy+0x84>
  return 0;
    80001442:	4501                	li	a0,0
}
    80001444:	60a6                	ld	ra,72(sp)
    80001446:	6406                	ld	s0,64(sp)
    80001448:	74e2                	ld	s1,56(sp)
    8000144a:	7942                	ld	s2,48(sp)
    8000144c:	79a2                	ld	s3,40(sp)
    8000144e:	7a02                	ld	s4,32(sp)
    80001450:	6ae2                	ld	s5,24(sp)
    80001452:	6b42                	ld	s6,16(sp)
    80001454:	6ba2                	ld	s7,8(sp)
    80001456:	6161                	addi	sp,sp,80
    80001458:	8082                	ret
  return 0;
    8000145a:	4501                	li	a0,0
}
    8000145c:	8082                	ret

000000008000145e <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    8000145e:	1141                	addi	sp,sp,-16
    80001460:	e406                	sd	ra,8(sp)
    80001462:	e022                	sd	s0,0(sp)
    80001464:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001466:	4601                	li	a2,0
    80001468:	aafff0ef          	jal	80000f16 <walk>
  if(pte == 0)
    8000146c:	c901                	beqz	a0,8000147c <uvmclear+0x1e>
    panic("uvmclear");
  *pte &= ~PTE_U;
    8000146e:	611c                	ld	a5,0(a0)
    80001470:	9bbd                	andi	a5,a5,-17
    80001472:	e11c                	sd	a5,0(a0)
}
    80001474:	60a2                	ld	ra,8(sp)
    80001476:	6402                	ld	s0,0(sp)
    80001478:	0141                	addi	sp,sp,16
    8000147a:	8082                	ret
    panic("uvmclear");
    8000147c:	00006517          	auipc	a0,0x6
    80001480:	ccc50513          	addi	a0,a0,-820 # 80007148 <etext+0x148>
    80001484:	b5cff0ef          	jal	800007e0 <panic>

0000000080001488 <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    80001488:	c6dd                	beqz	a3,80001536 <copyinstr+0xae>
{
    8000148a:	715d                	addi	sp,sp,-80
    8000148c:	e486                	sd	ra,72(sp)
    8000148e:	e0a2                	sd	s0,64(sp)
    80001490:	fc26                	sd	s1,56(sp)
    80001492:	f84a                	sd	s2,48(sp)
    80001494:	f44e                	sd	s3,40(sp)
    80001496:	f052                	sd	s4,32(sp)
    80001498:	ec56                	sd	s5,24(sp)
    8000149a:	e85a                	sd	s6,16(sp)
    8000149c:	e45e                	sd	s7,8(sp)
    8000149e:	0880                	addi	s0,sp,80
    800014a0:	8a2a                	mv	s4,a0
    800014a2:	8b2e                	mv	s6,a1
    800014a4:	8bb2                	mv	s7,a2
    800014a6:	8936                	mv	s2,a3
    va0 = PGROUNDDOWN(srcva);
    800014a8:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800014aa:	6985                	lui	s3,0x1
    800014ac:	a825                	j	800014e4 <copyinstr+0x5c>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800014ae:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800014b2:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800014b4:	37fd                	addiw	a5,a5,-1
    800014b6:	0007851b          	sext.w	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800014ba:	60a6                	ld	ra,72(sp)
    800014bc:	6406                	ld	s0,64(sp)
    800014be:	74e2                	ld	s1,56(sp)
    800014c0:	7942                	ld	s2,48(sp)
    800014c2:	79a2                	ld	s3,40(sp)
    800014c4:	7a02                	ld	s4,32(sp)
    800014c6:	6ae2                	ld	s5,24(sp)
    800014c8:	6b42                	ld	s6,16(sp)
    800014ca:	6ba2                	ld	s7,8(sp)
    800014cc:	6161                	addi	sp,sp,80
    800014ce:	8082                	ret
    800014d0:	fff90713          	addi	a4,s2,-1 # fff <_entry-0x7ffff001>
    800014d4:	9742                	add	a4,a4,a6
      --max;
    800014d6:	40b70933          	sub	s2,a4,a1
    srcva = va0 + PGSIZE;
    800014da:	01348bb3          	add	s7,s1,s3
  while(got_null == 0 && max > 0){
    800014de:	04e58463          	beq	a1,a4,80001526 <copyinstr+0x9e>
{
    800014e2:	8b3e                	mv	s6,a5
    va0 = PGROUNDDOWN(srcva);
    800014e4:	015bf4b3          	and	s1,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800014e8:	85a6                	mv	a1,s1
    800014ea:	8552                	mv	a0,s4
    800014ec:	ac5ff0ef          	jal	80000fb0 <walkaddr>
    if(pa0 == 0)
    800014f0:	cd0d                	beqz	a0,8000152a <copyinstr+0xa2>
    n = PGSIZE - (srcva - va0);
    800014f2:	417486b3          	sub	a3,s1,s7
    800014f6:	96ce                	add	a3,a3,s3
    if(n > max)
    800014f8:	00d97363          	bgeu	s2,a3,800014fe <copyinstr+0x76>
    800014fc:	86ca                	mv	a3,s2
    char *p = (char *) (pa0 + (srcva - va0));
    800014fe:	955e                	add	a0,a0,s7
    80001500:	8d05                	sub	a0,a0,s1
    while(n > 0){
    80001502:	c695                	beqz	a3,8000152e <copyinstr+0xa6>
    80001504:	87da                	mv	a5,s6
    80001506:	885a                	mv	a6,s6
      if(*p == '\0'){
    80001508:	41650633          	sub	a2,a0,s6
    while(n > 0){
    8000150c:	96da                	add	a3,a3,s6
    8000150e:	85be                	mv	a1,a5
      if(*p == '\0'){
    80001510:	00f60733          	add	a4,a2,a5
    80001514:	00074703          	lbu	a4,0(a4)
    80001518:	db59                	beqz	a4,800014ae <copyinstr+0x26>
        *dst = *p;
    8000151a:	00e78023          	sb	a4,0(a5)
      dst++;
    8000151e:	0785                	addi	a5,a5,1
    while(n > 0){
    80001520:	fed797e3          	bne	a5,a3,8000150e <copyinstr+0x86>
    80001524:	b775                	j	800014d0 <copyinstr+0x48>
    80001526:	4781                	li	a5,0
    80001528:	b771                	j	800014b4 <copyinstr+0x2c>
      return -1;
    8000152a:	557d                	li	a0,-1
    8000152c:	b779                	j	800014ba <copyinstr+0x32>
    srcva = va0 + PGSIZE;
    8000152e:	6b85                	lui	s7,0x1
    80001530:	9ba6                	add	s7,s7,s1
    80001532:	87da                	mv	a5,s6
    80001534:	b77d                	j	800014e2 <copyinstr+0x5a>
  int got_null = 0;
    80001536:	4781                	li	a5,0
  if(got_null){
    80001538:	37fd                	addiw	a5,a5,-1
    8000153a:	0007851b          	sext.w	a0,a5
}
    8000153e:	8082                	ret

0000000080001540 <ismapped>:
  return mem;
}

int
ismapped(pagetable_t pagetable, uint64 va)
{
    80001540:	1141                	addi	sp,sp,-16
    80001542:	e406                	sd	ra,8(sp)
    80001544:	e022                	sd	s0,0(sp)
    80001546:	0800                	addi	s0,sp,16
  pte_t *pte = walk(pagetable, va, 0);
    80001548:	4601                	li	a2,0
    8000154a:	9cdff0ef          	jal	80000f16 <walk>
  if (pte == 0) {
    8000154e:	c519                	beqz	a0,8000155c <ismapped+0x1c>
    return 0;
  }
  if (*pte & PTE_V){
    80001550:	6108                	ld	a0,0(a0)
    80001552:	8905                	andi	a0,a0,1
    return 1;
  }
  return 0;
}
    80001554:	60a2                	ld	ra,8(sp)
    80001556:	6402                	ld	s0,0(sp)
    80001558:	0141                	addi	sp,sp,16
    8000155a:	8082                	ret
    return 0;
    8000155c:	4501                	li	a0,0
    8000155e:	bfdd                	j	80001554 <ismapped+0x14>

0000000080001560 <vmfault>:
{
    80001560:	7179                	addi	sp,sp,-48
    80001562:	f406                	sd	ra,40(sp)
    80001564:	f022                	sd	s0,32(sp)
    80001566:	ec26                	sd	s1,24(sp)
    80001568:	e44e                	sd	s3,8(sp)
    8000156a:	1800                	addi	s0,sp,48
    8000156c:	89aa                	mv	s3,a0
    8000156e:	84ae                	mv	s1,a1
  struct proc *p = myproc();
    80001570:	35e000ef          	jal	800018ce <myproc>
  if (va >= p->sz)
    80001574:	653c                	ld	a5,72(a0)
    80001576:	00f4ea63          	bltu	s1,a5,8000158a <vmfault+0x2a>
    return 0;
    8000157a:	4981                	li	s3,0
}
    8000157c:	854e                	mv	a0,s3
    8000157e:	70a2                	ld	ra,40(sp)
    80001580:	7402                	ld	s0,32(sp)
    80001582:	64e2                	ld	s1,24(sp)
    80001584:	69a2                	ld	s3,8(sp)
    80001586:	6145                	addi	sp,sp,48
    80001588:	8082                	ret
    8000158a:	e84a                	sd	s2,16(sp)
    8000158c:	892a                	mv	s2,a0
  va = PGROUNDDOWN(va);
    8000158e:	77fd                	lui	a5,0xfffff
    80001590:	8cfd                	and	s1,s1,a5
  if(ismapped(pagetable, va)) {
    80001592:	85a6                	mv	a1,s1
    80001594:	854e                	mv	a0,s3
    80001596:	fabff0ef          	jal	80001540 <ismapped>
    return 0;
    8000159a:	4981                	li	s3,0
  if(ismapped(pagetable, va)) {
    8000159c:	c119                	beqz	a0,800015a2 <vmfault+0x42>
    8000159e:	6942                	ld	s2,16(sp)
    800015a0:	bff1                	j	8000157c <vmfault+0x1c>
    800015a2:	e052                	sd	s4,0(sp)
  mem = (uint64) kalloc();
    800015a4:	d5aff0ef          	jal	80000afe <kalloc>
    800015a8:	8a2a                	mv	s4,a0
  if(mem == 0)
    800015aa:	c90d                	beqz	a0,800015dc <vmfault+0x7c>
  mem = (uint64) kalloc();
    800015ac:	89aa                	mv	s3,a0
  memset((void *) mem, 0, PGSIZE);
    800015ae:	6605                	lui	a2,0x1
    800015b0:	4581                	li	a1,0
    800015b2:	ef0ff0ef          	jal	80000ca2 <memset>
  if (mappages(p->pagetable, va, PGSIZE, mem, PTE_W|PTE_U|PTE_R) != 0) {
    800015b6:	4759                	li	a4,22
    800015b8:	86d2                	mv	a3,s4
    800015ba:	6605                	lui	a2,0x1
    800015bc:	85a6                	mv	a1,s1
    800015be:	05093503          	ld	a0,80(s2)
    800015c2:	a2dff0ef          	jal	80000fee <mappages>
    800015c6:	e501                	bnez	a0,800015ce <vmfault+0x6e>
    800015c8:	6942                	ld	s2,16(sp)
    800015ca:	6a02                	ld	s4,0(sp)
    800015cc:	bf45                	j	8000157c <vmfault+0x1c>
    kfree((void *)mem);
    800015ce:	8552                	mv	a0,s4
    800015d0:	c4cff0ef          	jal	80000a1c <kfree>
    return 0;
    800015d4:	4981                	li	s3,0
    800015d6:	6942                	ld	s2,16(sp)
    800015d8:	6a02                	ld	s4,0(sp)
    800015da:	b74d                	j	8000157c <vmfault+0x1c>
    800015dc:	6942                	ld	s2,16(sp)
    800015de:	6a02                	ld	s4,0(sp)
    800015e0:	bf71                	j	8000157c <vmfault+0x1c>

00000000800015e2 <copyout>:
  while(len > 0){
    800015e2:	c2cd                	beqz	a3,80001684 <copyout+0xa2>
{
    800015e4:	711d                	addi	sp,sp,-96
    800015e6:	ec86                	sd	ra,88(sp)
    800015e8:	e8a2                	sd	s0,80(sp)
    800015ea:	e4a6                	sd	s1,72(sp)
    800015ec:	f852                	sd	s4,48(sp)
    800015ee:	f05a                	sd	s6,32(sp)
    800015f0:	ec5e                	sd	s7,24(sp)
    800015f2:	e862                	sd	s8,16(sp)
    800015f4:	1080                	addi	s0,sp,96
    800015f6:	8c2a                	mv	s8,a0
    800015f8:	8b2e                	mv	s6,a1
    800015fa:	8bb2                	mv	s7,a2
    800015fc:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(dstva);
    800015fe:	74fd                	lui	s1,0xfffff
    80001600:	8ced                	and	s1,s1,a1
    if(va0 >= MAXVA)
    80001602:	57fd                	li	a5,-1
    80001604:	83e9                	srli	a5,a5,0x1a
    80001606:	0897e163          	bltu	a5,s1,80001688 <copyout+0xa6>
    8000160a:	e0ca                	sd	s2,64(sp)
    8000160c:	fc4e                	sd	s3,56(sp)
    8000160e:	f456                	sd	s5,40(sp)
    80001610:	e466                	sd	s9,8(sp)
    80001612:	e06a                	sd	s10,0(sp)
    80001614:	6d05                	lui	s10,0x1
    80001616:	8cbe                	mv	s9,a5
    80001618:	a015                	j	8000163c <copyout+0x5a>
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    8000161a:	409b0533          	sub	a0,s6,s1
    8000161e:	0009861b          	sext.w	a2,s3
    80001622:	85de                	mv	a1,s7
    80001624:	954a                	add	a0,a0,s2
    80001626:	ed8ff0ef          	jal	80000cfe <memmove>
    len -= n;
    8000162a:	413a0a33          	sub	s4,s4,s3
    src += n;
    8000162e:	9bce                	add	s7,s7,s3
  while(len > 0){
    80001630:	040a0363          	beqz	s4,80001676 <copyout+0x94>
    if(va0 >= MAXVA)
    80001634:	055cec63          	bltu	s9,s5,8000168c <copyout+0xaa>
    80001638:	84d6                	mv	s1,s5
    8000163a:	8b56                	mv	s6,s5
    pa0 = walkaddr(pagetable, va0);
    8000163c:	85a6                	mv	a1,s1
    8000163e:	8562                	mv	a0,s8
    80001640:	971ff0ef          	jal	80000fb0 <walkaddr>
    80001644:	892a                	mv	s2,a0
    if(pa0 == 0) {
    80001646:	e901                	bnez	a0,80001656 <copyout+0x74>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001648:	4601                	li	a2,0
    8000164a:	85a6                	mv	a1,s1
    8000164c:	8562                	mv	a0,s8
    8000164e:	f13ff0ef          	jal	80001560 <vmfault>
    80001652:	892a                	mv	s2,a0
    80001654:	c139                	beqz	a0,8000169a <copyout+0xb8>
    pte = walk(pagetable, va0, 0);
    80001656:	4601                	li	a2,0
    80001658:	85a6                	mv	a1,s1
    8000165a:	8562                	mv	a0,s8
    8000165c:	8bbff0ef          	jal	80000f16 <walk>
    if((*pte & PTE_W) == 0)
    80001660:	611c                	ld	a5,0(a0)
    80001662:	8b91                	andi	a5,a5,4
    80001664:	c3b1                	beqz	a5,800016a8 <copyout+0xc6>
    n = PGSIZE - (dstva - va0);
    80001666:	01a48ab3          	add	s5,s1,s10
    8000166a:	416a89b3          	sub	s3,s5,s6
    if(n > len)
    8000166e:	fb3a76e3          	bgeu	s4,s3,8000161a <copyout+0x38>
    80001672:	89d2                	mv	s3,s4
    80001674:	b75d                	j	8000161a <copyout+0x38>
  return 0;
    80001676:	4501                	li	a0,0
    80001678:	6906                	ld	s2,64(sp)
    8000167a:	79e2                	ld	s3,56(sp)
    8000167c:	7aa2                	ld	s5,40(sp)
    8000167e:	6ca2                	ld	s9,8(sp)
    80001680:	6d02                	ld	s10,0(sp)
    80001682:	a80d                	j	800016b4 <copyout+0xd2>
    80001684:	4501                	li	a0,0
}
    80001686:	8082                	ret
      return -1;
    80001688:	557d                	li	a0,-1
    8000168a:	a02d                	j	800016b4 <copyout+0xd2>
    8000168c:	557d                	li	a0,-1
    8000168e:	6906                	ld	s2,64(sp)
    80001690:	79e2                	ld	s3,56(sp)
    80001692:	7aa2                	ld	s5,40(sp)
    80001694:	6ca2                	ld	s9,8(sp)
    80001696:	6d02                	ld	s10,0(sp)
    80001698:	a831                	j	800016b4 <copyout+0xd2>
        return -1;
    8000169a:	557d                	li	a0,-1
    8000169c:	6906                	ld	s2,64(sp)
    8000169e:	79e2                	ld	s3,56(sp)
    800016a0:	7aa2                	ld	s5,40(sp)
    800016a2:	6ca2                	ld	s9,8(sp)
    800016a4:	6d02                	ld	s10,0(sp)
    800016a6:	a039                	j	800016b4 <copyout+0xd2>
      return -1;
    800016a8:	557d                	li	a0,-1
    800016aa:	6906                	ld	s2,64(sp)
    800016ac:	79e2                	ld	s3,56(sp)
    800016ae:	7aa2                	ld	s5,40(sp)
    800016b0:	6ca2                	ld	s9,8(sp)
    800016b2:	6d02                	ld	s10,0(sp)
}
    800016b4:	60e6                	ld	ra,88(sp)
    800016b6:	6446                	ld	s0,80(sp)
    800016b8:	64a6                	ld	s1,72(sp)
    800016ba:	7a42                	ld	s4,48(sp)
    800016bc:	7b02                	ld	s6,32(sp)
    800016be:	6be2                	ld	s7,24(sp)
    800016c0:	6c42                	ld	s8,16(sp)
    800016c2:	6125                	addi	sp,sp,96
    800016c4:	8082                	ret

00000000800016c6 <copyin>:
  while(len > 0){
    800016c6:	c6c9                	beqz	a3,80001750 <copyin+0x8a>
{
    800016c8:	715d                	addi	sp,sp,-80
    800016ca:	e486                	sd	ra,72(sp)
    800016cc:	e0a2                	sd	s0,64(sp)
    800016ce:	fc26                	sd	s1,56(sp)
    800016d0:	f84a                	sd	s2,48(sp)
    800016d2:	f44e                	sd	s3,40(sp)
    800016d4:	f052                	sd	s4,32(sp)
    800016d6:	ec56                	sd	s5,24(sp)
    800016d8:	e85a                	sd	s6,16(sp)
    800016da:	e45e                	sd	s7,8(sp)
    800016dc:	e062                	sd	s8,0(sp)
    800016de:	0880                	addi	s0,sp,80
    800016e0:	8baa                	mv	s7,a0
    800016e2:	8aae                	mv	s5,a1
    800016e4:	8932                	mv	s2,a2
    800016e6:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(srcva);
    800016e8:	7c7d                	lui	s8,0xfffff
    n = PGSIZE - (srcva - va0);
    800016ea:	6b05                	lui	s6,0x1
    800016ec:	a035                	j	80001718 <copyin+0x52>
    800016ee:	412984b3          	sub	s1,s3,s2
    800016f2:	94da                	add	s1,s1,s6
    if(n > len)
    800016f4:	009a7363          	bgeu	s4,s1,800016fa <copyin+0x34>
    800016f8:	84d2                	mv	s1,s4
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    800016fa:	413905b3          	sub	a1,s2,s3
    800016fe:	0004861b          	sext.w	a2,s1
    80001702:	95aa                	add	a1,a1,a0
    80001704:	8556                	mv	a0,s5
    80001706:	df8ff0ef          	jal	80000cfe <memmove>
    len -= n;
    8000170a:	409a0a33          	sub	s4,s4,s1
    dst += n;
    8000170e:	9aa6                	add	s5,s5,s1
    srcva = va0 + PGSIZE;
    80001710:	01698933          	add	s2,s3,s6
  while(len > 0){
    80001714:	020a0163          	beqz	s4,80001736 <copyin+0x70>
    va0 = PGROUNDDOWN(srcva);
    80001718:	018979b3          	and	s3,s2,s8
    pa0 = walkaddr(pagetable, va0);
    8000171c:	85ce                	mv	a1,s3
    8000171e:	855e                	mv	a0,s7
    80001720:	891ff0ef          	jal	80000fb0 <walkaddr>
    if(pa0 == 0) {
    80001724:	f569                	bnez	a0,800016ee <copyin+0x28>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001726:	4601                	li	a2,0
    80001728:	85ce                	mv	a1,s3
    8000172a:	855e                	mv	a0,s7
    8000172c:	e35ff0ef          	jal	80001560 <vmfault>
    80001730:	fd5d                	bnez	a0,800016ee <copyin+0x28>
        return -1;
    80001732:	557d                	li	a0,-1
    80001734:	a011                	j	80001738 <copyin+0x72>
  return 0;
    80001736:	4501                	li	a0,0
}
    80001738:	60a6                	ld	ra,72(sp)
    8000173a:	6406                	ld	s0,64(sp)
    8000173c:	74e2                	ld	s1,56(sp)
    8000173e:	7942                	ld	s2,48(sp)
    80001740:	79a2                	ld	s3,40(sp)
    80001742:	7a02                	ld	s4,32(sp)
    80001744:	6ae2                	ld	s5,24(sp)
    80001746:	6b42                	ld	s6,16(sp)
    80001748:	6ba2                	ld	s7,8(sp)
    8000174a:	6c02                	ld	s8,0(sp)
    8000174c:	6161                	addi	sp,sp,80
    8000174e:	8082                	ret
  return 0;
    80001750:	4501                	li	a0,0
}
    80001752:	8082                	ret

0000000080001754 <proc_mapstacks>:
// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl)
{
    80001754:	7139                	addi	sp,sp,-64
    80001756:	fc06                	sd	ra,56(sp)
    80001758:	f822                	sd	s0,48(sp)
    8000175a:	f426                	sd	s1,40(sp)
    8000175c:	f04a                	sd	s2,32(sp)
    8000175e:	ec4e                	sd	s3,24(sp)
    80001760:	e852                	sd	s4,16(sp)
    80001762:	e456                	sd	s5,8(sp)
    80001764:	e05a                	sd	s6,0(sp)
    80001766:	0080                	addi	s0,sp,64
    80001768:	8a2a                	mv	s4,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    8000176a:	0000e497          	auipc	s1,0xe
    8000176e:	61e48493          	addi	s1,s1,1566 # 8000fd88 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    80001772:	8b26                	mv	s6,s1
    80001774:	00a36937          	lui	s2,0xa36
    80001778:	77d90913          	addi	s2,s2,1917 # a3677d <_entry-0x7f5c9883>
    8000177c:	0932                	slli	s2,s2,0xc
    8000177e:	46d90913          	addi	s2,s2,1133
    80001782:	0936                	slli	s2,s2,0xd
    80001784:	df590913          	addi	s2,s2,-523
    80001788:	093a                	slli	s2,s2,0xe
    8000178a:	6cf90913          	addi	s2,s2,1743
    8000178e:	040009b7          	lui	s3,0x4000
    80001792:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    80001794:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001796:	00014a97          	auipc	s5,0x14
    8000179a:	3f2a8a93          	addi	s5,s5,1010 # 80015b88 <tickslock>
    char *pa = kalloc();
    8000179e:	b60ff0ef          	jal	80000afe <kalloc>
    800017a2:	862a                	mv	a2,a0
    if(pa == 0)
    800017a4:	cd15                	beqz	a0,800017e0 <proc_mapstacks+0x8c>
    uint64 va = KSTACK((int) (p - proc));
    800017a6:	416485b3          	sub	a1,s1,s6
    800017aa:	858d                	srai	a1,a1,0x3
    800017ac:	032585b3          	mul	a1,a1,s2
    800017b0:	2585                	addiw	a1,a1,1
    800017b2:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    800017b6:	4719                	li	a4,6
    800017b8:	6685                	lui	a3,0x1
    800017ba:	40b985b3          	sub	a1,s3,a1
    800017be:	8552                	mv	a0,s4
    800017c0:	8dfff0ef          	jal	8000109e <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    800017c4:	17848493          	addi	s1,s1,376
    800017c8:	fd549be3          	bne	s1,s5,8000179e <proc_mapstacks+0x4a>
  }
}
    800017cc:	70e2                	ld	ra,56(sp)
    800017ce:	7442                	ld	s0,48(sp)
    800017d0:	74a2                	ld	s1,40(sp)
    800017d2:	7902                	ld	s2,32(sp)
    800017d4:	69e2                	ld	s3,24(sp)
    800017d6:	6a42                	ld	s4,16(sp)
    800017d8:	6aa2                	ld	s5,8(sp)
    800017da:	6b02                	ld	s6,0(sp)
    800017dc:	6121                	addi	sp,sp,64
    800017de:	8082                	ret
      panic("kalloc");
    800017e0:	00006517          	auipc	a0,0x6
    800017e4:	97850513          	addi	a0,a0,-1672 # 80007158 <etext+0x158>
    800017e8:	ff9fe0ef          	jal	800007e0 <panic>

00000000800017ec <procinit>:

// initialize the proc table.
void
procinit(void)
{
    800017ec:	7139                	addi	sp,sp,-64
    800017ee:	fc06                	sd	ra,56(sp)
    800017f0:	f822                	sd	s0,48(sp)
    800017f2:	f426                	sd	s1,40(sp)
    800017f4:	f04a                	sd	s2,32(sp)
    800017f6:	ec4e                	sd	s3,24(sp)
    800017f8:	e852                	sd	s4,16(sp)
    800017fa:	e456                	sd	s5,8(sp)
    800017fc:	e05a                	sd	s6,0(sp)
    800017fe:	0080                	addi	s0,sp,64
  struct proc *p;
  
  initlock(&pid_lock, "nextpid");
    80001800:	00006597          	auipc	a1,0x6
    80001804:	96058593          	addi	a1,a1,-1696 # 80007160 <etext+0x160>
    80001808:	0000e517          	auipc	a0,0xe
    8000180c:	15050513          	addi	a0,a0,336 # 8000f958 <pid_lock>
    80001810:	b3eff0ef          	jal	80000b4e <initlock>
  initlock(&wait_lock, "wait_lock");
    80001814:	00006597          	auipc	a1,0x6
    80001818:	95458593          	addi	a1,a1,-1708 # 80007168 <etext+0x168>
    8000181c:	0000e517          	auipc	a0,0xe
    80001820:	15450513          	addi	a0,a0,340 # 8000f970 <wait_lock>
    80001824:	b2aff0ef          	jal	80000b4e <initlock>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001828:	0000e497          	auipc	s1,0xe
    8000182c:	56048493          	addi	s1,s1,1376 # 8000fd88 <proc>
      initlock(&p->lock, "proc");
    80001830:	00006b17          	auipc	s6,0x6
    80001834:	948b0b13          	addi	s6,s6,-1720 # 80007178 <etext+0x178>
      p->state = UNUSED;
      p->kstack = KSTACK((int) (p - proc));
    80001838:	8aa6                	mv	s5,s1
    8000183a:	00a36937          	lui	s2,0xa36
    8000183e:	77d90913          	addi	s2,s2,1917 # a3677d <_entry-0x7f5c9883>
    80001842:	0932                	slli	s2,s2,0xc
    80001844:	46d90913          	addi	s2,s2,1133
    80001848:	0936                	slli	s2,s2,0xd
    8000184a:	df590913          	addi	s2,s2,-523
    8000184e:	093a                	slli	s2,s2,0xe
    80001850:	6cf90913          	addi	s2,s2,1743
    80001854:	040009b7          	lui	s3,0x4000
    80001858:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    8000185a:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    8000185c:	00014a17          	auipc	s4,0x14
    80001860:	32ca0a13          	addi	s4,s4,812 # 80015b88 <tickslock>
      initlock(&p->lock, "proc");
    80001864:	85da                	mv	a1,s6
    80001866:	8526                	mv	a0,s1
    80001868:	ae6ff0ef          	jal	80000b4e <initlock>
      p->state = UNUSED;
    8000186c:	0004ac23          	sw	zero,24(s1)
      p->kstack = KSTACK((int) (p - proc));
    80001870:	415487b3          	sub	a5,s1,s5
    80001874:	878d                	srai	a5,a5,0x3
    80001876:	032787b3          	mul	a5,a5,s2
    8000187a:	2785                	addiw	a5,a5,1 # fffffffffffff001 <end+0xffffffff7ffde099>
    8000187c:	00d7979b          	slliw	a5,a5,0xd
    80001880:	40f987b3          	sub	a5,s3,a5
    80001884:	e0bc                	sd	a5,64(s1)
  for(p = proc; p < &proc[NPROC]; p++) {
    80001886:	17848493          	addi	s1,s1,376
    8000188a:	fd449de3          	bne	s1,s4,80001864 <procinit+0x78>
  }
}
    8000188e:	70e2                	ld	ra,56(sp)
    80001890:	7442                	ld	s0,48(sp)
    80001892:	74a2                	ld	s1,40(sp)
    80001894:	7902                	ld	s2,32(sp)
    80001896:	69e2                	ld	s3,24(sp)
    80001898:	6a42                	ld	s4,16(sp)
    8000189a:	6aa2                	ld	s5,8(sp)
    8000189c:	6b02                	ld	s6,0(sp)
    8000189e:	6121                	addi	sp,sp,64
    800018a0:	8082                	ret

00000000800018a2 <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    800018a2:	1141                	addi	sp,sp,-16
    800018a4:	e422                	sd	s0,8(sp)
    800018a6:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    800018a8:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    800018aa:	2501                	sext.w	a0,a0
    800018ac:	6422                	ld	s0,8(sp)
    800018ae:	0141                	addi	sp,sp,16
    800018b0:	8082                	ret

00000000800018b2 <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void)
{
    800018b2:	1141                	addi	sp,sp,-16
    800018b4:	e422                	sd	s0,8(sp)
    800018b6:	0800                	addi	s0,sp,16
    800018b8:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    800018ba:	2781                	sext.w	a5,a5
    800018bc:	079e                	slli	a5,a5,0x7
  return c;
}
    800018be:	0000e517          	auipc	a0,0xe
    800018c2:	0ca50513          	addi	a0,a0,202 # 8000f988 <cpus>
    800018c6:	953e                	add	a0,a0,a5
    800018c8:	6422                	ld	s0,8(sp)
    800018ca:	0141                	addi	sp,sp,16
    800018cc:	8082                	ret

00000000800018ce <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void)
{
    800018ce:	1101                	addi	sp,sp,-32
    800018d0:	ec06                	sd	ra,24(sp)
    800018d2:	e822                	sd	s0,16(sp)
    800018d4:	e426                	sd	s1,8(sp)
    800018d6:	1000                	addi	s0,sp,32
  push_off();
    800018d8:	ab6ff0ef          	jal	80000b8e <push_off>
    800018dc:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    800018de:	2781                	sext.w	a5,a5
    800018e0:	079e                	slli	a5,a5,0x7
    800018e2:	0000e717          	auipc	a4,0xe
    800018e6:	07670713          	addi	a4,a4,118 # 8000f958 <pid_lock>
    800018ea:	97ba                	add	a5,a5,a4
    800018ec:	7b84                	ld	s1,48(a5)
  pop_off();
    800018ee:	b24ff0ef          	jal	80000c12 <pop_off>
  return p;
}
    800018f2:	8526                	mv	a0,s1
    800018f4:	60e2                	ld	ra,24(sp)
    800018f6:	6442                	ld	s0,16(sp)
    800018f8:	64a2                	ld	s1,8(sp)
    800018fa:	6105                	addi	sp,sp,32
    800018fc:	8082                	ret

00000000800018fe <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    800018fe:	7179                	addi	sp,sp,-48
    80001900:	f406                	sd	ra,40(sp)
    80001902:	f022                	sd	s0,32(sp)
    80001904:	ec26                	sd	s1,24(sp)
    80001906:	1800                	addi	s0,sp,48
  extern char userret[];
  static int first = 1;
  struct proc *p = myproc();
    80001908:	fc7ff0ef          	jal	800018ce <myproc>
    8000190c:	84aa                	mv	s1,a0

  // Still holding p->lock from scheduler.
  release(&p->lock);
    8000190e:	b58ff0ef          	jal	80000c66 <release>

  if (first) {
    80001912:	00006797          	auipc	a5,0x6
    80001916:	f0e7a783          	lw	a5,-242(a5) # 80007820 <first.1>
    8000191a:	cf8d                	beqz	a5,80001954 <forkret+0x56>
    // File system initialization must be run in the context of a
    // regular process (e.g., because it calls sleep), and thus cannot
    // be run from main().
    fsinit(ROOTDEV);
    8000191c:	4505                	li	a0,1
    8000191e:	40d010ef          	jal	8000352a <fsinit>

    first = 0;
    80001922:	00006797          	auipc	a5,0x6
    80001926:	ee07af23          	sw	zero,-258(a5) # 80007820 <first.1>
    // ensure other cores see first=0.
    __sync_synchronize();
    8000192a:	0ff0000f          	fence

    // We can invoke kexec() now that file system is initialized.
    // Put the return value (argc) of kexec into a0.
    p->trapframe->a0 = kexec("/init", (char *[]){ "/init", 0 });
    8000192e:	00006517          	auipc	a0,0x6
    80001932:	85250513          	addi	a0,a0,-1966 # 80007180 <etext+0x180>
    80001936:	fca43823          	sd	a0,-48(s0)
    8000193a:	fc043c23          	sd	zero,-40(s0)
    8000193e:	fd040593          	addi	a1,s0,-48
    80001942:	4f3020ef          	jal	80004634 <kexec>
    80001946:	6cbc                	ld	a5,88(s1)
    80001948:	fba8                	sd	a0,112(a5)
    if (p->trapframe->a0 == -1) {
    8000194a:	6cbc                	ld	a5,88(s1)
    8000194c:	7bb8                	ld	a4,112(a5)
    8000194e:	57fd                	li	a5,-1
    80001950:	02f70d63          	beq	a4,a5,8000198a <forkret+0x8c>
      panic("exec");
    }
  }

  // return to user space, mimicing usertrap()'s return.
  prepare_return();
    80001954:	2cb000ef          	jal	8000241e <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    80001958:	68a8                	ld	a0,80(s1)
    8000195a:	8131                	srli	a0,a0,0xc
  uint64 trampoline_userret = TRAMPOLINE + (userret - trampoline);
    8000195c:	04000737          	lui	a4,0x4000
    80001960:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    80001962:	0732                	slli	a4,a4,0xc
    80001964:	00004797          	auipc	a5,0x4
    80001968:	73878793          	addi	a5,a5,1848 # 8000609c <userret>
    8000196c:	00004697          	auipc	a3,0x4
    80001970:	69468693          	addi	a3,a3,1684 # 80006000 <_trampoline>
    80001974:	8f95                	sub	a5,a5,a3
    80001976:	97ba                	add	a5,a5,a4
  ((void (*)(uint64))trampoline_userret)(satp);
    80001978:	577d                	li	a4,-1
    8000197a:	177e                	slli	a4,a4,0x3f
    8000197c:	8d59                	or	a0,a0,a4
    8000197e:	9782                	jalr	a5
}
    80001980:	70a2                	ld	ra,40(sp)
    80001982:	7402                	ld	s0,32(sp)
    80001984:	64e2                	ld	s1,24(sp)
    80001986:	6145                	addi	sp,sp,48
    80001988:	8082                	ret
      panic("exec");
    8000198a:	00005517          	auipc	a0,0x5
    8000198e:	7fe50513          	addi	a0,a0,2046 # 80007188 <etext+0x188>
    80001992:	e4ffe0ef          	jal	800007e0 <panic>

0000000080001996 <allocpid>:
{
    80001996:	1101                	addi	sp,sp,-32
    80001998:	ec06                	sd	ra,24(sp)
    8000199a:	e822                	sd	s0,16(sp)
    8000199c:	e426                	sd	s1,8(sp)
    8000199e:	e04a                	sd	s2,0(sp)
    800019a0:	1000                	addi	s0,sp,32
  acquire(&pid_lock);
    800019a2:	0000e917          	auipc	s2,0xe
    800019a6:	fb690913          	addi	s2,s2,-74 # 8000f958 <pid_lock>
    800019aa:	854a                	mv	a0,s2
    800019ac:	a22ff0ef          	jal	80000bce <acquire>
  pid = nextpid;
    800019b0:	00006797          	auipc	a5,0x6
    800019b4:	e7478793          	addi	a5,a5,-396 # 80007824 <nextpid>
    800019b8:	4384                	lw	s1,0(a5)
  nextpid = nextpid + 1;
    800019ba:	0014871b          	addiw	a4,s1,1
    800019be:	c398                	sw	a4,0(a5)
  release(&pid_lock);
    800019c0:	854a                	mv	a0,s2
    800019c2:	aa4ff0ef          	jal	80000c66 <release>
}
    800019c6:	8526                	mv	a0,s1
    800019c8:	60e2                	ld	ra,24(sp)
    800019ca:	6442                	ld	s0,16(sp)
    800019cc:	64a2                	ld	s1,8(sp)
    800019ce:	6902                	ld	s2,0(sp)
    800019d0:	6105                	addi	sp,sp,32
    800019d2:	8082                	ret

00000000800019d4 <proc_pagetable>:
{
    800019d4:	1101                	addi	sp,sp,-32
    800019d6:	ec06                	sd	ra,24(sp)
    800019d8:	e822                	sd	s0,16(sp)
    800019da:	e426                	sd	s1,8(sp)
    800019dc:	e04a                	sd	s2,0(sp)
    800019de:	1000                	addi	s0,sp,32
    800019e0:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    800019e2:	fb2ff0ef          	jal	80001194 <uvmcreate>
    800019e6:	84aa                	mv	s1,a0
  if(pagetable == 0)
    800019e8:	cd05                	beqz	a0,80001a20 <proc_pagetable+0x4c>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    800019ea:	4729                	li	a4,10
    800019ec:	00004697          	auipc	a3,0x4
    800019f0:	61468693          	addi	a3,a3,1556 # 80006000 <_trampoline>
    800019f4:	6605                	lui	a2,0x1
    800019f6:	040005b7          	lui	a1,0x4000
    800019fa:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    800019fc:	05b2                	slli	a1,a1,0xc
    800019fe:	df0ff0ef          	jal	80000fee <mappages>
    80001a02:	02054663          	bltz	a0,80001a2e <proc_pagetable+0x5a>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001a06:	4719                	li	a4,6
    80001a08:	05893683          	ld	a3,88(s2)
    80001a0c:	6605                	lui	a2,0x1
    80001a0e:	020005b7          	lui	a1,0x2000
    80001a12:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001a14:	05b6                	slli	a1,a1,0xd
    80001a16:	8526                	mv	a0,s1
    80001a18:	dd6ff0ef          	jal	80000fee <mappages>
    80001a1c:	00054f63          	bltz	a0,80001a3a <proc_pagetable+0x66>
}
    80001a20:	8526                	mv	a0,s1
    80001a22:	60e2                	ld	ra,24(sp)
    80001a24:	6442                	ld	s0,16(sp)
    80001a26:	64a2                	ld	s1,8(sp)
    80001a28:	6902                	ld	s2,0(sp)
    80001a2a:	6105                	addi	sp,sp,32
    80001a2c:	8082                	ret
    uvmfree(pagetable, 0);
    80001a2e:	4581                	li	a1,0
    80001a30:	8526                	mv	a0,s1
    80001a32:	95dff0ef          	jal	8000138e <uvmfree>
    return 0;
    80001a36:	4481                	li	s1,0
    80001a38:	b7e5                	j	80001a20 <proc_pagetable+0x4c>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001a3a:	4681                	li	a3,0
    80001a3c:	4605                	li	a2,1
    80001a3e:	040005b7          	lui	a1,0x4000
    80001a42:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a44:	05b2                	slli	a1,a1,0xc
    80001a46:	8526                	mv	a0,s1
    80001a48:	f72ff0ef          	jal	800011ba <uvmunmap>
    uvmfree(pagetable, 0);
    80001a4c:	4581                	li	a1,0
    80001a4e:	8526                	mv	a0,s1
    80001a50:	93fff0ef          	jal	8000138e <uvmfree>
    return 0;
    80001a54:	4481                	li	s1,0
    80001a56:	b7e9                	j	80001a20 <proc_pagetable+0x4c>

0000000080001a58 <proc_freepagetable>:
{
    80001a58:	1101                	addi	sp,sp,-32
    80001a5a:	ec06                	sd	ra,24(sp)
    80001a5c:	e822                	sd	s0,16(sp)
    80001a5e:	e426                	sd	s1,8(sp)
    80001a60:	e04a                	sd	s2,0(sp)
    80001a62:	1000                	addi	s0,sp,32
    80001a64:	84aa                	mv	s1,a0
    80001a66:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001a68:	4681                	li	a3,0
    80001a6a:	4605                	li	a2,1
    80001a6c:	040005b7          	lui	a1,0x4000
    80001a70:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a72:	05b2                	slli	a1,a1,0xc
    80001a74:	f46ff0ef          	jal	800011ba <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001a78:	4681                	li	a3,0
    80001a7a:	4605                	li	a2,1
    80001a7c:	020005b7          	lui	a1,0x2000
    80001a80:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001a82:	05b6                	slli	a1,a1,0xd
    80001a84:	8526                	mv	a0,s1
    80001a86:	f34ff0ef          	jal	800011ba <uvmunmap>
  uvmfree(pagetable, sz);
    80001a8a:	85ca                	mv	a1,s2
    80001a8c:	8526                	mv	a0,s1
    80001a8e:	901ff0ef          	jal	8000138e <uvmfree>
}
    80001a92:	60e2                	ld	ra,24(sp)
    80001a94:	6442                	ld	s0,16(sp)
    80001a96:	64a2                	ld	s1,8(sp)
    80001a98:	6902                	ld	s2,0(sp)
    80001a9a:	6105                	addi	sp,sp,32
    80001a9c:	8082                	ret

0000000080001a9e <freeproc>:
{
    80001a9e:	1101                	addi	sp,sp,-32
    80001aa0:	ec06                	sd	ra,24(sp)
    80001aa2:	e822                	sd	s0,16(sp)
    80001aa4:	e426                	sd	s1,8(sp)
    80001aa6:	1000                	addi	s0,sp,32
    80001aa8:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001aaa:	6d28                	ld	a0,88(a0)
    80001aac:	c119                	beqz	a0,80001ab2 <freeproc+0x14>
    kfree((void*)p->trapframe);
    80001aae:	f6ffe0ef          	jal	80000a1c <kfree>
  p->trapframe = 0;
    80001ab2:	0404bc23          	sd	zero,88(s1)
  if(p->pagetable)
    80001ab6:	68a8                	ld	a0,80(s1)
    80001ab8:	c501                	beqz	a0,80001ac0 <freeproc+0x22>
    proc_freepagetable(p->pagetable, p->sz);
    80001aba:	64ac                	ld	a1,72(s1)
    80001abc:	f9dff0ef          	jal	80001a58 <proc_freepagetable>
  p->pagetable = 0;
    80001ac0:	0404b823          	sd	zero,80(s1)
  p->sz = 0;
    80001ac4:	0404b423          	sd	zero,72(s1)
  p->pid = 0;
    80001ac8:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001acc:	0204bc23          	sd	zero,56(s1)
  p->name[0] = 0;
    80001ad0:	14048c23          	sb	zero,344(s1)
  p->chan = 0;
    80001ad4:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001ad8:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001adc:	0204a623          	sw	zero,44(s1)
  p->state = UNUSED;
    80001ae0:	0004ac23          	sw	zero,24(s1)
}
    80001ae4:	60e2                	ld	ra,24(sp)
    80001ae6:	6442                	ld	s0,16(sp)
    80001ae8:	64a2                	ld	s1,8(sp)
    80001aea:	6105                	addi	sp,sp,32
    80001aec:	8082                	ret

0000000080001aee <allocproc>:
{
    80001aee:	1101                	addi	sp,sp,-32
    80001af0:	ec06                	sd	ra,24(sp)
    80001af2:	e822                	sd	s0,16(sp)
    80001af4:	e426                	sd	s1,8(sp)
    80001af6:	e04a                	sd	s2,0(sp)
    80001af8:	1000                	addi	s0,sp,32
  for(p = proc; p < &proc[NPROC]; p++) {
    80001afa:	0000e497          	auipc	s1,0xe
    80001afe:	28e48493          	addi	s1,s1,654 # 8000fd88 <proc>
    80001b02:	00014917          	auipc	s2,0x14
    80001b06:	08690913          	addi	s2,s2,134 # 80015b88 <tickslock>
    acquire(&p->lock);
    80001b0a:	8526                	mv	a0,s1
    80001b0c:	8c2ff0ef          	jal	80000bce <acquire>
    if(p->state == UNUSED) {
    80001b10:	4c9c                	lw	a5,24(s1)
    80001b12:	cb91                	beqz	a5,80001b26 <allocproc+0x38>
      release(&p->lock);
    80001b14:	8526                	mv	a0,s1
    80001b16:	950ff0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b1a:	17848493          	addi	s1,s1,376
    80001b1e:	ff2496e3          	bne	s1,s2,80001b0a <allocproc+0x1c>
  return 0;
    80001b22:	4481                	li	s1,0
    80001b24:	a0b9                	j	80001b72 <allocproc+0x84>
  p->pid = allocpid();
    80001b26:	e71ff0ef          	jal	80001996 <allocpid>
    80001b2a:	d888                	sw	a0,48(s1)
  p->queue = 0;
    80001b2c:	1604a423          	sw	zero,360(s1)
p->quantumleft = Q0_QUANTUM;
    80001b30:	4785                	li	a5,1
    80001b32:	16f4a623          	sw	a5,364(s1)
p->mlfq_ticks = 0;
    80001b36:	1604a823          	sw	zero,368(s1)
p->state = USED;
    80001b3a:	cc9c                	sw	a5,24(s1)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001b3c:	fc3fe0ef          	jal	80000afe <kalloc>
    80001b40:	892a                	mv	s2,a0
    80001b42:	eca8                	sd	a0,88(s1)
    80001b44:	cd15                	beqz	a0,80001b80 <allocproc+0x92>
  p->pagetable = proc_pagetable(p);
    80001b46:	8526                	mv	a0,s1
    80001b48:	e8dff0ef          	jal	800019d4 <proc_pagetable>
    80001b4c:	892a                	mv	s2,a0
    80001b4e:	e8a8                	sd	a0,80(s1)
  if(p->pagetable == 0){
    80001b50:	c121                	beqz	a0,80001b90 <allocproc+0xa2>
  memset(&p->context, 0, sizeof(p->context));
    80001b52:	07000613          	li	a2,112
    80001b56:	4581                	li	a1,0
    80001b58:	06048513          	addi	a0,s1,96
    80001b5c:	946ff0ef          	jal	80000ca2 <memset>
  p->context.ra = (uint64)forkret;
    80001b60:	00000797          	auipc	a5,0x0
    80001b64:	d9e78793          	addi	a5,a5,-610 # 800018fe <forkret>
    80001b68:	f0bc                	sd	a5,96(s1)
  p->context.sp = p->kstack + PGSIZE;
    80001b6a:	60bc                	ld	a5,64(s1)
    80001b6c:	6705                	lui	a4,0x1
    80001b6e:	97ba                	add	a5,a5,a4
    80001b70:	f4bc                	sd	a5,104(s1)
}
    80001b72:	8526                	mv	a0,s1
    80001b74:	60e2                	ld	ra,24(sp)
    80001b76:	6442                	ld	s0,16(sp)
    80001b78:	64a2                	ld	s1,8(sp)
    80001b7a:	6902                	ld	s2,0(sp)
    80001b7c:	6105                	addi	sp,sp,32
    80001b7e:	8082                	ret
    freeproc(p);
    80001b80:	8526                	mv	a0,s1
    80001b82:	f1dff0ef          	jal	80001a9e <freeproc>
    release(&p->lock);
    80001b86:	8526                	mv	a0,s1
    80001b88:	8deff0ef          	jal	80000c66 <release>
    return 0;
    80001b8c:	84ca                	mv	s1,s2
    80001b8e:	b7d5                	j	80001b72 <allocproc+0x84>
    freeproc(p);
    80001b90:	8526                	mv	a0,s1
    80001b92:	f0dff0ef          	jal	80001a9e <freeproc>
    release(&p->lock);
    80001b96:	8526                	mv	a0,s1
    80001b98:	8ceff0ef          	jal	80000c66 <release>
    return 0;
    80001b9c:	84ca                	mv	s1,s2
    80001b9e:	bfd1                	j	80001b72 <allocproc+0x84>

0000000080001ba0 <userinit>:
{
    80001ba0:	1101                	addi	sp,sp,-32
    80001ba2:	ec06                	sd	ra,24(sp)
    80001ba4:	e822                	sd	s0,16(sp)
    80001ba6:	e426                	sd	s1,8(sp)
    80001ba8:	1000                	addi	s0,sp,32
  p = allocproc();
    80001baa:	f45ff0ef          	jal	80001aee <allocproc>
    80001bae:	84aa                	mv	s1,a0
  initproc = p;
    80001bb0:	00006797          	auipc	a5,0x6
    80001bb4:	caa7b023          	sd	a0,-864(a5) # 80007850 <initproc>
  p->cwd = namei("/");
    80001bb8:	00005517          	auipc	a0,0x5
    80001bbc:	5d850513          	addi	a0,a0,1496 # 80007190 <etext+0x190>
    80001bc0:	68d010ef          	jal	80003a4c <namei>
    80001bc4:	14a4b823          	sd	a0,336(s1)
  p->state = RUNNABLE;
    80001bc8:	478d                	li	a5,3
    80001bca:	cc9c                	sw	a5,24(s1)
  release(&p->lock);
    80001bcc:	8526                	mv	a0,s1
    80001bce:	898ff0ef          	jal	80000c66 <release>
}
    80001bd2:	60e2                	ld	ra,24(sp)
    80001bd4:	6442                	ld	s0,16(sp)
    80001bd6:	64a2                	ld	s1,8(sp)
    80001bd8:	6105                	addi	sp,sp,32
    80001bda:	8082                	ret

0000000080001bdc <growproc>:
{
    80001bdc:	1101                	addi	sp,sp,-32
    80001bde:	ec06                	sd	ra,24(sp)
    80001be0:	e822                	sd	s0,16(sp)
    80001be2:	e426                	sd	s1,8(sp)
    80001be4:	e04a                	sd	s2,0(sp)
    80001be6:	1000                	addi	s0,sp,32
    80001be8:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80001bea:	ce5ff0ef          	jal	800018ce <myproc>
    80001bee:	892a                	mv	s2,a0
  sz = p->sz;
    80001bf0:	652c                	ld	a1,72(a0)
  if(n > 0){
    80001bf2:	02905963          	blez	s1,80001c24 <growproc+0x48>
    if(sz + n > TRAPFRAME) {
    80001bf6:	00b48633          	add	a2,s1,a1
    80001bfa:	020007b7          	lui	a5,0x2000
    80001bfe:	17fd                	addi	a5,a5,-1 # 1ffffff <_entry-0x7e000001>
    80001c00:	07b6                	slli	a5,a5,0xd
    80001c02:	02c7ea63          	bltu	a5,a2,80001c36 <growproc+0x5a>
    if((sz = uvmalloc(p->pagetable, sz, sz + n, PTE_W)) == 0) {
    80001c06:	4691                	li	a3,4
    80001c08:	6928                	ld	a0,80(a0)
    80001c0a:	e7eff0ef          	jal	80001288 <uvmalloc>
    80001c0e:	85aa                	mv	a1,a0
    80001c10:	c50d                	beqz	a0,80001c3a <growproc+0x5e>
  p->sz = sz;
    80001c12:	04b93423          	sd	a1,72(s2)
  return 0;
    80001c16:	4501                	li	a0,0
}
    80001c18:	60e2                	ld	ra,24(sp)
    80001c1a:	6442                	ld	s0,16(sp)
    80001c1c:	64a2                	ld	s1,8(sp)
    80001c1e:	6902                	ld	s2,0(sp)
    80001c20:	6105                	addi	sp,sp,32
    80001c22:	8082                	ret
  } else if(n < 0){
    80001c24:	fe04d7e3          	bgez	s1,80001c12 <growproc+0x36>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    80001c28:	00b48633          	add	a2,s1,a1
    80001c2c:	6928                	ld	a0,80(a0)
    80001c2e:	e16ff0ef          	jal	80001244 <uvmdealloc>
    80001c32:	85aa                	mv	a1,a0
    80001c34:	bff9                	j	80001c12 <growproc+0x36>
      return -1;
    80001c36:	557d                	li	a0,-1
    80001c38:	b7c5                	j	80001c18 <growproc+0x3c>
      return -1;
    80001c3a:	557d                	li	a0,-1
    80001c3c:	bff1                	j	80001c18 <growproc+0x3c>

0000000080001c3e <kfork>:
{
    80001c3e:	7139                	addi	sp,sp,-64
    80001c40:	fc06                	sd	ra,56(sp)
    80001c42:	f822                	sd	s0,48(sp)
    80001c44:	f04a                	sd	s2,32(sp)
    80001c46:	e456                	sd	s5,8(sp)
    80001c48:	0080                	addi	s0,sp,64
  struct proc *p = myproc();
    80001c4a:	c85ff0ef          	jal	800018ce <myproc>
    80001c4e:	8aaa                	mv	s5,a0
  if((np = allocproc()) == 0){
    80001c50:	e9fff0ef          	jal	80001aee <allocproc>
    80001c54:	0e050a63          	beqz	a0,80001d48 <kfork+0x10a>
    80001c58:	e852                	sd	s4,16(sp)
    80001c5a:	8a2a                	mv	s4,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    80001c5c:	048ab603          	ld	a2,72(s5)
    80001c60:	692c                	ld	a1,80(a0)
    80001c62:	050ab503          	ld	a0,80(s5)
    80001c66:	f5aff0ef          	jal	800013c0 <uvmcopy>
    80001c6a:	04054a63          	bltz	a0,80001cbe <kfork+0x80>
    80001c6e:	f426                	sd	s1,40(sp)
    80001c70:	ec4e                	sd	s3,24(sp)
  np->sz = p->sz;
    80001c72:	048ab783          	ld	a5,72(s5)
    80001c76:	04fa3423          	sd	a5,72(s4)
  *(np->trapframe) = *(p->trapframe);
    80001c7a:	058ab683          	ld	a3,88(s5)
    80001c7e:	87b6                	mv	a5,a3
    80001c80:	058a3703          	ld	a4,88(s4)
    80001c84:	12068693          	addi	a3,a3,288
    80001c88:	0007b803          	ld	a6,0(a5)
    80001c8c:	6788                	ld	a0,8(a5)
    80001c8e:	6b8c                	ld	a1,16(a5)
    80001c90:	6f90                	ld	a2,24(a5)
    80001c92:	01073023          	sd	a6,0(a4) # 1000 <_entry-0x7ffff000>
    80001c96:	e708                	sd	a0,8(a4)
    80001c98:	eb0c                	sd	a1,16(a4)
    80001c9a:	ef10                	sd	a2,24(a4)
    80001c9c:	02078793          	addi	a5,a5,32
    80001ca0:	02070713          	addi	a4,a4,32
    80001ca4:	fed792e3          	bne	a5,a3,80001c88 <kfork+0x4a>
  np->trapframe->a0 = 0;
    80001ca8:	058a3783          	ld	a5,88(s4)
    80001cac:	0607b823          	sd	zero,112(a5)
  for(i = 0; i < NOFILE; i++)
    80001cb0:	0d0a8493          	addi	s1,s5,208
    80001cb4:	0d0a0913          	addi	s2,s4,208
    80001cb8:	150a8993          	addi	s3,s5,336
    80001cbc:	a831                	j	80001cd8 <kfork+0x9a>
    freeproc(np);
    80001cbe:	8552                	mv	a0,s4
    80001cc0:	ddfff0ef          	jal	80001a9e <freeproc>
    release(&np->lock);
    80001cc4:	8552                	mv	a0,s4
    80001cc6:	fa1fe0ef          	jal	80000c66 <release>
    return -1;
    80001cca:	597d                	li	s2,-1
    80001ccc:	6a42                	ld	s4,16(sp)
    80001cce:	a0b5                	j	80001d3a <kfork+0xfc>
  for(i = 0; i < NOFILE; i++)
    80001cd0:	04a1                	addi	s1,s1,8
    80001cd2:	0921                	addi	s2,s2,8
    80001cd4:	01348963          	beq	s1,s3,80001ce6 <kfork+0xa8>
    if(p->ofile[i])
    80001cd8:	6088                	ld	a0,0(s1)
    80001cda:	d97d                	beqz	a0,80001cd0 <kfork+0x92>
      np->ofile[i] = filedup(p->ofile[i]);
    80001cdc:	30a020ef          	jal	80003fe6 <filedup>
    80001ce0:	00a93023          	sd	a0,0(s2)
    80001ce4:	b7f5                	j	80001cd0 <kfork+0x92>
  np->cwd = idup(p->cwd);
    80001ce6:	150ab503          	ld	a0,336(s5)
    80001cea:	516010ef          	jal	80003200 <idup>
    80001cee:	14aa3823          	sd	a0,336(s4)
  safestrcpy(np->name, p->name, sizeof(p->name));
    80001cf2:	4641                	li	a2,16
    80001cf4:	158a8593          	addi	a1,s5,344
    80001cf8:	158a0513          	addi	a0,s4,344
    80001cfc:	8e4ff0ef          	jal	80000de0 <safestrcpy>
  pid = np->pid;
    80001d00:	030a2903          	lw	s2,48(s4)
  release(&np->lock);
    80001d04:	8552                	mv	a0,s4
    80001d06:	f61fe0ef          	jal	80000c66 <release>
  acquire(&wait_lock);
    80001d0a:	0000e497          	auipc	s1,0xe
    80001d0e:	c6648493          	addi	s1,s1,-922 # 8000f970 <wait_lock>
    80001d12:	8526                	mv	a0,s1
    80001d14:	ebbfe0ef          	jal	80000bce <acquire>
  np->parent = p;
    80001d18:	035a3c23          	sd	s5,56(s4)
  release(&wait_lock);
    80001d1c:	8526                	mv	a0,s1
    80001d1e:	f49fe0ef          	jal	80000c66 <release>
  acquire(&np->lock);
    80001d22:	8552                	mv	a0,s4
    80001d24:	eabfe0ef          	jal	80000bce <acquire>
  np->state = RUNNABLE;
    80001d28:	478d                	li	a5,3
    80001d2a:	00fa2c23          	sw	a5,24(s4)
  release(&np->lock);
    80001d2e:	8552                	mv	a0,s4
    80001d30:	f37fe0ef          	jal	80000c66 <release>
  return pid;
    80001d34:	74a2                	ld	s1,40(sp)
    80001d36:	69e2                	ld	s3,24(sp)
    80001d38:	6a42                	ld	s4,16(sp)
}
    80001d3a:	854a                	mv	a0,s2
    80001d3c:	70e2                	ld	ra,56(sp)
    80001d3e:	7442                	ld	s0,48(sp)
    80001d40:	7902                	ld	s2,32(sp)
    80001d42:	6aa2                	ld	s5,8(sp)
    80001d44:	6121                	addi	sp,sp,64
    80001d46:	8082                	ret
    return -1;
    80001d48:	597d                	li	s2,-1
    80001d4a:	bfc5                	j	80001d3a <kfork+0xfc>

0000000080001d4c <scheduler>:
{
    80001d4c:	715d                	addi	sp,sp,-80
    80001d4e:	e486                	sd	ra,72(sp)
    80001d50:	e0a2                	sd	s0,64(sp)
    80001d52:	fc26                	sd	s1,56(sp)
    80001d54:	f84a                	sd	s2,48(sp)
    80001d56:	f44e                	sd	s3,40(sp)
    80001d58:	f052                	sd	s4,32(sp)
    80001d5a:	ec56                	sd	s5,24(sp)
    80001d5c:	e85a                	sd	s6,16(sp)
    80001d5e:	e45e                	sd	s7,8(sp)
    80001d60:	e062                	sd	s8,0(sp)
    80001d62:	0880                	addi	s0,sp,80
    80001d64:	8792                	mv	a5,tp
  int id = r_tp();
    80001d66:	2781                	sext.w	a5,a5
  c->proc = 0;
    80001d68:	00779b13          	slli	s6,a5,0x7
    80001d6c:	0000e717          	auipc	a4,0xe
    80001d70:	bec70713          	addi	a4,a4,-1044 # 8000f958 <pid_lock>
    80001d74:	975a                	add	a4,a4,s6
    80001d76:	02073823          	sd	zero,48(a4)
          swtch(&c->context, &p->context);
    80001d7a:	0000e717          	auipc	a4,0xe
    80001d7e:	c1670713          	addi	a4,a4,-1002 # 8000f990 <cpus+0x8>
    80001d82:	9b3a                	add	s6,s6,a4
    for(int q = 0; q < NMLFQ && !found; q++){
    80001d84:	4c01                	li	s8,0
        if(p->state == RUNNABLE && p->queue == q){
    80001d86:	490d                	li	s2,3
      for(p = proc; p < &proc[NPROC]; p++){
    80001d88:	00014997          	auipc	s3,0x14
    80001d8c:	e0098993          	addi	s3,s3,-512 # 80015b88 <tickslock>
          p->state = RUNNING;
    80001d90:	4b91                	li	s7,4
          c->proc = p;
    80001d92:	079e                	slli	a5,a5,0x7
    80001d94:	0000ea97          	auipc	s5,0xe
    80001d98:	bc4a8a93          	addi	s5,s5,-1084 # 8000f958 <pid_lock>
    80001d9c:	9abe                	add	s5,s5,a5
    80001d9e:	a0a1                	j	80001de6 <scheduler+0x9a>
    for(int q = 0; q < NMLFQ && !found; q++){
    80001da0:	2a05                	addiw	s4,s4,1
    80001da2:	052a1963          	bne	s4,s2,80001df4 <scheduler+0xa8>
    80001da6:	a081                	j	80001de6 <scheduler+0x9a>
        release(&p->lock);
    80001da8:	8526                	mv	a0,s1
    80001daa:	ebdfe0ef          	jal	80000c66 <release>
      for(p = proc; p < &proc[NPROC]; p++){
    80001dae:	17848493          	addi	s1,s1,376
    80001db2:	ff3487e3          	beq	s1,s3,80001da0 <scheduler+0x54>
        acquire(&p->lock);
    80001db6:	8526                	mv	a0,s1
    80001db8:	e17fe0ef          	jal	80000bce <acquire>
        if(p->state == RUNNABLE && p->queue == q){
    80001dbc:	4c9c                	lw	a5,24(s1)
    80001dbe:	ff2795e3          	bne	a5,s2,80001da8 <scheduler+0x5c>
    80001dc2:	1684a783          	lw	a5,360(s1)
    80001dc6:	ff4791e3          	bne	a5,s4,80001da8 <scheduler+0x5c>
          p->state = RUNNING;
    80001dca:	0174ac23          	sw	s7,24(s1)
          c->proc = p;
    80001dce:	029ab823          	sd	s1,48(s5)
          swtch(&c->context, &p->context);
    80001dd2:	06048593          	addi	a1,s1,96
    80001dd6:	855a                	mv	a0,s6
    80001dd8:	5a0000ef          	jal	80002378 <swtch>
          c->proc = 0;
    80001ddc:	020ab823          	sd	zero,48(s5)
        release(&p->lock);
    80001de0:	8526                	mv	a0,s1
    80001de2:	e85fe0ef          	jal	80000c66 <release>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001de6:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80001dea:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001dee:	10079073          	csrw	sstatus,a5
    for(int q = 0; q < NMLFQ && !found; q++){
    80001df2:	8a62                	mv	s4,s8
      for(p = proc; p < &proc[NPROC]; p++){
    80001df4:	0000e497          	auipc	s1,0xe
    80001df8:	f9448493          	addi	s1,s1,-108 # 8000fd88 <proc>
    80001dfc:	bf6d                	j	80001db6 <scheduler+0x6a>

0000000080001dfe <sched>:
{
    80001dfe:	7179                	addi	sp,sp,-48
    80001e00:	f406                	sd	ra,40(sp)
    80001e02:	f022                	sd	s0,32(sp)
    80001e04:	ec26                	sd	s1,24(sp)
    80001e06:	e84a                	sd	s2,16(sp)
    80001e08:	e44e                	sd	s3,8(sp)
    80001e0a:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80001e0c:	ac3ff0ef          	jal	800018ce <myproc>
    80001e10:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    80001e12:	d53fe0ef          	jal	80000b64 <holding>
    80001e16:	c92d                	beqz	a0,80001e88 <sched+0x8a>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e18:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    80001e1a:	2781                	sext.w	a5,a5
    80001e1c:	079e                	slli	a5,a5,0x7
    80001e1e:	0000e717          	auipc	a4,0xe
    80001e22:	b3a70713          	addi	a4,a4,-1222 # 8000f958 <pid_lock>
    80001e26:	97ba                	add	a5,a5,a4
    80001e28:	0a87a703          	lw	a4,168(a5)
    80001e2c:	4785                	li	a5,1
    80001e2e:	06f71363          	bne	a4,a5,80001e94 <sched+0x96>
  if(p->state == RUNNING)
    80001e32:	4c98                	lw	a4,24(s1)
    80001e34:	4791                	li	a5,4
    80001e36:	06f70563          	beq	a4,a5,80001ea0 <sched+0xa2>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001e3a:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80001e3e:	8b89                	andi	a5,a5,2
  if(intr_get())
    80001e40:	e7b5                	bnez	a5,80001eac <sched+0xae>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e42:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    80001e44:	0000e917          	auipc	s2,0xe
    80001e48:	b1490913          	addi	s2,s2,-1260 # 8000f958 <pid_lock>
    80001e4c:	2781                	sext.w	a5,a5
    80001e4e:	079e                	slli	a5,a5,0x7
    80001e50:	97ca                	add	a5,a5,s2
    80001e52:	0ac7a983          	lw	s3,172(a5)
    80001e56:	8792                	mv	a5,tp
  swtch(&p->context, &mycpu()->context);
    80001e58:	2781                	sext.w	a5,a5
    80001e5a:	079e                	slli	a5,a5,0x7
    80001e5c:	0000e597          	auipc	a1,0xe
    80001e60:	b3458593          	addi	a1,a1,-1228 # 8000f990 <cpus+0x8>
    80001e64:	95be                	add	a1,a1,a5
    80001e66:	06048513          	addi	a0,s1,96
    80001e6a:	50e000ef          	jal	80002378 <swtch>
    80001e6e:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    80001e70:	2781                	sext.w	a5,a5
    80001e72:	079e                	slli	a5,a5,0x7
    80001e74:	993e                	add	s2,s2,a5
    80001e76:	0b392623          	sw	s3,172(s2)
}
    80001e7a:	70a2                	ld	ra,40(sp)
    80001e7c:	7402                	ld	s0,32(sp)
    80001e7e:	64e2                	ld	s1,24(sp)
    80001e80:	6942                	ld	s2,16(sp)
    80001e82:	69a2                	ld	s3,8(sp)
    80001e84:	6145                	addi	sp,sp,48
    80001e86:	8082                	ret
    panic("sched p->lock");
    80001e88:	00005517          	auipc	a0,0x5
    80001e8c:	31050513          	addi	a0,a0,784 # 80007198 <etext+0x198>
    80001e90:	951fe0ef          	jal	800007e0 <panic>
    panic("sched locks");
    80001e94:	00005517          	auipc	a0,0x5
    80001e98:	31450513          	addi	a0,a0,788 # 800071a8 <etext+0x1a8>
    80001e9c:	945fe0ef          	jal	800007e0 <panic>
    panic("sched RUNNING");
    80001ea0:	00005517          	auipc	a0,0x5
    80001ea4:	31850513          	addi	a0,a0,792 # 800071b8 <etext+0x1b8>
    80001ea8:	939fe0ef          	jal	800007e0 <panic>
    panic("sched interruptible");
    80001eac:	00005517          	auipc	a0,0x5
    80001eb0:	31c50513          	addi	a0,a0,796 # 800071c8 <etext+0x1c8>
    80001eb4:	92dfe0ef          	jal	800007e0 <panic>

0000000080001eb8 <yield>:
{
    80001eb8:	1101                	addi	sp,sp,-32
    80001eba:	ec06                	sd	ra,24(sp)
    80001ebc:	e822                	sd	s0,16(sp)
    80001ebe:	e426                	sd	s1,8(sp)
    80001ec0:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    80001ec2:	a0dff0ef          	jal	800018ce <myproc>
    80001ec6:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80001ec8:	d07fe0ef          	jal	80000bce <acquire>
  p->state = RUNNABLE;
    80001ecc:	478d                	li	a5,3
    80001ece:	cc9c                	sw	a5,24(s1)
  sched();
    80001ed0:	f2fff0ef          	jal	80001dfe <sched>
  release(&p->lock);
    80001ed4:	8526                	mv	a0,s1
    80001ed6:	d91fe0ef          	jal	80000c66 <release>
}
    80001eda:	60e2                	ld	ra,24(sp)
    80001edc:	6442                	ld	s0,16(sp)
    80001ede:	64a2                	ld	s1,8(sp)
    80001ee0:	6105                	addi	sp,sp,32
    80001ee2:	8082                	ret

0000000080001ee4 <sleep>:

// Sleep on channel chan, releasing condition lock lk.
// Re-acquires lk when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    80001ee4:	7179                	addi	sp,sp,-48
    80001ee6:	f406                	sd	ra,40(sp)
    80001ee8:	f022                	sd	s0,32(sp)
    80001eea:	ec26                	sd	s1,24(sp)
    80001eec:	e84a                	sd	s2,16(sp)
    80001eee:	e44e                	sd	s3,8(sp)
    80001ef0:	1800                	addi	s0,sp,48
    80001ef2:	89aa                	mv	s3,a0
    80001ef4:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80001ef6:	9d9ff0ef          	jal	800018ce <myproc>
    80001efa:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    80001efc:	cd3fe0ef          	jal	80000bce <acquire>
  release(lk);
    80001f00:	854a                	mv	a0,s2
    80001f02:	d65fe0ef          	jal	80000c66 <release>

  // Go to sleep.
  p->chan = chan;
    80001f06:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    80001f0a:	4789                	li	a5,2
    80001f0c:	cc9c                	sw	a5,24(s1)

  sched();
    80001f0e:	ef1ff0ef          	jal	80001dfe <sched>

  // Tidy up.
  p->chan = 0;
    80001f12:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    80001f16:	8526                	mv	a0,s1
    80001f18:	d4ffe0ef          	jal	80000c66 <release>
  acquire(lk);
    80001f1c:	854a                	mv	a0,s2
    80001f1e:	cb1fe0ef          	jal	80000bce <acquire>
}
    80001f22:	70a2                	ld	ra,40(sp)
    80001f24:	7402                	ld	s0,32(sp)
    80001f26:	64e2                	ld	s1,24(sp)
    80001f28:	6942                	ld	s2,16(sp)
    80001f2a:	69a2                	ld	s3,8(sp)
    80001f2c:	6145                	addi	sp,sp,48
    80001f2e:	8082                	ret

0000000080001f30 <wakeup>:

// Wake up all processes sleeping on channel chan.
// Caller should hold the condition lock.
void
wakeup(void *chan)
{
    80001f30:	7139                	addi	sp,sp,-64
    80001f32:	fc06                	sd	ra,56(sp)
    80001f34:	f822                	sd	s0,48(sp)
    80001f36:	f426                	sd	s1,40(sp)
    80001f38:	f04a                	sd	s2,32(sp)
    80001f3a:	ec4e                	sd	s3,24(sp)
    80001f3c:	e852                	sd	s4,16(sp)
    80001f3e:	e456                	sd	s5,8(sp)
    80001f40:	0080                	addi	s0,sp,64
    80001f42:	8a2a                	mv	s4,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++) {
    80001f44:	0000e497          	auipc	s1,0xe
    80001f48:	e4448493          	addi	s1,s1,-444 # 8000fd88 <proc>
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    80001f4c:	4989                	li	s3,2
        p->state = RUNNABLE;
    80001f4e:	4a8d                	li	s5,3
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f50:	00014917          	auipc	s2,0x14
    80001f54:	c3890913          	addi	s2,s2,-968 # 80015b88 <tickslock>
    80001f58:	a801                	j	80001f68 <wakeup+0x38>
      }
      release(&p->lock);
    80001f5a:	8526                	mv	a0,s1
    80001f5c:	d0bfe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f60:	17848493          	addi	s1,s1,376
    80001f64:	03248263          	beq	s1,s2,80001f88 <wakeup+0x58>
    if(p != myproc()){
    80001f68:	967ff0ef          	jal	800018ce <myproc>
    80001f6c:	fea48ae3          	beq	s1,a0,80001f60 <wakeup+0x30>
      acquire(&p->lock);
    80001f70:	8526                	mv	a0,s1
    80001f72:	c5dfe0ef          	jal	80000bce <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    80001f76:	4c9c                	lw	a5,24(s1)
    80001f78:	ff3791e3          	bne	a5,s3,80001f5a <wakeup+0x2a>
    80001f7c:	709c                	ld	a5,32(s1)
    80001f7e:	fd479ee3          	bne	a5,s4,80001f5a <wakeup+0x2a>
        p->state = RUNNABLE;
    80001f82:	0154ac23          	sw	s5,24(s1)
    80001f86:	bfd1                	j	80001f5a <wakeup+0x2a>
    }
  }
}
    80001f88:	70e2                	ld	ra,56(sp)
    80001f8a:	7442                	ld	s0,48(sp)
    80001f8c:	74a2                	ld	s1,40(sp)
    80001f8e:	7902                	ld	s2,32(sp)
    80001f90:	69e2                	ld	s3,24(sp)
    80001f92:	6a42                	ld	s4,16(sp)
    80001f94:	6aa2                	ld	s5,8(sp)
    80001f96:	6121                	addi	sp,sp,64
    80001f98:	8082                	ret

0000000080001f9a <reparent>:
{
    80001f9a:	7179                	addi	sp,sp,-48
    80001f9c:	f406                	sd	ra,40(sp)
    80001f9e:	f022                	sd	s0,32(sp)
    80001fa0:	ec26                	sd	s1,24(sp)
    80001fa2:	e84a                	sd	s2,16(sp)
    80001fa4:	e44e                	sd	s3,8(sp)
    80001fa6:	e052                	sd	s4,0(sp)
    80001fa8:	1800                	addi	s0,sp,48
    80001faa:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fac:	0000e497          	auipc	s1,0xe
    80001fb0:	ddc48493          	addi	s1,s1,-548 # 8000fd88 <proc>
      pp->parent = initproc;
    80001fb4:	00006a17          	auipc	s4,0x6
    80001fb8:	89ca0a13          	addi	s4,s4,-1892 # 80007850 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fbc:	00014997          	auipc	s3,0x14
    80001fc0:	bcc98993          	addi	s3,s3,-1076 # 80015b88 <tickslock>
    80001fc4:	a029                	j	80001fce <reparent+0x34>
    80001fc6:	17848493          	addi	s1,s1,376
    80001fca:	01348b63          	beq	s1,s3,80001fe0 <reparent+0x46>
    if(pp->parent == p){
    80001fce:	7c9c                	ld	a5,56(s1)
    80001fd0:	ff279be3          	bne	a5,s2,80001fc6 <reparent+0x2c>
      pp->parent = initproc;
    80001fd4:	000a3503          	ld	a0,0(s4)
    80001fd8:	fc88                	sd	a0,56(s1)
      wakeup(initproc);
    80001fda:	f57ff0ef          	jal	80001f30 <wakeup>
    80001fde:	b7e5                	j	80001fc6 <reparent+0x2c>
}
    80001fe0:	70a2                	ld	ra,40(sp)
    80001fe2:	7402                	ld	s0,32(sp)
    80001fe4:	64e2                	ld	s1,24(sp)
    80001fe6:	6942                	ld	s2,16(sp)
    80001fe8:	69a2                	ld	s3,8(sp)
    80001fea:	6a02                	ld	s4,0(sp)
    80001fec:	6145                	addi	sp,sp,48
    80001fee:	8082                	ret

0000000080001ff0 <kexit>:
{
    80001ff0:	7179                	addi	sp,sp,-48
    80001ff2:	f406                	sd	ra,40(sp)
    80001ff4:	f022                	sd	s0,32(sp)
    80001ff6:	ec26                	sd	s1,24(sp)
    80001ff8:	e84a                	sd	s2,16(sp)
    80001ffa:	e44e                	sd	s3,8(sp)
    80001ffc:	e052                	sd	s4,0(sp)
    80001ffe:	1800                	addi	s0,sp,48
    80002000:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    80002002:	8cdff0ef          	jal	800018ce <myproc>
    80002006:	89aa                	mv	s3,a0
  if(p == initproc)
    80002008:	00006797          	auipc	a5,0x6
    8000200c:	8487b783          	ld	a5,-1976(a5) # 80007850 <initproc>
    80002010:	0d050493          	addi	s1,a0,208
    80002014:	15050913          	addi	s2,a0,336
    80002018:	00a79f63          	bne	a5,a0,80002036 <kexit+0x46>
    panic("init exiting");
    8000201c:	00005517          	auipc	a0,0x5
    80002020:	1c450513          	addi	a0,a0,452 # 800071e0 <etext+0x1e0>
    80002024:	fbcfe0ef          	jal	800007e0 <panic>
      fileclose(f);
    80002028:	004020ef          	jal	8000402c <fileclose>
      p->ofile[fd] = 0;
    8000202c:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    80002030:	04a1                	addi	s1,s1,8
    80002032:	01248563          	beq	s1,s2,8000203c <kexit+0x4c>
    if(p->ofile[fd]){
    80002036:	6088                	ld	a0,0(s1)
    80002038:	f965                	bnez	a0,80002028 <kexit+0x38>
    8000203a:	bfdd                	j	80002030 <kexit+0x40>
  begin_op();
    8000203c:	3e5010ef          	jal	80003c20 <begin_op>
  iput(p->cwd);
    80002040:	1509b503          	ld	a0,336(s3)
    80002044:	374010ef          	jal	800033b8 <iput>
  end_op();
    80002048:	443010ef          	jal	80003c8a <end_op>
  p->cwd = 0;
    8000204c:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    80002050:	0000e497          	auipc	s1,0xe
    80002054:	92048493          	addi	s1,s1,-1760 # 8000f970 <wait_lock>
    80002058:	8526                	mv	a0,s1
    8000205a:	b75fe0ef          	jal	80000bce <acquire>
  reparent(p);
    8000205e:	854e                	mv	a0,s3
    80002060:	f3bff0ef          	jal	80001f9a <reparent>
  wakeup(p->parent);
    80002064:	0389b503          	ld	a0,56(s3)
    80002068:	ec9ff0ef          	jal	80001f30 <wakeup>
  acquire(&p->lock);
    8000206c:	854e                	mv	a0,s3
    8000206e:	b61fe0ef          	jal	80000bce <acquire>
  p->xstate = status;
    80002072:	0349a623          	sw	s4,44(s3)
  p->state = ZOMBIE;
    80002076:	4795                	li	a5,5
    80002078:	00f9ac23          	sw	a5,24(s3)
  release(&wait_lock);
    8000207c:	8526                	mv	a0,s1
    8000207e:	be9fe0ef          	jal	80000c66 <release>
  sched();
    80002082:	d7dff0ef          	jal	80001dfe <sched>
  panic("zombie exit");
    80002086:	00005517          	auipc	a0,0x5
    8000208a:	16a50513          	addi	a0,a0,362 # 800071f0 <etext+0x1f0>
    8000208e:	f52fe0ef          	jal	800007e0 <panic>

0000000080002092 <kkill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kkill(int pid)
{
    80002092:	7179                	addi	sp,sp,-48
    80002094:	f406                	sd	ra,40(sp)
    80002096:	f022                	sd	s0,32(sp)
    80002098:	ec26                	sd	s1,24(sp)
    8000209a:	e84a                	sd	s2,16(sp)
    8000209c:	e44e                	sd	s3,8(sp)
    8000209e:	1800                	addi	s0,sp,48
    800020a0:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    800020a2:	0000e497          	auipc	s1,0xe
    800020a6:	ce648493          	addi	s1,s1,-794 # 8000fd88 <proc>
    800020aa:	00014997          	auipc	s3,0x14
    800020ae:	ade98993          	addi	s3,s3,-1314 # 80015b88 <tickslock>
    acquire(&p->lock);
    800020b2:	8526                	mv	a0,s1
    800020b4:	b1bfe0ef          	jal	80000bce <acquire>
    if(p->pid == pid){
    800020b8:	589c                	lw	a5,48(s1)
    800020ba:	01278b63          	beq	a5,s2,800020d0 <kkill+0x3e>
        p->state = RUNNABLE;
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    800020be:	8526                	mv	a0,s1
    800020c0:	ba7fe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    800020c4:	17848493          	addi	s1,s1,376
    800020c8:	ff3495e3          	bne	s1,s3,800020b2 <kkill+0x20>
  }
  return -1;
    800020cc:	557d                	li	a0,-1
    800020ce:	a819                	j	800020e4 <kkill+0x52>
      p->killed = 1;
    800020d0:	4785                	li	a5,1
    800020d2:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    800020d4:	4c98                	lw	a4,24(s1)
    800020d6:	4789                	li	a5,2
    800020d8:	00f70d63          	beq	a4,a5,800020f2 <kkill+0x60>
      release(&p->lock);
    800020dc:	8526                	mv	a0,s1
    800020de:	b89fe0ef          	jal	80000c66 <release>
      return 0;
    800020e2:	4501                	li	a0,0
}
    800020e4:	70a2                	ld	ra,40(sp)
    800020e6:	7402                	ld	s0,32(sp)
    800020e8:	64e2                	ld	s1,24(sp)
    800020ea:	6942                	ld	s2,16(sp)
    800020ec:	69a2                	ld	s3,8(sp)
    800020ee:	6145                	addi	sp,sp,48
    800020f0:	8082                	ret
        p->state = RUNNABLE;
    800020f2:	478d                	li	a5,3
    800020f4:	cc9c                	sw	a5,24(s1)
    800020f6:	b7dd                	j	800020dc <kkill+0x4a>

00000000800020f8 <setkilled>:

void
setkilled(struct proc *p)
{
    800020f8:	1101                	addi	sp,sp,-32
    800020fa:	ec06                	sd	ra,24(sp)
    800020fc:	e822                	sd	s0,16(sp)
    800020fe:	e426                	sd	s1,8(sp)
    80002100:	1000                	addi	s0,sp,32
    80002102:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80002104:	acbfe0ef          	jal	80000bce <acquire>
  p->killed = 1;
    80002108:	4785                	li	a5,1
    8000210a:	d49c                	sw	a5,40(s1)
  release(&p->lock);
    8000210c:	8526                	mv	a0,s1
    8000210e:	b59fe0ef          	jal	80000c66 <release>
}
    80002112:	60e2                	ld	ra,24(sp)
    80002114:	6442                	ld	s0,16(sp)
    80002116:	64a2                	ld	s1,8(sp)
    80002118:	6105                	addi	sp,sp,32
    8000211a:	8082                	ret

000000008000211c <killed>:

int
killed(struct proc *p)
{
    8000211c:	1101                	addi	sp,sp,-32
    8000211e:	ec06                	sd	ra,24(sp)
    80002120:	e822                	sd	s0,16(sp)
    80002122:	e426                	sd	s1,8(sp)
    80002124:	e04a                	sd	s2,0(sp)
    80002126:	1000                	addi	s0,sp,32
    80002128:	84aa                	mv	s1,a0
  int k;
  
  acquire(&p->lock);
    8000212a:	aa5fe0ef          	jal	80000bce <acquire>
  k = p->killed;
    8000212e:	0284a903          	lw	s2,40(s1)
  release(&p->lock);
    80002132:	8526                	mv	a0,s1
    80002134:	b33fe0ef          	jal	80000c66 <release>
  return k;
}
    80002138:	854a                	mv	a0,s2
    8000213a:	60e2                	ld	ra,24(sp)
    8000213c:	6442                	ld	s0,16(sp)
    8000213e:	64a2                	ld	s1,8(sp)
    80002140:	6902                	ld	s2,0(sp)
    80002142:	6105                	addi	sp,sp,32
    80002144:	8082                	ret

0000000080002146 <kwait>:
{
    80002146:	715d                	addi	sp,sp,-80
    80002148:	e486                	sd	ra,72(sp)
    8000214a:	e0a2                	sd	s0,64(sp)
    8000214c:	fc26                	sd	s1,56(sp)
    8000214e:	f84a                	sd	s2,48(sp)
    80002150:	f44e                	sd	s3,40(sp)
    80002152:	f052                	sd	s4,32(sp)
    80002154:	ec56                	sd	s5,24(sp)
    80002156:	e85a                	sd	s6,16(sp)
    80002158:	e45e                	sd	s7,8(sp)
    8000215a:	e062                	sd	s8,0(sp)
    8000215c:	0880                	addi	s0,sp,80
    8000215e:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    80002160:	f6eff0ef          	jal	800018ce <myproc>
    80002164:	892a                	mv	s2,a0
  acquire(&wait_lock);
    80002166:	0000e517          	auipc	a0,0xe
    8000216a:	80a50513          	addi	a0,a0,-2038 # 8000f970 <wait_lock>
    8000216e:	a61fe0ef          	jal	80000bce <acquire>
    havekids = 0;
    80002172:	4b81                	li	s7,0
        if(pp->state == ZOMBIE){
    80002174:	4a15                	li	s4,5
        havekids = 1;
    80002176:	4a85                	li	s5,1
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002178:	00014997          	auipc	s3,0x14
    8000217c:	a1098993          	addi	s3,s3,-1520 # 80015b88 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002180:	0000dc17          	auipc	s8,0xd
    80002184:	7f0c0c13          	addi	s8,s8,2032 # 8000f970 <wait_lock>
    80002188:	a871                	j	80002224 <kwait+0xde>
          pid = pp->pid;
    8000218a:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&pp->xstate,
    8000218e:	000b0c63          	beqz	s6,800021a6 <kwait+0x60>
    80002192:	4691                	li	a3,4
    80002194:	02c48613          	addi	a2,s1,44
    80002198:	85da                	mv	a1,s6
    8000219a:	05093503          	ld	a0,80(s2)
    8000219e:	c44ff0ef          	jal	800015e2 <copyout>
    800021a2:	02054b63          	bltz	a0,800021d8 <kwait+0x92>
          freeproc(pp);
    800021a6:	8526                	mv	a0,s1
    800021a8:	8f7ff0ef          	jal	80001a9e <freeproc>
          release(&pp->lock);
    800021ac:	8526                	mv	a0,s1
    800021ae:	ab9fe0ef          	jal	80000c66 <release>
          release(&wait_lock);
    800021b2:	0000d517          	auipc	a0,0xd
    800021b6:	7be50513          	addi	a0,a0,1982 # 8000f970 <wait_lock>
    800021ba:	aadfe0ef          	jal	80000c66 <release>
}
    800021be:	854e                	mv	a0,s3
    800021c0:	60a6                	ld	ra,72(sp)
    800021c2:	6406                	ld	s0,64(sp)
    800021c4:	74e2                	ld	s1,56(sp)
    800021c6:	7942                	ld	s2,48(sp)
    800021c8:	79a2                	ld	s3,40(sp)
    800021ca:	7a02                	ld	s4,32(sp)
    800021cc:	6ae2                	ld	s5,24(sp)
    800021ce:	6b42                	ld	s6,16(sp)
    800021d0:	6ba2                	ld	s7,8(sp)
    800021d2:	6c02                	ld	s8,0(sp)
    800021d4:	6161                	addi	sp,sp,80
    800021d6:	8082                	ret
            release(&pp->lock);
    800021d8:	8526                	mv	a0,s1
    800021da:	a8dfe0ef          	jal	80000c66 <release>
            release(&wait_lock);
    800021de:	0000d517          	auipc	a0,0xd
    800021e2:	79250513          	addi	a0,a0,1938 # 8000f970 <wait_lock>
    800021e6:	a81fe0ef          	jal	80000c66 <release>
            return -1;
    800021ea:	59fd                	li	s3,-1
    800021ec:	bfc9                	j	800021be <kwait+0x78>
    for(pp = proc; pp < &proc[NPROC]; pp++){
    800021ee:	17848493          	addi	s1,s1,376
    800021f2:	03348063          	beq	s1,s3,80002212 <kwait+0xcc>
      if(pp->parent == p){
    800021f6:	7c9c                	ld	a5,56(s1)
    800021f8:	ff279be3          	bne	a5,s2,800021ee <kwait+0xa8>
        acquire(&pp->lock);
    800021fc:	8526                	mv	a0,s1
    800021fe:	9d1fe0ef          	jal	80000bce <acquire>
        if(pp->state == ZOMBIE){
    80002202:	4c9c                	lw	a5,24(s1)
    80002204:	f94783e3          	beq	a5,s4,8000218a <kwait+0x44>
        release(&pp->lock);
    80002208:	8526                	mv	a0,s1
    8000220a:	a5dfe0ef          	jal	80000c66 <release>
        havekids = 1;
    8000220e:	8756                	mv	a4,s5
    80002210:	bff9                	j	800021ee <kwait+0xa8>
    if(!havekids || killed(p)){
    80002212:	cf19                	beqz	a4,80002230 <kwait+0xea>
    80002214:	854a                	mv	a0,s2
    80002216:	f07ff0ef          	jal	8000211c <killed>
    8000221a:	e919                	bnez	a0,80002230 <kwait+0xea>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    8000221c:	85e2                	mv	a1,s8
    8000221e:	854a                	mv	a0,s2
    80002220:	cc5ff0ef          	jal	80001ee4 <sleep>
    havekids = 0;
    80002224:	875e                	mv	a4,s7
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002226:	0000e497          	auipc	s1,0xe
    8000222a:	b6248493          	addi	s1,s1,-1182 # 8000fd88 <proc>
    8000222e:	b7e1                	j	800021f6 <kwait+0xb0>
      release(&wait_lock);
    80002230:	0000d517          	auipc	a0,0xd
    80002234:	74050513          	addi	a0,a0,1856 # 8000f970 <wait_lock>
    80002238:	a2ffe0ef          	jal	80000c66 <release>
      return -1;
    8000223c:	59fd                	li	s3,-1
    8000223e:	b741                	j	800021be <kwait+0x78>

0000000080002240 <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    80002240:	7179                	addi	sp,sp,-48
    80002242:	f406                	sd	ra,40(sp)
    80002244:	f022                	sd	s0,32(sp)
    80002246:	ec26                	sd	s1,24(sp)
    80002248:	e84a                	sd	s2,16(sp)
    8000224a:	e44e                	sd	s3,8(sp)
    8000224c:	e052                	sd	s4,0(sp)
    8000224e:	1800                	addi	s0,sp,48
    80002250:	84aa                	mv	s1,a0
    80002252:	892e                	mv	s2,a1
    80002254:	89b2                	mv	s3,a2
    80002256:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    80002258:	e76ff0ef          	jal	800018ce <myproc>
  if(user_dst){
    8000225c:	cc99                	beqz	s1,8000227a <either_copyout+0x3a>
    return copyout(p->pagetable, dst, src, len);
    8000225e:	86d2                	mv	a3,s4
    80002260:	864e                	mv	a2,s3
    80002262:	85ca                	mv	a1,s2
    80002264:	6928                	ld	a0,80(a0)
    80002266:	b7cff0ef          	jal	800015e2 <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    8000226a:	70a2                	ld	ra,40(sp)
    8000226c:	7402                	ld	s0,32(sp)
    8000226e:	64e2                	ld	s1,24(sp)
    80002270:	6942                	ld	s2,16(sp)
    80002272:	69a2                	ld	s3,8(sp)
    80002274:	6a02                	ld	s4,0(sp)
    80002276:	6145                	addi	sp,sp,48
    80002278:	8082                	ret
    memmove((char *)dst, src, len);
    8000227a:	000a061b          	sext.w	a2,s4
    8000227e:	85ce                	mv	a1,s3
    80002280:	854a                	mv	a0,s2
    80002282:	a7dfe0ef          	jal	80000cfe <memmove>
    return 0;
    80002286:	8526                	mv	a0,s1
    80002288:	b7cd                	j	8000226a <either_copyout+0x2a>

000000008000228a <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    8000228a:	7179                	addi	sp,sp,-48
    8000228c:	f406                	sd	ra,40(sp)
    8000228e:	f022                	sd	s0,32(sp)
    80002290:	ec26                	sd	s1,24(sp)
    80002292:	e84a                	sd	s2,16(sp)
    80002294:	e44e                	sd	s3,8(sp)
    80002296:	e052                	sd	s4,0(sp)
    80002298:	1800                	addi	s0,sp,48
    8000229a:	892a                	mv	s2,a0
    8000229c:	84ae                	mv	s1,a1
    8000229e:	89b2                	mv	s3,a2
    800022a0:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800022a2:	e2cff0ef          	jal	800018ce <myproc>
  if(user_src){
    800022a6:	cc99                	beqz	s1,800022c4 <either_copyin+0x3a>
    return copyin(p->pagetable, dst, src, len);
    800022a8:	86d2                	mv	a3,s4
    800022aa:	864e                	mv	a2,s3
    800022ac:	85ca                	mv	a1,s2
    800022ae:	6928                	ld	a0,80(a0)
    800022b0:	c16ff0ef          	jal	800016c6 <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    800022b4:	70a2                	ld	ra,40(sp)
    800022b6:	7402                	ld	s0,32(sp)
    800022b8:	64e2                	ld	s1,24(sp)
    800022ba:	6942                	ld	s2,16(sp)
    800022bc:	69a2                	ld	s3,8(sp)
    800022be:	6a02                	ld	s4,0(sp)
    800022c0:	6145                	addi	sp,sp,48
    800022c2:	8082                	ret
    memmove(dst, (char*)src, len);
    800022c4:	000a061b          	sext.w	a2,s4
    800022c8:	85ce                	mv	a1,s3
    800022ca:	854a                	mv	a0,s2
    800022cc:	a33fe0ef          	jal	80000cfe <memmove>
    return 0;
    800022d0:	8526                	mv	a0,s1
    800022d2:	b7cd                	j	800022b4 <either_copyin+0x2a>

00000000800022d4 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
    800022d4:	715d                	addi	sp,sp,-80
    800022d6:	e486                	sd	ra,72(sp)
    800022d8:	e0a2                	sd	s0,64(sp)
    800022da:	fc26                	sd	s1,56(sp)
    800022dc:	f84a                	sd	s2,48(sp)
    800022de:	f44e                	sd	s3,40(sp)
    800022e0:	f052                	sd	s4,32(sp)
    800022e2:	ec56                	sd	s5,24(sp)
    800022e4:	e85a                	sd	s6,16(sp)
    800022e6:	e45e                	sd	s7,8(sp)
    800022e8:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    800022ea:	00005517          	auipc	a0,0x5
    800022ee:	d8e50513          	addi	a0,a0,-626 # 80007078 <etext+0x78>
    800022f2:	a08fe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    800022f6:	0000e497          	auipc	s1,0xe
    800022fa:	bea48493          	addi	s1,s1,-1046 # 8000fee0 <proc+0x158>
    800022fe:	00014917          	auipc	s2,0x14
    80002302:	9e290913          	addi	s2,s2,-1566 # 80015ce0 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002306:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???";
    80002308:	00005997          	auipc	s3,0x5
    8000230c:	ef898993          	addi	s3,s3,-264 # 80007200 <etext+0x200>
    printf("%d %s %s", p->pid, state, p->name);
    80002310:	00005a97          	auipc	s5,0x5
    80002314:	ef8a8a93          	addi	s5,s5,-264 # 80007208 <etext+0x208>
    printf("\n");
    80002318:	00005a17          	auipc	s4,0x5
    8000231c:	d60a0a13          	addi	s4,s4,-672 # 80007078 <etext+0x78>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002320:	00005b97          	auipc	s7,0x5
    80002324:	408b8b93          	addi	s7,s7,1032 # 80007728 <states.0>
    80002328:	a829                	j	80002342 <procdump+0x6e>
    printf("%d %s %s", p->pid, state, p->name);
    8000232a:	ed86a583          	lw	a1,-296(a3)
    8000232e:	8556                	mv	a0,s5
    80002330:	9cafe0ef          	jal	800004fa <printf>
    printf("\n");
    80002334:	8552                	mv	a0,s4
    80002336:	9c4fe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    8000233a:	17848493          	addi	s1,s1,376
    8000233e:	03248263          	beq	s1,s2,80002362 <procdump+0x8e>
    if(p->state == UNUSED)
    80002342:	86a6                	mv	a3,s1
    80002344:	ec04a783          	lw	a5,-320(s1)
    80002348:	dbed                	beqz	a5,8000233a <procdump+0x66>
      state = "???";
    8000234a:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    8000234c:	fcfb6fe3          	bltu	s6,a5,8000232a <procdump+0x56>
    80002350:	02079713          	slli	a4,a5,0x20
    80002354:	01d75793          	srli	a5,a4,0x1d
    80002358:	97de                	add	a5,a5,s7
    8000235a:	6390                	ld	a2,0(a5)
    8000235c:	f679                	bnez	a2,8000232a <procdump+0x56>
      state = "???";
    8000235e:	864e                	mv	a2,s3
    80002360:	b7e9                	j	8000232a <procdump+0x56>
  }
}
    80002362:	60a6                	ld	ra,72(sp)
    80002364:	6406                	ld	s0,64(sp)
    80002366:	74e2                	ld	s1,56(sp)
    80002368:	7942                	ld	s2,48(sp)
    8000236a:	79a2                	ld	s3,40(sp)
    8000236c:	7a02                	ld	s4,32(sp)
    8000236e:	6ae2                	ld	s5,24(sp)
    80002370:	6b42                	ld	s6,16(sp)
    80002372:	6ba2                	ld	s7,8(sp)
    80002374:	6161                	addi	sp,sp,80
    80002376:	8082                	ret

0000000080002378 <swtch>:
# Save current registers in old. Load from new.	


.globl swtch
swtch:
        sd ra, 0(a0)
    80002378:	00153023          	sd	ra,0(a0)
        sd sp, 8(a0)
    8000237c:	00253423          	sd	sp,8(a0)
        sd s0, 16(a0)
    80002380:	e900                	sd	s0,16(a0)
        sd s1, 24(a0)
    80002382:	ed04                	sd	s1,24(a0)
        sd s2, 32(a0)
    80002384:	03253023          	sd	s2,32(a0)
        sd s3, 40(a0)
    80002388:	03353423          	sd	s3,40(a0)
        sd s4, 48(a0)
    8000238c:	03453823          	sd	s4,48(a0)
        sd s5, 56(a0)
    80002390:	03553c23          	sd	s5,56(a0)
        sd s6, 64(a0)
    80002394:	05653023          	sd	s6,64(a0)
        sd s7, 72(a0)
    80002398:	05753423          	sd	s7,72(a0)
        sd s8, 80(a0)
    8000239c:	05853823          	sd	s8,80(a0)
        sd s9, 88(a0)
    800023a0:	05953c23          	sd	s9,88(a0)
        sd s10, 96(a0)
    800023a4:	07a53023          	sd	s10,96(a0)
        sd s11, 104(a0)
    800023a8:	07b53423          	sd	s11,104(a0)

        ld ra, 0(a1)
    800023ac:	0005b083          	ld	ra,0(a1)
        ld sp, 8(a1)
    800023b0:	0085b103          	ld	sp,8(a1)
        ld s0, 16(a1)
    800023b4:	6980                	ld	s0,16(a1)
        ld s1, 24(a1)
    800023b6:	6d84                	ld	s1,24(a1)
        ld s2, 32(a1)
    800023b8:	0205b903          	ld	s2,32(a1)
        ld s3, 40(a1)
    800023bc:	0285b983          	ld	s3,40(a1)
        ld s4, 48(a1)
    800023c0:	0305ba03          	ld	s4,48(a1)
        ld s5, 56(a1)
    800023c4:	0385ba83          	ld	s5,56(a1)
        ld s6, 64(a1)
    800023c8:	0405bb03          	ld	s6,64(a1)
        ld s7, 72(a1)
    800023cc:	0485bb83          	ld	s7,72(a1)
        ld s8, 80(a1)
    800023d0:	0505bc03          	ld	s8,80(a1)
        ld s9, 88(a1)
    800023d4:	0585bc83          	ld	s9,88(a1)
        ld s10, 96(a1)
    800023d8:	0605bd03          	ld	s10,96(a1)
        ld s11, 104(a1)
    800023dc:	0685bd83          	ld	s11,104(a1)
        
        ret
    800023e0:	8082                	ret

00000000800023e2 <trapinit>:

extern int devintr();

void
trapinit(void)
{
    800023e2:	1141                	addi	sp,sp,-16
    800023e4:	e406                	sd	ra,8(sp)
    800023e6:	e022                	sd	s0,0(sp)
    800023e8:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    800023ea:	00005597          	auipc	a1,0x5
    800023ee:	e5e58593          	addi	a1,a1,-418 # 80007248 <etext+0x248>
    800023f2:	00013517          	auipc	a0,0x13
    800023f6:	79650513          	addi	a0,a0,1942 # 80015b88 <tickslock>
    800023fa:	f54fe0ef          	jal	80000b4e <initlock>
}
    800023fe:	60a2                	ld	ra,8(sp)
    80002400:	6402                	ld	s0,0(sp)
    80002402:	0141                	addi	sp,sp,16
    80002404:	8082                	ret

0000000080002406 <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    80002406:	1141                	addi	sp,sp,-16
    80002408:	e422                	sd	s0,8(sp)
    8000240a:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    8000240c:	00003797          	auipc	a5,0x3
    80002410:	f9478793          	addi	a5,a5,-108 # 800053a0 <kernelvec>
    80002414:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002418:	6422                	ld	s0,8(sp)
    8000241a:	0141                	addi	sp,sp,16
    8000241c:	8082                	ret

000000008000241e <prepare_return>:
//
// set up trapframe and control registers for a return to user space
//
void
prepare_return(void)
{
    8000241e:	1141                	addi	sp,sp,-16
    80002420:	e406                	sd	ra,8(sp)
    80002422:	e022                	sd	s0,0(sp)
    80002424:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002426:	ca8ff0ef          	jal	800018ce <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000242a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    8000242e:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002430:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(). because a trap from kernel
  // code to usertrap would be a disaster, turn off interrupts.
  intr_off();

  // send syscalls, interrupts, and exceptions to uservec in trampoline.S
  uint64 trampoline_uservec = TRAMPOLINE + (uservec - trampoline);
    80002434:	04000737          	lui	a4,0x4000
    80002438:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    8000243a:	0732                	slli	a4,a4,0xc
    8000243c:	00004797          	auipc	a5,0x4
    80002440:	bc478793          	addi	a5,a5,-1084 # 80006000 <_trampoline>
    80002444:	00004697          	auipc	a3,0x4
    80002448:	bbc68693          	addi	a3,a3,-1092 # 80006000 <_trampoline>
    8000244c:	8f95                	sub	a5,a5,a3
    8000244e:	97ba                	add	a5,a5,a4
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002450:	10579073          	csrw	stvec,a5
  w_stvec(trampoline_uservec);

  // set up trapframe values that uservec will need when
  // the process next traps into the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    80002454:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    80002456:	18002773          	csrr	a4,satp
    8000245a:	e398                	sd	a4,0(a5)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    8000245c:	6d38                	ld	a4,88(a0)
    8000245e:	613c                	ld	a5,64(a0)
    80002460:	6685                	lui	a3,0x1
    80002462:	97b6                	add	a5,a5,a3
    80002464:	e71c                	sd	a5,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    80002466:	6d3c                	ld	a5,88(a0)
    80002468:	00000717          	auipc	a4,0x0
    8000246c:	0f870713          	addi	a4,a4,248 # 80002560 <usertrap>
    80002470:	eb98                	sd	a4,16(a5)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    80002472:	6d3c                	ld	a5,88(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    80002474:	8712                	mv	a4,tp
    80002476:	f398                	sd	a4,32(a5)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002478:	100027f3          	csrr	a5,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    8000247c:	eff7f793          	andi	a5,a5,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    80002480:	0207e793          	ori	a5,a5,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002484:	10079073          	csrw	sstatus,a5
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    80002488:	6d3c                	ld	a5,88(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    8000248a:	6f9c                	ld	a5,24(a5)
    8000248c:	14179073          	csrw	sepc,a5
}
    80002490:	60a2                	ld	ra,8(sp)
    80002492:	6402                	ld	s0,0(sp)
    80002494:	0141                	addi	sp,sp,16
    80002496:	8082                	ret

0000000080002498 <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    80002498:	1101                	addi	sp,sp,-32
    8000249a:	ec06                	sd	ra,24(sp)
    8000249c:	e822                	sd	s0,16(sp)
    8000249e:	1000                	addi	s0,sp,32
  if(cpuid() == 0){
    800024a0:	c02ff0ef          	jal	800018a2 <cpuid>
    800024a4:	cd11                	beqz	a0,800024c0 <clockintr+0x28>
  asm volatile("csrr %0, time" : "=r" (x) );
    800024a6:	c01027f3          	rdtime	a5
  }

  // ask for the next timer interrupt. this also clears
  // the interrupt request. 1000000 is about a tenth
  // of a second.
  w_stimecmp(r_time() + 1000000);
    800024aa:	000f4737          	lui	a4,0xf4
    800024ae:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    800024b2:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    800024b4:	14d79073          	csrw	stimecmp,a5
}
    800024b8:	60e2                	ld	ra,24(sp)
    800024ba:	6442                	ld	s0,16(sp)
    800024bc:	6105                	addi	sp,sp,32
    800024be:	8082                	ret
    800024c0:	e426                	sd	s1,8(sp)
    acquire(&tickslock);
    800024c2:	00013497          	auipc	s1,0x13
    800024c6:	6c648493          	addi	s1,s1,1734 # 80015b88 <tickslock>
    800024ca:	8526                	mv	a0,s1
    800024cc:	f02fe0ef          	jal	80000bce <acquire>
    ticks++;
    800024d0:	00005517          	auipc	a0,0x5
    800024d4:	38850513          	addi	a0,a0,904 # 80007858 <ticks>
    800024d8:	411c                	lw	a5,0(a0)
    800024da:	2785                	addiw	a5,a5,1
    800024dc:	c11c                	sw	a5,0(a0)
    wakeup(&ticks);
    800024de:	a53ff0ef          	jal	80001f30 <wakeup>
    release(&tickslock);
    800024e2:	8526                	mv	a0,s1
    800024e4:	f82fe0ef          	jal	80000c66 <release>
    800024e8:	64a2                	ld	s1,8(sp)
    800024ea:	bf75                	j	800024a6 <clockintr+0xe>

00000000800024ec <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    800024ec:	1101                	addi	sp,sp,-32
    800024ee:	ec06                	sd	ra,24(sp)
    800024f0:	e822                	sd	s0,16(sp)
    800024f2:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    800024f4:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if(scause == 0x8000000000000009L){
    800024f8:	57fd                	li	a5,-1
    800024fa:	17fe                	slli	a5,a5,0x3f
    800024fc:	07a5                	addi	a5,a5,9
    800024fe:	00f70c63          	beq	a4,a5,80002516 <devintr+0x2a>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000005L){
    80002502:	57fd                	li	a5,-1
    80002504:	17fe                	slli	a5,a5,0x3f
    80002506:	0795                	addi	a5,a5,5
    // timer interrupt.
    clockintr();
    return 2;
  } else {
    return 0;
    80002508:	4501                	li	a0,0
  } else if(scause == 0x8000000000000005L){
    8000250a:	04f70763          	beq	a4,a5,80002558 <devintr+0x6c>
  }
}
    8000250e:	60e2                	ld	ra,24(sp)
    80002510:	6442                	ld	s0,16(sp)
    80002512:	6105                	addi	sp,sp,32
    80002514:	8082                	ret
    80002516:	e426                	sd	s1,8(sp)
    int irq = plic_claim();
    80002518:	735020ef          	jal	8000544c <plic_claim>
    8000251c:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    8000251e:	47a9                	li	a5,10
    80002520:	00f50963          	beq	a0,a5,80002532 <devintr+0x46>
    } else if(irq == VIRTIO0_IRQ){
    80002524:	4785                	li	a5,1
    80002526:	00f50963          	beq	a0,a5,80002538 <devintr+0x4c>
    return 1;
    8000252a:	4505                	li	a0,1
    } else if(irq){
    8000252c:	e889                	bnez	s1,8000253e <devintr+0x52>
    8000252e:	64a2                	ld	s1,8(sp)
    80002530:	bff9                	j	8000250e <devintr+0x22>
      uartintr();
    80002532:	c7efe0ef          	jal	800009b0 <uartintr>
    if(irq)
    80002536:	a819                	j	8000254c <devintr+0x60>
      virtio_disk_intr();
    80002538:	3da030ef          	jal	80005912 <virtio_disk_intr>
    if(irq)
    8000253c:	a801                	j	8000254c <devintr+0x60>
      printf("unexpected interrupt irq=%d\n", irq);
    8000253e:	85a6                	mv	a1,s1
    80002540:	00005517          	auipc	a0,0x5
    80002544:	d1050513          	addi	a0,a0,-752 # 80007250 <etext+0x250>
    80002548:	fb3fd0ef          	jal	800004fa <printf>
      plic_complete(irq);
    8000254c:	8526                	mv	a0,s1
    8000254e:	71f020ef          	jal	8000546c <plic_complete>
    return 1;
    80002552:	4505                	li	a0,1
    80002554:	64a2                	ld	s1,8(sp)
    80002556:	bf65                	j	8000250e <devintr+0x22>
    clockintr();
    80002558:	f41ff0ef          	jal	80002498 <clockintr>
    return 2;
    8000255c:	4509                	li	a0,2
    8000255e:	bf45                	j	8000250e <devintr+0x22>

0000000080002560 <usertrap>:
{
    80002560:	1101                	addi	sp,sp,-32
    80002562:	ec06                	sd	ra,24(sp)
    80002564:	e822                	sd	s0,16(sp)
    80002566:	e426                	sd	s1,8(sp)
    80002568:	e04a                	sd	s2,0(sp)
    8000256a:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000256c:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    80002570:	1007f793          	andi	a5,a5,256
    80002574:	eba5                	bnez	a5,800025e4 <usertrap+0x84>
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002576:	00003797          	auipc	a5,0x3
    8000257a:	e2a78793          	addi	a5,a5,-470 # 800053a0 <kernelvec>
    8000257e:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    80002582:	b4cff0ef          	jal	800018ce <myproc>
    80002586:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    80002588:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    8000258a:	14102773          	csrr	a4,sepc
    8000258e:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002590:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    80002594:	47a1                	li	a5,8
    80002596:	04f70d63          	beq	a4,a5,800025f0 <usertrap+0x90>
  } else if((which_dev = devintr()) != 0){
    8000259a:	f53ff0ef          	jal	800024ec <devintr>
    8000259e:	892a                	mv	s2,a0
    800025a0:	e945                	bnez	a0,80002650 <usertrap+0xf0>
    800025a2:	14202773          	csrr	a4,scause
  } else if((r_scause() == 15 || r_scause() == 13) &&
    800025a6:	47bd                	li	a5,15
    800025a8:	08f70863          	beq	a4,a5,80002638 <usertrap+0xd8>
    800025ac:	14202773          	csrr	a4,scause
    800025b0:	47b5                	li	a5,13
    800025b2:	08f70363          	beq	a4,a5,80002638 <usertrap+0xd8>
    800025b6:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause 0x%lx pid=%d\n", r_scause(), p->pid);
    800025ba:	5890                	lw	a2,48(s1)
    800025bc:	00005517          	auipc	a0,0x5
    800025c0:	cd450513          	addi	a0,a0,-812 # 80007290 <etext+0x290>
    800025c4:	f37fd0ef          	jal	800004fa <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800025c8:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    800025cc:	14302673          	csrr	a2,stval
    printf("            sepc=0x%lx stval=0x%lx\n", r_sepc(), r_stval());
    800025d0:	00005517          	auipc	a0,0x5
    800025d4:	cf050513          	addi	a0,a0,-784 # 800072c0 <etext+0x2c0>
    800025d8:	f23fd0ef          	jal	800004fa <printf>
    setkilled(p);
    800025dc:	8526                	mv	a0,s1
    800025de:	b1bff0ef          	jal	800020f8 <setkilled>
    800025e2:	a035                	j	8000260e <usertrap+0xae>
    panic("usertrap: not from user mode");
    800025e4:	00005517          	auipc	a0,0x5
    800025e8:	c8c50513          	addi	a0,a0,-884 # 80007270 <etext+0x270>
    800025ec:	9f4fe0ef          	jal	800007e0 <panic>
    if(killed(p))
    800025f0:	b2dff0ef          	jal	8000211c <killed>
    800025f4:	ed15                	bnez	a0,80002630 <usertrap+0xd0>
    p->trapframe->epc += 4;
    800025f6:	6cb8                	ld	a4,88(s1)
    800025f8:	6f1c                	ld	a5,24(a4)
    800025fa:	0791                	addi	a5,a5,4
    800025fc:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800025fe:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002602:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002606:	10079073          	csrw	sstatus,a5
    syscall();
    8000260a:	28e000ef          	jal	80002898 <syscall>
  if(killed(p))
    8000260e:	8526                	mv	a0,s1
    80002610:	b0dff0ef          	jal	8000211c <killed>
    80002614:	e139                	bnez	a0,8000265a <usertrap+0xfa>
  prepare_return();
    80002616:	e09ff0ef          	jal	8000241e <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    8000261a:	68a8                	ld	a0,80(s1)
    8000261c:	8131                	srli	a0,a0,0xc
    8000261e:	57fd                	li	a5,-1
    80002620:	17fe                	slli	a5,a5,0x3f
    80002622:	8d5d                	or	a0,a0,a5
}
    80002624:	60e2                	ld	ra,24(sp)
    80002626:	6442                	ld	s0,16(sp)
    80002628:	64a2                	ld	s1,8(sp)
    8000262a:	6902                	ld	s2,0(sp)
    8000262c:	6105                	addi	sp,sp,32
    8000262e:	8082                	ret
      kexit(-1);
    80002630:	557d                	li	a0,-1
    80002632:	9bfff0ef          	jal	80001ff0 <kexit>
    80002636:	b7c1                	j	800025f6 <usertrap+0x96>
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002638:	143025f3          	csrr	a1,stval
  asm volatile("csrr %0, scause" : "=r" (x) );
    8000263c:	14202673          	csrr	a2,scause
            vmfault(p->pagetable, r_stval(), (r_scause() == 13)? 1 : 0) != 0) {
    80002640:	164d                	addi	a2,a2,-13 # ff3 <_entry-0x7ffff00d>
    80002642:	00163613          	seqz	a2,a2
    80002646:	68a8                	ld	a0,80(s1)
    80002648:	f19fe0ef          	jal	80001560 <vmfault>
  } else if((r_scause() == 15 || r_scause() == 13) &&
    8000264c:	f169                	bnez	a0,8000260e <usertrap+0xae>
    8000264e:	b7a5                	j	800025b6 <usertrap+0x56>
  if(killed(p))
    80002650:	8526                	mv	a0,s1
    80002652:	acbff0ef          	jal	8000211c <killed>
    80002656:	c511                	beqz	a0,80002662 <usertrap+0x102>
    80002658:	a011                	j	8000265c <usertrap+0xfc>
    8000265a:	4901                	li	s2,0
    kexit(-1);
    8000265c:	557d                	li	a0,-1
    8000265e:	993ff0ef          	jal	80001ff0 <kexit>
if(which_dev == 2){
    80002662:	4789                	li	a5,2
    80002664:	faf919e3          	bne	s2,a5,80002616 <usertrap+0xb6>
  struct proc *p = myproc();
    80002668:	a66ff0ef          	jal	800018ce <myproc>
  p->quantumleft--;
    8000266c:	16c52783          	lw	a5,364(a0)
    80002670:	37fd                	addiw	a5,a5,-1
    80002672:	0007871b          	sext.w	a4,a5
    80002676:	16f52623          	sw	a5,364(a0)
  p->mlfq_ticks++;
    8000267a:	17052783          	lw	a5,368(a0)
    8000267e:	2785                	addiw	a5,a5,1
    80002680:	16f52823          	sw	a5,368(a0)
  if(p->quantumleft <= 0){
    80002684:	f8e049e3          	bgtz	a4,80002616 <usertrap+0xb6>
    if(p->queue < NMLFQ - 1){
    80002688:	16852783          	lw	a5,360(a0)
    8000268c:	4705                	li	a4,1
    8000268e:	00f75463          	bge	a4,a5,80002696 <usertrap+0x136>
      p->quantumleft = Q2_QUANTUM;
    80002692:	4911                	li	s2,4
    80002694:	a801                	j	800026a4 <usertrap+0x144>
      p->queue++;
    80002696:	2785                	addiw	a5,a5,1
    80002698:	0007871b          	sext.w	a4,a5
    8000269c:	16f52423          	sw	a5,360(a0)
    if(p->queue == 0)
    800026a0:	e719                	bnez	a4,800026ae <usertrap+0x14e>
    800026a2:	4905                	li	s2,1
      p->quantumleft = Q0_QUANTUM;
    800026a4:	17252623          	sw	s2,364(a0)
    yield();
    800026a8:	811ff0ef          	jal	80001eb8 <yield>
    800026ac:	b7ad                	j	80002616 <usertrap+0xb6>
    else if(p->queue == 1)
    800026ae:	4785                	li	a5,1
    800026b0:	fef70ae3          	beq	a4,a5,800026a4 <usertrap+0x144>
    800026b4:	bff9                	j	80002692 <usertrap+0x132>

00000000800026b6 <kerneltrap>:
{
    800026b6:	7179                	addi	sp,sp,-48
    800026b8:	f406                	sd	ra,40(sp)
    800026ba:	f022                	sd	s0,32(sp)
    800026bc:	ec26                	sd	s1,24(sp)
    800026be:	e84a                	sd	s2,16(sp)
    800026c0:	e44e                	sd	s3,8(sp)
    800026c2:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800026c4:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800026c8:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    800026cc:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    800026d0:	1004f793          	andi	a5,s1,256
    800026d4:	c795                	beqz	a5,80002700 <kerneltrap+0x4a>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800026d6:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    800026da:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    800026dc:	eb85                	bnez	a5,8000270c <kerneltrap+0x56>
  if((which_dev = devintr()) == 0){
    800026de:	e0fff0ef          	jal	800024ec <devintr>
    800026e2:	c91d                	beqz	a0,80002718 <kerneltrap+0x62>
  if(which_dev == 2 && myproc() != 0)
    800026e4:	4789                	li	a5,2
    800026e6:	04f50a63          	beq	a0,a5,8000273a <kerneltrap+0x84>
  asm volatile("csrw sepc, %0" : : "r" (x));
    800026ea:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    800026ee:	10049073          	csrw	sstatus,s1
}
    800026f2:	70a2                	ld	ra,40(sp)
    800026f4:	7402                	ld	s0,32(sp)
    800026f6:	64e2                	ld	s1,24(sp)
    800026f8:	6942                	ld	s2,16(sp)
    800026fa:	69a2                	ld	s3,8(sp)
    800026fc:	6145                	addi	sp,sp,48
    800026fe:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    80002700:	00005517          	auipc	a0,0x5
    80002704:	be850513          	addi	a0,a0,-1048 # 800072e8 <etext+0x2e8>
    80002708:	8d8fe0ef          	jal	800007e0 <panic>
    panic("kerneltrap: interrupts enabled");
    8000270c:	00005517          	auipc	a0,0x5
    80002710:	c0450513          	addi	a0,a0,-1020 # 80007310 <etext+0x310>
    80002714:	8ccfe0ef          	jal	800007e0 <panic>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    80002718:	14102673          	csrr	a2,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    8000271c:	143026f3          	csrr	a3,stval
    printf("scause=0x%lx sepc=0x%lx stval=0x%lx\n", scause, r_sepc(), r_stval());
    80002720:	85ce                	mv	a1,s3
    80002722:	00005517          	auipc	a0,0x5
    80002726:	c0e50513          	addi	a0,a0,-1010 # 80007330 <etext+0x330>
    8000272a:	dd1fd0ef          	jal	800004fa <printf>
    panic("kerneltrap");
    8000272e:	00005517          	auipc	a0,0x5
    80002732:	c2a50513          	addi	a0,a0,-982 # 80007358 <etext+0x358>
    80002736:	8aafe0ef          	jal	800007e0 <panic>
  if(which_dev == 2 && myproc() != 0)
    8000273a:	994ff0ef          	jal	800018ce <myproc>
    8000273e:	d555                	beqz	a0,800026ea <kerneltrap+0x34>
    yield();
    80002740:	f78ff0ef          	jal	80001eb8 <yield>
    80002744:	b75d                	j	800026ea <kerneltrap+0x34>

0000000080002746 <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    80002746:	1101                	addi	sp,sp,-32
    80002748:	ec06                	sd	ra,24(sp)
    8000274a:	e822                	sd	s0,16(sp)
    8000274c:	e426                	sd	s1,8(sp)
    8000274e:	1000                	addi	s0,sp,32
    80002750:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80002752:	97cff0ef          	jal	800018ce <myproc>
  switch (n) {
    80002756:	4795                	li	a5,5
    80002758:	0497e163          	bltu	a5,s1,8000279a <argraw+0x54>
    8000275c:	048a                	slli	s1,s1,0x2
    8000275e:	00005717          	auipc	a4,0x5
    80002762:	ffa70713          	addi	a4,a4,-6 # 80007758 <states.0+0x30>
    80002766:	94ba                	add	s1,s1,a4
    80002768:	409c                	lw	a5,0(s1)
    8000276a:	97ba                	add	a5,a5,a4
    8000276c:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    8000276e:	6d3c                	ld	a5,88(a0)
    80002770:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    80002772:	60e2                	ld	ra,24(sp)
    80002774:	6442                	ld	s0,16(sp)
    80002776:	64a2                	ld	s1,8(sp)
    80002778:	6105                	addi	sp,sp,32
    8000277a:	8082                	ret
    return p->trapframe->a1;
    8000277c:	6d3c                	ld	a5,88(a0)
    8000277e:	7fa8                	ld	a0,120(a5)
    80002780:	bfcd                	j	80002772 <argraw+0x2c>
    return p->trapframe->a2;
    80002782:	6d3c                	ld	a5,88(a0)
    80002784:	63c8                	ld	a0,128(a5)
    80002786:	b7f5                	j	80002772 <argraw+0x2c>
    return p->trapframe->a3;
    80002788:	6d3c                	ld	a5,88(a0)
    8000278a:	67c8                	ld	a0,136(a5)
    8000278c:	b7dd                	j	80002772 <argraw+0x2c>
    return p->trapframe->a4;
    8000278e:	6d3c                	ld	a5,88(a0)
    80002790:	6bc8                	ld	a0,144(a5)
    80002792:	b7c5                	j	80002772 <argraw+0x2c>
    return p->trapframe->a5;
    80002794:	6d3c                	ld	a5,88(a0)
    80002796:	6fc8                	ld	a0,152(a5)
    80002798:	bfe9                	j	80002772 <argraw+0x2c>
  panic("argraw");
    8000279a:	00005517          	auipc	a0,0x5
    8000279e:	bce50513          	addi	a0,a0,-1074 # 80007368 <etext+0x368>
    800027a2:	83efe0ef          	jal	800007e0 <panic>

00000000800027a6 <fetchaddr>:
{
    800027a6:	1101                	addi	sp,sp,-32
    800027a8:	ec06                	sd	ra,24(sp)
    800027aa:	e822                	sd	s0,16(sp)
    800027ac:	e426                	sd	s1,8(sp)
    800027ae:	e04a                	sd	s2,0(sp)
    800027b0:	1000                	addi	s0,sp,32
    800027b2:	84aa                	mv	s1,a0
    800027b4:	892e                	mv	s2,a1
  struct proc *p = myproc();
    800027b6:	918ff0ef          	jal	800018ce <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz) // both tests needed, in case of overflow
    800027ba:	653c                	ld	a5,72(a0)
    800027bc:	02f4f663          	bgeu	s1,a5,800027e8 <fetchaddr+0x42>
    800027c0:	00848713          	addi	a4,s1,8
    800027c4:	02e7e463          	bltu	a5,a4,800027ec <fetchaddr+0x46>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    800027c8:	46a1                	li	a3,8
    800027ca:	8626                	mv	a2,s1
    800027cc:	85ca                	mv	a1,s2
    800027ce:	6928                	ld	a0,80(a0)
    800027d0:	ef7fe0ef          	jal	800016c6 <copyin>
    800027d4:	00a03533          	snez	a0,a0
    800027d8:	40a00533          	neg	a0,a0
}
    800027dc:	60e2                	ld	ra,24(sp)
    800027de:	6442                	ld	s0,16(sp)
    800027e0:	64a2                	ld	s1,8(sp)
    800027e2:	6902                	ld	s2,0(sp)
    800027e4:	6105                	addi	sp,sp,32
    800027e6:	8082                	ret
    return -1;
    800027e8:	557d                	li	a0,-1
    800027ea:	bfcd                	j	800027dc <fetchaddr+0x36>
    800027ec:	557d                	li	a0,-1
    800027ee:	b7fd                	j	800027dc <fetchaddr+0x36>

00000000800027f0 <fetchstr>:
{
    800027f0:	7179                	addi	sp,sp,-48
    800027f2:	f406                	sd	ra,40(sp)
    800027f4:	f022                	sd	s0,32(sp)
    800027f6:	ec26                	sd	s1,24(sp)
    800027f8:	e84a                	sd	s2,16(sp)
    800027fa:	e44e                	sd	s3,8(sp)
    800027fc:	1800                	addi	s0,sp,48
    800027fe:	892a                	mv	s2,a0
    80002800:	84ae                	mv	s1,a1
    80002802:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    80002804:	8caff0ef          	jal	800018ce <myproc>
  if(copyinstr(p->pagetable, buf, addr, max) < 0)
    80002808:	86ce                	mv	a3,s3
    8000280a:	864a                	mv	a2,s2
    8000280c:	85a6                	mv	a1,s1
    8000280e:	6928                	ld	a0,80(a0)
    80002810:	c79fe0ef          	jal	80001488 <copyinstr>
    80002814:	00054c63          	bltz	a0,8000282c <fetchstr+0x3c>
  return strlen(buf);
    80002818:	8526                	mv	a0,s1
    8000281a:	df8fe0ef          	jal	80000e12 <strlen>
}
    8000281e:	70a2                	ld	ra,40(sp)
    80002820:	7402                	ld	s0,32(sp)
    80002822:	64e2                	ld	s1,24(sp)
    80002824:	6942                	ld	s2,16(sp)
    80002826:	69a2                	ld	s3,8(sp)
    80002828:	6145                	addi	sp,sp,48
    8000282a:	8082                	ret
    return -1;
    8000282c:	557d                	li	a0,-1
    8000282e:	bfc5                	j	8000281e <fetchstr+0x2e>

0000000080002830 <argint>:

// Fetch the nth 32-bit system call argument.
void
argint(int n, int *ip)
{
    80002830:	1101                	addi	sp,sp,-32
    80002832:	ec06                	sd	ra,24(sp)
    80002834:	e822                	sd	s0,16(sp)
    80002836:	e426                	sd	s1,8(sp)
    80002838:	1000                	addi	s0,sp,32
    8000283a:	84ae                	mv	s1,a1
  *ip = argraw(n);
    8000283c:	f0bff0ef          	jal	80002746 <argraw>
    80002840:	c088                	sw	a0,0(s1)
}
    80002842:	60e2                	ld	ra,24(sp)
    80002844:	6442                	ld	s0,16(sp)
    80002846:	64a2                	ld	s1,8(sp)
    80002848:	6105                	addi	sp,sp,32
    8000284a:	8082                	ret

000000008000284c <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
void
argaddr(int n, uint64 *ip)
{
    8000284c:	1101                	addi	sp,sp,-32
    8000284e:	ec06                	sd	ra,24(sp)
    80002850:	e822                	sd	s0,16(sp)
    80002852:	e426                	sd	s1,8(sp)
    80002854:	1000                	addi	s0,sp,32
    80002856:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002858:	eefff0ef          	jal	80002746 <argraw>
    8000285c:	e088                	sd	a0,0(s1)
}
    8000285e:	60e2                	ld	ra,24(sp)
    80002860:	6442                	ld	s0,16(sp)
    80002862:	64a2                	ld	s1,8(sp)
    80002864:	6105                	addi	sp,sp,32
    80002866:	8082                	ret

0000000080002868 <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002868:	7179                	addi	sp,sp,-48
    8000286a:	f406                	sd	ra,40(sp)
    8000286c:	f022                	sd	s0,32(sp)
    8000286e:	ec26                	sd	s1,24(sp)
    80002870:	e84a                	sd	s2,16(sp)
    80002872:	1800                	addi	s0,sp,48
    80002874:	84ae                	mv	s1,a1
    80002876:	8932                	mv	s2,a2
  uint64 addr;
  argaddr(n, &addr);
    80002878:	fd840593          	addi	a1,s0,-40
    8000287c:	fd1ff0ef          	jal	8000284c <argaddr>
  return fetchstr(addr, buf, max);
    80002880:	864a                	mv	a2,s2
    80002882:	85a6                	mv	a1,s1
    80002884:	fd843503          	ld	a0,-40(s0)
    80002888:	f69ff0ef          	jal	800027f0 <fetchstr>
}
    8000288c:	70a2                	ld	ra,40(sp)
    8000288e:	7402                	ld	s0,32(sp)
    80002890:	64e2                	ld	s1,24(sp)
    80002892:	6942                	ld	s2,16(sp)
    80002894:	6145                	addi	sp,sp,48
    80002896:	8082                	ret

0000000080002898 <syscall>:
[SYS_close]   sys_close,
};

void
syscall(void)
{
    80002898:	1101                	addi	sp,sp,-32
    8000289a:	ec06                	sd	ra,24(sp)
    8000289c:	e822                	sd	s0,16(sp)
    8000289e:	e426                	sd	s1,8(sp)
    800028a0:	e04a                	sd	s2,0(sp)
    800028a2:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    800028a4:	82aff0ef          	jal	800018ce <myproc>
    800028a8:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    800028aa:	05853903          	ld	s2,88(a0)
    800028ae:	0a893783          	ld	a5,168(s2)
    800028b2:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    800028b6:	37fd                	addiw	a5,a5,-1
    800028b8:	4751                	li	a4,20
    800028ba:	00f76f63          	bltu	a4,a5,800028d8 <syscall+0x40>
    800028be:	00369713          	slli	a4,a3,0x3
    800028c2:	00005797          	auipc	a5,0x5
    800028c6:	eae78793          	addi	a5,a5,-338 # 80007770 <syscalls>
    800028ca:	97ba                	add	a5,a5,a4
    800028cc:	639c                	ld	a5,0(a5)
    800028ce:	c789                	beqz	a5,800028d8 <syscall+0x40>
    // Use num to lookup the system call function for num, call it,
    // and store its return value in p->trapframe->a0
    p->trapframe->a0 = syscalls[num]();
    800028d0:	9782                	jalr	a5
    800028d2:	06a93823          	sd	a0,112(s2)
    800028d6:	a829                	j	800028f0 <syscall+0x58>
  } else {
    printf("%d %s: unknown sys call %d\n",
    800028d8:	15848613          	addi	a2,s1,344
    800028dc:	588c                	lw	a1,48(s1)
    800028de:	00005517          	auipc	a0,0x5
    800028e2:	a9250513          	addi	a0,a0,-1390 # 80007370 <etext+0x370>
    800028e6:	c15fd0ef          	jal	800004fa <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    800028ea:	6cbc                	ld	a5,88(s1)
    800028ec:	577d                	li	a4,-1
    800028ee:	fbb8                	sd	a4,112(a5)
  }
}
    800028f0:	60e2                	ld	ra,24(sp)
    800028f2:	6442                	ld	s0,16(sp)
    800028f4:	64a2                	ld	s1,8(sp)
    800028f6:	6902                	ld	s2,0(sp)
    800028f8:	6105                	addi	sp,sp,32
    800028fa:	8082                	ret

00000000800028fc <sys_exit>:
#include "proc.h"
#include "vm.h"

uint64
sys_exit(void)
{
    800028fc:	1101                	addi	sp,sp,-32
    800028fe:	ec06                	sd	ra,24(sp)
    80002900:	e822                	sd	s0,16(sp)
    80002902:	1000                	addi	s0,sp,32
  int n;
  argint(0, &n);
    80002904:	fec40593          	addi	a1,s0,-20
    80002908:	4501                	li	a0,0
    8000290a:	f27ff0ef          	jal	80002830 <argint>
  kexit(n);
    8000290e:	fec42503          	lw	a0,-20(s0)
    80002912:	edeff0ef          	jal	80001ff0 <kexit>
  return 0;  // not reached
}
    80002916:	4501                	li	a0,0
    80002918:	60e2                	ld	ra,24(sp)
    8000291a:	6442                	ld	s0,16(sp)
    8000291c:	6105                	addi	sp,sp,32
    8000291e:	8082                	ret

0000000080002920 <sys_getpid>:

uint64
sys_getpid(void)
{
    80002920:	1141                	addi	sp,sp,-16
    80002922:	e406                	sd	ra,8(sp)
    80002924:	e022                	sd	s0,0(sp)
    80002926:	0800                	addi	s0,sp,16
  return myproc()->pid;
    80002928:	fa7fe0ef          	jal	800018ce <myproc>
}
    8000292c:	5908                	lw	a0,48(a0)
    8000292e:	60a2                	ld	ra,8(sp)
    80002930:	6402                	ld	s0,0(sp)
    80002932:	0141                	addi	sp,sp,16
    80002934:	8082                	ret

0000000080002936 <sys_fork>:

uint64
sys_fork(void)
{
    80002936:	1141                	addi	sp,sp,-16
    80002938:	e406                	sd	ra,8(sp)
    8000293a:	e022                	sd	s0,0(sp)
    8000293c:	0800                	addi	s0,sp,16
  return kfork();
    8000293e:	b00ff0ef          	jal	80001c3e <kfork>
}
    80002942:	60a2                	ld	ra,8(sp)
    80002944:	6402                	ld	s0,0(sp)
    80002946:	0141                	addi	sp,sp,16
    80002948:	8082                	ret

000000008000294a <sys_wait>:

uint64
sys_wait(void)
{
    8000294a:	1101                	addi	sp,sp,-32
    8000294c:	ec06                	sd	ra,24(sp)
    8000294e:	e822                	sd	s0,16(sp)
    80002950:	1000                	addi	s0,sp,32
  uint64 p;
  argaddr(0, &p);
    80002952:	fe840593          	addi	a1,s0,-24
    80002956:	4501                	li	a0,0
    80002958:	ef5ff0ef          	jal	8000284c <argaddr>
  return kwait(p);
    8000295c:	fe843503          	ld	a0,-24(s0)
    80002960:	fe6ff0ef          	jal	80002146 <kwait>
}
    80002964:	60e2                	ld	ra,24(sp)
    80002966:	6442                	ld	s0,16(sp)
    80002968:	6105                	addi	sp,sp,32
    8000296a:	8082                	ret

000000008000296c <sys_sbrk>:

uint64
sys_sbrk(void)
{
    8000296c:	7179                	addi	sp,sp,-48
    8000296e:	f406                	sd	ra,40(sp)
    80002970:	f022                	sd	s0,32(sp)
    80002972:	ec26                	sd	s1,24(sp)
    80002974:	1800                	addi	s0,sp,48
  uint64 addr;
  int t;
  int n;

  argint(0, &n);
    80002976:	fd840593          	addi	a1,s0,-40
    8000297a:	4501                	li	a0,0
    8000297c:	eb5ff0ef          	jal	80002830 <argint>
  argint(1, &t);
    80002980:	fdc40593          	addi	a1,s0,-36
    80002984:	4505                	li	a0,1
    80002986:	eabff0ef          	jal	80002830 <argint>
  addr = myproc()->sz;
    8000298a:	f45fe0ef          	jal	800018ce <myproc>
    8000298e:	6524                	ld	s1,72(a0)

  if(t == SBRK_EAGER || n < 0) {
    80002990:	fdc42703          	lw	a4,-36(s0)
    80002994:	4785                	li	a5,1
    80002996:	02f70763          	beq	a4,a5,800029c4 <sys_sbrk+0x58>
    8000299a:	fd842783          	lw	a5,-40(s0)
    8000299e:	0207c363          	bltz	a5,800029c4 <sys_sbrk+0x58>
    }
  } else {
    // Lazily allocate memory for this process: increase its memory
    // size but don't allocate memory. If the processes uses the
    // memory, vmfault() will allocate it.
    if(addr + n < addr)
    800029a2:	97a6                	add	a5,a5,s1
    800029a4:	0297ee63          	bltu	a5,s1,800029e0 <sys_sbrk+0x74>
      return -1;
    if(addr + n > TRAPFRAME)
    800029a8:	02000737          	lui	a4,0x2000
    800029ac:	177d                	addi	a4,a4,-1 # 1ffffff <_entry-0x7e000001>
    800029ae:	0736                	slli	a4,a4,0xd
    800029b0:	02f76a63          	bltu	a4,a5,800029e4 <sys_sbrk+0x78>
      return -1;
    myproc()->sz += n;
    800029b4:	f1bfe0ef          	jal	800018ce <myproc>
    800029b8:	fd842703          	lw	a4,-40(s0)
    800029bc:	653c                	ld	a5,72(a0)
    800029be:	97ba                	add	a5,a5,a4
    800029c0:	e53c                	sd	a5,72(a0)
    800029c2:	a039                	j	800029d0 <sys_sbrk+0x64>
    if(growproc(n) < 0) {
    800029c4:	fd842503          	lw	a0,-40(s0)
    800029c8:	a14ff0ef          	jal	80001bdc <growproc>
    800029cc:	00054863          	bltz	a0,800029dc <sys_sbrk+0x70>
  }
  return addr;
}
    800029d0:	8526                	mv	a0,s1
    800029d2:	70a2                	ld	ra,40(sp)
    800029d4:	7402                	ld	s0,32(sp)
    800029d6:	64e2                	ld	s1,24(sp)
    800029d8:	6145                	addi	sp,sp,48
    800029da:	8082                	ret
      return -1;
    800029dc:	54fd                	li	s1,-1
    800029de:	bfcd                	j	800029d0 <sys_sbrk+0x64>
      return -1;
    800029e0:	54fd                	li	s1,-1
    800029e2:	b7fd                	j	800029d0 <sys_sbrk+0x64>
      return -1;
    800029e4:	54fd                	li	s1,-1
    800029e6:	b7ed                	j	800029d0 <sys_sbrk+0x64>

00000000800029e8 <sys_pause>:

uint64
sys_pause(void)
{
    800029e8:	7139                	addi	sp,sp,-64
    800029ea:	fc06                	sd	ra,56(sp)
    800029ec:	f822                	sd	s0,48(sp)
    800029ee:	f04a                	sd	s2,32(sp)
    800029f0:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  argint(0, &n);
    800029f2:	fcc40593          	addi	a1,s0,-52
    800029f6:	4501                	li	a0,0
    800029f8:	e39ff0ef          	jal	80002830 <argint>
  if(n < 0)
    800029fc:	fcc42783          	lw	a5,-52(s0)
    80002a00:	0607c763          	bltz	a5,80002a6e <sys_pause+0x86>
    n = 0;
  acquire(&tickslock);
    80002a04:	00013517          	auipc	a0,0x13
    80002a08:	18450513          	addi	a0,a0,388 # 80015b88 <tickslock>
    80002a0c:	9c2fe0ef          	jal	80000bce <acquire>
  ticks0 = ticks;
    80002a10:	00005917          	auipc	s2,0x5
    80002a14:	e4892903          	lw	s2,-440(s2) # 80007858 <ticks>
  while(ticks - ticks0 < n){
    80002a18:	fcc42783          	lw	a5,-52(s0)
    80002a1c:	cf8d                	beqz	a5,80002a56 <sys_pause+0x6e>
    80002a1e:	f426                	sd	s1,40(sp)
    80002a20:	ec4e                	sd	s3,24(sp)
    if(killed(myproc())){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    80002a22:	00013997          	auipc	s3,0x13
    80002a26:	16698993          	addi	s3,s3,358 # 80015b88 <tickslock>
    80002a2a:	00005497          	auipc	s1,0x5
    80002a2e:	e2e48493          	addi	s1,s1,-466 # 80007858 <ticks>
    if(killed(myproc())){
    80002a32:	e9dfe0ef          	jal	800018ce <myproc>
    80002a36:	ee6ff0ef          	jal	8000211c <killed>
    80002a3a:	ed0d                	bnez	a0,80002a74 <sys_pause+0x8c>
    sleep(&ticks, &tickslock);
    80002a3c:	85ce                	mv	a1,s3
    80002a3e:	8526                	mv	a0,s1
    80002a40:	ca4ff0ef          	jal	80001ee4 <sleep>
  while(ticks - ticks0 < n){
    80002a44:	409c                	lw	a5,0(s1)
    80002a46:	412787bb          	subw	a5,a5,s2
    80002a4a:	fcc42703          	lw	a4,-52(s0)
    80002a4e:	fee7e2e3          	bltu	a5,a4,80002a32 <sys_pause+0x4a>
    80002a52:	74a2                	ld	s1,40(sp)
    80002a54:	69e2                	ld	s3,24(sp)
  }
  release(&tickslock);
    80002a56:	00013517          	auipc	a0,0x13
    80002a5a:	13250513          	addi	a0,a0,306 # 80015b88 <tickslock>
    80002a5e:	a08fe0ef          	jal	80000c66 <release>
  return 0;
    80002a62:	4501                	li	a0,0
}
    80002a64:	70e2                	ld	ra,56(sp)
    80002a66:	7442                	ld	s0,48(sp)
    80002a68:	7902                	ld	s2,32(sp)
    80002a6a:	6121                	addi	sp,sp,64
    80002a6c:	8082                	ret
    n = 0;
    80002a6e:	fc042623          	sw	zero,-52(s0)
    80002a72:	bf49                	j	80002a04 <sys_pause+0x1c>
      release(&tickslock);
    80002a74:	00013517          	auipc	a0,0x13
    80002a78:	11450513          	addi	a0,a0,276 # 80015b88 <tickslock>
    80002a7c:	9eafe0ef          	jal	80000c66 <release>
      return -1;
    80002a80:	557d                	li	a0,-1
    80002a82:	74a2                	ld	s1,40(sp)
    80002a84:	69e2                	ld	s3,24(sp)
    80002a86:	bff9                	j	80002a64 <sys_pause+0x7c>

0000000080002a88 <sys_kill>:

uint64
sys_kill(void)
{
    80002a88:	1101                	addi	sp,sp,-32
    80002a8a:	ec06                	sd	ra,24(sp)
    80002a8c:	e822                	sd	s0,16(sp)
    80002a8e:	1000                	addi	s0,sp,32
  int pid;

  argint(0, &pid);
    80002a90:	fec40593          	addi	a1,s0,-20
    80002a94:	4501                	li	a0,0
    80002a96:	d9bff0ef          	jal	80002830 <argint>
  return kkill(pid);
    80002a9a:	fec42503          	lw	a0,-20(s0)
    80002a9e:	df4ff0ef          	jal	80002092 <kkill>
}
    80002aa2:	60e2                	ld	ra,24(sp)
    80002aa4:	6442                	ld	s0,16(sp)
    80002aa6:	6105                	addi	sp,sp,32
    80002aa8:	8082                	ret

0000000080002aaa <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    80002aaa:	1101                	addi	sp,sp,-32
    80002aac:	ec06                	sd	ra,24(sp)
    80002aae:	e822                	sd	s0,16(sp)
    80002ab0:	e426                	sd	s1,8(sp)
    80002ab2:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    80002ab4:	00013517          	auipc	a0,0x13
    80002ab8:	0d450513          	addi	a0,a0,212 # 80015b88 <tickslock>
    80002abc:	912fe0ef          	jal	80000bce <acquire>
  xticks = ticks;
    80002ac0:	00005497          	auipc	s1,0x5
    80002ac4:	d984a483          	lw	s1,-616(s1) # 80007858 <ticks>
  release(&tickslock);
    80002ac8:	00013517          	auipc	a0,0x13
    80002acc:	0c050513          	addi	a0,a0,192 # 80015b88 <tickslock>
    80002ad0:	996fe0ef          	jal	80000c66 <release>
  return xticks;
}
    80002ad4:	02049513          	slli	a0,s1,0x20
    80002ad8:	9101                	srli	a0,a0,0x20
    80002ada:	60e2                	ld	ra,24(sp)
    80002adc:	6442                	ld	s0,16(sp)
    80002ade:	64a2                	ld	s1,8(sp)
    80002ae0:	6105                	addi	sp,sp,32
    80002ae2:	8082                	ret

0000000080002ae4 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    80002ae4:	7179                	addi	sp,sp,-48
    80002ae6:	f406                	sd	ra,40(sp)
    80002ae8:	f022                	sd	s0,32(sp)
    80002aea:	ec26                	sd	s1,24(sp)
    80002aec:	e84a                	sd	s2,16(sp)
    80002aee:	e44e                	sd	s3,8(sp)
    80002af0:	e052                	sd	s4,0(sp)
    80002af2:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    80002af4:	00005597          	auipc	a1,0x5
    80002af8:	89c58593          	addi	a1,a1,-1892 # 80007390 <etext+0x390>
    80002afc:	00013517          	auipc	a0,0x13
    80002b00:	0a450513          	addi	a0,a0,164 # 80015ba0 <bcache>
    80002b04:	84afe0ef          	jal	80000b4e <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002b08:	0001b797          	auipc	a5,0x1b
    80002b0c:	09878793          	addi	a5,a5,152 # 8001dba0 <bcache+0x8000>
    80002b10:	0001b717          	auipc	a4,0x1b
    80002b14:	2f870713          	addi	a4,a4,760 # 8001de08 <bcache+0x8268>
    80002b18:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002b1c:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002b20:	00013497          	auipc	s1,0x13
    80002b24:	09848493          	addi	s1,s1,152 # 80015bb8 <bcache+0x18>
    b->next = bcache.head.next;
    80002b28:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80002b2a:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80002b2c:	00005a17          	auipc	s4,0x5
    80002b30:	86ca0a13          	addi	s4,s4,-1940 # 80007398 <etext+0x398>
    b->next = bcache.head.next;
    80002b34:	2b893783          	ld	a5,696(s2)
    80002b38:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80002b3a:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    80002b3e:	85d2                	mv	a1,s4
    80002b40:	01048513          	addi	a0,s1,16
    80002b44:	322010ef          	jal	80003e66 <initsleeplock>
    bcache.head.next->prev = b;
    80002b48:	2b893783          	ld	a5,696(s2)
    80002b4c:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80002b4e:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002b52:	45848493          	addi	s1,s1,1112
    80002b56:	fd349fe3          	bne	s1,s3,80002b34 <binit+0x50>
  }
}
    80002b5a:	70a2                	ld	ra,40(sp)
    80002b5c:	7402                	ld	s0,32(sp)
    80002b5e:	64e2                	ld	s1,24(sp)
    80002b60:	6942                	ld	s2,16(sp)
    80002b62:	69a2                	ld	s3,8(sp)
    80002b64:	6a02                	ld	s4,0(sp)
    80002b66:	6145                	addi	sp,sp,48
    80002b68:	8082                	ret

0000000080002b6a <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    80002b6a:	7179                	addi	sp,sp,-48
    80002b6c:	f406                	sd	ra,40(sp)
    80002b6e:	f022                	sd	s0,32(sp)
    80002b70:	ec26                	sd	s1,24(sp)
    80002b72:	e84a                	sd	s2,16(sp)
    80002b74:	e44e                	sd	s3,8(sp)
    80002b76:	1800                	addi	s0,sp,48
    80002b78:	892a                	mv	s2,a0
    80002b7a:	89ae                	mv	s3,a1
  acquire(&bcache.lock);
    80002b7c:	00013517          	auipc	a0,0x13
    80002b80:	02450513          	addi	a0,a0,36 # 80015ba0 <bcache>
    80002b84:	84afe0ef          	jal	80000bce <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002b88:	0001b497          	auipc	s1,0x1b
    80002b8c:	2d04b483          	ld	s1,720(s1) # 8001de58 <bcache+0x82b8>
    80002b90:	0001b797          	auipc	a5,0x1b
    80002b94:	27878793          	addi	a5,a5,632 # 8001de08 <bcache+0x8268>
    80002b98:	02f48b63          	beq	s1,a5,80002bce <bread+0x64>
    80002b9c:	873e                	mv	a4,a5
    80002b9e:	a021                	j	80002ba6 <bread+0x3c>
    80002ba0:	68a4                	ld	s1,80(s1)
    80002ba2:	02e48663          	beq	s1,a4,80002bce <bread+0x64>
    if(b->dev == dev && b->blockno == blockno){
    80002ba6:	449c                	lw	a5,8(s1)
    80002ba8:	ff279ce3          	bne	a5,s2,80002ba0 <bread+0x36>
    80002bac:	44dc                	lw	a5,12(s1)
    80002bae:	ff3799e3          	bne	a5,s3,80002ba0 <bread+0x36>
      b->refcnt++;
    80002bb2:	40bc                	lw	a5,64(s1)
    80002bb4:	2785                	addiw	a5,a5,1
    80002bb6:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002bb8:	00013517          	auipc	a0,0x13
    80002bbc:	fe850513          	addi	a0,a0,-24 # 80015ba0 <bcache>
    80002bc0:	8a6fe0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002bc4:	01048513          	addi	a0,s1,16
    80002bc8:	2d4010ef          	jal	80003e9c <acquiresleep>
      return b;
    80002bcc:	a889                	j	80002c1e <bread+0xb4>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002bce:	0001b497          	auipc	s1,0x1b
    80002bd2:	2824b483          	ld	s1,642(s1) # 8001de50 <bcache+0x82b0>
    80002bd6:	0001b797          	auipc	a5,0x1b
    80002bda:	23278793          	addi	a5,a5,562 # 8001de08 <bcache+0x8268>
    80002bde:	00f48863          	beq	s1,a5,80002bee <bread+0x84>
    80002be2:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    80002be4:	40bc                	lw	a5,64(s1)
    80002be6:	cb91                	beqz	a5,80002bfa <bread+0x90>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002be8:	64a4                	ld	s1,72(s1)
    80002bea:	fee49de3          	bne	s1,a4,80002be4 <bread+0x7a>
  panic("bget: no buffers");
    80002bee:	00004517          	auipc	a0,0x4
    80002bf2:	7b250513          	addi	a0,a0,1970 # 800073a0 <etext+0x3a0>
    80002bf6:	bebfd0ef          	jal	800007e0 <panic>
      b->dev = dev;
    80002bfa:	0124a423          	sw	s2,8(s1)
      b->blockno = blockno;
    80002bfe:	0134a623          	sw	s3,12(s1)
      b->valid = 0;
    80002c02:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    80002c06:	4785                	li	a5,1
    80002c08:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002c0a:	00013517          	auipc	a0,0x13
    80002c0e:	f9650513          	addi	a0,a0,-106 # 80015ba0 <bcache>
    80002c12:	854fe0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002c16:	01048513          	addi	a0,s1,16
    80002c1a:	282010ef          	jal	80003e9c <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80002c1e:	409c                	lw	a5,0(s1)
    80002c20:	cb89                	beqz	a5,80002c32 <bread+0xc8>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    80002c22:	8526                	mv	a0,s1
    80002c24:	70a2                	ld	ra,40(sp)
    80002c26:	7402                	ld	s0,32(sp)
    80002c28:	64e2                	ld	s1,24(sp)
    80002c2a:	6942                	ld	s2,16(sp)
    80002c2c:	69a2                	ld	s3,8(sp)
    80002c2e:	6145                	addi	sp,sp,48
    80002c30:	8082                	ret
    virtio_disk_rw(b, 0);
    80002c32:	4581                	li	a1,0
    80002c34:	8526                	mv	a0,s1
    80002c36:	2cb020ef          	jal	80005700 <virtio_disk_rw>
    b->valid = 1;
    80002c3a:	4785                	li	a5,1
    80002c3c:	c09c                	sw	a5,0(s1)
  return b;
    80002c3e:	b7d5                	j	80002c22 <bread+0xb8>

0000000080002c40 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    80002c40:	1101                	addi	sp,sp,-32
    80002c42:	ec06                	sd	ra,24(sp)
    80002c44:	e822                	sd	s0,16(sp)
    80002c46:	e426                	sd	s1,8(sp)
    80002c48:	1000                	addi	s0,sp,32
    80002c4a:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002c4c:	0541                	addi	a0,a0,16
    80002c4e:	2cc010ef          	jal	80003f1a <holdingsleep>
    80002c52:	c911                	beqz	a0,80002c66 <bwrite+0x26>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80002c54:	4585                	li	a1,1
    80002c56:	8526                	mv	a0,s1
    80002c58:	2a9020ef          	jal	80005700 <virtio_disk_rw>
}
    80002c5c:	60e2                	ld	ra,24(sp)
    80002c5e:	6442                	ld	s0,16(sp)
    80002c60:	64a2                	ld	s1,8(sp)
    80002c62:	6105                	addi	sp,sp,32
    80002c64:	8082                	ret
    panic("bwrite");
    80002c66:	00004517          	auipc	a0,0x4
    80002c6a:	75250513          	addi	a0,a0,1874 # 800073b8 <etext+0x3b8>
    80002c6e:	b73fd0ef          	jal	800007e0 <panic>

0000000080002c72 <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    80002c72:	1101                	addi	sp,sp,-32
    80002c74:	ec06                	sd	ra,24(sp)
    80002c76:	e822                	sd	s0,16(sp)
    80002c78:	e426                	sd	s1,8(sp)
    80002c7a:	e04a                	sd	s2,0(sp)
    80002c7c:	1000                	addi	s0,sp,32
    80002c7e:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002c80:	01050913          	addi	s2,a0,16
    80002c84:	854a                	mv	a0,s2
    80002c86:	294010ef          	jal	80003f1a <holdingsleep>
    80002c8a:	c135                	beqz	a0,80002cee <brelse+0x7c>
    panic("brelse");

  releasesleep(&b->lock);
    80002c8c:	854a                	mv	a0,s2
    80002c8e:	254010ef          	jal	80003ee2 <releasesleep>

  acquire(&bcache.lock);
    80002c92:	00013517          	auipc	a0,0x13
    80002c96:	f0e50513          	addi	a0,a0,-242 # 80015ba0 <bcache>
    80002c9a:	f35fd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002c9e:	40bc                	lw	a5,64(s1)
    80002ca0:	37fd                	addiw	a5,a5,-1
    80002ca2:	0007871b          	sext.w	a4,a5
    80002ca6:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    80002ca8:	e71d                	bnez	a4,80002cd6 <brelse+0x64>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80002caa:	68b8                	ld	a4,80(s1)
    80002cac:	64bc                	ld	a5,72(s1)
    80002cae:	e73c                	sd	a5,72(a4)
    b->prev->next = b->next;
    80002cb0:	68b8                	ld	a4,80(s1)
    80002cb2:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80002cb4:	0001b797          	auipc	a5,0x1b
    80002cb8:	eec78793          	addi	a5,a5,-276 # 8001dba0 <bcache+0x8000>
    80002cbc:	2b87b703          	ld	a4,696(a5)
    80002cc0:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002cc2:	0001b717          	auipc	a4,0x1b
    80002cc6:	14670713          	addi	a4,a4,326 # 8001de08 <bcache+0x8268>
    80002cca:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80002ccc:	2b87b703          	ld	a4,696(a5)
    80002cd0:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80002cd2:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    80002cd6:	00013517          	auipc	a0,0x13
    80002cda:	eca50513          	addi	a0,a0,-310 # 80015ba0 <bcache>
    80002cde:	f89fd0ef          	jal	80000c66 <release>
}
    80002ce2:	60e2                	ld	ra,24(sp)
    80002ce4:	6442                	ld	s0,16(sp)
    80002ce6:	64a2                	ld	s1,8(sp)
    80002ce8:	6902                	ld	s2,0(sp)
    80002cea:	6105                	addi	sp,sp,32
    80002cec:	8082                	ret
    panic("brelse");
    80002cee:	00004517          	auipc	a0,0x4
    80002cf2:	6d250513          	addi	a0,a0,1746 # 800073c0 <etext+0x3c0>
    80002cf6:	aebfd0ef          	jal	800007e0 <panic>

0000000080002cfa <bpin>:

void
bpin(struct buf *b) {
    80002cfa:	1101                	addi	sp,sp,-32
    80002cfc:	ec06                	sd	ra,24(sp)
    80002cfe:	e822                	sd	s0,16(sp)
    80002d00:	e426                	sd	s1,8(sp)
    80002d02:	1000                	addi	s0,sp,32
    80002d04:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002d06:	00013517          	auipc	a0,0x13
    80002d0a:	e9a50513          	addi	a0,a0,-358 # 80015ba0 <bcache>
    80002d0e:	ec1fd0ef          	jal	80000bce <acquire>
  b->refcnt++;
    80002d12:	40bc                	lw	a5,64(s1)
    80002d14:	2785                	addiw	a5,a5,1
    80002d16:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002d18:	00013517          	auipc	a0,0x13
    80002d1c:	e8850513          	addi	a0,a0,-376 # 80015ba0 <bcache>
    80002d20:	f47fd0ef          	jal	80000c66 <release>
}
    80002d24:	60e2                	ld	ra,24(sp)
    80002d26:	6442                	ld	s0,16(sp)
    80002d28:	64a2                	ld	s1,8(sp)
    80002d2a:	6105                	addi	sp,sp,32
    80002d2c:	8082                	ret

0000000080002d2e <bunpin>:

void
bunpin(struct buf *b) {
    80002d2e:	1101                	addi	sp,sp,-32
    80002d30:	ec06                	sd	ra,24(sp)
    80002d32:	e822                	sd	s0,16(sp)
    80002d34:	e426                	sd	s1,8(sp)
    80002d36:	1000                	addi	s0,sp,32
    80002d38:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002d3a:	00013517          	auipc	a0,0x13
    80002d3e:	e6650513          	addi	a0,a0,-410 # 80015ba0 <bcache>
    80002d42:	e8dfd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002d46:	40bc                	lw	a5,64(s1)
    80002d48:	37fd                	addiw	a5,a5,-1
    80002d4a:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002d4c:	00013517          	auipc	a0,0x13
    80002d50:	e5450513          	addi	a0,a0,-428 # 80015ba0 <bcache>
    80002d54:	f13fd0ef          	jal	80000c66 <release>
}
    80002d58:	60e2                	ld	ra,24(sp)
    80002d5a:	6442                	ld	s0,16(sp)
    80002d5c:	64a2                	ld	s1,8(sp)
    80002d5e:	6105                	addi	sp,sp,32
    80002d60:	8082                	ret

0000000080002d62 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    80002d62:	1101                	addi	sp,sp,-32
    80002d64:	ec06                	sd	ra,24(sp)
    80002d66:	e822                	sd	s0,16(sp)
    80002d68:	e426                	sd	s1,8(sp)
    80002d6a:	e04a                	sd	s2,0(sp)
    80002d6c:	1000                	addi	s0,sp,32
    80002d6e:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    80002d70:	00d5d59b          	srliw	a1,a1,0xd
    80002d74:	0001b797          	auipc	a5,0x1b
    80002d78:	5087a783          	lw	a5,1288(a5) # 8001e27c <sb+0x1c>
    80002d7c:	9dbd                	addw	a1,a1,a5
    80002d7e:	dedff0ef          	jal	80002b6a <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    80002d82:	0074f713          	andi	a4,s1,7
    80002d86:	4785                	li	a5,1
    80002d88:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    80002d8c:	14ce                	slli	s1,s1,0x33
    80002d8e:	90d9                	srli	s1,s1,0x36
    80002d90:	00950733          	add	a4,a0,s1
    80002d94:	05874703          	lbu	a4,88(a4)
    80002d98:	00e7f6b3          	and	a3,a5,a4
    80002d9c:	c29d                	beqz	a3,80002dc2 <bfree+0x60>
    80002d9e:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80002da0:	94aa                	add	s1,s1,a0
    80002da2:	fff7c793          	not	a5,a5
    80002da6:	8f7d                	and	a4,a4,a5
    80002da8:	04e48c23          	sb	a4,88(s1)
  log_write(bp);
    80002dac:	7f9000ef          	jal	80003da4 <log_write>
  brelse(bp);
    80002db0:	854a                	mv	a0,s2
    80002db2:	ec1ff0ef          	jal	80002c72 <brelse>
}
    80002db6:	60e2                	ld	ra,24(sp)
    80002db8:	6442                	ld	s0,16(sp)
    80002dba:	64a2                	ld	s1,8(sp)
    80002dbc:	6902                	ld	s2,0(sp)
    80002dbe:	6105                	addi	sp,sp,32
    80002dc0:	8082                	ret
    panic("freeing free block");
    80002dc2:	00004517          	auipc	a0,0x4
    80002dc6:	60650513          	addi	a0,a0,1542 # 800073c8 <etext+0x3c8>
    80002dca:	a17fd0ef          	jal	800007e0 <panic>

0000000080002dce <balloc>:
{
    80002dce:	711d                	addi	sp,sp,-96
    80002dd0:	ec86                	sd	ra,88(sp)
    80002dd2:	e8a2                	sd	s0,80(sp)
    80002dd4:	e4a6                	sd	s1,72(sp)
    80002dd6:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80002dd8:	0001b797          	auipc	a5,0x1b
    80002ddc:	48c7a783          	lw	a5,1164(a5) # 8001e264 <sb+0x4>
    80002de0:	0e078f63          	beqz	a5,80002ede <balloc+0x110>
    80002de4:	e0ca                	sd	s2,64(sp)
    80002de6:	fc4e                	sd	s3,56(sp)
    80002de8:	f852                	sd	s4,48(sp)
    80002dea:	f456                	sd	s5,40(sp)
    80002dec:	f05a                	sd	s6,32(sp)
    80002dee:	ec5e                	sd	s7,24(sp)
    80002df0:	e862                	sd	s8,16(sp)
    80002df2:	e466                	sd	s9,8(sp)
    80002df4:	8baa                	mv	s7,a0
    80002df6:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80002df8:	0001bb17          	auipc	s6,0x1b
    80002dfc:	468b0b13          	addi	s6,s6,1128 # 8001e260 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e00:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80002e02:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e04:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80002e06:	6c89                	lui	s9,0x2
    80002e08:	a0b5                	j	80002e74 <balloc+0xa6>
        bp->data[bi/8] |= m;  // Mark block in use.
    80002e0a:	97ca                	add	a5,a5,s2
    80002e0c:	8e55                	or	a2,a2,a3
    80002e0e:	04c78c23          	sb	a2,88(a5)
        log_write(bp);
    80002e12:	854a                	mv	a0,s2
    80002e14:	791000ef          	jal	80003da4 <log_write>
        brelse(bp);
    80002e18:	854a                	mv	a0,s2
    80002e1a:	e59ff0ef          	jal	80002c72 <brelse>
  bp = bread(dev, bno);
    80002e1e:	85a6                	mv	a1,s1
    80002e20:	855e                	mv	a0,s7
    80002e22:	d49ff0ef          	jal	80002b6a <bread>
    80002e26:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    80002e28:	40000613          	li	a2,1024
    80002e2c:	4581                	li	a1,0
    80002e2e:	05850513          	addi	a0,a0,88
    80002e32:	e71fd0ef          	jal	80000ca2 <memset>
  log_write(bp);
    80002e36:	854a                	mv	a0,s2
    80002e38:	76d000ef          	jal	80003da4 <log_write>
  brelse(bp);
    80002e3c:	854a                	mv	a0,s2
    80002e3e:	e35ff0ef          	jal	80002c72 <brelse>
}
    80002e42:	6906                	ld	s2,64(sp)
    80002e44:	79e2                	ld	s3,56(sp)
    80002e46:	7a42                	ld	s4,48(sp)
    80002e48:	7aa2                	ld	s5,40(sp)
    80002e4a:	7b02                	ld	s6,32(sp)
    80002e4c:	6be2                	ld	s7,24(sp)
    80002e4e:	6c42                	ld	s8,16(sp)
    80002e50:	6ca2                	ld	s9,8(sp)
}
    80002e52:	8526                	mv	a0,s1
    80002e54:	60e6                	ld	ra,88(sp)
    80002e56:	6446                	ld	s0,80(sp)
    80002e58:	64a6                	ld	s1,72(sp)
    80002e5a:	6125                	addi	sp,sp,96
    80002e5c:	8082                	ret
    brelse(bp);
    80002e5e:	854a                	mv	a0,s2
    80002e60:	e13ff0ef          	jal	80002c72 <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80002e64:	015c87bb          	addw	a5,s9,s5
    80002e68:	00078a9b          	sext.w	s5,a5
    80002e6c:	004b2703          	lw	a4,4(s6)
    80002e70:	04eaff63          	bgeu	s5,a4,80002ece <balloc+0x100>
    bp = bread(dev, BBLOCK(b, sb));
    80002e74:	41fad79b          	sraiw	a5,s5,0x1f
    80002e78:	0137d79b          	srliw	a5,a5,0x13
    80002e7c:	015787bb          	addw	a5,a5,s5
    80002e80:	40d7d79b          	sraiw	a5,a5,0xd
    80002e84:	01cb2583          	lw	a1,28(s6)
    80002e88:	9dbd                	addw	a1,a1,a5
    80002e8a:	855e                	mv	a0,s7
    80002e8c:	cdfff0ef          	jal	80002b6a <bread>
    80002e90:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e92:	004b2503          	lw	a0,4(s6)
    80002e96:	000a849b          	sext.w	s1,s5
    80002e9a:	8762                	mv	a4,s8
    80002e9c:	fca4f1e3          	bgeu	s1,a0,80002e5e <balloc+0x90>
      m = 1 << (bi % 8);
    80002ea0:	00777693          	andi	a3,a4,7
    80002ea4:	00d996bb          	sllw	a3,s3,a3
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    80002ea8:	41f7579b          	sraiw	a5,a4,0x1f
    80002eac:	01d7d79b          	srliw	a5,a5,0x1d
    80002eb0:	9fb9                	addw	a5,a5,a4
    80002eb2:	4037d79b          	sraiw	a5,a5,0x3
    80002eb6:	00f90633          	add	a2,s2,a5
    80002eba:	05864603          	lbu	a2,88(a2)
    80002ebe:	00c6f5b3          	and	a1,a3,a2
    80002ec2:	d5a1                	beqz	a1,80002e0a <balloc+0x3c>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002ec4:	2705                	addiw	a4,a4,1
    80002ec6:	2485                	addiw	s1,s1,1
    80002ec8:	fd471ae3          	bne	a4,s4,80002e9c <balloc+0xce>
    80002ecc:	bf49                	j	80002e5e <balloc+0x90>
    80002ece:	6906                	ld	s2,64(sp)
    80002ed0:	79e2                	ld	s3,56(sp)
    80002ed2:	7a42                	ld	s4,48(sp)
    80002ed4:	7aa2                	ld	s5,40(sp)
    80002ed6:	7b02                	ld	s6,32(sp)
    80002ed8:	6be2                	ld	s7,24(sp)
    80002eda:	6c42                	ld	s8,16(sp)
    80002edc:	6ca2                	ld	s9,8(sp)
  printf("balloc: out of blocks\n");
    80002ede:	00004517          	auipc	a0,0x4
    80002ee2:	50250513          	addi	a0,a0,1282 # 800073e0 <etext+0x3e0>
    80002ee6:	e14fd0ef          	jal	800004fa <printf>
  return 0;
    80002eea:	4481                	li	s1,0
    80002eec:	b79d                	j	80002e52 <balloc+0x84>

0000000080002eee <bmap>:
// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
// returns 0 if out of disk space.
static uint
bmap(struct inode *ip, uint bn)
{
    80002eee:	7179                	addi	sp,sp,-48
    80002ef0:	f406                	sd	ra,40(sp)
    80002ef2:	f022                	sd	s0,32(sp)
    80002ef4:	ec26                	sd	s1,24(sp)
    80002ef6:	e84a                	sd	s2,16(sp)
    80002ef8:	e44e                	sd	s3,8(sp)
    80002efa:	1800                	addi	s0,sp,48
    80002efc:	89aa                	mv	s3,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    80002efe:	47ad                	li	a5,11
    80002f00:	02b7e663          	bltu	a5,a1,80002f2c <bmap+0x3e>
    if((addr = ip->addrs[bn]) == 0){
    80002f04:	02059793          	slli	a5,a1,0x20
    80002f08:	01e7d593          	srli	a1,a5,0x1e
    80002f0c:	00b504b3          	add	s1,a0,a1
    80002f10:	0504a903          	lw	s2,80(s1)
    80002f14:	06091a63          	bnez	s2,80002f88 <bmap+0x9a>
      addr = balloc(ip->dev);
    80002f18:	4108                	lw	a0,0(a0)
    80002f1a:	eb5ff0ef          	jal	80002dce <balloc>
    80002f1e:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002f22:	06090363          	beqz	s2,80002f88 <bmap+0x9a>
        return 0;
      ip->addrs[bn] = addr;
    80002f26:	0524a823          	sw	s2,80(s1)
    80002f2a:	a8b9                	j	80002f88 <bmap+0x9a>
    }
    return addr;
  }
  bn -= NDIRECT;
    80002f2c:	ff45849b          	addiw	s1,a1,-12
    80002f30:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80002f34:	0ff00793          	li	a5,255
    80002f38:	06e7ee63          	bltu	a5,a4,80002fb4 <bmap+0xc6>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0){
    80002f3c:	08052903          	lw	s2,128(a0)
    80002f40:	00091d63          	bnez	s2,80002f5a <bmap+0x6c>
      addr = balloc(ip->dev);
    80002f44:	4108                	lw	a0,0(a0)
    80002f46:	e89ff0ef          	jal	80002dce <balloc>
    80002f4a:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002f4e:	02090d63          	beqz	s2,80002f88 <bmap+0x9a>
    80002f52:	e052                	sd	s4,0(sp)
        return 0;
      ip->addrs[NDIRECT] = addr;
    80002f54:	0929a023          	sw	s2,128(s3)
    80002f58:	a011                	j	80002f5c <bmap+0x6e>
    80002f5a:	e052                	sd	s4,0(sp)
    }
    bp = bread(ip->dev, addr);
    80002f5c:	85ca                	mv	a1,s2
    80002f5e:	0009a503          	lw	a0,0(s3)
    80002f62:	c09ff0ef          	jal	80002b6a <bread>
    80002f66:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80002f68:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80002f6c:	02049713          	slli	a4,s1,0x20
    80002f70:	01e75593          	srli	a1,a4,0x1e
    80002f74:	00b784b3          	add	s1,a5,a1
    80002f78:	0004a903          	lw	s2,0(s1)
    80002f7c:	00090e63          	beqz	s2,80002f98 <bmap+0xaa>
      if(addr){
        a[bn] = addr;
        log_write(bp);
      }
    }
    brelse(bp);
    80002f80:	8552                	mv	a0,s4
    80002f82:	cf1ff0ef          	jal	80002c72 <brelse>
    return addr;
    80002f86:	6a02                	ld	s4,0(sp)
  }

  panic("bmap: out of range");
}
    80002f88:	854a                	mv	a0,s2
    80002f8a:	70a2                	ld	ra,40(sp)
    80002f8c:	7402                	ld	s0,32(sp)
    80002f8e:	64e2                	ld	s1,24(sp)
    80002f90:	6942                	ld	s2,16(sp)
    80002f92:	69a2                	ld	s3,8(sp)
    80002f94:	6145                	addi	sp,sp,48
    80002f96:	8082                	ret
      addr = balloc(ip->dev);
    80002f98:	0009a503          	lw	a0,0(s3)
    80002f9c:	e33ff0ef          	jal	80002dce <balloc>
    80002fa0:	0005091b          	sext.w	s2,a0
      if(addr){
    80002fa4:	fc090ee3          	beqz	s2,80002f80 <bmap+0x92>
        a[bn] = addr;
    80002fa8:	0124a023          	sw	s2,0(s1)
        log_write(bp);
    80002fac:	8552                	mv	a0,s4
    80002fae:	5f7000ef          	jal	80003da4 <log_write>
    80002fb2:	b7f9                	j	80002f80 <bmap+0x92>
    80002fb4:	e052                	sd	s4,0(sp)
  panic("bmap: out of range");
    80002fb6:	00004517          	auipc	a0,0x4
    80002fba:	44250513          	addi	a0,a0,1090 # 800073f8 <etext+0x3f8>
    80002fbe:	823fd0ef          	jal	800007e0 <panic>

0000000080002fc2 <iget>:
{
    80002fc2:	7179                	addi	sp,sp,-48
    80002fc4:	f406                	sd	ra,40(sp)
    80002fc6:	f022                	sd	s0,32(sp)
    80002fc8:	ec26                	sd	s1,24(sp)
    80002fca:	e84a                	sd	s2,16(sp)
    80002fcc:	e44e                	sd	s3,8(sp)
    80002fce:	e052                	sd	s4,0(sp)
    80002fd0:	1800                	addi	s0,sp,48
    80002fd2:	89aa                	mv	s3,a0
    80002fd4:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    80002fd6:	0001b517          	auipc	a0,0x1b
    80002fda:	2aa50513          	addi	a0,a0,682 # 8001e280 <itable>
    80002fde:	bf1fd0ef          	jal	80000bce <acquire>
  empty = 0;
    80002fe2:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80002fe4:	0001b497          	auipc	s1,0x1b
    80002fe8:	2b448493          	addi	s1,s1,692 # 8001e298 <itable+0x18>
    80002fec:	0001d697          	auipc	a3,0x1d
    80002ff0:	d3c68693          	addi	a3,a3,-708 # 8001fd28 <log>
    80002ff4:	a039                	j	80003002 <iget+0x40>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80002ff6:	02090963          	beqz	s2,80003028 <iget+0x66>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80002ffa:	08848493          	addi	s1,s1,136
    80002ffe:	02d48863          	beq	s1,a3,8000302e <iget+0x6c>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    80003002:	449c                	lw	a5,8(s1)
    80003004:	fef059e3          	blez	a5,80002ff6 <iget+0x34>
    80003008:	4098                	lw	a4,0(s1)
    8000300a:	ff3716e3          	bne	a4,s3,80002ff6 <iget+0x34>
    8000300e:	40d8                	lw	a4,4(s1)
    80003010:	ff4713e3          	bne	a4,s4,80002ff6 <iget+0x34>
      ip->ref++;
    80003014:	2785                	addiw	a5,a5,1
    80003016:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    80003018:	0001b517          	auipc	a0,0x1b
    8000301c:	26850513          	addi	a0,a0,616 # 8001e280 <itable>
    80003020:	c47fd0ef          	jal	80000c66 <release>
      return ip;
    80003024:	8926                	mv	s2,s1
    80003026:	a02d                	j	80003050 <iget+0x8e>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003028:	fbe9                	bnez	a5,80002ffa <iget+0x38>
      empty = ip;
    8000302a:	8926                	mv	s2,s1
    8000302c:	b7f9                	j	80002ffa <iget+0x38>
  if(empty == 0)
    8000302e:	02090a63          	beqz	s2,80003062 <iget+0xa0>
  ip->dev = dev;
    80003032:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    80003036:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    8000303a:	4785                	li	a5,1
    8000303c:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80003040:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    80003044:	0001b517          	auipc	a0,0x1b
    80003048:	23c50513          	addi	a0,a0,572 # 8001e280 <itable>
    8000304c:	c1bfd0ef          	jal	80000c66 <release>
}
    80003050:	854a                	mv	a0,s2
    80003052:	70a2                	ld	ra,40(sp)
    80003054:	7402                	ld	s0,32(sp)
    80003056:	64e2                	ld	s1,24(sp)
    80003058:	6942                	ld	s2,16(sp)
    8000305a:	69a2                	ld	s3,8(sp)
    8000305c:	6a02                	ld	s4,0(sp)
    8000305e:	6145                	addi	sp,sp,48
    80003060:	8082                	ret
    panic("iget: no inodes");
    80003062:	00004517          	auipc	a0,0x4
    80003066:	3ae50513          	addi	a0,a0,942 # 80007410 <etext+0x410>
    8000306a:	f76fd0ef          	jal	800007e0 <panic>

000000008000306e <iinit>:
{
    8000306e:	7179                	addi	sp,sp,-48
    80003070:	f406                	sd	ra,40(sp)
    80003072:	f022                	sd	s0,32(sp)
    80003074:	ec26                	sd	s1,24(sp)
    80003076:	e84a                	sd	s2,16(sp)
    80003078:	e44e                	sd	s3,8(sp)
    8000307a:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    8000307c:	00004597          	auipc	a1,0x4
    80003080:	3a458593          	addi	a1,a1,932 # 80007420 <etext+0x420>
    80003084:	0001b517          	auipc	a0,0x1b
    80003088:	1fc50513          	addi	a0,a0,508 # 8001e280 <itable>
    8000308c:	ac3fd0ef          	jal	80000b4e <initlock>
  for(i = 0; i < NINODE; i++) {
    80003090:	0001b497          	auipc	s1,0x1b
    80003094:	21848493          	addi	s1,s1,536 # 8001e2a8 <itable+0x28>
    80003098:	0001d997          	auipc	s3,0x1d
    8000309c:	ca098993          	addi	s3,s3,-864 # 8001fd38 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    800030a0:	00004917          	auipc	s2,0x4
    800030a4:	38890913          	addi	s2,s2,904 # 80007428 <etext+0x428>
    800030a8:	85ca                	mv	a1,s2
    800030aa:	8526                	mv	a0,s1
    800030ac:	5bb000ef          	jal	80003e66 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    800030b0:	08848493          	addi	s1,s1,136
    800030b4:	ff349ae3          	bne	s1,s3,800030a8 <iinit+0x3a>
}
    800030b8:	70a2                	ld	ra,40(sp)
    800030ba:	7402                	ld	s0,32(sp)
    800030bc:	64e2                	ld	s1,24(sp)
    800030be:	6942                	ld	s2,16(sp)
    800030c0:	69a2                	ld	s3,8(sp)
    800030c2:	6145                	addi	sp,sp,48
    800030c4:	8082                	ret

00000000800030c6 <ialloc>:
{
    800030c6:	7139                	addi	sp,sp,-64
    800030c8:	fc06                	sd	ra,56(sp)
    800030ca:	f822                	sd	s0,48(sp)
    800030cc:	0080                	addi	s0,sp,64
  for(inum = 1; inum < sb.ninodes; inum++){
    800030ce:	0001b717          	auipc	a4,0x1b
    800030d2:	19e72703          	lw	a4,414(a4) # 8001e26c <sb+0xc>
    800030d6:	4785                	li	a5,1
    800030d8:	06e7f063          	bgeu	a5,a4,80003138 <ialloc+0x72>
    800030dc:	f426                	sd	s1,40(sp)
    800030de:	f04a                	sd	s2,32(sp)
    800030e0:	ec4e                	sd	s3,24(sp)
    800030e2:	e852                	sd	s4,16(sp)
    800030e4:	e456                	sd	s5,8(sp)
    800030e6:	e05a                	sd	s6,0(sp)
    800030e8:	8aaa                	mv	s5,a0
    800030ea:	8b2e                	mv	s6,a1
    800030ec:	4905                	li	s2,1
    bp = bread(dev, IBLOCK(inum, sb));
    800030ee:	0001ba17          	auipc	s4,0x1b
    800030f2:	172a0a13          	addi	s4,s4,370 # 8001e260 <sb>
    800030f6:	00495593          	srli	a1,s2,0x4
    800030fa:	018a2783          	lw	a5,24(s4)
    800030fe:	9dbd                	addw	a1,a1,a5
    80003100:	8556                	mv	a0,s5
    80003102:	a69ff0ef          	jal	80002b6a <bread>
    80003106:	84aa                	mv	s1,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    80003108:	05850993          	addi	s3,a0,88
    8000310c:	00f97793          	andi	a5,s2,15
    80003110:	079a                	slli	a5,a5,0x6
    80003112:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    80003114:	00099783          	lh	a5,0(s3)
    80003118:	cb9d                	beqz	a5,8000314e <ialloc+0x88>
    brelse(bp);
    8000311a:	b59ff0ef          	jal	80002c72 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    8000311e:	0905                	addi	s2,s2,1
    80003120:	00ca2703          	lw	a4,12(s4)
    80003124:	0009079b          	sext.w	a5,s2
    80003128:	fce7e7e3          	bltu	a5,a4,800030f6 <ialloc+0x30>
    8000312c:	74a2                	ld	s1,40(sp)
    8000312e:	7902                	ld	s2,32(sp)
    80003130:	69e2                	ld	s3,24(sp)
    80003132:	6a42                	ld	s4,16(sp)
    80003134:	6aa2                	ld	s5,8(sp)
    80003136:	6b02                	ld	s6,0(sp)
  printf("ialloc: no inodes\n");
    80003138:	00004517          	auipc	a0,0x4
    8000313c:	2f850513          	addi	a0,a0,760 # 80007430 <etext+0x430>
    80003140:	bbafd0ef          	jal	800004fa <printf>
  return 0;
    80003144:	4501                	li	a0,0
}
    80003146:	70e2                	ld	ra,56(sp)
    80003148:	7442                	ld	s0,48(sp)
    8000314a:	6121                	addi	sp,sp,64
    8000314c:	8082                	ret
      memset(dip, 0, sizeof(*dip));
    8000314e:	04000613          	li	a2,64
    80003152:	4581                	li	a1,0
    80003154:	854e                	mv	a0,s3
    80003156:	b4dfd0ef          	jal	80000ca2 <memset>
      dip->type = type;
    8000315a:	01699023          	sh	s6,0(s3)
      log_write(bp);   // mark it allocated on the disk
    8000315e:	8526                	mv	a0,s1
    80003160:	445000ef          	jal	80003da4 <log_write>
      brelse(bp);
    80003164:	8526                	mv	a0,s1
    80003166:	b0dff0ef          	jal	80002c72 <brelse>
      return iget(dev, inum);
    8000316a:	0009059b          	sext.w	a1,s2
    8000316e:	8556                	mv	a0,s5
    80003170:	e53ff0ef          	jal	80002fc2 <iget>
    80003174:	74a2                	ld	s1,40(sp)
    80003176:	7902                	ld	s2,32(sp)
    80003178:	69e2                	ld	s3,24(sp)
    8000317a:	6a42                	ld	s4,16(sp)
    8000317c:	6aa2                	ld	s5,8(sp)
    8000317e:	6b02                	ld	s6,0(sp)
    80003180:	b7d9                	j	80003146 <ialloc+0x80>

0000000080003182 <iupdate>:
{
    80003182:	1101                	addi	sp,sp,-32
    80003184:	ec06                	sd	ra,24(sp)
    80003186:	e822                	sd	s0,16(sp)
    80003188:	e426                	sd	s1,8(sp)
    8000318a:	e04a                	sd	s2,0(sp)
    8000318c:	1000                	addi	s0,sp,32
    8000318e:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003190:	415c                	lw	a5,4(a0)
    80003192:	0047d79b          	srliw	a5,a5,0x4
    80003196:	0001b597          	auipc	a1,0x1b
    8000319a:	0e25a583          	lw	a1,226(a1) # 8001e278 <sb+0x18>
    8000319e:	9dbd                	addw	a1,a1,a5
    800031a0:	4108                	lw	a0,0(a0)
    800031a2:	9c9ff0ef          	jal	80002b6a <bread>
    800031a6:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    800031a8:	05850793          	addi	a5,a0,88
    800031ac:	40d8                	lw	a4,4(s1)
    800031ae:	8b3d                	andi	a4,a4,15
    800031b0:	071a                	slli	a4,a4,0x6
    800031b2:	97ba                	add	a5,a5,a4
  dip->type = ip->type;
    800031b4:	04449703          	lh	a4,68(s1)
    800031b8:	00e79023          	sh	a4,0(a5)
  dip->major = ip->major;
    800031bc:	04649703          	lh	a4,70(s1)
    800031c0:	00e79123          	sh	a4,2(a5)
  dip->minor = ip->minor;
    800031c4:	04849703          	lh	a4,72(s1)
    800031c8:	00e79223          	sh	a4,4(a5)
  dip->nlink = ip->nlink;
    800031cc:	04a49703          	lh	a4,74(s1)
    800031d0:	00e79323          	sh	a4,6(a5)
  dip->size = ip->size;
    800031d4:	44f8                	lw	a4,76(s1)
    800031d6:	c798                	sw	a4,8(a5)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    800031d8:	03400613          	li	a2,52
    800031dc:	05048593          	addi	a1,s1,80
    800031e0:	00c78513          	addi	a0,a5,12
    800031e4:	b1bfd0ef          	jal	80000cfe <memmove>
  log_write(bp);
    800031e8:	854a                	mv	a0,s2
    800031ea:	3bb000ef          	jal	80003da4 <log_write>
  brelse(bp);
    800031ee:	854a                	mv	a0,s2
    800031f0:	a83ff0ef          	jal	80002c72 <brelse>
}
    800031f4:	60e2                	ld	ra,24(sp)
    800031f6:	6442                	ld	s0,16(sp)
    800031f8:	64a2                	ld	s1,8(sp)
    800031fa:	6902                	ld	s2,0(sp)
    800031fc:	6105                	addi	sp,sp,32
    800031fe:	8082                	ret

0000000080003200 <idup>:
{
    80003200:	1101                	addi	sp,sp,-32
    80003202:	ec06                	sd	ra,24(sp)
    80003204:	e822                	sd	s0,16(sp)
    80003206:	e426                	sd	s1,8(sp)
    80003208:	1000                	addi	s0,sp,32
    8000320a:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    8000320c:	0001b517          	auipc	a0,0x1b
    80003210:	07450513          	addi	a0,a0,116 # 8001e280 <itable>
    80003214:	9bbfd0ef          	jal	80000bce <acquire>
  ip->ref++;
    80003218:	449c                	lw	a5,8(s1)
    8000321a:	2785                	addiw	a5,a5,1
    8000321c:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    8000321e:	0001b517          	auipc	a0,0x1b
    80003222:	06250513          	addi	a0,a0,98 # 8001e280 <itable>
    80003226:	a41fd0ef          	jal	80000c66 <release>
}
    8000322a:	8526                	mv	a0,s1
    8000322c:	60e2                	ld	ra,24(sp)
    8000322e:	6442                	ld	s0,16(sp)
    80003230:	64a2                	ld	s1,8(sp)
    80003232:	6105                	addi	sp,sp,32
    80003234:	8082                	ret

0000000080003236 <ilock>:
{
    80003236:	1101                	addi	sp,sp,-32
    80003238:	ec06                	sd	ra,24(sp)
    8000323a:	e822                	sd	s0,16(sp)
    8000323c:	e426                	sd	s1,8(sp)
    8000323e:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    80003240:	cd19                	beqz	a0,8000325e <ilock+0x28>
    80003242:	84aa                	mv	s1,a0
    80003244:	451c                	lw	a5,8(a0)
    80003246:	00f05c63          	blez	a5,8000325e <ilock+0x28>
  acquiresleep(&ip->lock);
    8000324a:	0541                	addi	a0,a0,16
    8000324c:	451000ef          	jal	80003e9c <acquiresleep>
  if(ip->valid == 0){
    80003250:	40bc                	lw	a5,64(s1)
    80003252:	cf89                	beqz	a5,8000326c <ilock+0x36>
}
    80003254:	60e2                	ld	ra,24(sp)
    80003256:	6442                	ld	s0,16(sp)
    80003258:	64a2                	ld	s1,8(sp)
    8000325a:	6105                	addi	sp,sp,32
    8000325c:	8082                	ret
    8000325e:	e04a                	sd	s2,0(sp)
    panic("ilock");
    80003260:	00004517          	auipc	a0,0x4
    80003264:	1e850513          	addi	a0,a0,488 # 80007448 <etext+0x448>
    80003268:	d78fd0ef          	jal	800007e0 <panic>
    8000326c:	e04a                	sd	s2,0(sp)
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    8000326e:	40dc                	lw	a5,4(s1)
    80003270:	0047d79b          	srliw	a5,a5,0x4
    80003274:	0001b597          	auipc	a1,0x1b
    80003278:	0045a583          	lw	a1,4(a1) # 8001e278 <sb+0x18>
    8000327c:	9dbd                	addw	a1,a1,a5
    8000327e:	4088                	lw	a0,0(s1)
    80003280:	8ebff0ef          	jal	80002b6a <bread>
    80003284:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003286:	05850593          	addi	a1,a0,88
    8000328a:	40dc                	lw	a5,4(s1)
    8000328c:	8bbd                	andi	a5,a5,15
    8000328e:	079a                	slli	a5,a5,0x6
    80003290:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    80003292:	00059783          	lh	a5,0(a1)
    80003296:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    8000329a:	00259783          	lh	a5,2(a1)
    8000329e:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    800032a2:	00459783          	lh	a5,4(a1)
    800032a6:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    800032aa:	00659783          	lh	a5,6(a1)
    800032ae:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    800032b2:	459c                	lw	a5,8(a1)
    800032b4:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    800032b6:	03400613          	li	a2,52
    800032ba:	05b1                	addi	a1,a1,12
    800032bc:	05048513          	addi	a0,s1,80
    800032c0:	a3ffd0ef          	jal	80000cfe <memmove>
    brelse(bp);
    800032c4:	854a                	mv	a0,s2
    800032c6:	9adff0ef          	jal	80002c72 <brelse>
    ip->valid = 1;
    800032ca:	4785                	li	a5,1
    800032cc:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    800032ce:	04449783          	lh	a5,68(s1)
    800032d2:	c399                	beqz	a5,800032d8 <ilock+0xa2>
    800032d4:	6902                	ld	s2,0(sp)
    800032d6:	bfbd                	j	80003254 <ilock+0x1e>
      panic("ilock: no type");
    800032d8:	00004517          	auipc	a0,0x4
    800032dc:	17850513          	addi	a0,a0,376 # 80007450 <etext+0x450>
    800032e0:	d00fd0ef          	jal	800007e0 <panic>

00000000800032e4 <iunlock>:
{
    800032e4:	1101                	addi	sp,sp,-32
    800032e6:	ec06                	sd	ra,24(sp)
    800032e8:	e822                	sd	s0,16(sp)
    800032ea:	e426                	sd	s1,8(sp)
    800032ec:	e04a                	sd	s2,0(sp)
    800032ee:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    800032f0:	c505                	beqz	a0,80003318 <iunlock+0x34>
    800032f2:	84aa                	mv	s1,a0
    800032f4:	01050913          	addi	s2,a0,16
    800032f8:	854a                	mv	a0,s2
    800032fa:	421000ef          	jal	80003f1a <holdingsleep>
    800032fe:	cd09                	beqz	a0,80003318 <iunlock+0x34>
    80003300:	449c                	lw	a5,8(s1)
    80003302:	00f05b63          	blez	a5,80003318 <iunlock+0x34>
  releasesleep(&ip->lock);
    80003306:	854a                	mv	a0,s2
    80003308:	3db000ef          	jal	80003ee2 <releasesleep>
}
    8000330c:	60e2                	ld	ra,24(sp)
    8000330e:	6442                	ld	s0,16(sp)
    80003310:	64a2                	ld	s1,8(sp)
    80003312:	6902                	ld	s2,0(sp)
    80003314:	6105                	addi	sp,sp,32
    80003316:	8082                	ret
    panic("iunlock");
    80003318:	00004517          	auipc	a0,0x4
    8000331c:	14850513          	addi	a0,a0,328 # 80007460 <etext+0x460>
    80003320:	cc0fd0ef          	jal	800007e0 <panic>

0000000080003324 <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    80003324:	7179                	addi	sp,sp,-48
    80003326:	f406                	sd	ra,40(sp)
    80003328:	f022                	sd	s0,32(sp)
    8000332a:	ec26                	sd	s1,24(sp)
    8000332c:	e84a                	sd	s2,16(sp)
    8000332e:	e44e                	sd	s3,8(sp)
    80003330:	1800                	addi	s0,sp,48
    80003332:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    80003334:	05050493          	addi	s1,a0,80
    80003338:	08050913          	addi	s2,a0,128
    8000333c:	a021                	j	80003344 <itrunc+0x20>
    8000333e:	0491                	addi	s1,s1,4
    80003340:	01248b63          	beq	s1,s2,80003356 <itrunc+0x32>
    if(ip->addrs[i]){
    80003344:	408c                	lw	a1,0(s1)
    80003346:	dde5                	beqz	a1,8000333e <itrunc+0x1a>
      bfree(ip->dev, ip->addrs[i]);
    80003348:	0009a503          	lw	a0,0(s3)
    8000334c:	a17ff0ef          	jal	80002d62 <bfree>
      ip->addrs[i] = 0;
    80003350:	0004a023          	sw	zero,0(s1)
    80003354:	b7ed                	j	8000333e <itrunc+0x1a>
    }
  }

  if(ip->addrs[NDIRECT]){
    80003356:	0809a583          	lw	a1,128(s3)
    8000335a:	ed89                	bnez	a1,80003374 <itrunc+0x50>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    8000335c:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003360:	854e                	mv	a0,s3
    80003362:	e21ff0ef          	jal	80003182 <iupdate>
}
    80003366:	70a2                	ld	ra,40(sp)
    80003368:	7402                	ld	s0,32(sp)
    8000336a:	64e2                	ld	s1,24(sp)
    8000336c:	6942                	ld	s2,16(sp)
    8000336e:	69a2                	ld	s3,8(sp)
    80003370:	6145                	addi	sp,sp,48
    80003372:	8082                	ret
    80003374:	e052                	sd	s4,0(sp)
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    80003376:	0009a503          	lw	a0,0(s3)
    8000337a:	ff0ff0ef          	jal	80002b6a <bread>
    8000337e:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003380:	05850493          	addi	s1,a0,88
    80003384:	45850913          	addi	s2,a0,1112
    80003388:	a021                	j	80003390 <itrunc+0x6c>
    8000338a:	0491                	addi	s1,s1,4
    8000338c:	01248963          	beq	s1,s2,8000339e <itrunc+0x7a>
      if(a[j])
    80003390:	408c                	lw	a1,0(s1)
    80003392:	dde5                	beqz	a1,8000338a <itrunc+0x66>
        bfree(ip->dev, a[j]);
    80003394:	0009a503          	lw	a0,0(s3)
    80003398:	9cbff0ef          	jal	80002d62 <bfree>
    8000339c:	b7fd                	j	8000338a <itrunc+0x66>
    brelse(bp);
    8000339e:	8552                	mv	a0,s4
    800033a0:	8d3ff0ef          	jal	80002c72 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    800033a4:	0809a583          	lw	a1,128(s3)
    800033a8:	0009a503          	lw	a0,0(s3)
    800033ac:	9b7ff0ef          	jal	80002d62 <bfree>
    ip->addrs[NDIRECT] = 0;
    800033b0:	0809a023          	sw	zero,128(s3)
    800033b4:	6a02                	ld	s4,0(sp)
    800033b6:	b75d                	j	8000335c <itrunc+0x38>

00000000800033b8 <iput>:
{
    800033b8:	1101                	addi	sp,sp,-32
    800033ba:	ec06                	sd	ra,24(sp)
    800033bc:	e822                	sd	s0,16(sp)
    800033be:	e426                	sd	s1,8(sp)
    800033c0:	1000                	addi	s0,sp,32
    800033c2:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    800033c4:	0001b517          	auipc	a0,0x1b
    800033c8:	ebc50513          	addi	a0,a0,-324 # 8001e280 <itable>
    800033cc:	803fd0ef          	jal	80000bce <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    800033d0:	4498                	lw	a4,8(s1)
    800033d2:	4785                	li	a5,1
    800033d4:	02f70063          	beq	a4,a5,800033f4 <iput+0x3c>
  ip->ref--;
    800033d8:	449c                	lw	a5,8(s1)
    800033da:	37fd                	addiw	a5,a5,-1
    800033dc:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    800033de:	0001b517          	auipc	a0,0x1b
    800033e2:	ea250513          	addi	a0,a0,-350 # 8001e280 <itable>
    800033e6:	881fd0ef          	jal	80000c66 <release>
}
    800033ea:	60e2                	ld	ra,24(sp)
    800033ec:	6442                	ld	s0,16(sp)
    800033ee:	64a2                	ld	s1,8(sp)
    800033f0:	6105                	addi	sp,sp,32
    800033f2:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    800033f4:	40bc                	lw	a5,64(s1)
    800033f6:	d3ed                	beqz	a5,800033d8 <iput+0x20>
    800033f8:	04a49783          	lh	a5,74(s1)
    800033fc:	fff1                	bnez	a5,800033d8 <iput+0x20>
    800033fe:	e04a                	sd	s2,0(sp)
    acquiresleep(&ip->lock);
    80003400:	01048913          	addi	s2,s1,16
    80003404:	854a                	mv	a0,s2
    80003406:	297000ef          	jal	80003e9c <acquiresleep>
    release(&itable.lock);
    8000340a:	0001b517          	auipc	a0,0x1b
    8000340e:	e7650513          	addi	a0,a0,-394 # 8001e280 <itable>
    80003412:	855fd0ef          	jal	80000c66 <release>
    itrunc(ip);
    80003416:	8526                	mv	a0,s1
    80003418:	f0dff0ef          	jal	80003324 <itrunc>
    ip->type = 0;
    8000341c:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003420:	8526                	mv	a0,s1
    80003422:	d61ff0ef          	jal	80003182 <iupdate>
    ip->valid = 0;
    80003426:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    8000342a:	854a                	mv	a0,s2
    8000342c:	2b7000ef          	jal	80003ee2 <releasesleep>
    acquire(&itable.lock);
    80003430:	0001b517          	auipc	a0,0x1b
    80003434:	e5050513          	addi	a0,a0,-432 # 8001e280 <itable>
    80003438:	f96fd0ef          	jal	80000bce <acquire>
    8000343c:	6902                	ld	s2,0(sp)
    8000343e:	bf69                	j	800033d8 <iput+0x20>

0000000080003440 <iunlockput>:
{
    80003440:	1101                	addi	sp,sp,-32
    80003442:	ec06                	sd	ra,24(sp)
    80003444:	e822                	sd	s0,16(sp)
    80003446:	e426                	sd	s1,8(sp)
    80003448:	1000                	addi	s0,sp,32
    8000344a:	84aa                	mv	s1,a0
  iunlock(ip);
    8000344c:	e99ff0ef          	jal	800032e4 <iunlock>
  iput(ip);
    80003450:	8526                	mv	a0,s1
    80003452:	f67ff0ef          	jal	800033b8 <iput>
}
    80003456:	60e2                	ld	ra,24(sp)
    80003458:	6442                	ld	s0,16(sp)
    8000345a:	64a2                	ld	s1,8(sp)
    8000345c:	6105                	addi	sp,sp,32
    8000345e:	8082                	ret

0000000080003460 <ireclaim>:
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003460:	0001b717          	auipc	a4,0x1b
    80003464:	e0c72703          	lw	a4,-500(a4) # 8001e26c <sb+0xc>
    80003468:	4785                	li	a5,1
    8000346a:	0ae7ff63          	bgeu	a5,a4,80003528 <ireclaim+0xc8>
{
    8000346e:	7139                	addi	sp,sp,-64
    80003470:	fc06                	sd	ra,56(sp)
    80003472:	f822                	sd	s0,48(sp)
    80003474:	f426                	sd	s1,40(sp)
    80003476:	f04a                	sd	s2,32(sp)
    80003478:	ec4e                	sd	s3,24(sp)
    8000347a:	e852                	sd	s4,16(sp)
    8000347c:	e456                	sd	s5,8(sp)
    8000347e:	e05a                	sd	s6,0(sp)
    80003480:	0080                	addi	s0,sp,64
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003482:	4485                	li	s1,1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    80003484:	00050a1b          	sext.w	s4,a0
    80003488:	0001ba97          	auipc	s5,0x1b
    8000348c:	dd8a8a93          	addi	s5,s5,-552 # 8001e260 <sb>
      printf("ireclaim: orphaned inode %d\n", inum);
    80003490:	00004b17          	auipc	s6,0x4
    80003494:	fd8b0b13          	addi	s6,s6,-40 # 80007468 <etext+0x468>
    80003498:	a099                	j	800034de <ireclaim+0x7e>
    8000349a:	85ce                	mv	a1,s3
    8000349c:	855a                	mv	a0,s6
    8000349e:	85cfd0ef          	jal	800004fa <printf>
      ip = iget(dev, inum);
    800034a2:	85ce                	mv	a1,s3
    800034a4:	8552                	mv	a0,s4
    800034a6:	b1dff0ef          	jal	80002fc2 <iget>
    800034aa:	89aa                	mv	s3,a0
    brelse(bp);
    800034ac:	854a                	mv	a0,s2
    800034ae:	fc4ff0ef          	jal	80002c72 <brelse>
    if (ip) {
    800034b2:	00098f63          	beqz	s3,800034d0 <ireclaim+0x70>
      begin_op();
    800034b6:	76a000ef          	jal	80003c20 <begin_op>
      ilock(ip);
    800034ba:	854e                	mv	a0,s3
    800034bc:	d7bff0ef          	jal	80003236 <ilock>
      iunlock(ip);
    800034c0:	854e                	mv	a0,s3
    800034c2:	e23ff0ef          	jal	800032e4 <iunlock>
      iput(ip);
    800034c6:	854e                	mv	a0,s3
    800034c8:	ef1ff0ef          	jal	800033b8 <iput>
      end_op();
    800034cc:	7be000ef          	jal	80003c8a <end_op>
  for (int inum = 1; inum < sb.ninodes; inum++) {
    800034d0:	0485                	addi	s1,s1,1
    800034d2:	00caa703          	lw	a4,12(s5)
    800034d6:	0004879b          	sext.w	a5,s1
    800034da:	02e7fd63          	bgeu	a5,a4,80003514 <ireclaim+0xb4>
    800034de:	0004899b          	sext.w	s3,s1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    800034e2:	0044d593          	srli	a1,s1,0x4
    800034e6:	018aa783          	lw	a5,24(s5)
    800034ea:	9dbd                	addw	a1,a1,a5
    800034ec:	8552                	mv	a0,s4
    800034ee:	e7cff0ef          	jal	80002b6a <bread>
    800034f2:	892a                	mv	s2,a0
    struct dinode *dip = (struct dinode *)bp->data + inum % IPB;
    800034f4:	05850793          	addi	a5,a0,88
    800034f8:	00f9f713          	andi	a4,s3,15
    800034fc:	071a                	slli	a4,a4,0x6
    800034fe:	97ba                	add	a5,a5,a4
    if (dip->type != 0 && dip->nlink == 0) {  // is an orphaned inode
    80003500:	00079703          	lh	a4,0(a5)
    80003504:	c701                	beqz	a4,8000350c <ireclaim+0xac>
    80003506:	00679783          	lh	a5,6(a5)
    8000350a:	dbc1                	beqz	a5,8000349a <ireclaim+0x3a>
    brelse(bp);
    8000350c:	854a                	mv	a0,s2
    8000350e:	f64ff0ef          	jal	80002c72 <brelse>
    if (ip) {
    80003512:	bf7d                	j	800034d0 <ireclaim+0x70>
}
    80003514:	70e2                	ld	ra,56(sp)
    80003516:	7442                	ld	s0,48(sp)
    80003518:	74a2                	ld	s1,40(sp)
    8000351a:	7902                	ld	s2,32(sp)
    8000351c:	69e2                	ld	s3,24(sp)
    8000351e:	6a42                	ld	s4,16(sp)
    80003520:	6aa2                	ld	s5,8(sp)
    80003522:	6b02                	ld	s6,0(sp)
    80003524:	6121                	addi	sp,sp,64
    80003526:	8082                	ret
    80003528:	8082                	ret

000000008000352a <fsinit>:
fsinit(int dev) {
    8000352a:	7179                	addi	sp,sp,-48
    8000352c:	f406                	sd	ra,40(sp)
    8000352e:	f022                	sd	s0,32(sp)
    80003530:	ec26                	sd	s1,24(sp)
    80003532:	e84a                	sd	s2,16(sp)
    80003534:	e44e                	sd	s3,8(sp)
    80003536:	1800                	addi	s0,sp,48
    80003538:	84aa                	mv	s1,a0
  bp = bread(dev, 1);
    8000353a:	4585                	li	a1,1
    8000353c:	e2eff0ef          	jal	80002b6a <bread>
    80003540:	892a                	mv	s2,a0
  memmove(sb, bp->data, sizeof(*sb));
    80003542:	0001b997          	auipc	s3,0x1b
    80003546:	d1e98993          	addi	s3,s3,-738 # 8001e260 <sb>
    8000354a:	02000613          	li	a2,32
    8000354e:	05850593          	addi	a1,a0,88
    80003552:	854e                	mv	a0,s3
    80003554:	faafd0ef          	jal	80000cfe <memmove>
  brelse(bp);
    80003558:	854a                	mv	a0,s2
    8000355a:	f18ff0ef          	jal	80002c72 <brelse>
  if(sb.magic != FSMAGIC)
    8000355e:	0009a703          	lw	a4,0(s3)
    80003562:	102037b7          	lui	a5,0x10203
    80003566:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    8000356a:	02f71363          	bne	a4,a5,80003590 <fsinit+0x66>
  initlog(dev, &sb);
    8000356e:	0001b597          	auipc	a1,0x1b
    80003572:	cf258593          	addi	a1,a1,-782 # 8001e260 <sb>
    80003576:	8526                	mv	a0,s1
    80003578:	62a000ef          	jal	80003ba2 <initlog>
  ireclaim(dev);
    8000357c:	8526                	mv	a0,s1
    8000357e:	ee3ff0ef          	jal	80003460 <ireclaim>
}
    80003582:	70a2                	ld	ra,40(sp)
    80003584:	7402                	ld	s0,32(sp)
    80003586:	64e2                	ld	s1,24(sp)
    80003588:	6942                	ld	s2,16(sp)
    8000358a:	69a2                	ld	s3,8(sp)
    8000358c:	6145                	addi	sp,sp,48
    8000358e:	8082                	ret
    panic("invalid file system");
    80003590:	00004517          	auipc	a0,0x4
    80003594:	ef850513          	addi	a0,a0,-264 # 80007488 <etext+0x488>
    80003598:	a48fd0ef          	jal	800007e0 <panic>

000000008000359c <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    8000359c:	1141                	addi	sp,sp,-16
    8000359e:	e422                	sd	s0,8(sp)
    800035a0:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    800035a2:	411c                	lw	a5,0(a0)
    800035a4:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    800035a6:	415c                	lw	a5,4(a0)
    800035a8:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    800035aa:	04451783          	lh	a5,68(a0)
    800035ae:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    800035b2:	04a51783          	lh	a5,74(a0)
    800035b6:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    800035ba:	04c56783          	lwu	a5,76(a0)
    800035be:	e99c                	sd	a5,16(a1)
}
    800035c0:	6422                	ld	s0,8(sp)
    800035c2:	0141                	addi	sp,sp,16
    800035c4:	8082                	ret

00000000800035c6 <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    800035c6:	457c                	lw	a5,76(a0)
    800035c8:	0ed7eb63          	bltu	a5,a3,800036be <readi+0xf8>
{
    800035cc:	7159                	addi	sp,sp,-112
    800035ce:	f486                	sd	ra,104(sp)
    800035d0:	f0a2                	sd	s0,96(sp)
    800035d2:	eca6                	sd	s1,88(sp)
    800035d4:	e0d2                	sd	s4,64(sp)
    800035d6:	fc56                	sd	s5,56(sp)
    800035d8:	f85a                	sd	s6,48(sp)
    800035da:	f45e                	sd	s7,40(sp)
    800035dc:	1880                	addi	s0,sp,112
    800035de:	8b2a                	mv	s6,a0
    800035e0:	8bae                	mv	s7,a1
    800035e2:	8a32                	mv	s4,a2
    800035e4:	84b6                	mv	s1,a3
    800035e6:	8aba                	mv	s5,a4
  if(off > ip->size || off + n < off)
    800035e8:	9f35                	addw	a4,a4,a3
    return 0;
    800035ea:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    800035ec:	0cd76063          	bltu	a4,a3,800036ac <readi+0xe6>
    800035f0:	e4ce                	sd	s3,72(sp)
  if(off + n > ip->size)
    800035f2:	00e7f463          	bgeu	a5,a4,800035fa <readi+0x34>
    n = ip->size - off;
    800035f6:	40d78abb          	subw	s5,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800035fa:	080a8f63          	beqz	s5,80003698 <readi+0xd2>
    800035fe:	e8ca                	sd	s2,80(sp)
    80003600:	f062                	sd	s8,32(sp)
    80003602:	ec66                	sd	s9,24(sp)
    80003604:	e86a                	sd	s10,16(sp)
    80003606:	e46e                	sd	s11,8(sp)
    80003608:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    8000360a:	40000c93          	li	s9,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    8000360e:	5c7d                	li	s8,-1
    80003610:	a80d                	j	80003642 <readi+0x7c>
    80003612:	020d1d93          	slli	s11,s10,0x20
    80003616:	020ddd93          	srli	s11,s11,0x20
    8000361a:	05890613          	addi	a2,s2,88
    8000361e:	86ee                	mv	a3,s11
    80003620:	963a                	add	a2,a2,a4
    80003622:	85d2                	mv	a1,s4
    80003624:	855e                	mv	a0,s7
    80003626:	c1bfe0ef          	jal	80002240 <either_copyout>
    8000362a:	05850763          	beq	a0,s8,80003678 <readi+0xb2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    8000362e:	854a                	mv	a0,s2
    80003630:	e42ff0ef          	jal	80002c72 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003634:	013d09bb          	addw	s3,s10,s3
    80003638:	009d04bb          	addw	s1,s10,s1
    8000363c:	9a6e                	add	s4,s4,s11
    8000363e:	0559f763          	bgeu	s3,s5,8000368c <readi+0xc6>
    uint addr = bmap(ip, off/BSIZE);
    80003642:	00a4d59b          	srliw	a1,s1,0xa
    80003646:	855a                	mv	a0,s6
    80003648:	8a7ff0ef          	jal	80002eee <bmap>
    8000364c:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    80003650:	c5b1                	beqz	a1,8000369c <readi+0xd6>
    bp = bread(ip->dev, addr);
    80003652:	000b2503          	lw	a0,0(s6)
    80003656:	d14ff0ef          	jal	80002b6a <bread>
    8000365a:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    8000365c:	3ff4f713          	andi	a4,s1,1023
    80003660:	40ec87bb          	subw	a5,s9,a4
    80003664:	413a86bb          	subw	a3,s5,s3
    80003668:	8d3e                	mv	s10,a5
    8000366a:	2781                	sext.w	a5,a5
    8000366c:	0006861b          	sext.w	a2,a3
    80003670:	faf671e3          	bgeu	a2,a5,80003612 <readi+0x4c>
    80003674:	8d36                	mv	s10,a3
    80003676:	bf71                	j	80003612 <readi+0x4c>
      brelse(bp);
    80003678:	854a                	mv	a0,s2
    8000367a:	df8ff0ef          	jal	80002c72 <brelse>
      tot = -1;
    8000367e:	59fd                	li	s3,-1
      break;
    80003680:	6946                	ld	s2,80(sp)
    80003682:	7c02                	ld	s8,32(sp)
    80003684:	6ce2                	ld	s9,24(sp)
    80003686:	6d42                	ld	s10,16(sp)
    80003688:	6da2                	ld	s11,8(sp)
    8000368a:	a831                	j	800036a6 <readi+0xe0>
    8000368c:	6946                	ld	s2,80(sp)
    8000368e:	7c02                	ld	s8,32(sp)
    80003690:	6ce2                	ld	s9,24(sp)
    80003692:	6d42                	ld	s10,16(sp)
    80003694:	6da2                	ld	s11,8(sp)
    80003696:	a801                	j	800036a6 <readi+0xe0>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003698:	89d6                	mv	s3,s5
    8000369a:	a031                	j	800036a6 <readi+0xe0>
    8000369c:	6946                	ld	s2,80(sp)
    8000369e:	7c02                	ld	s8,32(sp)
    800036a0:	6ce2                	ld	s9,24(sp)
    800036a2:	6d42                	ld	s10,16(sp)
    800036a4:	6da2                	ld	s11,8(sp)
  }
  return tot;
    800036a6:	0009851b          	sext.w	a0,s3
    800036aa:	69a6                	ld	s3,72(sp)
}
    800036ac:	70a6                	ld	ra,104(sp)
    800036ae:	7406                	ld	s0,96(sp)
    800036b0:	64e6                	ld	s1,88(sp)
    800036b2:	6a06                	ld	s4,64(sp)
    800036b4:	7ae2                	ld	s5,56(sp)
    800036b6:	7b42                	ld	s6,48(sp)
    800036b8:	7ba2                	ld	s7,40(sp)
    800036ba:	6165                	addi	sp,sp,112
    800036bc:	8082                	ret
    return 0;
    800036be:	4501                	li	a0,0
}
    800036c0:	8082                	ret

00000000800036c2 <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    800036c2:	457c                	lw	a5,76(a0)
    800036c4:	10d7e063          	bltu	a5,a3,800037c4 <writei+0x102>
{
    800036c8:	7159                	addi	sp,sp,-112
    800036ca:	f486                	sd	ra,104(sp)
    800036cc:	f0a2                	sd	s0,96(sp)
    800036ce:	e8ca                	sd	s2,80(sp)
    800036d0:	e0d2                	sd	s4,64(sp)
    800036d2:	fc56                	sd	s5,56(sp)
    800036d4:	f85a                	sd	s6,48(sp)
    800036d6:	f45e                	sd	s7,40(sp)
    800036d8:	1880                	addi	s0,sp,112
    800036da:	8aaa                	mv	s5,a0
    800036dc:	8bae                	mv	s7,a1
    800036de:	8a32                	mv	s4,a2
    800036e0:	8936                	mv	s2,a3
    800036e2:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    800036e4:	00e687bb          	addw	a5,a3,a4
    800036e8:	0ed7e063          	bltu	a5,a3,800037c8 <writei+0x106>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    800036ec:	00043737          	lui	a4,0x43
    800036f0:	0cf76e63          	bltu	a4,a5,800037cc <writei+0x10a>
    800036f4:	e4ce                	sd	s3,72(sp)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800036f6:	0a0b0f63          	beqz	s6,800037b4 <writei+0xf2>
    800036fa:	eca6                	sd	s1,88(sp)
    800036fc:	f062                	sd	s8,32(sp)
    800036fe:	ec66                	sd	s9,24(sp)
    80003700:	e86a                	sd	s10,16(sp)
    80003702:	e46e                	sd	s11,8(sp)
    80003704:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    80003706:	40000c93          	li	s9,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    8000370a:	5c7d                	li	s8,-1
    8000370c:	a825                	j	80003744 <writei+0x82>
    8000370e:	020d1d93          	slli	s11,s10,0x20
    80003712:	020ddd93          	srli	s11,s11,0x20
    80003716:	05848513          	addi	a0,s1,88
    8000371a:	86ee                	mv	a3,s11
    8000371c:	8652                	mv	a2,s4
    8000371e:	85de                	mv	a1,s7
    80003720:	953a                	add	a0,a0,a4
    80003722:	b69fe0ef          	jal	8000228a <either_copyin>
    80003726:	05850a63          	beq	a0,s8,8000377a <writei+0xb8>
      brelse(bp);
      break;
    }
    log_write(bp);
    8000372a:	8526                	mv	a0,s1
    8000372c:	678000ef          	jal	80003da4 <log_write>
    brelse(bp);
    80003730:	8526                	mv	a0,s1
    80003732:	d40ff0ef          	jal	80002c72 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    80003736:	013d09bb          	addw	s3,s10,s3
    8000373a:	012d093b          	addw	s2,s10,s2
    8000373e:	9a6e                	add	s4,s4,s11
    80003740:	0569f063          	bgeu	s3,s6,80003780 <writei+0xbe>
    uint addr = bmap(ip, off/BSIZE);
    80003744:	00a9559b          	srliw	a1,s2,0xa
    80003748:	8556                	mv	a0,s5
    8000374a:	fa4ff0ef          	jal	80002eee <bmap>
    8000374e:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    80003752:	c59d                	beqz	a1,80003780 <writei+0xbe>
    bp = bread(ip->dev, addr);
    80003754:	000aa503          	lw	a0,0(s5)
    80003758:	c12ff0ef          	jal	80002b6a <bread>
    8000375c:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    8000375e:	3ff97713          	andi	a4,s2,1023
    80003762:	40ec87bb          	subw	a5,s9,a4
    80003766:	413b06bb          	subw	a3,s6,s3
    8000376a:	8d3e                	mv	s10,a5
    8000376c:	2781                	sext.w	a5,a5
    8000376e:	0006861b          	sext.w	a2,a3
    80003772:	f8f67ee3          	bgeu	a2,a5,8000370e <writei+0x4c>
    80003776:	8d36                	mv	s10,a3
    80003778:	bf59                	j	8000370e <writei+0x4c>
      brelse(bp);
    8000377a:	8526                	mv	a0,s1
    8000377c:	cf6ff0ef          	jal	80002c72 <brelse>
  }

  if(off > ip->size)
    80003780:	04caa783          	lw	a5,76(s5)
    80003784:	0327fa63          	bgeu	a5,s2,800037b8 <writei+0xf6>
    ip->size = off;
    80003788:	052aa623          	sw	s2,76(s5)
    8000378c:	64e6                	ld	s1,88(sp)
    8000378e:	7c02                	ld	s8,32(sp)
    80003790:	6ce2                	ld	s9,24(sp)
    80003792:	6d42                	ld	s10,16(sp)
    80003794:	6da2                	ld	s11,8(sp)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    80003796:	8556                	mv	a0,s5
    80003798:	9ebff0ef          	jal	80003182 <iupdate>

  return tot;
    8000379c:	0009851b          	sext.w	a0,s3
    800037a0:	69a6                	ld	s3,72(sp)
}
    800037a2:	70a6                	ld	ra,104(sp)
    800037a4:	7406                	ld	s0,96(sp)
    800037a6:	6946                	ld	s2,80(sp)
    800037a8:	6a06                	ld	s4,64(sp)
    800037aa:	7ae2                	ld	s5,56(sp)
    800037ac:	7b42                	ld	s6,48(sp)
    800037ae:	7ba2                	ld	s7,40(sp)
    800037b0:	6165                	addi	sp,sp,112
    800037b2:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800037b4:	89da                	mv	s3,s6
    800037b6:	b7c5                	j	80003796 <writei+0xd4>
    800037b8:	64e6                	ld	s1,88(sp)
    800037ba:	7c02                	ld	s8,32(sp)
    800037bc:	6ce2                	ld	s9,24(sp)
    800037be:	6d42                	ld	s10,16(sp)
    800037c0:	6da2                	ld	s11,8(sp)
    800037c2:	bfd1                	j	80003796 <writei+0xd4>
    return -1;
    800037c4:	557d                	li	a0,-1
}
    800037c6:	8082                	ret
    return -1;
    800037c8:	557d                	li	a0,-1
    800037ca:	bfe1                	j	800037a2 <writei+0xe0>
    return -1;
    800037cc:	557d                	li	a0,-1
    800037ce:	bfd1                	j	800037a2 <writei+0xe0>

00000000800037d0 <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    800037d0:	1141                	addi	sp,sp,-16
    800037d2:	e406                	sd	ra,8(sp)
    800037d4:	e022                	sd	s0,0(sp)
    800037d6:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    800037d8:	4639                	li	a2,14
    800037da:	d94fd0ef          	jal	80000d6e <strncmp>
}
    800037de:	60a2                	ld	ra,8(sp)
    800037e0:	6402                	ld	s0,0(sp)
    800037e2:	0141                	addi	sp,sp,16
    800037e4:	8082                	ret

00000000800037e6 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    800037e6:	7139                	addi	sp,sp,-64
    800037e8:	fc06                	sd	ra,56(sp)
    800037ea:	f822                	sd	s0,48(sp)
    800037ec:	f426                	sd	s1,40(sp)
    800037ee:	f04a                	sd	s2,32(sp)
    800037f0:	ec4e                	sd	s3,24(sp)
    800037f2:	e852                	sd	s4,16(sp)
    800037f4:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    800037f6:	04451703          	lh	a4,68(a0)
    800037fa:	4785                	li	a5,1
    800037fc:	00f71a63          	bne	a4,a5,80003810 <dirlookup+0x2a>
    80003800:	892a                	mv	s2,a0
    80003802:	89ae                	mv	s3,a1
    80003804:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    80003806:	457c                	lw	a5,76(a0)
    80003808:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    8000380a:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    8000380c:	e39d                	bnez	a5,80003832 <dirlookup+0x4c>
    8000380e:	a095                	j	80003872 <dirlookup+0x8c>
    panic("dirlookup not DIR");
    80003810:	00004517          	auipc	a0,0x4
    80003814:	c9050513          	addi	a0,a0,-880 # 800074a0 <etext+0x4a0>
    80003818:	fc9fc0ef          	jal	800007e0 <panic>
      panic("dirlookup read");
    8000381c:	00004517          	auipc	a0,0x4
    80003820:	c9c50513          	addi	a0,a0,-868 # 800074b8 <etext+0x4b8>
    80003824:	fbdfc0ef          	jal	800007e0 <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003828:	24c1                	addiw	s1,s1,16
    8000382a:	04c92783          	lw	a5,76(s2)
    8000382e:	04f4f163          	bgeu	s1,a5,80003870 <dirlookup+0x8a>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003832:	4741                	li	a4,16
    80003834:	86a6                	mv	a3,s1
    80003836:	fc040613          	addi	a2,s0,-64
    8000383a:	4581                	li	a1,0
    8000383c:	854a                	mv	a0,s2
    8000383e:	d89ff0ef          	jal	800035c6 <readi>
    80003842:	47c1                	li	a5,16
    80003844:	fcf51ce3          	bne	a0,a5,8000381c <dirlookup+0x36>
    if(de.inum == 0)
    80003848:	fc045783          	lhu	a5,-64(s0)
    8000384c:	dff1                	beqz	a5,80003828 <dirlookup+0x42>
    if(namecmp(name, de.name) == 0){
    8000384e:	fc240593          	addi	a1,s0,-62
    80003852:	854e                	mv	a0,s3
    80003854:	f7dff0ef          	jal	800037d0 <namecmp>
    80003858:	f961                	bnez	a0,80003828 <dirlookup+0x42>
      if(poff)
    8000385a:	000a0463          	beqz	s4,80003862 <dirlookup+0x7c>
        *poff = off;
    8000385e:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    80003862:	fc045583          	lhu	a1,-64(s0)
    80003866:	00092503          	lw	a0,0(s2)
    8000386a:	f58ff0ef          	jal	80002fc2 <iget>
    8000386e:	a011                	j	80003872 <dirlookup+0x8c>
  return 0;
    80003870:	4501                	li	a0,0
}
    80003872:	70e2                	ld	ra,56(sp)
    80003874:	7442                	ld	s0,48(sp)
    80003876:	74a2                	ld	s1,40(sp)
    80003878:	7902                	ld	s2,32(sp)
    8000387a:	69e2                	ld	s3,24(sp)
    8000387c:	6a42                	ld	s4,16(sp)
    8000387e:	6121                	addi	sp,sp,64
    80003880:	8082                	ret

0000000080003882 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    80003882:	711d                	addi	sp,sp,-96
    80003884:	ec86                	sd	ra,88(sp)
    80003886:	e8a2                	sd	s0,80(sp)
    80003888:	e4a6                	sd	s1,72(sp)
    8000388a:	e0ca                	sd	s2,64(sp)
    8000388c:	fc4e                	sd	s3,56(sp)
    8000388e:	f852                	sd	s4,48(sp)
    80003890:	f456                	sd	s5,40(sp)
    80003892:	f05a                	sd	s6,32(sp)
    80003894:	ec5e                	sd	s7,24(sp)
    80003896:	e862                	sd	s8,16(sp)
    80003898:	e466                	sd	s9,8(sp)
    8000389a:	1080                	addi	s0,sp,96
    8000389c:	84aa                	mv	s1,a0
    8000389e:	8b2e                	mv	s6,a1
    800038a0:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    800038a2:	00054703          	lbu	a4,0(a0)
    800038a6:	02f00793          	li	a5,47
    800038aa:	00f70e63          	beq	a4,a5,800038c6 <namex+0x44>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    800038ae:	820fe0ef          	jal	800018ce <myproc>
    800038b2:	15053503          	ld	a0,336(a0)
    800038b6:	94bff0ef          	jal	80003200 <idup>
    800038ba:	8a2a                	mv	s4,a0
  while(*path == '/')
    800038bc:	02f00913          	li	s2,47
  if(len >= DIRSIZ)
    800038c0:	4c35                	li	s8,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    800038c2:	4b85                	li	s7,1
    800038c4:	a871                	j	80003960 <namex+0xde>
    ip = iget(ROOTDEV, ROOTINO);
    800038c6:	4585                	li	a1,1
    800038c8:	4505                	li	a0,1
    800038ca:	ef8ff0ef          	jal	80002fc2 <iget>
    800038ce:	8a2a                	mv	s4,a0
    800038d0:	b7f5                	j	800038bc <namex+0x3a>
      iunlockput(ip);
    800038d2:	8552                	mv	a0,s4
    800038d4:	b6dff0ef          	jal	80003440 <iunlockput>
      return 0;
    800038d8:	4a01                	li	s4,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    800038da:	8552                	mv	a0,s4
    800038dc:	60e6                	ld	ra,88(sp)
    800038de:	6446                	ld	s0,80(sp)
    800038e0:	64a6                	ld	s1,72(sp)
    800038e2:	6906                	ld	s2,64(sp)
    800038e4:	79e2                	ld	s3,56(sp)
    800038e6:	7a42                	ld	s4,48(sp)
    800038e8:	7aa2                	ld	s5,40(sp)
    800038ea:	7b02                	ld	s6,32(sp)
    800038ec:	6be2                	ld	s7,24(sp)
    800038ee:	6c42                	ld	s8,16(sp)
    800038f0:	6ca2                	ld	s9,8(sp)
    800038f2:	6125                	addi	sp,sp,96
    800038f4:	8082                	ret
      iunlock(ip);
    800038f6:	8552                	mv	a0,s4
    800038f8:	9edff0ef          	jal	800032e4 <iunlock>
      return ip;
    800038fc:	bff9                	j	800038da <namex+0x58>
      iunlockput(ip);
    800038fe:	8552                	mv	a0,s4
    80003900:	b41ff0ef          	jal	80003440 <iunlockput>
      return 0;
    80003904:	8a4e                	mv	s4,s3
    80003906:	bfd1                	j	800038da <namex+0x58>
  len = path - s;
    80003908:	40998633          	sub	a2,s3,s1
    8000390c:	00060c9b          	sext.w	s9,a2
  if(len >= DIRSIZ)
    80003910:	099c5063          	bge	s8,s9,80003990 <namex+0x10e>
    memmove(name, s, DIRSIZ);
    80003914:	4639                	li	a2,14
    80003916:	85a6                	mv	a1,s1
    80003918:	8556                	mv	a0,s5
    8000391a:	be4fd0ef          	jal	80000cfe <memmove>
    8000391e:	84ce                	mv	s1,s3
  while(*path == '/')
    80003920:	0004c783          	lbu	a5,0(s1)
    80003924:	01279763          	bne	a5,s2,80003932 <namex+0xb0>
    path++;
    80003928:	0485                	addi	s1,s1,1
  while(*path == '/')
    8000392a:	0004c783          	lbu	a5,0(s1)
    8000392e:	ff278de3          	beq	a5,s2,80003928 <namex+0xa6>
    ilock(ip);
    80003932:	8552                	mv	a0,s4
    80003934:	903ff0ef          	jal	80003236 <ilock>
    if(ip->type != T_DIR){
    80003938:	044a1783          	lh	a5,68(s4)
    8000393c:	f9779be3          	bne	a5,s7,800038d2 <namex+0x50>
    if(nameiparent && *path == '\0'){
    80003940:	000b0563          	beqz	s6,8000394a <namex+0xc8>
    80003944:	0004c783          	lbu	a5,0(s1)
    80003948:	d7dd                	beqz	a5,800038f6 <namex+0x74>
    if((next = dirlookup(ip, name, 0)) == 0){
    8000394a:	4601                	li	a2,0
    8000394c:	85d6                	mv	a1,s5
    8000394e:	8552                	mv	a0,s4
    80003950:	e97ff0ef          	jal	800037e6 <dirlookup>
    80003954:	89aa                	mv	s3,a0
    80003956:	d545                	beqz	a0,800038fe <namex+0x7c>
    iunlockput(ip);
    80003958:	8552                	mv	a0,s4
    8000395a:	ae7ff0ef          	jal	80003440 <iunlockput>
    ip = next;
    8000395e:	8a4e                	mv	s4,s3
  while(*path == '/')
    80003960:	0004c783          	lbu	a5,0(s1)
    80003964:	01279763          	bne	a5,s2,80003972 <namex+0xf0>
    path++;
    80003968:	0485                	addi	s1,s1,1
  while(*path == '/')
    8000396a:	0004c783          	lbu	a5,0(s1)
    8000396e:	ff278de3          	beq	a5,s2,80003968 <namex+0xe6>
  if(*path == 0)
    80003972:	cb8d                	beqz	a5,800039a4 <namex+0x122>
  while(*path != '/' && *path != 0)
    80003974:	0004c783          	lbu	a5,0(s1)
    80003978:	89a6                	mv	s3,s1
  len = path - s;
    8000397a:	4c81                	li	s9,0
    8000397c:	4601                	li	a2,0
  while(*path != '/' && *path != 0)
    8000397e:	01278963          	beq	a5,s2,80003990 <namex+0x10e>
    80003982:	d3d9                	beqz	a5,80003908 <namex+0x86>
    path++;
    80003984:	0985                	addi	s3,s3,1
  while(*path != '/' && *path != 0)
    80003986:	0009c783          	lbu	a5,0(s3)
    8000398a:	ff279ce3          	bne	a5,s2,80003982 <namex+0x100>
    8000398e:	bfad                	j	80003908 <namex+0x86>
    memmove(name, s, len);
    80003990:	2601                	sext.w	a2,a2
    80003992:	85a6                	mv	a1,s1
    80003994:	8556                	mv	a0,s5
    80003996:	b68fd0ef          	jal	80000cfe <memmove>
    name[len] = 0;
    8000399a:	9cd6                	add	s9,s9,s5
    8000399c:	000c8023          	sb	zero,0(s9) # 2000 <_entry-0x7fffe000>
    800039a0:	84ce                	mv	s1,s3
    800039a2:	bfbd                	j	80003920 <namex+0x9e>
  if(nameiparent){
    800039a4:	f20b0be3          	beqz	s6,800038da <namex+0x58>
    iput(ip);
    800039a8:	8552                	mv	a0,s4
    800039aa:	a0fff0ef          	jal	800033b8 <iput>
    return 0;
    800039ae:	4a01                	li	s4,0
    800039b0:	b72d                	j	800038da <namex+0x58>

00000000800039b2 <dirlink>:
{
    800039b2:	7139                	addi	sp,sp,-64
    800039b4:	fc06                	sd	ra,56(sp)
    800039b6:	f822                	sd	s0,48(sp)
    800039b8:	f04a                	sd	s2,32(sp)
    800039ba:	ec4e                	sd	s3,24(sp)
    800039bc:	e852                	sd	s4,16(sp)
    800039be:	0080                	addi	s0,sp,64
    800039c0:	892a                	mv	s2,a0
    800039c2:	8a2e                	mv	s4,a1
    800039c4:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    800039c6:	4601                	li	a2,0
    800039c8:	e1fff0ef          	jal	800037e6 <dirlookup>
    800039cc:	e535                	bnez	a0,80003a38 <dirlink+0x86>
    800039ce:	f426                	sd	s1,40(sp)
  for(off = 0; off < dp->size; off += sizeof(de)){
    800039d0:	04c92483          	lw	s1,76(s2)
    800039d4:	c48d                	beqz	s1,800039fe <dirlink+0x4c>
    800039d6:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800039d8:	4741                	li	a4,16
    800039da:	86a6                	mv	a3,s1
    800039dc:	fc040613          	addi	a2,s0,-64
    800039e0:	4581                	li	a1,0
    800039e2:	854a                	mv	a0,s2
    800039e4:	be3ff0ef          	jal	800035c6 <readi>
    800039e8:	47c1                	li	a5,16
    800039ea:	04f51b63          	bne	a0,a5,80003a40 <dirlink+0x8e>
    if(de.inum == 0)
    800039ee:	fc045783          	lhu	a5,-64(s0)
    800039f2:	c791                	beqz	a5,800039fe <dirlink+0x4c>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800039f4:	24c1                	addiw	s1,s1,16
    800039f6:	04c92783          	lw	a5,76(s2)
    800039fa:	fcf4efe3          	bltu	s1,a5,800039d8 <dirlink+0x26>
  strncpy(de.name, name, DIRSIZ);
    800039fe:	4639                	li	a2,14
    80003a00:	85d2                	mv	a1,s4
    80003a02:	fc240513          	addi	a0,s0,-62
    80003a06:	b9efd0ef          	jal	80000da4 <strncpy>
  de.inum = inum;
    80003a0a:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003a0e:	4741                	li	a4,16
    80003a10:	86a6                	mv	a3,s1
    80003a12:	fc040613          	addi	a2,s0,-64
    80003a16:	4581                	li	a1,0
    80003a18:	854a                	mv	a0,s2
    80003a1a:	ca9ff0ef          	jal	800036c2 <writei>
    80003a1e:	1541                	addi	a0,a0,-16
    80003a20:	00a03533          	snez	a0,a0
    80003a24:	40a00533          	neg	a0,a0
    80003a28:	74a2                	ld	s1,40(sp)
}
    80003a2a:	70e2                	ld	ra,56(sp)
    80003a2c:	7442                	ld	s0,48(sp)
    80003a2e:	7902                	ld	s2,32(sp)
    80003a30:	69e2                	ld	s3,24(sp)
    80003a32:	6a42                	ld	s4,16(sp)
    80003a34:	6121                	addi	sp,sp,64
    80003a36:	8082                	ret
    iput(ip);
    80003a38:	981ff0ef          	jal	800033b8 <iput>
    return -1;
    80003a3c:	557d                	li	a0,-1
    80003a3e:	b7f5                	j	80003a2a <dirlink+0x78>
      panic("dirlink read");
    80003a40:	00004517          	auipc	a0,0x4
    80003a44:	a8850513          	addi	a0,a0,-1400 # 800074c8 <etext+0x4c8>
    80003a48:	d99fc0ef          	jal	800007e0 <panic>

0000000080003a4c <namei>:

struct inode*
namei(char *path)
{
    80003a4c:	1101                	addi	sp,sp,-32
    80003a4e:	ec06                	sd	ra,24(sp)
    80003a50:	e822                	sd	s0,16(sp)
    80003a52:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    80003a54:	fe040613          	addi	a2,s0,-32
    80003a58:	4581                	li	a1,0
    80003a5a:	e29ff0ef          	jal	80003882 <namex>
}
    80003a5e:	60e2                	ld	ra,24(sp)
    80003a60:	6442                	ld	s0,16(sp)
    80003a62:	6105                	addi	sp,sp,32
    80003a64:	8082                	ret

0000000080003a66 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    80003a66:	1141                	addi	sp,sp,-16
    80003a68:	e406                	sd	ra,8(sp)
    80003a6a:	e022                	sd	s0,0(sp)
    80003a6c:	0800                	addi	s0,sp,16
    80003a6e:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80003a70:	4585                	li	a1,1
    80003a72:	e11ff0ef          	jal	80003882 <namex>
}
    80003a76:	60a2                	ld	ra,8(sp)
    80003a78:	6402                	ld	s0,0(sp)
    80003a7a:	0141                	addi	sp,sp,16
    80003a7c:	8082                	ret

0000000080003a7e <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    80003a7e:	1101                	addi	sp,sp,-32
    80003a80:	ec06                	sd	ra,24(sp)
    80003a82:	e822                	sd	s0,16(sp)
    80003a84:	e426                	sd	s1,8(sp)
    80003a86:	e04a                	sd	s2,0(sp)
    80003a88:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    80003a8a:	0001c917          	auipc	s2,0x1c
    80003a8e:	29e90913          	addi	s2,s2,670 # 8001fd28 <log>
    80003a92:	01892583          	lw	a1,24(s2)
    80003a96:	02492503          	lw	a0,36(s2)
    80003a9a:	8d0ff0ef          	jal	80002b6a <bread>
    80003a9e:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    80003aa0:	02892603          	lw	a2,40(s2)
    80003aa4:	cd30                	sw	a2,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    80003aa6:	00c05f63          	blez	a2,80003ac4 <write_head+0x46>
    80003aaa:	0001c717          	auipc	a4,0x1c
    80003aae:	2aa70713          	addi	a4,a4,682 # 8001fd54 <log+0x2c>
    80003ab2:	87aa                	mv	a5,a0
    80003ab4:	060a                	slli	a2,a2,0x2
    80003ab6:	962a                	add	a2,a2,a0
    hb->block[i] = log.lh.block[i];
    80003ab8:	4314                	lw	a3,0(a4)
    80003aba:	cff4                	sw	a3,92(a5)
  for (i = 0; i < log.lh.n; i++) {
    80003abc:	0711                	addi	a4,a4,4
    80003abe:	0791                	addi	a5,a5,4
    80003ac0:	fec79ce3          	bne	a5,a2,80003ab8 <write_head+0x3a>
  }
  bwrite(buf);
    80003ac4:	8526                	mv	a0,s1
    80003ac6:	97aff0ef          	jal	80002c40 <bwrite>
  brelse(buf);
    80003aca:	8526                	mv	a0,s1
    80003acc:	9a6ff0ef          	jal	80002c72 <brelse>
}
    80003ad0:	60e2                	ld	ra,24(sp)
    80003ad2:	6442                	ld	s0,16(sp)
    80003ad4:	64a2                	ld	s1,8(sp)
    80003ad6:	6902                	ld	s2,0(sp)
    80003ad8:	6105                	addi	sp,sp,32
    80003ada:	8082                	ret

0000000080003adc <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    80003adc:	0001c797          	auipc	a5,0x1c
    80003ae0:	2747a783          	lw	a5,628(a5) # 8001fd50 <log+0x28>
    80003ae4:	0af05e63          	blez	a5,80003ba0 <install_trans+0xc4>
{
    80003ae8:	715d                	addi	sp,sp,-80
    80003aea:	e486                	sd	ra,72(sp)
    80003aec:	e0a2                	sd	s0,64(sp)
    80003aee:	fc26                	sd	s1,56(sp)
    80003af0:	f84a                	sd	s2,48(sp)
    80003af2:	f44e                	sd	s3,40(sp)
    80003af4:	f052                	sd	s4,32(sp)
    80003af6:	ec56                	sd	s5,24(sp)
    80003af8:	e85a                	sd	s6,16(sp)
    80003afa:	e45e                	sd	s7,8(sp)
    80003afc:	0880                	addi	s0,sp,80
    80003afe:	8b2a                	mv	s6,a0
    80003b00:	0001ca97          	auipc	s5,0x1c
    80003b04:	254a8a93          	addi	s5,s5,596 # 8001fd54 <log+0x2c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003b08:	4981                	li	s3,0
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003b0a:	00004b97          	auipc	s7,0x4
    80003b0e:	9ceb8b93          	addi	s7,s7,-1586 # 800074d8 <etext+0x4d8>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003b12:	0001ca17          	auipc	s4,0x1c
    80003b16:	216a0a13          	addi	s4,s4,534 # 8001fd28 <log>
    80003b1a:	a025                	j	80003b42 <install_trans+0x66>
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003b1c:	000aa603          	lw	a2,0(s5)
    80003b20:	85ce                	mv	a1,s3
    80003b22:	855e                	mv	a0,s7
    80003b24:	9d7fc0ef          	jal	800004fa <printf>
    80003b28:	a839                	j	80003b46 <install_trans+0x6a>
    brelse(lbuf);
    80003b2a:	854a                	mv	a0,s2
    80003b2c:	946ff0ef          	jal	80002c72 <brelse>
    brelse(dbuf);
    80003b30:	8526                	mv	a0,s1
    80003b32:	940ff0ef          	jal	80002c72 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003b36:	2985                	addiw	s3,s3,1
    80003b38:	0a91                	addi	s5,s5,4
    80003b3a:	028a2783          	lw	a5,40(s4)
    80003b3e:	04f9d663          	bge	s3,a5,80003b8a <install_trans+0xae>
    if(recovering) {
    80003b42:	fc0b1de3          	bnez	s6,80003b1c <install_trans+0x40>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003b46:	018a2583          	lw	a1,24(s4)
    80003b4a:	013585bb          	addw	a1,a1,s3
    80003b4e:	2585                	addiw	a1,a1,1
    80003b50:	024a2503          	lw	a0,36(s4)
    80003b54:	816ff0ef          	jal	80002b6a <bread>
    80003b58:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80003b5a:	000aa583          	lw	a1,0(s5)
    80003b5e:	024a2503          	lw	a0,36(s4)
    80003b62:	808ff0ef          	jal	80002b6a <bread>
    80003b66:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80003b68:	40000613          	li	a2,1024
    80003b6c:	05890593          	addi	a1,s2,88
    80003b70:	05850513          	addi	a0,a0,88
    80003b74:	98afd0ef          	jal	80000cfe <memmove>
    bwrite(dbuf);  // write dst to disk
    80003b78:	8526                	mv	a0,s1
    80003b7a:	8c6ff0ef          	jal	80002c40 <bwrite>
    if(recovering == 0)
    80003b7e:	fa0b16e3          	bnez	s6,80003b2a <install_trans+0x4e>
      bunpin(dbuf);
    80003b82:	8526                	mv	a0,s1
    80003b84:	9aaff0ef          	jal	80002d2e <bunpin>
    80003b88:	b74d                	j	80003b2a <install_trans+0x4e>
}
    80003b8a:	60a6                	ld	ra,72(sp)
    80003b8c:	6406                	ld	s0,64(sp)
    80003b8e:	74e2                	ld	s1,56(sp)
    80003b90:	7942                	ld	s2,48(sp)
    80003b92:	79a2                	ld	s3,40(sp)
    80003b94:	7a02                	ld	s4,32(sp)
    80003b96:	6ae2                	ld	s5,24(sp)
    80003b98:	6b42                	ld	s6,16(sp)
    80003b9a:	6ba2                	ld	s7,8(sp)
    80003b9c:	6161                	addi	sp,sp,80
    80003b9e:	8082                	ret
    80003ba0:	8082                	ret

0000000080003ba2 <initlog>:
{
    80003ba2:	7179                	addi	sp,sp,-48
    80003ba4:	f406                	sd	ra,40(sp)
    80003ba6:	f022                	sd	s0,32(sp)
    80003ba8:	ec26                	sd	s1,24(sp)
    80003baa:	e84a                	sd	s2,16(sp)
    80003bac:	e44e                	sd	s3,8(sp)
    80003bae:	1800                	addi	s0,sp,48
    80003bb0:	892a                	mv	s2,a0
    80003bb2:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    80003bb4:	0001c497          	auipc	s1,0x1c
    80003bb8:	17448493          	addi	s1,s1,372 # 8001fd28 <log>
    80003bbc:	00004597          	auipc	a1,0x4
    80003bc0:	93c58593          	addi	a1,a1,-1732 # 800074f8 <etext+0x4f8>
    80003bc4:	8526                	mv	a0,s1
    80003bc6:	f89fc0ef          	jal	80000b4e <initlock>
  log.start = sb->logstart;
    80003bca:	0149a583          	lw	a1,20(s3)
    80003bce:	cc8c                	sw	a1,24(s1)
  log.dev = dev;
    80003bd0:	0324a223          	sw	s2,36(s1)
  struct buf *buf = bread(log.dev, log.start);
    80003bd4:	854a                	mv	a0,s2
    80003bd6:	f95fe0ef          	jal	80002b6a <bread>
  log.lh.n = lh->n;
    80003bda:	4d30                	lw	a2,88(a0)
    80003bdc:	d490                	sw	a2,40(s1)
  for (i = 0; i < log.lh.n; i++) {
    80003bde:	00c05f63          	blez	a2,80003bfc <initlog+0x5a>
    80003be2:	87aa                	mv	a5,a0
    80003be4:	0001c717          	auipc	a4,0x1c
    80003be8:	17070713          	addi	a4,a4,368 # 8001fd54 <log+0x2c>
    80003bec:	060a                	slli	a2,a2,0x2
    80003bee:	962a                	add	a2,a2,a0
    log.lh.block[i] = lh->block[i];
    80003bf0:	4ff4                	lw	a3,92(a5)
    80003bf2:	c314                	sw	a3,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80003bf4:	0791                	addi	a5,a5,4
    80003bf6:	0711                	addi	a4,a4,4
    80003bf8:	fec79ce3          	bne	a5,a2,80003bf0 <initlog+0x4e>
  brelse(buf);
    80003bfc:	876ff0ef          	jal	80002c72 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    80003c00:	4505                	li	a0,1
    80003c02:	edbff0ef          	jal	80003adc <install_trans>
  log.lh.n = 0;
    80003c06:	0001c797          	auipc	a5,0x1c
    80003c0a:	1407a523          	sw	zero,330(a5) # 8001fd50 <log+0x28>
  write_head(); // clear the log
    80003c0e:	e71ff0ef          	jal	80003a7e <write_head>
}
    80003c12:	70a2                	ld	ra,40(sp)
    80003c14:	7402                	ld	s0,32(sp)
    80003c16:	64e2                	ld	s1,24(sp)
    80003c18:	6942                	ld	s2,16(sp)
    80003c1a:	69a2                	ld	s3,8(sp)
    80003c1c:	6145                	addi	sp,sp,48
    80003c1e:	8082                	ret

0000000080003c20 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    80003c20:	1101                	addi	sp,sp,-32
    80003c22:	ec06                	sd	ra,24(sp)
    80003c24:	e822                	sd	s0,16(sp)
    80003c26:	e426                	sd	s1,8(sp)
    80003c28:	e04a                	sd	s2,0(sp)
    80003c2a:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    80003c2c:	0001c517          	auipc	a0,0x1c
    80003c30:	0fc50513          	addi	a0,a0,252 # 8001fd28 <log>
    80003c34:	f9bfc0ef          	jal	80000bce <acquire>
  while(1){
    if(log.committing){
    80003c38:	0001c497          	auipc	s1,0x1c
    80003c3c:	0f048493          	addi	s1,s1,240 # 8001fd28 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003c40:	4979                	li	s2,30
    80003c42:	a029                	j	80003c4c <begin_op+0x2c>
      sleep(&log, &log.lock);
    80003c44:	85a6                	mv	a1,s1
    80003c46:	8526                	mv	a0,s1
    80003c48:	a9cfe0ef          	jal	80001ee4 <sleep>
    if(log.committing){
    80003c4c:	509c                	lw	a5,32(s1)
    80003c4e:	fbfd                	bnez	a5,80003c44 <begin_op+0x24>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003c50:	4cd8                	lw	a4,28(s1)
    80003c52:	2705                	addiw	a4,a4,1
    80003c54:	0027179b          	slliw	a5,a4,0x2
    80003c58:	9fb9                	addw	a5,a5,a4
    80003c5a:	0017979b          	slliw	a5,a5,0x1
    80003c5e:	5494                	lw	a3,40(s1)
    80003c60:	9fb5                	addw	a5,a5,a3
    80003c62:	00f95763          	bge	s2,a5,80003c70 <begin_op+0x50>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80003c66:	85a6                	mv	a1,s1
    80003c68:	8526                	mv	a0,s1
    80003c6a:	a7afe0ef          	jal	80001ee4 <sleep>
    80003c6e:	bff9                	j	80003c4c <begin_op+0x2c>
    } else {
      log.outstanding += 1;
    80003c70:	0001c517          	auipc	a0,0x1c
    80003c74:	0b850513          	addi	a0,a0,184 # 8001fd28 <log>
    80003c78:	cd58                	sw	a4,28(a0)
      release(&log.lock);
    80003c7a:	fedfc0ef          	jal	80000c66 <release>
      break;
    }
  }
}
    80003c7e:	60e2                	ld	ra,24(sp)
    80003c80:	6442                	ld	s0,16(sp)
    80003c82:	64a2                	ld	s1,8(sp)
    80003c84:	6902                	ld	s2,0(sp)
    80003c86:	6105                	addi	sp,sp,32
    80003c88:	8082                	ret

0000000080003c8a <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    80003c8a:	7139                	addi	sp,sp,-64
    80003c8c:	fc06                	sd	ra,56(sp)
    80003c8e:	f822                	sd	s0,48(sp)
    80003c90:	f426                	sd	s1,40(sp)
    80003c92:	f04a                	sd	s2,32(sp)
    80003c94:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    80003c96:	0001c497          	auipc	s1,0x1c
    80003c9a:	09248493          	addi	s1,s1,146 # 8001fd28 <log>
    80003c9e:	8526                	mv	a0,s1
    80003ca0:	f2ffc0ef          	jal	80000bce <acquire>
  log.outstanding -= 1;
    80003ca4:	4cdc                	lw	a5,28(s1)
    80003ca6:	37fd                	addiw	a5,a5,-1
    80003ca8:	0007891b          	sext.w	s2,a5
    80003cac:	ccdc                	sw	a5,28(s1)
  if(log.committing)
    80003cae:	509c                	lw	a5,32(s1)
    80003cb0:	ef9d                	bnez	a5,80003cee <end_op+0x64>
    panic("log.committing");
  if(log.outstanding == 0){
    80003cb2:	04091763          	bnez	s2,80003d00 <end_op+0x76>
    do_commit = 1;
    log.committing = 1;
    80003cb6:	0001c497          	auipc	s1,0x1c
    80003cba:	07248493          	addi	s1,s1,114 # 8001fd28 <log>
    80003cbe:	4785                	li	a5,1
    80003cc0:	d09c                	sw	a5,32(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    80003cc2:	8526                	mv	a0,s1
    80003cc4:	fa3fc0ef          	jal	80000c66 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    80003cc8:	549c                	lw	a5,40(s1)
    80003cca:	04f04b63          	bgtz	a5,80003d20 <end_op+0x96>
    acquire(&log.lock);
    80003cce:	0001c497          	auipc	s1,0x1c
    80003cd2:	05a48493          	addi	s1,s1,90 # 8001fd28 <log>
    80003cd6:	8526                	mv	a0,s1
    80003cd8:	ef7fc0ef          	jal	80000bce <acquire>
    log.committing = 0;
    80003cdc:	0204a023          	sw	zero,32(s1)
    wakeup(&log);
    80003ce0:	8526                	mv	a0,s1
    80003ce2:	a4efe0ef          	jal	80001f30 <wakeup>
    release(&log.lock);
    80003ce6:	8526                	mv	a0,s1
    80003ce8:	f7ffc0ef          	jal	80000c66 <release>
}
    80003cec:	a025                	j	80003d14 <end_op+0x8a>
    80003cee:	ec4e                	sd	s3,24(sp)
    80003cf0:	e852                	sd	s4,16(sp)
    80003cf2:	e456                	sd	s5,8(sp)
    panic("log.committing");
    80003cf4:	00004517          	auipc	a0,0x4
    80003cf8:	80c50513          	addi	a0,a0,-2036 # 80007500 <etext+0x500>
    80003cfc:	ae5fc0ef          	jal	800007e0 <panic>
    wakeup(&log);
    80003d00:	0001c497          	auipc	s1,0x1c
    80003d04:	02848493          	addi	s1,s1,40 # 8001fd28 <log>
    80003d08:	8526                	mv	a0,s1
    80003d0a:	a26fe0ef          	jal	80001f30 <wakeup>
  release(&log.lock);
    80003d0e:	8526                	mv	a0,s1
    80003d10:	f57fc0ef          	jal	80000c66 <release>
}
    80003d14:	70e2                	ld	ra,56(sp)
    80003d16:	7442                	ld	s0,48(sp)
    80003d18:	74a2                	ld	s1,40(sp)
    80003d1a:	7902                	ld	s2,32(sp)
    80003d1c:	6121                	addi	sp,sp,64
    80003d1e:	8082                	ret
    80003d20:	ec4e                	sd	s3,24(sp)
    80003d22:	e852                	sd	s4,16(sp)
    80003d24:	e456                	sd	s5,8(sp)
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d26:	0001ca97          	auipc	s5,0x1c
    80003d2a:	02ea8a93          	addi	s5,s5,46 # 8001fd54 <log+0x2c>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003d2e:	0001ca17          	auipc	s4,0x1c
    80003d32:	ffaa0a13          	addi	s4,s4,-6 # 8001fd28 <log>
    80003d36:	018a2583          	lw	a1,24(s4)
    80003d3a:	012585bb          	addw	a1,a1,s2
    80003d3e:	2585                	addiw	a1,a1,1
    80003d40:	024a2503          	lw	a0,36(s4)
    80003d44:	e27fe0ef          	jal	80002b6a <bread>
    80003d48:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80003d4a:	000aa583          	lw	a1,0(s5)
    80003d4e:	024a2503          	lw	a0,36(s4)
    80003d52:	e19fe0ef          	jal	80002b6a <bread>
    80003d56:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80003d58:	40000613          	li	a2,1024
    80003d5c:	05850593          	addi	a1,a0,88
    80003d60:	05848513          	addi	a0,s1,88
    80003d64:	f9bfc0ef          	jal	80000cfe <memmove>
    bwrite(to);  // write the log
    80003d68:	8526                	mv	a0,s1
    80003d6a:	ed7fe0ef          	jal	80002c40 <bwrite>
    brelse(from);
    80003d6e:	854e                	mv	a0,s3
    80003d70:	f03fe0ef          	jal	80002c72 <brelse>
    brelse(to);
    80003d74:	8526                	mv	a0,s1
    80003d76:	efdfe0ef          	jal	80002c72 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d7a:	2905                	addiw	s2,s2,1
    80003d7c:	0a91                	addi	s5,s5,4
    80003d7e:	028a2783          	lw	a5,40(s4)
    80003d82:	faf94ae3          	blt	s2,a5,80003d36 <end_op+0xac>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80003d86:	cf9ff0ef          	jal	80003a7e <write_head>
    install_trans(0); // Now install writes to home locations
    80003d8a:	4501                	li	a0,0
    80003d8c:	d51ff0ef          	jal	80003adc <install_trans>
    log.lh.n = 0;
    80003d90:	0001c797          	auipc	a5,0x1c
    80003d94:	fc07a023          	sw	zero,-64(a5) # 8001fd50 <log+0x28>
    write_head();    // Erase the transaction from the log
    80003d98:	ce7ff0ef          	jal	80003a7e <write_head>
    80003d9c:	69e2                	ld	s3,24(sp)
    80003d9e:	6a42                	ld	s4,16(sp)
    80003da0:	6aa2                	ld	s5,8(sp)
    80003da2:	b735                	j	80003cce <end_op+0x44>

0000000080003da4 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    80003da4:	1101                	addi	sp,sp,-32
    80003da6:	ec06                	sd	ra,24(sp)
    80003da8:	e822                	sd	s0,16(sp)
    80003daa:	e426                	sd	s1,8(sp)
    80003dac:	e04a                	sd	s2,0(sp)
    80003dae:	1000                	addi	s0,sp,32
    80003db0:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    80003db2:	0001c917          	auipc	s2,0x1c
    80003db6:	f7690913          	addi	s2,s2,-138 # 8001fd28 <log>
    80003dba:	854a                	mv	a0,s2
    80003dbc:	e13fc0ef          	jal	80000bce <acquire>
  if (log.lh.n >= LOGBLOCKS)
    80003dc0:	02892603          	lw	a2,40(s2)
    80003dc4:	47f5                	li	a5,29
    80003dc6:	04c7cc63          	blt	a5,a2,80003e1e <log_write+0x7a>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80003dca:	0001c797          	auipc	a5,0x1c
    80003dce:	f7a7a783          	lw	a5,-134(a5) # 8001fd44 <log+0x1c>
    80003dd2:	04f05c63          	blez	a5,80003e2a <log_write+0x86>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80003dd6:	4781                	li	a5,0
    80003dd8:	04c05f63          	blez	a2,80003e36 <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003ddc:	44cc                	lw	a1,12(s1)
    80003dde:	0001c717          	auipc	a4,0x1c
    80003de2:	f7670713          	addi	a4,a4,-138 # 8001fd54 <log+0x2c>
  for (i = 0; i < log.lh.n; i++) {
    80003de6:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003de8:	4314                	lw	a3,0(a4)
    80003dea:	04b68663          	beq	a3,a1,80003e36 <log_write+0x92>
  for (i = 0; i < log.lh.n; i++) {
    80003dee:	2785                	addiw	a5,a5,1
    80003df0:	0711                	addi	a4,a4,4
    80003df2:	fef61be3          	bne	a2,a5,80003de8 <log_write+0x44>
      break;
  }
  log.lh.block[i] = b->blockno;
    80003df6:	0621                	addi	a2,a2,8
    80003df8:	060a                	slli	a2,a2,0x2
    80003dfa:	0001c797          	auipc	a5,0x1c
    80003dfe:	f2e78793          	addi	a5,a5,-210 # 8001fd28 <log>
    80003e02:	97b2                	add	a5,a5,a2
    80003e04:	44d8                	lw	a4,12(s1)
    80003e06:	c7d8                	sw	a4,12(a5)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80003e08:	8526                	mv	a0,s1
    80003e0a:	ef1fe0ef          	jal	80002cfa <bpin>
    log.lh.n++;
    80003e0e:	0001c717          	auipc	a4,0x1c
    80003e12:	f1a70713          	addi	a4,a4,-230 # 8001fd28 <log>
    80003e16:	571c                	lw	a5,40(a4)
    80003e18:	2785                	addiw	a5,a5,1
    80003e1a:	d71c                	sw	a5,40(a4)
    80003e1c:	a80d                	j	80003e4e <log_write+0xaa>
    panic("too big a transaction");
    80003e1e:	00003517          	auipc	a0,0x3
    80003e22:	6f250513          	addi	a0,a0,1778 # 80007510 <etext+0x510>
    80003e26:	9bbfc0ef          	jal	800007e0 <panic>
    panic("log_write outside of trans");
    80003e2a:	00003517          	auipc	a0,0x3
    80003e2e:	6fe50513          	addi	a0,a0,1790 # 80007528 <etext+0x528>
    80003e32:	9affc0ef          	jal	800007e0 <panic>
  log.lh.block[i] = b->blockno;
    80003e36:	00878693          	addi	a3,a5,8
    80003e3a:	068a                	slli	a3,a3,0x2
    80003e3c:	0001c717          	auipc	a4,0x1c
    80003e40:	eec70713          	addi	a4,a4,-276 # 8001fd28 <log>
    80003e44:	9736                	add	a4,a4,a3
    80003e46:	44d4                	lw	a3,12(s1)
    80003e48:	c754                	sw	a3,12(a4)
  if (i == log.lh.n) {  // Add new block to log?
    80003e4a:	faf60fe3          	beq	a2,a5,80003e08 <log_write+0x64>
  }
  release(&log.lock);
    80003e4e:	0001c517          	auipc	a0,0x1c
    80003e52:	eda50513          	addi	a0,a0,-294 # 8001fd28 <log>
    80003e56:	e11fc0ef          	jal	80000c66 <release>
}
    80003e5a:	60e2                	ld	ra,24(sp)
    80003e5c:	6442                	ld	s0,16(sp)
    80003e5e:	64a2                	ld	s1,8(sp)
    80003e60:	6902                	ld	s2,0(sp)
    80003e62:	6105                	addi	sp,sp,32
    80003e64:	8082                	ret

0000000080003e66 <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    80003e66:	1101                	addi	sp,sp,-32
    80003e68:	ec06                	sd	ra,24(sp)
    80003e6a:	e822                	sd	s0,16(sp)
    80003e6c:	e426                	sd	s1,8(sp)
    80003e6e:	e04a                	sd	s2,0(sp)
    80003e70:	1000                	addi	s0,sp,32
    80003e72:	84aa                	mv	s1,a0
    80003e74:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    80003e76:	00003597          	auipc	a1,0x3
    80003e7a:	6d258593          	addi	a1,a1,1746 # 80007548 <etext+0x548>
    80003e7e:	0521                	addi	a0,a0,8
    80003e80:	ccffc0ef          	jal	80000b4e <initlock>
  lk->name = name;
    80003e84:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    80003e88:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003e8c:	0204a423          	sw	zero,40(s1)
}
    80003e90:	60e2                	ld	ra,24(sp)
    80003e92:	6442                	ld	s0,16(sp)
    80003e94:	64a2                	ld	s1,8(sp)
    80003e96:	6902                	ld	s2,0(sp)
    80003e98:	6105                	addi	sp,sp,32
    80003e9a:	8082                	ret

0000000080003e9c <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    80003e9c:	1101                	addi	sp,sp,-32
    80003e9e:	ec06                	sd	ra,24(sp)
    80003ea0:	e822                	sd	s0,16(sp)
    80003ea2:	e426                	sd	s1,8(sp)
    80003ea4:	e04a                	sd	s2,0(sp)
    80003ea6:	1000                	addi	s0,sp,32
    80003ea8:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003eaa:	00850913          	addi	s2,a0,8
    80003eae:	854a                	mv	a0,s2
    80003eb0:	d1ffc0ef          	jal	80000bce <acquire>
  while (lk->locked) {
    80003eb4:	409c                	lw	a5,0(s1)
    80003eb6:	c799                	beqz	a5,80003ec4 <acquiresleep+0x28>
    sleep(lk, &lk->lk);
    80003eb8:	85ca                	mv	a1,s2
    80003eba:	8526                	mv	a0,s1
    80003ebc:	828fe0ef          	jal	80001ee4 <sleep>
  while (lk->locked) {
    80003ec0:	409c                	lw	a5,0(s1)
    80003ec2:	fbfd                	bnez	a5,80003eb8 <acquiresleep+0x1c>
  }
  lk->locked = 1;
    80003ec4:	4785                	li	a5,1
    80003ec6:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    80003ec8:	a07fd0ef          	jal	800018ce <myproc>
    80003ecc:	591c                	lw	a5,48(a0)
    80003ece:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    80003ed0:	854a                	mv	a0,s2
    80003ed2:	d95fc0ef          	jal	80000c66 <release>
}
    80003ed6:	60e2                	ld	ra,24(sp)
    80003ed8:	6442                	ld	s0,16(sp)
    80003eda:	64a2                	ld	s1,8(sp)
    80003edc:	6902                	ld	s2,0(sp)
    80003ede:	6105                	addi	sp,sp,32
    80003ee0:	8082                	ret

0000000080003ee2 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    80003ee2:	1101                	addi	sp,sp,-32
    80003ee4:	ec06                	sd	ra,24(sp)
    80003ee6:	e822                	sd	s0,16(sp)
    80003ee8:	e426                	sd	s1,8(sp)
    80003eea:	e04a                	sd	s2,0(sp)
    80003eec:	1000                	addi	s0,sp,32
    80003eee:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003ef0:	00850913          	addi	s2,a0,8
    80003ef4:	854a                	mv	a0,s2
    80003ef6:	cd9fc0ef          	jal	80000bce <acquire>
  lk->locked = 0;
    80003efa:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003efe:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    80003f02:	8526                	mv	a0,s1
    80003f04:	82cfe0ef          	jal	80001f30 <wakeup>
  release(&lk->lk);
    80003f08:	854a                	mv	a0,s2
    80003f0a:	d5dfc0ef          	jal	80000c66 <release>
}
    80003f0e:	60e2                	ld	ra,24(sp)
    80003f10:	6442                	ld	s0,16(sp)
    80003f12:	64a2                	ld	s1,8(sp)
    80003f14:	6902                	ld	s2,0(sp)
    80003f16:	6105                	addi	sp,sp,32
    80003f18:	8082                	ret

0000000080003f1a <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    80003f1a:	7179                	addi	sp,sp,-48
    80003f1c:	f406                	sd	ra,40(sp)
    80003f1e:	f022                	sd	s0,32(sp)
    80003f20:	ec26                	sd	s1,24(sp)
    80003f22:	e84a                	sd	s2,16(sp)
    80003f24:	1800                	addi	s0,sp,48
    80003f26:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    80003f28:	00850913          	addi	s2,a0,8
    80003f2c:	854a                	mv	a0,s2
    80003f2e:	ca1fc0ef          	jal	80000bce <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    80003f32:	409c                	lw	a5,0(s1)
    80003f34:	ef81                	bnez	a5,80003f4c <holdingsleep+0x32>
    80003f36:	4481                	li	s1,0
  release(&lk->lk);
    80003f38:	854a                	mv	a0,s2
    80003f3a:	d2dfc0ef          	jal	80000c66 <release>
  return r;
}
    80003f3e:	8526                	mv	a0,s1
    80003f40:	70a2                	ld	ra,40(sp)
    80003f42:	7402                	ld	s0,32(sp)
    80003f44:	64e2                	ld	s1,24(sp)
    80003f46:	6942                	ld	s2,16(sp)
    80003f48:	6145                	addi	sp,sp,48
    80003f4a:	8082                	ret
    80003f4c:	e44e                	sd	s3,8(sp)
  r = lk->locked && (lk->pid == myproc()->pid);
    80003f4e:	0284a983          	lw	s3,40(s1)
    80003f52:	97dfd0ef          	jal	800018ce <myproc>
    80003f56:	5904                	lw	s1,48(a0)
    80003f58:	413484b3          	sub	s1,s1,s3
    80003f5c:	0014b493          	seqz	s1,s1
    80003f60:	69a2                	ld	s3,8(sp)
    80003f62:	bfd9                	j	80003f38 <holdingsleep+0x1e>

0000000080003f64 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    80003f64:	1141                	addi	sp,sp,-16
    80003f66:	e406                	sd	ra,8(sp)
    80003f68:	e022                	sd	s0,0(sp)
    80003f6a:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    80003f6c:	00003597          	auipc	a1,0x3
    80003f70:	5ec58593          	addi	a1,a1,1516 # 80007558 <etext+0x558>
    80003f74:	0001c517          	auipc	a0,0x1c
    80003f78:	efc50513          	addi	a0,a0,-260 # 8001fe70 <ftable>
    80003f7c:	bd3fc0ef          	jal	80000b4e <initlock>
}
    80003f80:	60a2                	ld	ra,8(sp)
    80003f82:	6402                	ld	s0,0(sp)
    80003f84:	0141                	addi	sp,sp,16
    80003f86:	8082                	ret

0000000080003f88 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    80003f88:	1101                	addi	sp,sp,-32
    80003f8a:	ec06                	sd	ra,24(sp)
    80003f8c:	e822                	sd	s0,16(sp)
    80003f8e:	e426                	sd	s1,8(sp)
    80003f90:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    80003f92:	0001c517          	auipc	a0,0x1c
    80003f96:	ede50513          	addi	a0,a0,-290 # 8001fe70 <ftable>
    80003f9a:	c35fc0ef          	jal	80000bce <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003f9e:	0001c497          	auipc	s1,0x1c
    80003fa2:	eea48493          	addi	s1,s1,-278 # 8001fe88 <ftable+0x18>
    80003fa6:	0001d717          	auipc	a4,0x1d
    80003faa:	e8270713          	addi	a4,a4,-382 # 80020e28 <disk>
    if(f->ref == 0){
    80003fae:	40dc                	lw	a5,4(s1)
    80003fb0:	cf89                	beqz	a5,80003fca <filealloc+0x42>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003fb2:	02848493          	addi	s1,s1,40
    80003fb6:	fee49ce3          	bne	s1,a4,80003fae <filealloc+0x26>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    80003fba:	0001c517          	auipc	a0,0x1c
    80003fbe:	eb650513          	addi	a0,a0,-330 # 8001fe70 <ftable>
    80003fc2:	ca5fc0ef          	jal	80000c66 <release>
  return 0;
    80003fc6:	4481                	li	s1,0
    80003fc8:	a809                	j	80003fda <filealloc+0x52>
      f->ref = 1;
    80003fca:	4785                	li	a5,1
    80003fcc:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    80003fce:	0001c517          	auipc	a0,0x1c
    80003fd2:	ea250513          	addi	a0,a0,-350 # 8001fe70 <ftable>
    80003fd6:	c91fc0ef          	jal	80000c66 <release>
}
    80003fda:	8526                	mv	a0,s1
    80003fdc:	60e2                	ld	ra,24(sp)
    80003fde:	6442                	ld	s0,16(sp)
    80003fe0:	64a2                	ld	s1,8(sp)
    80003fe2:	6105                	addi	sp,sp,32
    80003fe4:	8082                	ret

0000000080003fe6 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    80003fe6:	1101                	addi	sp,sp,-32
    80003fe8:	ec06                	sd	ra,24(sp)
    80003fea:	e822                	sd	s0,16(sp)
    80003fec:	e426                	sd	s1,8(sp)
    80003fee:	1000                	addi	s0,sp,32
    80003ff0:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    80003ff2:	0001c517          	auipc	a0,0x1c
    80003ff6:	e7e50513          	addi	a0,a0,-386 # 8001fe70 <ftable>
    80003ffa:	bd5fc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    80003ffe:	40dc                	lw	a5,4(s1)
    80004000:	02f05063          	blez	a5,80004020 <filedup+0x3a>
    panic("filedup");
  f->ref++;
    80004004:	2785                	addiw	a5,a5,1
    80004006:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    80004008:	0001c517          	auipc	a0,0x1c
    8000400c:	e6850513          	addi	a0,a0,-408 # 8001fe70 <ftable>
    80004010:	c57fc0ef          	jal	80000c66 <release>
  return f;
}
    80004014:	8526                	mv	a0,s1
    80004016:	60e2                	ld	ra,24(sp)
    80004018:	6442                	ld	s0,16(sp)
    8000401a:	64a2                	ld	s1,8(sp)
    8000401c:	6105                	addi	sp,sp,32
    8000401e:	8082                	ret
    panic("filedup");
    80004020:	00003517          	auipc	a0,0x3
    80004024:	54050513          	addi	a0,a0,1344 # 80007560 <etext+0x560>
    80004028:	fb8fc0ef          	jal	800007e0 <panic>

000000008000402c <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    8000402c:	7139                	addi	sp,sp,-64
    8000402e:	fc06                	sd	ra,56(sp)
    80004030:	f822                	sd	s0,48(sp)
    80004032:	f426                	sd	s1,40(sp)
    80004034:	0080                	addi	s0,sp,64
    80004036:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    80004038:	0001c517          	auipc	a0,0x1c
    8000403c:	e3850513          	addi	a0,a0,-456 # 8001fe70 <ftable>
    80004040:	b8ffc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    80004044:	40dc                	lw	a5,4(s1)
    80004046:	04f05a63          	blez	a5,8000409a <fileclose+0x6e>
    panic("fileclose");
  if(--f->ref > 0){
    8000404a:	37fd                	addiw	a5,a5,-1
    8000404c:	0007871b          	sext.w	a4,a5
    80004050:	c0dc                	sw	a5,4(s1)
    80004052:	04e04e63          	bgtz	a4,800040ae <fileclose+0x82>
    80004056:	f04a                	sd	s2,32(sp)
    80004058:	ec4e                	sd	s3,24(sp)
    8000405a:	e852                	sd	s4,16(sp)
    8000405c:	e456                	sd	s5,8(sp)
    release(&ftable.lock);
    return;
  }
  ff = *f;
    8000405e:	0004a903          	lw	s2,0(s1)
    80004062:	0094ca83          	lbu	s5,9(s1)
    80004066:	0104ba03          	ld	s4,16(s1)
    8000406a:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    8000406e:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    80004072:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    80004076:	0001c517          	auipc	a0,0x1c
    8000407a:	dfa50513          	addi	a0,a0,-518 # 8001fe70 <ftable>
    8000407e:	be9fc0ef          	jal	80000c66 <release>

  if(ff.type == FD_PIPE){
    80004082:	4785                	li	a5,1
    80004084:	04f90063          	beq	s2,a5,800040c4 <fileclose+0x98>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    80004088:	3979                	addiw	s2,s2,-2
    8000408a:	4785                	li	a5,1
    8000408c:	0527f563          	bgeu	a5,s2,800040d6 <fileclose+0xaa>
    80004090:	7902                	ld	s2,32(sp)
    80004092:	69e2                	ld	s3,24(sp)
    80004094:	6a42                	ld	s4,16(sp)
    80004096:	6aa2                	ld	s5,8(sp)
    80004098:	a00d                	j	800040ba <fileclose+0x8e>
    8000409a:	f04a                	sd	s2,32(sp)
    8000409c:	ec4e                	sd	s3,24(sp)
    8000409e:	e852                	sd	s4,16(sp)
    800040a0:	e456                	sd	s5,8(sp)
    panic("fileclose");
    800040a2:	00003517          	auipc	a0,0x3
    800040a6:	4c650513          	addi	a0,a0,1222 # 80007568 <etext+0x568>
    800040aa:	f36fc0ef          	jal	800007e0 <panic>
    release(&ftable.lock);
    800040ae:	0001c517          	auipc	a0,0x1c
    800040b2:	dc250513          	addi	a0,a0,-574 # 8001fe70 <ftable>
    800040b6:	bb1fc0ef          	jal	80000c66 <release>
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
    800040ba:	70e2                	ld	ra,56(sp)
    800040bc:	7442                	ld	s0,48(sp)
    800040be:	74a2                	ld	s1,40(sp)
    800040c0:	6121                	addi	sp,sp,64
    800040c2:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    800040c4:	85d6                	mv	a1,s5
    800040c6:	8552                	mv	a0,s4
    800040c8:	336000ef          	jal	800043fe <pipeclose>
    800040cc:	7902                	ld	s2,32(sp)
    800040ce:	69e2                	ld	s3,24(sp)
    800040d0:	6a42                	ld	s4,16(sp)
    800040d2:	6aa2                	ld	s5,8(sp)
    800040d4:	b7dd                	j	800040ba <fileclose+0x8e>
    begin_op();
    800040d6:	b4bff0ef          	jal	80003c20 <begin_op>
    iput(ff.ip);
    800040da:	854e                	mv	a0,s3
    800040dc:	adcff0ef          	jal	800033b8 <iput>
    end_op();
    800040e0:	babff0ef          	jal	80003c8a <end_op>
    800040e4:	7902                	ld	s2,32(sp)
    800040e6:	69e2                	ld	s3,24(sp)
    800040e8:	6a42                	ld	s4,16(sp)
    800040ea:	6aa2                	ld	s5,8(sp)
    800040ec:	b7f9                	j	800040ba <fileclose+0x8e>

00000000800040ee <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    800040ee:	715d                	addi	sp,sp,-80
    800040f0:	e486                	sd	ra,72(sp)
    800040f2:	e0a2                	sd	s0,64(sp)
    800040f4:	fc26                	sd	s1,56(sp)
    800040f6:	f44e                	sd	s3,40(sp)
    800040f8:	0880                	addi	s0,sp,80
    800040fa:	84aa                	mv	s1,a0
    800040fc:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    800040fe:	fd0fd0ef          	jal	800018ce <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    80004102:	409c                	lw	a5,0(s1)
    80004104:	37f9                	addiw	a5,a5,-2
    80004106:	4705                	li	a4,1
    80004108:	04f76063          	bltu	a4,a5,80004148 <filestat+0x5a>
    8000410c:	f84a                	sd	s2,48(sp)
    8000410e:	892a                	mv	s2,a0
    ilock(f->ip);
    80004110:	6c88                	ld	a0,24(s1)
    80004112:	924ff0ef          	jal	80003236 <ilock>
    stati(f->ip, &st);
    80004116:	fb840593          	addi	a1,s0,-72
    8000411a:	6c88                	ld	a0,24(s1)
    8000411c:	c80ff0ef          	jal	8000359c <stati>
    iunlock(f->ip);
    80004120:	6c88                	ld	a0,24(s1)
    80004122:	9c2ff0ef          	jal	800032e4 <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    80004126:	46e1                	li	a3,24
    80004128:	fb840613          	addi	a2,s0,-72
    8000412c:	85ce                	mv	a1,s3
    8000412e:	05093503          	ld	a0,80(s2)
    80004132:	cb0fd0ef          	jal	800015e2 <copyout>
    80004136:	41f5551b          	sraiw	a0,a0,0x1f
    8000413a:	7942                	ld	s2,48(sp)
      return -1;
    return 0;
  }
  return -1;
}
    8000413c:	60a6                	ld	ra,72(sp)
    8000413e:	6406                	ld	s0,64(sp)
    80004140:	74e2                	ld	s1,56(sp)
    80004142:	79a2                	ld	s3,40(sp)
    80004144:	6161                	addi	sp,sp,80
    80004146:	8082                	ret
  return -1;
    80004148:	557d                	li	a0,-1
    8000414a:	bfcd                	j	8000413c <filestat+0x4e>

000000008000414c <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    8000414c:	7179                	addi	sp,sp,-48
    8000414e:	f406                	sd	ra,40(sp)
    80004150:	f022                	sd	s0,32(sp)
    80004152:	e84a                	sd	s2,16(sp)
    80004154:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    80004156:	00854783          	lbu	a5,8(a0)
    8000415a:	cfd1                	beqz	a5,800041f6 <fileread+0xaa>
    8000415c:	ec26                	sd	s1,24(sp)
    8000415e:	e44e                	sd	s3,8(sp)
    80004160:	84aa                	mv	s1,a0
    80004162:	89ae                	mv	s3,a1
    80004164:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    80004166:	411c                	lw	a5,0(a0)
    80004168:	4705                	li	a4,1
    8000416a:	04e78363          	beq	a5,a4,800041b0 <fileread+0x64>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    8000416e:	470d                	li	a4,3
    80004170:	04e78763          	beq	a5,a4,800041be <fileread+0x72>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    80004174:	4709                	li	a4,2
    80004176:	06e79a63          	bne	a5,a4,800041ea <fileread+0x9e>
    ilock(f->ip);
    8000417a:	6d08                	ld	a0,24(a0)
    8000417c:	8baff0ef          	jal	80003236 <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    80004180:	874a                	mv	a4,s2
    80004182:	5094                	lw	a3,32(s1)
    80004184:	864e                	mv	a2,s3
    80004186:	4585                	li	a1,1
    80004188:	6c88                	ld	a0,24(s1)
    8000418a:	c3cff0ef          	jal	800035c6 <readi>
    8000418e:	892a                	mv	s2,a0
    80004190:	00a05563          	blez	a0,8000419a <fileread+0x4e>
      f->off += r;
    80004194:	509c                	lw	a5,32(s1)
    80004196:	9fa9                	addw	a5,a5,a0
    80004198:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    8000419a:	6c88                	ld	a0,24(s1)
    8000419c:	948ff0ef          	jal	800032e4 <iunlock>
    800041a0:	64e2                	ld	s1,24(sp)
    800041a2:	69a2                	ld	s3,8(sp)
  } else {
    panic("fileread");
  }

  return r;
}
    800041a4:	854a                	mv	a0,s2
    800041a6:	70a2                	ld	ra,40(sp)
    800041a8:	7402                	ld	s0,32(sp)
    800041aa:	6942                	ld	s2,16(sp)
    800041ac:	6145                	addi	sp,sp,48
    800041ae:	8082                	ret
    r = piperead(f->pipe, addr, n);
    800041b0:	6908                	ld	a0,16(a0)
    800041b2:	388000ef          	jal	8000453a <piperead>
    800041b6:	892a                	mv	s2,a0
    800041b8:	64e2                	ld	s1,24(sp)
    800041ba:	69a2                	ld	s3,8(sp)
    800041bc:	b7e5                	j	800041a4 <fileread+0x58>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    800041be:	02451783          	lh	a5,36(a0)
    800041c2:	03079693          	slli	a3,a5,0x30
    800041c6:	92c1                	srli	a3,a3,0x30
    800041c8:	4725                	li	a4,9
    800041ca:	02d76863          	bltu	a4,a3,800041fa <fileread+0xae>
    800041ce:	0792                	slli	a5,a5,0x4
    800041d0:	0001c717          	auipc	a4,0x1c
    800041d4:	c0070713          	addi	a4,a4,-1024 # 8001fdd0 <devsw>
    800041d8:	97ba                	add	a5,a5,a4
    800041da:	639c                	ld	a5,0(a5)
    800041dc:	c39d                	beqz	a5,80004202 <fileread+0xb6>
    r = devsw[f->major].read(1, addr, n);
    800041de:	4505                	li	a0,1
    800041e0:	9782                	jalr	a5
    800041e2:	892a                	mv	s2,a0
    800041e4:	64e2                	ld	s1,24(sp)
    800041e6:	69a2                	ld	s3,8(sp)
    800041e8:	bf75                	j	800041a4 <fileread+0x58>
    panic("fileread");
    800041ea:	00003517          	auipc	a0,0x3
    800041ee:	38e50513          	addi	a0,a0,910 # 80007578 <etext+0x578>
    800041f2:	deefc0ef          	jal	800007e0 <panic>
    return -1;
    800041f6:	597d                	li	s2,-1
    800041f8:	b775                	j	800041a4 <fileread+0x58>
      return -1;
    800041fa:	597d                	li	s2,-1
    800041fc:	64e2                	ld	s1,24(sp)
    800041fe:	69a2                	ld	s3,8(sp)
    80004200:	b755                	j	800041a4 <fileread+0x58>
    80004202:	597d                	li	s2,-1
    80004204:	64e2                	ld	s1,24(sp)
    80004206:	69a2                	ld	s3,8(sp)
    80004208:	bf71                	j	800041a4 <fileread+0x58>

000000008000420a <filewrite>:
int
filewrite(struct file *f, uint64 addr, int n)
{
  int r, ret = 0;

  if(f->writable == 0)
    8000420a:	00954783          	lbu	a5,9(a0)
    8000420e:	10078b63          	beqz	a5,80004324 <filewrite+0x11a>
{
    80004212:	715d                	addi	sp,sp,-80
    80004214:	e486                	sd	ra,72(sp)
    80004216:	e0a2                	sd	s0,64(sp)
    80004218:	f84a                	sd	s2,48(sp)
    8000421a:	f052                	sd	s4,32(sp)
    8000421c:	e85a                	sd	s6,16(sp)
    8000421e:	0880                	addi	s0,sp,80
    80004220:	892a                	mv	s2,a0
    80004222:	8b2e                	mv	s6,a1
    80004224:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    80004226:	411c                	lw	a5,0(a0)
    80004228:	4705                	li	a4,1
    8000422a:	02e78763          	beq	a5,a4,80004258 <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    8000422e:	470d                	li	a4,3
    80004230:	02e78863          	beq	a5,a4,80004260 <filewrite+0x56>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    80004234:	4709                	li	a4,2
    80004236:	0ce79c63          	bne	a5,a4,8000430e <filewrite+0x104>
    8000423a:	f44e                	sd	s3,40(sp)
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    8000423c:	0ac05863          	blez	a2,800042ec <filewrite+0xe2>
    80004240:	fc26                	sd	s1,56(sp)
    80004242:	ec56                	sd	s5,24(sp)
    80004244:	e45e                	sd	s7,8(sp)
    80004246:	e062                	sd	s8,0(sp)
    int i = 0;
    80004248:	4981                	li	s3,0
      int n1 = n - i;
      if(n1 > max)
    8000424a:	6b85                	lui	s7,0x1
    8000424c:	c00b8b93          	addi	s7,s7,-1024 # c00 <_entry-0x7ffff400>
    80004250:	6c05                	lui	s8,0x1
    80004252:	c00c0c1b          	addiw	s8,s8,-1024 # c00 <_entry-0x7ffff400>
    80004256:	a8b5                	j	800042d2 <filewrite+0xc8>
    ret = pipewrite(f->pipe, addr, n);
    80004258:	6908                	ld	a0,16(a0)
    8000425a:	1fc000ef          	jal	80004456 <pipewrite>
    8000425e:	a04d                	j	80004300 <filewrite+0xf6>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    80004260:	02451783          	lh	a5,36(a0)
    80004264:	03079693          	slli	a3,a5,0x30
    80004268:	92c1                	srli	a3,a3,0x30
    8000426a:	4725                	li	a4,9
    8000426c:	0ad76e63          	bltu	a4,a3,80004328 <filewrite+0x11e>
    80004270:	0792                	slli	a5,a5,0x4
    80004272:	0001c717          	auipc	a4,0x1c
    80004276:	b5e70713          	addi	a4,a4,-1186 # 8001fdd0 <devsw>
    8000427a:	97ba                	add	a5,a5,a4
    8000427c:	679c                	ld	a5,8(a5)
    8000427e:	c7dd                	beqz	a5,8000432c <filewrite+0x122>
    ret = devsw[f->major].write(1, addr, n);
    80004280:	4505                	li	a0,1
    80004282:	9782                	jalr	a5
    80004284:	a8b5                	j	80004300 <filewrite+0xf6>
      if(n1 > max)
    80004286:	00048a9b          	sext.w	s5,s1
        n1 = max;

      begin_op();
    8000428a:	997ff0ef          	jal	80003c20 <begin_op>
      ilock(f->ip);
    8000428e:	01893503          	ld	a0,24(s2)
    80004292:	fa5fe0ef          	jal	80003236 <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    80004296:	8756                	mv	a4,s5
    80004298:	02092683          	lw	a3,32(s2)
    8000429c:	01698633          	add	a2,s3,s6
    800042a0:	4585                	li	a1,1
    800042a2:	01893503          	ld	a0,24(s2)
    800042a6:	c1cff0ef          	jal	800036c2 <writei>
    800042aa:	84aa                	mv	s1,a0
    800042ac:	00a05763          	blez	a0,800042ba <filewrite+0xb0>
        f->off += r;
    800042b0:	02092783          	lw	a5,32(s2)
    800042b4:	9fa9                	addw	a5,a5,a0
    800042b6:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    800042ba:	01893503          	ld	a0,24(s2)
    800042be:	826ff0ef          	jal	800032e4 <iunlock>
      end_op();
    800042c2:	9c9ff0ef          	jal	80003c8a <end_op>

      if(r != n1){
    800042c6:	029a9563          	bne	s5,s1,800042f0 <filewrite+0xe6>
        // error from writei
        break;
      }
      i += r;
    800042ca:	013489bb          	addw	s3,s1,s3
    while(i < n){
    800042ce:	0149da63          	bge	s3,s4,800042e2 <filewrite+0xd8>
      int n1 = n - i;
    800042d2:	413a04bb          	subw	s1,s4,s3
      if(n1 > max)
    800042d6:	0004879b          	sext.w	a5,s1
    800042da:	fafbd6e3          	bge	s7,a5,80004286 <filewrite+0x7c>
    800042de:	84e2                	mv	s1,s8
    800042e0:	b75d                	j	80004286 <filewrite+0x7c>
    800042e2:	74e2                	ld	s1,56(sp)
    800042e4:	6ae2                	ld	s5,24(sp)
    800042e6:	6ba2                	ld	s7,8(sp)
    800042e8:	6c02                	ld	s8,0(sp)
    800042ea:	a039                	j	800042f8 <filewrite+0xee>
    int i = 0;
    800042ec:	4981                	li	s3,0
    800042ee:	a029                	j	800042f8 <filewrite+0xee>
    800042f0:	74e2                	ld	s1,56(sp)
    800042f2:	6ae2                	ld	s5,24(sp)
    800042f4:	6ba2                	ld	s7,8(sp)
    800042f6:	6c02                	ld	s8,0(sp)
    }
    ret = (i == n ? n : -1);
    800042f8:	033a1c63          	bne	s4,s3,80004330 <filewrite+0x126>
    800042fc:	8552                	mv	a0,s4
    800042fe:	79a2                	ld	s3,40(sp)
  } else {
    panic("filewrite");
  }

  return ret;
}
    80004300:	60a6                	ld	ra,72(sp)
    80004302:	6406                	ld	s0,64(sp)
    80004304:	7942                	ld	s2,48(sp)
    80004306:	7a02                	ld	s4,32(sp)
    80004308:	6b42                	ld	s6,16(sp)
    8000430a:	6161                	addi	sp,sp,80
    8000430c:	8082                	ret
    8000430e:	fc26                	sd	s1,56(sp)
    80004310:	f44e                	sd	s3,40(sp)
    80004312:	ec56                	sd	s5,24(sp)
    80004314:	e45e                	sd	s7,8(sp)
    80004316:	e062                	sd	s8,0(sp)
    panic("filewrite");
    80004318:	00003517          	auipc	a0,0x3
    8000431c:	27050513          	addi	a0,a0,624 # 80007588 <etext+0x588>
    80004320:	cc0fc0ef          	jal	800007e0 <panic>
    return -1;
    80004324:	557d                	li	a0,-1
}
    80004326:	8082                	ret
      return -1;
    80004328:	557d                	li	a0,-1
    8000432a:	bfd9                	j	80004300 <filewrite+0xf6>
    8000432c:	557d                	li	a0,-1
    8000432e:	bfc9                	j	80004300 <filewrite+0xf6>
    ret = (i == n ? n : -1);
    80004330:	557d                	li	a0,-1
    80004332:	79a2                	ld	s3,40(sp)
    80004334:	b7f1                	j	80004300 <filewrite+0xf6>

0000000080004336 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    80004336:	7179                	addi	sp,sp,-48
    80004338:	f406                	sd	ra,40(sp)
    8000433a:	f022                	sd	s0,32(sp)
    8000433c:	ec26                	sd	s1,24(sp)
    8000433e:	e052                	sd	s4,0(sp)
    80004340:	1800                	addi	s0,sp,48
    80004342:	84aa                	mv	s1,a0
    80004344:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    80004346:	0005b023          	sd	zero,0(a1)
    8000434a:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    8000434e:	c3bff0ef          	jal	80003f88 <filealloc>
    80004352:	e088                	sd	a0,0(s1)
    80004354:	c549                	beqz	a0,800043de <pipealloc+0xa8>
    80004356:	c33ff0ef          	jal	80003f88 <filealloc>
    8000435a:	00aa3023          	sd	a0,0(s4)
    8000435e:	cd25                	beqz	a0,800043d6 <pipealloc+0xa0>
    80004360:	e84a                	sd	s2,16(sp)
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    80004362:	f9cfc0ef          	jal	80000afe <kalloc>
    80004366:	892a                	mv	s2,a0
    80004368:	c12d                	beqz	a0,800043ca <pipealloc+0x94>
    8000436a:	e44e                	sd	s3,8(sp)
    goto bad;
  pi->readopen = 1;
    8000436c:	4985                	li	s3,1
    8000436e:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    80004372:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    80004376:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    8000437a:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    8000437e:	00003597          	auipc	a1,0x3
    80004382:	21a58593          	addi	a1,a1,538 # 80007598 <etext+0x598>
    80004386:	fc8fc0ef          	jal	80000b4e <initlock>
  (*f0)->type = FD_PIPE;
    8000438a:	609c                	ld	a5,0(s1)
    8000438c:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    80004390:	609c                	ld	a5,0(s1)
    80004392:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    80004396:	609c                	ld	a5,0(s1)
    80004398:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    8000439c:	609c                	ld	a5,0(s1)
    8000439e:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    800043a2:	000a3783          	ld	a5,0(s4)
    800043a6:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    800043aa:	000a3783          	ld	a5,0(s4)
    800043ae:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    800043b2:	000a3783          	ld	a5,0(s4)
    800043b6:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    800043ba:	000a3783          	ld	a5,0(s4)
    800043be:	0127b823          	sd	s2,16(a5)
  return 0;
    800043c2:	4501                	li	a0,0
    800043c4:	6942                	ld	s2,16(sp)
    800043c6:	69a2                	ld	s3,8(sp)
    800043c8:	a01d                	j	800043ee <pipealloc+0xb8>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    800043ca:	6088                	ld	a0,0(s1)
    800043cc:	c119                	beqz	a0,800043d2 <pipealloc+0x9c>
    800043ce:	6942                	ld	s2,16(sp)
    800043d0:	a029                	j	800043da <pipealloc+0xa4>
    800043d2:	6942                	ld	s2,16(sp)
    800043d4:	a029                	j	800043de <pipealloc+0xa8>
    800043d6:	6088                	ld	a0,0(s1)
    800043d8:	c10d                	beqz	a0,800043fa <pipealloc+0xc4>
    fileclose(*f0);
    800043da:	c53ff0ef          	jal	8000402c <fileclose>
  if(*f1)
    800043de:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    800043e2:	557d                	li	a0,-1
  if(*f1)
    800043e4:	c789                	beqz	a5,800043ee <pipealloc+0xb8>
    fileclose(*f1);
    800043e6:	853e                	mv	a0,a5
    800043e8:	c45ff0ef          	jal	8000402c <fileclose>
  return -1;
    800043ec:	557d                	li	a0,-1
}
    800043ee:	70a2                	ld	ra,40(sp)
    800043f0:	7402                	ld	s0,32(sp)
    800043f2:	64e2                	ld	s1,24(sp)
    800043f4:	6a02                	ld	s4,0(sp)
    800043f6:	6145                	addi	sp,sp,48
    800043f8:	8082                	ret
  return -1;
    800043fa:	557d                	li	a0,-1
    800043fc:	bfcd                	j	800043ee <pipealloc+0xb8>

00000000800043fe <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    800043fe:	1101                	addi	sp,sp,-32
    80004400:	ec06                	sd	ra,24(sp)
    80004402:	e822                	sd	s0,16(sp)
    80004404:	e426                	sd	s1,8(sp)
    80004406:	e04a                	sd	s2,0(sp)
    80004408:	1000                	addi	s0,sp,32
    8000440a:	84aa                	mv	s1,a0
    8000440c:	892e                	mv	s2,a1
  acquire(&pi->lock);
    8000440e:	fc0fc0ef          	jal	80000bce <acquire>
  if(writable){
    80004412:	02090763          	beqz	s2,80004440 <pipeclose+0x42>
    pi->writeopen = 0;
    80004416:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    8000441a:	21848513          	addi	a0,s1,536
    8000441e:	b13fd0ef          	jal	80001f30 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    80004422:	2204b783          	ld	a5,544(s1)
    80004426:	e785                	bnez	a5,8000444e <pipeclose+0x50>
    release(&pi->lock);
    80004428:	8526                	mv	a0,s1
    8000442a:	83dfc0ef          	jal	80000c66 <release>
    kfree((char*)pi);
    8000442e:	8526                	mv	a0,s1
    80004430:	decfc0ef          	jal	80000a1c <kfree>
  } else
    release(&pi->lock);
}
    80004434:	60e2                	ld	ra,24(sp)
    80004436:	6442                	ld	s0,16(sp)
    80004438:	64a2                	ld	s1,8(sp)
    8000443a:	6902                	ld	s2,0(sp)
    8000443c:	6105                	addi	sp,sp,32
    8000443e:	8082                	ret
    pi->readopen = 0;
    80004440:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    80004444:	21c48513          	addi	a0,s1,540
    80004448:	ae9fd0ef          	jal	80001f30 <wakeup>
    8000444c:	bfd9                	j	80004422 <pipeclose+0x24>
    release(&pi->lock);
    8000444e:	8526                	mv	a0,s1
    80004450:	817fc0ef          	jal	80000c66 <release>
}
    80004454:	b7c5                	j	80004434 <pipeclose+0x36>

0000000080004456 <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    80004456:	711d                	addi	sp,sp,-96
    80004458:	ec86                	sd	ra,88(sp)
    8000445a:	e8a2                	sd	s0,80(sp)
    8000445c:	e4a6                	sd	s1,72(sp)
    8000445e:	e0ca                	sd	s2,64(sp)
    80004460:	fc4e                	sd	s3,56(sp)
    80004462:	f852                	sd	s4,48(sp)
    80004464:	f456                	sd	s5,40(sp)
    80004466:	1080                	addi	s0,sp,96
    80004468:	84aa                	mv	s1,a0
    8000446a:	8aae                	mv	s5,a1
    8000446c:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    8000446e:	c60fd0ef          	jal	800018ce <myproc>
    80004472:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    80004474:	8526                	mv	a0,s1
    80004476:	f58fc0ef          	jal	80000bce <acquire>
  while(i < n){
    8000447a:	0b405a63          	blez	s4,8000452e <pipewrite+0xd8>
    8000447e:	f05a                	sd	s6,32(sp)
    80004480:	ec5e                	sd	s7,24(sp)
    80004482:	e862                	sd	s8,16(sp)
  int i = 0;
    80004484:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    80004486:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    80004488:	21848c13          	addi	s8,s1,536
      sleep(&pi->nwrite, &pi->lock);
    8000448c:	21c48b93          	addi	s7,s1,540
    80004490:	a81d                	j	800044c6 <pipewrite+0x70>
      release(&pi->lock);
    80004492:	8526                	mv	a0,s1
    80004494:	fd2fc0ef          	jal	80000c66 <release>
      return -1;
    80004498:	597d                	li	s2,-1
    8000449a:	7b02                	ld	s6,32(sp)
    8000449c:	6be2                	ld	s7,24(sp)
    8000449e:	6c42                	ld	s8,16(sp)
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    800044a0:	854a                	mv	a0,s2
    800044a2:	60e6                	ld	ra,88(sp)
    800044a4:	6446                	ld	s0,80(sp)
    800044a6:	64a6                	ld	s1,72(sp)
    800044a8:	6906                	ld	s2,64(sp)
    800044aa:	79e2                	ld	s3,56(sp)
    800044ac:	7a42                	ld	s4,48(sp)
    800044ae:	7aa2                	ld	s5,40(sp)
    800044b0:	6125                	addi	sp,sp,96
    800044b2:	8082                	ret
      wakeup(&pi->nread);
    800044b4:	8562                	mv	a0,s8
    800044b6:	a7bfd0ef          	jal	80001f30 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    800044ba:	85a6                	mv	a1,s1
    800044bc:	855e                	mv	a0,s7
    800044be:	a27fd0ef          	jal	80001ee4 <sleep>
  while(i < n){
    800044c2:	05495b63          	bge	s2,s4,80004518 <pipewrite+0xc2>
    if(pi->readopen == 0 || killed(pr)){
    800044c6:	2204a783          	lw	a5,544(s1)
    800044ca:	d7e1                	beqz	a5,80004492 <pipewrite+0x3c>
    800044cc:	854e                	mv	a0,s3
    800044ce:	c4ffd0ef          	jal	8000211c <killed>
    800044d2:	f161                	bnez	a0,80004492 <pipewrite+0x3c>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    800044d4:	2184a783          	lw	a5,536(s1)
    800044d8:	21c4a703          	lw	a4,540(s1)
    800044dc:	2007879b          	addiw	a5,a5,512
    800044e0:	fcf70ae3          	beq	a4,a5,800044b4 <pipewrite+0x5e>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    800044e4:	4685                	li	a3,1
    800044e6:	01590633          	add	a2,s2,s5
    800044ea:	faf40593          	addi	a1,s0,-81
    800044ee:	0509b503          	ld	a0,80(s3)
    800044f2:	9d4fd0ef          	jal	800016c6 <copyin>
    800044f6:	03650e63          	beq	a0,s6,80004532 <pipewrite+0xdc>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    800044fa:	21c4a783          	lw	a5,540(s1)
    800044fe:	0017871b          	addiw	a4,a5,1
    80004502:	20e4ae23          	sw	a4,540(s1)
    80004506:	1ff7f793          	andi	a5,a5,511
    8000450a:	97a6                	add	a5,a5,s1
    8000450c:	faf44703          	lbu	a4,-81(s0)
    80004510:	00e78c23          	sb	a4,24(a5)
      i++;
    80004514:	2905                	addiw	s2,s2,1
    80004516:	b775                	j	800044c2 <pipewrite+0x6c>
    80004518:	7b02                	ld	s6,32(sp)
    8000451a:	6be2                	ld	s7,24(sp)
    8000451c:	6c42                	ld	s8,16(sp)
  wakeup(&pi->nread);
    8000451e:	21848513          	addi	a0,s1,536
    80004522:	a0ffd0ef          	jal	80001f30 <wakeup>
  release(&pi->lock);
    80004526:	8526                	mv	a0,s1
    80004528:	f3efc0ef          	jal	80000c66 <release>
  return i;
    8000452c:	bf95                	j	800044a0 <pipewrite+0x4a>
  int i = 0;
    8000452e:	4901                	li	s2,0
    80004530:	b7fd                	j	8000451e <pipewrite+0xc8>
    80004532:	7b02                	ld	s6,32(sp)
    80004534:	6be2                	ld	s7,24(sp)
    80004536:	6c42                	ld	s8,16(sp)
    80004538:	b7dd                	j	8000451e <pipewrite+0xc8>

000000008000453a <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    8000453a:	715d                	addi	sp,sp,-80
    8000453c:	e486                	sd	ra,72(sp)
    8000453e:	e0a2                	sd	s0,64(sp)
    80004540:	fc26                	sd	s1,56(sp)
    80004542:	f84a                	sd	s2,48(sp)
    80004544:	f44e                	sd	s3,40(sp)
    80004546:	f052                	sd	s4,32(sp)
    80004548:	ec56                	sd	s5,24(sp)
    8000454a:	0880                	addi	s0,sp,80
    8000454c:	84aa                	mv	s1,a0
    8000454e:	892e                	mv	s2,a1
    80004550:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    80004552:	b7cfd0ef          	jal	800018ce <myproc>
    80004556:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004558:	8526                	mv	a0,s1
    8000455a:	e74fc0ef          	jal	80000bce <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    8000455e:	2184a703          	lw	a4,536(s1)
    80004562:	21c4a783          	lw	a5,540(s1)
    if(killed(pr)){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004566:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    8000456a:	02f71563          	bne	a4,a5,80004594 <piperead+0x5a>
    8000456e:	2244a783          	lw	a5,548(s1)
    80004572:	cb85                	beqz	a5,800045a2 <piperead+0x68>
    if(killed(pr)){
    80004574:	8552                	mv	a0,s4
    80004576:	ba7fd0ef          	jal	8000211c <killed>
    8000457a:	ed19                	bnez	a0,80004598 <piperead+0x5e>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    8000457c:	85a6                	mv	a1,s1
    8000457e:	854e                	mv	a0,s3
    80004580:	965fd0ef          	jal	80001ee4 <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004584:	2184a703          	lw	a4,536(s1)
    80004588:	21c4a783          	lw	a5,540(s1)
    8000458c:	fef701e3          	beq	a4,a5,8000456e <piperead+0x34>
    80004590:	e85a                	sd	s6,16(sp)
    80004592:	a809                	j	800045a4 <piperead+0x6a>
    80004594:	e85a                	sd	s6,16(sp)
    80004596:	a039                	j	800045a4 <piperead+0x6a>
      release(&pi->lock);
    80004598:	8526                	mv	a0,s1
    8000459a:	eccfc0ef          	jal	80000c66 <release>
      return -1;
    8000459e:	59fd                	li	s3,-1
    800045a0:	a8b9                	j	800045fe <piperead+0xc4>
    800045a2:	e85a                	sd	s6,16(sp)
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800045a4:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    800045a6:	5b7d                	li	s6,-1
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800045a8:	05505363          	blez	s5,800045ee <piperead+0xb4>
    if(pi->nread == pi->nwrite)
    800045ac:	2184a783          	lw	a5,536(s1)
    800045b0:	21c4a703          	lw	a4,540(s1)
    800045b4:	02f70d63          	beq	a4,a5,800045ee <piperead+0xb4>
    ch = pi->data[pi->nread % PIPESIZE];
    800045b8:	1ff7f793          	andi	a5,a5,511
    800045bc:	97a6                	add	a5,a5,s1
    800045be:	0187c783          	lbu	a5,24(a5)
    800045c2:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    800045c6:	4685                	li	a3,1
    800045c8:	fbf40613          	addi	a2,s0,-65
    800045cc:	85ca                	mv	a1,s2
    800045ce:	050a3503          	ld	a0,80(s4)
    800045d2:	810fd0ef          	jal	800015e2 <copyout>
    800045d6:	03650e63          	beq	a0,s6,80004612 <piperead+0xd8>
      if(i == 0)
        i = -1;
      break;
    }
    pi->nread++;
    800045da:	2184a783          	lw	a5,536(s1)
    800045de:	2785                	addiw	a5,a5,1
    800045e0:	20f4ac23          	sw	a5,536(s1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800045e4:	2985                	addiw	s3,s3,1
    800045e6:	0905                	addi	s2,s2,1
    800045e8:	fd3a92e3          	bne	s5,s3,800045ac <piperead+0x72>
    800045ec:	89d6                	mv	s3,s5
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    800045ee:	21c48513          	addi	a0,s1,540
    800045f2:	93ffd0ef          	jal	80001f30 <wakeup>
  release(&pi->lock);
    800045f6:	8526                	mv	a0,s1
    800045f8:	e6efc0ef          	jal	80000c66 <release>
    800045fc:	6b42                	ld	s6,16(sp)
  return i;
}
    800045fe:	854e                	mv	a0,s3
    80004600:	60a6                	ld	ra,72(sp)
    80004602:	6406                	ld	s0,64(sp)
    80004604:	74e2                	ld	s1,56(sp)
    80004606:	7942                	ld	s2,48(sp)
    80004608:	79a2                	ld	s3,40(sp)
    8000460a:	7a02                	ld	s4,32(sp)
    8000460c:	6ae2                	ld	s5,24(sp)
    8000460e:	6161                	addi	sp,sp,80
    80004610:	8082                	ret
      if(i == 0)
    80004612:	fc099ee3          	bnez	s3,800045ee <piperead+0xb4>
        i = -1;
    80004616:	89aa                	mv	s3,a0
    80004618:	bfd9                	j	800045ee <piperead+0xb4>

000000008000461a <flags2perm>:

static int loadseg(pde_t *, uint64, struct inode *, uint, uint);

// map ELF permissions to PTE permission bits.
int flags2perm(int flags)
{
    8000461a:	1141                	addi	sp,sp,-16
    8000461c:	e422                	sd	s0,8(sp)
    8000461e:	0800                	addi	s0,sp,16
    80004620:	87aa                	mv	a5,a0
    int perm = 0;
    if(flags & 0x1)
    80004622:	8905                	andi	a0,a0,1
    80004624:	050e                	slli	a0,a0,0x3
      perm = PTE_X;
    if(flags & 0x2)
    80004626:	8b89                	andi	a5,a5,2
    80004628:	c399                	beqz	a5,8000462e <flags2perm+0x14>
      perm |= PTE_W;
    8000462a:	00456513          	ori	a0,a0,4
    return perm;
}
    8000462e:	6422                	ld	s0,8(sp)
    80004630:	0141                	addi	sp,sp,16
    80004632:	8082                	ret

0000000080004634 <kexec>:
//
// the implementation of the exec() system call
//
int
kexec(char *path, char **argv)
{
    80004634:	df010113          	addi	sp,sp,-528
    80004638:	20113423          	sd	ra,520(sp)
    8000463c:	20813023          	sd	s0,512(sp)
    80004640:	ffa6                	sd	s1,504(sp)
    80004642:	fbca                	sd	s2,496(sp)
    80004644:	0c00                	addi	s0,sp,528
    80004646:	892a                	mv	s2,a0
    80004648:	dea43c23          	sd	a0,-520(s0)
    8000464c:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    80004650:	a7efd0ef          	jal	800018ce <myproc>
    80004654:	84aa                	mv	s1,a0

  begin_op();
    80004656:	dcaff0ef          	jal	80003c20 <begin_op>

  // Open the executable file.
  if((ip = namei(path)) == 0){
    8000465a:	854a                	mv	a0,s2
    8000465c:	bf0ff0ef          	jal	80003a4c <namei>
    80004660:	c931                	beqz	a0,800046b4 <kexec+0x80>
    80004662:	f3d2                	sd	s4,480(sp)
    80004664:	8a2a                	mv	s4,a0
    end_op();
    return -1;
  }
  ilock(ip);
    80004666:	bd1fe0ef          	jal	80003236 <ilock>

  // Read the ELF header.
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    8000466a:	04000713          	li	a4,64
    8000466e:	4681                	li	a3,0
    80004670:	e5040613          	addi	a2,s0,-432
    80004674:	4581                	li	a1,0
    80004676:	8552                	mv	a0,s4
    80004678:	f4ffe0ef          	jal	800035c6 <readi>
    8000467c:	04000793          	li	a5,64
    80004680:	00f51a63          	bne	a0,a5,80004694 <kexec+0x60>
    goto bad;

  // Is this really an ELF file?
  if(elf.magic != ELF_MAGIC)
    80004684:	e5042703          	lw	a4,-432(s0)
    80004688:	464c47b7          	lui	a5,0x464c4
    8000468c:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    80004690:	02f70663          	beq	a4,a5,800046bc <kexec+0x88>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    80004694:	8552                	mv	a0,s4
    80004696:	dabfe0ef          	jal	80003440 <iunlockput>
    end_op();
    8000469a:	df0ff0ef          	jal	80003c8a <end_op>
  }
  return -1;
    8000469e:	557d                	li	a0,-1
    800046a0:	7a1e                	ld	s4,480(sp)
}
    800046a2:	20813083          	ld	ra,520(sp)
    800046a6:	20013403          	ld	s0,512(sp)
    800046aa:	74fe                	ld	s1,504(sp)
    800046ac:	795e                	ld	s2,496(sp)
    800046ae:	21010113          	addi	sp,sp,528
    800046b2:	8082                	ret
    end_op();
    800046b4:	dd6ff0ef          	jal	80003c8a <end_op>
    return -1;
    800046b8:	557d                	li	a0,-1
    800046ba:	b7e5                	j	800046a2 <kexec+0x6e>
    800046bc:	ebda                	sd	s6,464(sp)
  if((pagetable = proc_pagetable(p)) == 0)
    800046be:	8526                	mv	a0,s1
    800046c0:	b14fd0ef          	jal	800019d4 <proc_pagetable>
    800046c4:	8b2a                	mv	s6,a0
    800046c6:	2c050b63          	beqz	a0,8000499c <kexec+0x368>
    800046ca:	f7ce                	sd	s3,488(sp)
    800046cc:	efd6                	sd	s5,472(sp)
    800046ce:	e7de                	sd	s7,456(sp)
    800046d0:	e3e2                	sd	s8,448(sp)
    800046d2:	ff66                	sd	s9,440(sp)
    800046d4:	fb6a                	sd	s10,432(sp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    800046d6:	e7042d03          	lw	s10,-400(s0)
    800046da:	e8845783          	lhu	a5,-376(s0)
    800046de:	12078963          	beqz	a5,80004810 <kexec+0x1dc>
    800046e2:	f76e                	sd	s11,424(sp)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    800046e4:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    800046e6:	4d81                	li	s11,0
    if(ph.vaddr % PGSIZE != 0)
    800046e8:	6c85                	lui	s9,0x1
    800046ea:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    800046ee:	def43823          	sd	a5,-528(s0)

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    if(sz - i < PGSIZE)
    800046f2:	6a85                	lui	s5,0x1
    800046f4:	a085                	j	80004754 <kexec+0x120>
      panic("loadseg: address should exist");
    800046f6:	00003517          	auipc	a0,0x3
    800046fa:	eaa50513          	addi	a0,a0,-342 # 800075a0 <etext+0x5a0>
    800046fe:	8e2fc0ef          	jal	800007e0 <panic>
    if(sz - i < PGSIZE)
    80004702:	2481                	sext.w	s1,s1
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    80004704:	8726                	mv	a4,s1
    80004706:	012c06bb          	addw	a3,s8,s2
    8000470a:	4581                	li	a1,0
    8000470c:	8552                	mv	a0,s4
    8000470e:	eb9fe0ef          	jal	800035c6 <readi>
    80004712:	2501                	sext.w	a0,a0
    80004714:	24a49a63          	bne	s1,a0,80004968 <kexec+0x334>
  for(i = 0; i < sz; i += PGSIZE){
    80004718:	012a893b          	addw	s2,s5,s2
    8000471c:	03397363          	bgeu	s2,s3,80004742 <kexec+0x10e>
    pa = walkaddr(pagetable, va + i);
    80004720:	02091593          	slli	a1,s2,0x20
    80004724:	9181                	srli	a1,a1,0x20
    80004726:	95de                	add	a1,a1,s7
    80004728:	855a                	mv	a0,s6
    8000472a:	887fc0ef          	jal	80000fb0 <walkaddr>
    8000472e:	862a                	mv	a2,a0
    if(pa == 0)
    80004730:	d179                	beqz	a0,800046f6 <kexec+0xc2>
    if(sz - i < PGSIZE)
    80004732:	412984bb          	subw	s1,s3,s2
    80004736:	0004879b          	sext.w	a5,s1
    8000473a:	fcfcf4e3          	bgeu	s9,a5,80004702 <kexec+0xce>
    8000473e:	84d6                	mv	s1,s5
    80004740:	b7c9                	j	80004702 <kexec+0xce>
    sz = sz1;
    80004742:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004746:	2d85                	addiw	s11,s11,1
    80004748:	038d0d1b          	addiw	s10,s10,56 # 1038 <_entry-0x7fffefc8>
    8000474c:	e8845783          	lhu	a5,-376(s0)
    80004750:	08fdd063          	bge	s11,a5,800047d0 <kexec+0x19c>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    80004754:	2d01                	sext.w	s10,s10
    80004756:	03800713          	li	a4,56
    8000475a:	86ea                	mv	a3,s10
    8000475c:	e1840613          	addi	a2,s0,-488
    80004760:	4581                	li	a1,0
    80004762:	8552                	mv	a0,s4
    80004764:	e63fe0ef          	jal	800035c6 <readi>
    80004768:	03800793          	li	a5,56
    8000476c:	1cf51663          	bne	a0,a5,80004938 <kexec+0x304>
    if(ph.type != ELF_PROG_LOAD)
    80004770:	e1842783          	lw	a5,-488(s0)
    80004774:	4705                	li	a4,1
    80004776:	fce798e3          	bne	a5,a4,80004746 <kexec+0x112>
    if(ph.memsz < ph.filesz)
    8000477a:	e4043483          	ld	s1,-448(s0)
    8000477e:	e3843783          	ld	a5,-456(s0)
    80004782:	1af4ef63          	bltu	s1,a5,80004940 <kexec+0x30c>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    80004786:	e2843783          	ld	a5,-472(s0)
    8000478a:	94be                	add	s1,s1,a5
    8000478c:	1af4ee63          	bltu	s1,a5,80004948 <kexec+0x314>
    if(ph.vaddr % PGSIZE != 0)
    80004790:	df043703          	ld	a4,-528(s0)
    80004794:	8ff9                	and	a5,a5,a4
    80004796:	1a079d63          	bnez	a5,80004950 <kexec+0x31c>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz, flags2perm(ph.flags))) == 0)
    8000479a:	e1c42503          	lw	a0,-484(s0)
    8000479e:	e7dff0ef          	jal	8000461a <flags2perm>
    800047a2:	86aa                	mv	a3,a0
    800047a4:	8626                	mv	a2,s1
    800047a6:	85ca                	mv	a1,s2
    800047a8:	855a                	mv	a0,s6
    800047aa:	adffc0ef          	jal	80001288 <uvmalloc>
    800047ae:	e0a43423          	sd	a0,-504(s0)
    800047b2:	1a050363          	beqz	a0,80004958 <kexec+0x324>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    800047b6:	e2843b83          	ld	s7,-472(s0)
    800047ba:	e2042c03          	lw	s8,-480(s0)
    800047be:	e3842983          	lw	s3,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    800047c2:	00098463          	beqz	s3,800047ca <kexec+0x196>
    800047c6:	4901                	li	s2,0
    800047c8:	bfa1                	j	80004720 <kexec+0xec>
    sz = sz1;
    800047ca:	e0843903          	ld	s2,-504(s0)
    800047ce:	bfa5                	j	80004746 <kexec+0x112>
    800047d0:	7dba                	ld	s11,424(sp)
  iunlockput(ip);
    800047d2:	8552                	mv	a0,s4
    800047d4:	c6dfe0ef          	jal	80003440 <iunlockput>
  end_op();
    800047d8:	cb2ff0ef          	jal	80003c8a <end_op>
  p = myproc();
    800047dc:	8f2fd0ef          	jal	800018ce <myproc>
    800047e0:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    800047e2:	04853c83          	ld	s9,72(a0)
  sz = PGROUNDUP(sz);
    800047e6:	6985                	lui	s3,0x1
    800047e8:	19fd                	addi	s3,s3,-1 # fff <_entry-0x7ffff001>
    800047ea:	99ca                	add	s3,s3,s2
    800047ec:	77fd                	lui	a5,0xfffff
    800047ee:	00f9f9b3          	and	s3,s3,a5
  if((sz1 = uvmalloc(pagetable, sz, sz + (USERSTACK+1)*PGSIZE, PTE_W)) == 0)
    800047f2:	4691                	li	a3,4
    800047f4:	6609                	lui	a2,0x2
    800047f6:	964e                	add	a2,a2,s3
    800047f8:	85ce                	mv	a1,s3
    800047fa:	855a                	mv	a0,s6
    800047fc:	a8dfc0ef          	jal	80001288 <uvmalloc>
    80004800:	892a                	mv	s2,a0
    80004802:	e0a43423          	sd	a0,-504(s0)
    80004806:	e519                	bnez	a0,80004814 <kexec+0x1e0>
  if(pagetable)
    80004808:	e1343423          	sd	s3,-504(s0)
    8000480c:	4a01                	li	s4,0
    8000480e:	aab1                	j	8000496a <kexec+0x336>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    80004810:	4901                	li	s2,0
    80004812:	b7c1                	j	800047d2 <kexec+0x19e>
  uvmclear(pagetable, sz-(USERSTACK+1)*PGSIZE);
    80004814:	75f9                	lui	a1,0xffffe
    80004816:	95aa                	add	a1,a1,a0
    80004818:	855a                	mv	a0,s6
    8000481a:	c45fc0ef          	jal	8000145e <uvmclear>
  stackbase = sp - USERSTACK*PGSIZE;
    8000481e:	7bfd                	lui	s7,0xfffff
    80004820:	9bca                	add	s7,s7,s2
  for(argc = 0; argv[argc]; argc++) {
    80004822:	e0043783          	ld	a5,-512(s0)
    80004826:	6388                	ld	a0,0(a5)
    80004828:	cd39                	beqz	a0,80004886 <kexec+0x252>
    8000482a:	e9040993          	addi	s3,s0,-368
    8000482e:	f9040c13          	addi	s8,s0,-112
    80004832:	4481                	li	s1,0
    sp -= strlen(argv[argc]) + 1;
    80004834:	ddefc0ef          	jal	80000e12 <strlen>
    80004838:	0015079b          	addiw	a5,a0,1
    8000483c:	40f907b3          	sub	a5,s2,a5
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    80004840:	ff07f913          	andi	s2,a5,-16
    if(sp < stackbase)
    80004844:	11796e63          	bltu	s2,s7,80004960 <kexec+0x32c>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    80004848:	e0043d03          	ld	s10,-512(s0)
    8000484c:	000d3a03          	ld	s4,0(s10)
    80004850:	8552                	mv	a0,s4
    80004852:	dc0fc0ef          	jal	80000e12 <strlen>
    80004856:	0015069b          	addiw	a3,a0,1
    8000485a:	8652                	mv	a2,s4
    8000485c:	85ca                	mv	a1,s2
    8000485e:	855a                	mv	a0,s6
    80004860:	d83fc0ef          	jal	800015e2 <copyout>
    80004864:	10054063          	bltz	a0,80004964 <kexec+0x330>
    ustack[argc] = sp;
    80004868:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    8000486c:	0485                	addi	s1,s1,1
    8000486e:	008d0793          	addi	a5,s10,8
    80004872:	e0f43023          	sd	a5,-512(s0)
    80004876:	008d3503          	ld	a0,8(s10)
    8000487a:	c909                	beqz	a0,8000488c <kexec+0x258>
    if(argc >= MAXARG)
    8000487c:	09a1                	addi	s3,s3,8
    8000487e:	fb899be3          	bne	s3,s8,80004834 <kexec+0x200>
  ip = 0;
    80004882:	4a01                	li	s4,0
    80004884:	a0dd                	j	8000496a <kexec+0x336>
  sp = sz;
    80004886:	e0843903          	ld	s2,-504(s0)
  for(argc = 0; argv[argc]; argc++) {
    8000488a:	4481                	li	s1,0
  ustack[argc] = 0;
    8000488c:	00349793          	slli	a5,s1,0x3
    80004890:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffde028>
    80004894:	97a2                	add	a5,a5,s0
    80004896:	f007b023          	sd	zero,-256(a5)
  sp -= (argc+1) * sizeof(uint64);
    8000489a:	00148693          	addi	a3,s1,1
    8000489e:	068e                	slli	a3,a3,0x3
    800048a0:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    800048a4:	ff097913          	andi	s2,s2,-16
  sz = sz1;
    800048a8:	e0843983          	ld	s3,-504(s0)
  if(sp < stackbase)
    800048ac:	f5796ee3          	bltu	s2,s7,80004808 <kexec+0x1d4>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    800048b0:	e9040613          	addi	a2,s0,-368
    800048b4:	85ca                	mv	a1,s2
    800048b6:	855a                	mv	a0,s6
    800048b8:	d2bfc0ef          	jal	800015e2 <copyout>
    800048bc:	0e054263          	bltz	a0,800049a0 <kexec+0x36c>
  p->trapframe->a1 = sp;
    800048c0:	058ab783          	ld	a5,88(s5) # 1058 <_entry-0x7fffefa8>
    800048c4:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    800048c8:	df843783          	ld	a5,-520(s0)
    800048cc:	0007c703          	lbu	a4,0(a5)
    800048d0:	cf11                	beqz	a4,800048ec <kexec+0x2b8>
    800048d2:	0785                	addi	a5,a5,1
    if(*s == '/')
    800048d4:	02f00693          	li	a3,47
    800048d8:	a039                	j	800048e6 <kexec+0x2b2>
      last = s+1;
    800048da:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    800048de:	0785                	addi	a5,a5,1
    800048e0:	fff7c703          	lbu	a4,-1(a5)
    800048e4:	c701                	beqz	a4,800048ec <kexec+0x2b8>
    if(*s == '/')
    800048e6:	fed71ce3          	bne	a4,a3,800048de <kexec+0x2aa>
    800048ea:	bfc5                	j	800048da <kexec+0x2a6>
  safestrcpy(p->name, last, sizeof(p->name));
    800048ec:	4641                	li	a2,16
    800048ee:	df843583          	ld	a1,-520(s0)
    800048f2:	158a8513          	addi	a0,s5,344
    800048f6:	ceafc0ef          	jal	80000de0 <safestrcpy>
  oldpagetable = p->pagetable;
    800048fa:	050ab503          	ld	a0,80(s5)
  p->pagetable = pagetable;
    800048fe:	056ab823          	sd	s6,80(s5)
  p->sz = sz;
    80004902:	e0843783          	ld	a5,-504(s0)
    80004906:	04fab423          	sd	a5,72(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = ulib.c:start()
    8000490a:	058ab783          	ld	a5,88(s5)
    8000490e:	e6843703          	ld	a4,-408(s0)
    80004912:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    80004914:	058ab783          	ld	a5,88(s5)
    80004918:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    8000491c:	85e6                	mv	a1,s9
    8000491e:	93afd0ef          	jal	80001a58 <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    80004922:	0004851b          	sext.w	a0,s1
    80004926:	79be                	ld	s3,488(sp)
    80004928:	7a1e                	ld	s4,480(sp)
    8000492a:	6afe                	ld	s5,472(sp)
    8000492c:	6b5e                	ld	s6,464(sp)
    8000492e:	6bbe                	ld	s7,456(sp)
    80004930:	6c1e                	ld	s8,448(sp)
    80004932:	7cfa                	ld	s9,440(sp)
    80004934:	7d5a                	ld	s10,432(sp)
    80004936:	b3b5                	j	800046a2 <kexec+0x6e>
    80004938:	e1243423          	sd	s2,-504(s0)
    8000493c:	7dba                	ld	s11,424(sp)
    8000493e:	a035                	j	8000496a <kexec+0x336>
    80004940:	e1243423          	sd	s2,-504(s0)
    80004944:	7dba                	ld	s11,424(sp)
    80004946:	a015                	j	8000496a <kexec+0x336>
    80004948:	e1243423          	sd	s2,-504(s0)
    8000494c:	7dba                	ld	s11,424(sp)
    8000494e:	a831                	j	8000496a <kexec+0x336>
    80004950:	e1243423          	sd	s2,-504(s0)
    80004954:	7dba                	ld	s11,424(sp)
    80004956:	a811                	j	8000496a <kexec+0x336>
    80004958:	e1243423          	sd	s2,-504(s0)
    8000495c:	7dba                	ld	s11,424(sp)
    8000495e:	a031                	j	8000496a <kexec+0x336>
  ip = 0;
    80004960:	4a01                	li	s4,0
    80004962:	a021                	j	8000496a <kexec+0x336>
    80004964:	4a01                	li	s4,0
  if(pagetable)
    80004966:	a011                	j	8000496a <kexec+0x336>
    80004968:	7dba                	ld	s11,424(sp)
    proc_freepagetable(pagetable, sz);
    8000496a:	e0843583          	ld	a1,-504(s0)
    8000496e:	855a                	mv	a0,s6
    80004970:	8e8fd0ef          	jal	80001a58 <proc_freepagetable>
  return -1;
    80004974:	557d                	li	a0,-1
  if(ip){
    80004976:	000a1b63          	bnez	s4,8000498c <kexec+0x358>
    8000497a:	79be                	ld	s3,488(sp)
    8000497c:	7a1e                	ld	s4,480(sp)
    8000497e:	6afe                	ld	s5,472(sp)
    80004980:	6b5e                	ld	s6,464(sp)
    80004982:	6bbe                	ld	s7,456(sp)
    80004984:	6c1e                	ld	s8,448(sp)
    80004986:	7cfa                	ld	s9,440(sp)
    80004988:	7d5a                	ld	s10,432(sp)
    8000498a:	bb21                	j	800046a2 <kexec+0x6e>
    8000498c:	79be                	ld	s3,488(sp)
    8000498e:	6afe                	ld	s5,472(sp)
    80004990:	6b5e                	ld	s6,464(sp)
    80004992:	6bbe                	ld	s7,456(sp)
    80004994:	6c1e                	ld	s8,448(sp)
    80004996:	7cfa                	ld	s9,440(sp)
    80004998:	7d5a                	ld	s10,432(sp)
    8000499a:	b9ed                	j	80004694 <kexec+0x60>
    8000499c:	6b5e                	ld	s6,464(sp)
    8000499e:	b9dd                	j	80004694 <kexec+0x60>
  sz = sz1;
    800049a0:	e0843983          	ld	s3,-504(s0)
    800049a4:	b595                	j	80004808 <kexec+0x1d4>

00000000800049a6 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    800049a6:	7179                	addi	sp,sp,-48
    800049a8:	f406                	sd	ra,40(sp)
    800049aa:	f022                	sd	s0,32(sp)
    800049ac:	ec26                	sd	s1,24(sp)
    800049ae:	e84a                	sd	s2,16(sp)
    800049b0:	1800                	addi	s0,sp,48
    800049b2:	892e                	mv	s2,a1
    800049b4:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  argint(n, &fd);
    800049b6:	fdc40593          	addi	a1,s0,-36
    800049ba:	e77fd0ef          	jal	80002830 <argint>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    800049be:	fdc42703          	lw	a4,-36(s0)
    800049c2:	47bd                	li	a5,15
    800049c4:	02e7e963          	bltu	a5,a4,800049f6 <argfd+0x50>
    800049c8:	f07fc0ef          	jal	800018ce <myproc>
    800049cc:	fdc42703          	lw	a4,-36(s0)
    800049d0:	01a70793          	addi	a5,a4,26
    800049d4:	078e                	slli	a5,a5,0x3
    800049d6:	953e                	add	a0,a0,a5
    800049d8:	611c                	ld	a5,0(a0)
    800049da:	c385                	beqz	a5,800049fa <argfd+0x54>
    return -1;
  if(pfd)
    800049dc:	00090463          	beqz	s2,800049e4 <argfd+0x3e>
    *pfd = fd;
    800049e0:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    800049e4:	4501                	li	a0,0
  if(pf)
    800049e6:	c091                	beqz	s1,800049ea <argfd+0x44>
    *pf = f;
    800049e8:	e09c                	sd	a5,0(s1)
}
    800049ea:	70a2                	ld	ra,40(sp)
    800049ec:	7402                	ld	s0,32(sp)
    800049ee:	64e2                	ld	s1,24(sp)
    800049f0:	6942                	ld	s2,16(sp)
    800049f2:	6145                	addi	sp,sp,48
    800049f4:	8082                	ret
    return -1;
    800049f6:	557d                	li	a0,-1
    800049f8:	bfcd                	j	800049ea <argfd+0x44>
    800049fa:	557d                	li	a0,-1
    800049fc:	b7fd                	j	800049ea <argfd+0x44>

00000000800049fe <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    800049fe:	1101                	addi	sp,sp,-32
    80004a00:	ec06                	sd	ra,24(sp)
    80004a02:	e822                	sd	s0,16(sp)
    80004a04:	e426                	sd	s1,8(sp)
    80004a06:	1000                	addi	s0,sp,32
    80004a08:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    80004a0a:	ec5fc0ef          	jal	800018ce <myproc>
    80004a0e:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    80004a10:	0d050793          	addi	a5,a0,208
    80004a14:	4501                	li	a0,0
    80004a16:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    80004a18:	6398                	ld	a4,0(a5)
    80004a1a:	cb19                	beqz	a4,80004a30 <fdalloc+0x32>
  for(fd = 0; fd < NOFILE; fd++){
    80004a1c:	2505                	addiw	a0,a0,1
    80004a1e:	07a1                	addi	a5,a5,8
    80004a20:	fed51ce3          	bne	a0,a3,80004a18 <fdalloc+0x1a>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    80004a24:	557d                	li	a0,-1
}
    80004a26:	60e2                	ld	ra,24(sp)
    80004a28:	6442                	ld	s0,16(sp)
    80004a2a:	64a2                	ld	s1,8(sp)
    80004a2c:	6105                	addi	sp,sp,32
    80004a2e:	8082                	ret
      p->ofile[fd] = f;
    80004a30:	01a50793          	addi	a5,a0,26
    80004a34:	078e                	slli	a5,a5,0x3
    80004a36:	963e                	add	a2,a2,a5
    80004a38:	e204                	sd	s1,0(a2)
      return fd;
    80004a3a:	b7f5                	j	80004a26 <fdalloc+0x28>

0000000080004a3c <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    80004a3c:	715d                	addi	sp,sp,-80
    80004a3e:	e486                	sd	ra,72(sp)
    80004a40:	e0a2                	sd	s0,64(sp)
    80004a42:	fc26                	sd	s1,56(sp)
    80004a44:	f84a                	sd	s2,48(sp)
    80004a46:	f44e                	sd	s3,40(sp)
    80004a48:	ec56                	sd	s5,24(sp)
    80004a4a:	e85a                	sd	s6,16(sp)
    80004a4c:	0880                	addi	s0,sp,80
    80004a4e:	8b2e                	mv	s6,a1
    80004a50:	89b2                	mv	s3,a2
    80004a52:	8936                	mv	s2,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    80004a54:	fb040593          	addi	a1,s0,-80
    80004a58:	80eff0ef          	jal	80003a66 <nameiparent>
    80004a5c:	84aa                	mv	s1,a0
    80004a5e:	10050a63          	beqz	a0,80004b72 <create+0x136>
    return 0;

  ilock(dp);
    80004a62:	fd4fe0ef          	jal	80003236 <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    80004a66:	4601                	li	a2,0
    80004a68:	fb040593          	addi	a1,s0,-80
    80004a6c:	8526                	mv	a0,s1
    80004a6e:	d79fe0ef          	jal	800037e6 <dirlookup>
    80004a72:	8aaa                	mv	s5,a0
    80004a74:	c129                	beqz	a0,80004ab6 <create+0x7a>
    iunlockput(dp);
    80004a76:	8526                	mv	a0,s1
    80004a78:	9c9fe0ef          	jal	80003440 <iunlockput>
    ilock(ip);
    80004a7c:	8556                	mv	a0,s5
    80004a7e:	fb8fe0ef          	jal	80003236 <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    80004a82:	4789                	li	a5,2
    80004a84:	02fb1463          	bne	s6,a5,80004aac <create+0x70>
    80004a88:	044ad783          	lhu	a5,68(s5)
    80004a8c:	37f9                	addiw	a5,a5,-2
    80004a8e:	17c2                	slli	a5,a5,0x30
    80004a90:	93c1                	srli	a5,a5,0x30
    80004a92:	4705                	li	a4,1
    80004a94:	00f76c63          	bltu	a4,a5,80004aac <create+0x70>
  ip->nlink = 0;
  iupdate(ip);
  iunlockput(ip);
  iunlockput(dp);
  return 0;
}
    80004a98:	8556                	mv	a0,s5
    80004a9a:	60a6                	ld	ra,72(sp)
    80004a9c:	6406                	ld	s0,64(sp)
    80004a9e:	74e2                	ld	s1,56(sp)
    80004aa0:	7942                	ld	s2,48(sp)
    80004aa2:	79a2                	ld	s3,40(sp)
    80004aa4:	6ae2                	ld	s5,24(sp)
    80004aa6:	6b42                	ld	s6,16(sp)
    80004aa8:	6161                	addi	sp,sp,80
    80004aaa:	8082                	ret
    iunlockput(ip);
    80004aac:	8556                	mv	a0,s5
    80004aae:	993fe0ef          	jal	80003440 <iunlockput>
    return 0;
    80004ab2:	4a81                	li	s5,0
    80004ab4:	b7d5                	j	80004a98 <create+0x5c>
    80004ab6:	f052                	sd	s4,32(sp)
  if((ip = ialloc(dp->dev, type)) == 0){
    80004ab8:	85da                	mv	a1,s6
    80004aba:	4088                	lw	a0,0(s1)
    80004abc:	e0afe0ef          	jal	800030c6 <ialloc>
    80004ac0:	8a2a                	mv	s4,a0
    80004ac2:	cd15                	beqz	a0,80004afe <create+0xc2>
  ilock(ip);
    80004ac4:	f72fe0ef          	jal	80003236 <ilock>
  ip->major = major;
    80004ac8:	053a1323          	sh	s3,70(s4)
  ip->minor = minor;
    80004acc:	052a1423          	sh	s2,72(s4)
  ip->nlink = 1;
    80004ad0:	4905                	li	s2,1
    80004ad2:	052a1523          	sh	s2,74(s4)
  iupdate(ip);
    80004ad6:	8552                	mv	a0,s4
    80004ad8:	eaafe0ef          	jal	80003182 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80004adc:	032b0763          	beq	s6,s2,80004b0a <create+0xce>
  if(dirlink(dp, name, ip->inum) < 0)
    80004ae0:	004a2603          	lw	a2,4(s4)
    80004ae4:	fb040593          	addi	a1,s0,-80
    80004ae8:	8526                	mv	a0,s1
    80004aea:	ec9fe0ef          	jal	800039b2 <dirlink>
    80004aee:	06054563          	bltz	a0,80004b58 <create+0x11c>
  iunlockput(dp);
    80004af2:	8526                	mv	a0,s1
    80004af4:	94dfe0ef          	jal	80003440 <iunlockput>
  return ip;
    80004af8:	8ad2                	mv	s5,s4
    80004afa:	7a02                	ld	s4,32(sp)
    80004afc:	bf71                	j	80004a98 <create+0x5c>
    iunlockput(dp);
    80004afe:	8526                	mv	a0,s1
    80004b00:	941fe0ef          	jal	80003440 <iunlockput>
    return 0;
    80004b04:	8ad2                	mv	s5,s4
    80004b06:	7a02                	ld	s4,32(sp)
    80004b08:	bf41                	j	80004a98 <create+0x5c>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80004b0a:	004a2603          	lw	a2,4(s4)
    80004b0e:	00003597          	auipc	a1,0x3
    80004b12:	ab258593          	addi	a1,a1,-1358 # 800075c0 <etext+0x5c0>
    80004b16:	8552                	mv	a0,s4
    80004b18:	e9bfe0ef          	jal	800039b2 <dirlink>
    80004b1c:	02054e63          	bltz	a0,80004b58 <create+0x11c>
    80004b20:	40d0                	lw	a2,4(s1)
    80004b22:	00003597          	auipc	a1,0x3
    80004b26:	aa658593          	addi	a1,a1,-1370 # 800075c8 <etext+0x5c8>
    80004b2a:	8552                	mv	a0,s4
    80004b2c:	e87fe0ef          	jal	800039b2 <dirlink>
    80004b30:	02054463          	bltz	a0,80004b58 <create+0x11c>
  if(dirlink(dp, name, ip->inum) < 0)
    80004b34:	004a2603          	lw	a2,4(s4)
    80004b38:	fb040593          	addi	a1,s0,-80
    80004b3c:	8526                	mv	a0,s1
    80004b3e:	e75fe0ef          	jal	800039b2 <dirlink>
    80004b42:	00054b63          	bltz	a0,80004b58 <create+0x11c>
    dp->nlink++;  // for ".."
    80004b46:	04a4d783          	lhu	a5,74(s1)
    80004b4a:	2785                	addiw	a5,a5,1
    80004b4c:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004b50:	8526                	mv	a0,s1
    80004b52:	e30fe0ef          	jal	80003182 <iupdate>
    80004b56:	bf71                	j	80004af2 <create+0xb6>
  ip->nlink = 0;
    80004b58:	040a1523          	sh	zero,74(s4)
  iupdate(ip);
    80004b5c:	8552                	mv	a0,s4
    80004b5e:	e24fe0ef          	jal	80003182 <iupdate>
  iunlockput(ip);
    80004b62:	8552                	mv	a0,s4
    80004b64:	8ddfe0ef          	jal	80003440 <iunlockput>
  iunlockput(dp);
    80004b68:	8526                	mv	a0,s1
    80004b6a:	8d7fe0ef          	jal	80003440 <iunlockput>
  return 0;
    80004b6e:	7a02                	ld	s4,32(sp)
    80004b70:	b725                	j	80004a98 <create+0x5c>
    return 0;
    80004b72:	8aaa                	mv	s5,a0
    80004b74:	b715                	j	80004a98 <create+0x5c>

0000000080004b76 <sys_dup>:
{
    80004b76:	7179                	addi	sp,sp,-48
    80004b78:	f406                	sd	ra,40(sp)
    80004b7a:	f022                	sd	s0,32(sp)
    80004b7c:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    80004b7e:	fd840613          	addi	a2,s0,-40
    80004b82:	4581                	li	a1,0
    80004b84:	4501                	li	a0,0
    80004b86:	e21ff0ef          	jal	800049a6 <argfd>
    return -1;
    80004b8a:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80004b8c:	02054363          	bltz	a0,80004bb2 <sys_dup+0x3c>
    80004b90:	ec26                	sd	s1,24(sp)
    80004b92:	e84a                	sd	s2,16(sp)
  if((fd=fdalloc(f)) < 0)
    80004b94:	fd843903          	ld	s2,-40(s0)
    80004b98:	854a                	mv	a0,s2
    80004b9a:	e65ff0ef          	jal	800049fe <fdalloc>
    80004b9e:	84aa                	mv	s1,a0
    return -1;
    80004ba0:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80004ba2:	00054d63          	bltz	a0,80004bbc <sys_dup+0x46>
  filedup(f);
    80004ba6:	854a                	mv	a0,s2
    80004ba8:	c3eff0ef          	jal	80003fe6 <filedup>
  return fd;
    80004bac:	87a6                	mv	a5,s1
    80004bae:	64e2                	ld	s1,24(sp)
    80004bb0:	6942                	ld	s2,16(sp)
}
    80004bb2:	853e                	mv	a0,a5
    80004bb4:	70a2                	ld	ra,40(sp)
    80004bb6:	7402                	ld	s0,32(sp)
    80004bb8:	6145                	addi	sp,sp,48
    80004bba:	8082                	ret
    80004bbc:	64e2                	ld	s1,24(sp)
    80004bbe:	6942                	ld	s2,16(sp)
    80004bc0:	bfcd                	j	80004bb2 <sys_dup+0x3c>

0000000080004bc2 <sys_read>:
{
    80004bc2:	7179                	addi	sp,sp,-48
    80004bc4:	f406                	sd	ra,40(sp)
    80004bc6:	f022                	sd	s0,32(sp)
    80004bc8:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004bca:	fd840593          	addi	a1,s0,-40
    80004bce:	4505                	li	a0,1
    80004bd0:	c7dfd0ef          	jal	8000284c <argaddr>
  argint(2, &n);
    80004bd4:	fe440593          	addi	a1,s0,-28
    80004bd8:	4509                	li	a0,2
    80004bda:	c57fd0ef          	jal	80002830 <argint>
  if(argfd(0, 0, &f) < 0)
    80004bde:	fe840613          	addi	a2,s0,-24
    80004be2:	4581                	li	a1,0
    80004be4:	4501                	li	a0,0
    80004be6:	dc1ff0ef          	jal	800049a6 <argfd>
    80004bea:	87aa                	mv	a5,a0
    return -1;
    80004bec:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004bee:	0007ca63          	bltz	a5,80004c02 <sys_read+0x40>
  return fileread(f, p, n);
    80004bf2:	fe442603          	lw	a2,-28(s0)
    80004bf6:	fd843583          	ld	a1,-40(s0)
    80004bfa:	fe843503          	ld	a0,-24(s0)
    80004bfe:	d4eff0ef          	jal	8000414c <fileread>
}
    80004c02:	70a2                	ld	ra,40(sp)
    80004c04:	7402                	ld	s0,32(sp)
    80004c06:	6145                	addi	sp,sp,48
    80004c08:	8082                	ret

0000000080004c0a <sys_write>:
{
    80004c0a:	7179                	addi	sp,sp,-48
    80004c0c:	f406                	sd	ra,40(sp)
    80004c0e:	f022                	sd	s0,32(sp)
    80004c10:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004c12:	fd840593          	addi	a1,s0,-40
    80004c16:	4505                	li	a0,1
    80004c18:	c35fd0ef          	jal	8000284c <argaddr>
  argint(2, &n);
    80004c1c:	fe440593          	addi	a1,s0,-28
    80004c20:	4509                	li	a0,2
    80004c22:	c0ffd0ef          	jal	80002830 <argint>
  if(argfd(0, 0, &f) < 0)
    80004c26:	fe840613          	addi	a2,s0,-24
    80004c2a:	4581                	li	a1,0
    80004c2c:	4501                	li	a0,0
    80004c2e:	d79ff0ef          	jal	800049a6 <argfd>
    80004c32:	87aa                	mv	a5,a0
    return -1;
    80004c34:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004c36:	0007ca63          	bltz	a5,80004c4a <sys_write+0x40>
  return filewrite(f, p, n);
    80004c3a:	fe442603          	lw	a2,-28(s0)
    80004c3e:	fd843583          	ld	a1,-40(s0)
    80004c42:	fe843503          	ld	a0,-24(s0)
    80004c46:	dc4ff0ef          	jal	8000420a <filewrite>
}
    80004c4a:	70a2                	ld	ra,40(sp)
    80004c4c:	7402                	ld	s0,32(sp)
    80004c4e:	6145                	addi	sp,sp,48
    80004c50:	8082                	ret

0000000080004c52 <sys_close>:
{
    80004c52:	1101                	addi	sp,sp,-32
    80004c54:	ec06                	sd	ra,24(sp)
    80004c56:	e822                	sd	s0,16(sp)
    80004c58:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    80004c5a:	fe040613          	addi	a2,s0,-32
    80004c5e:	fec40593          	addi	a1,s0,-20
    80004c62:	4501                	li	a0,0
    80004c64:	d43ff0ef          	jal	800049a6 <argfd>
    return -1;
    80004c68:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    80004c6a:	02054063          	bltz	a0,80004c8a <sys_close+0x38>
  myproc()->ofile[fd] = 0;
    80004c6e:	c61fc0ef          	jal	800018ce <myproc>
    80004c72:	fec42783          	lw	a5,-20(s0)
    80004c76:	07e9                	addi	a5,a5,26
    80004c78:	078e                	slli	a5,a5,0x3
    80004c7a:	953e                	add	a0,a0,a5
    80004c7c:	00053023          	sd	zero,0(a0)
  fileclose(f);
    80004c80:	fe043503          	ld	a0,-32(s0)
    80004c84:	ba8ff0ef          	jal	8000402c <fileclose>
  return 0;
    80004c88:	4781                	li	a5,0
}
    80004c8a:	853e                	mv	a0,a5
    80004c8c:	60e2                	ld	ra,24(sp)
    80004c8e:	6442                	ld	s0,16(sp)
    80004c90:	6105                	addi	sp,sp,32
    80004c92:	8082                	ret

0000000080004c94 <sys_fstat>:
{
    80004c94:	1101                	addi	sp,sp,-32
    80004c96:	ec06                	sd	ra,24(sp)
    80004c98:	e822                	sd	s0,16(sp)
    80004c9a:	1000                	addi	s0,sp,32
  argaddr(1, &st);
    80004c9c:	fe040593          	addi	a1,s0,-32
    80004ca0:	4505                	li	a0,1
    80004ca2:	babfd0ef          	jal	8000284c <argaddr>
  if(argfd(0, 0, &f) < 0)
    80004ca6:	fe840613          	addi	a2,s0,-24
    80004caa:	4581                	li	a1,0
    80004cac:	4501                	li	a0,0
    80004cae:	cf9ff0ef          	jal	800049a6 <argfd>
    80004cb2:	87aa                	mv	a5,a0
    return -1;
    80004cb4:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004cb6:	0007c863          	bltz	a5,80004cc6 <sys_fstat+0x32>
  return filestat(f, st);
    80004cba:	fe043583          	ld	a1,-32(s0)
    80004cbe:	fe843503          	ld	a0,-24(s0)
    80004cc2:	c2cff0ef          	jal	800040ee <filestat>
}
    80004cc6:	60e2                	ld	ra,24(sp)
    80004cc8:	6442                	ld	s0,16(sp)
    80004cca:	6105                	addi	sp,sp,32
    80004ccc:	8082                	ret

0000000080004cce <sys_link>:
{
    80004cce:	7169                	addi	sp,sp,-304
    80004cd0:	f606                	sd	ra,296(sp)
    80004cd2:	f222                	sd	s0,288(sp)
    80004cd4:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004cd6:	08000613          	li	a2,128
    80004cda:	ed040593          	addi	a1,s0,-304
    80004cde:	4501                	li	a0,0
    80004ce0:	b89fd0ef          	jal	80002868 <argstr>
    return -1;
    80004ce4:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004ce6:	0c054e63          	bltz	a0,80004dc2 <sys_link+0xf4>
    80004cea:	08000613          	li	a2,128
    80004cee:	f5040593          	addi	a1,s0,-176
    80004cf2:	4505                	li	a0,1
    80004cf4:	b75fd0ef          	jal	80002868 <argstr>
    return -1;
    80004cf8:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004cfa:	0c054463          	bltz	a0,80004dc2 <sys_link+0xf4>
    80004cfe:	ee26                	sd	s1,280(sp)
  begin_op();
    80004d00:	f21fe0ef          	jal	80003c20 <begin_op>
  if((ip = namei(old)) == 0){
    80004d04:	ed040513          	addi	a0,s0,-304
    80004d08:	d45fe0ef          	jal	80003a4c <namei>
    80004d0c:	84aa                	mv	s1,a0
    80004d0e:	c53d                	beqz	a0,80004d7c <sys_link+0xae>
  ilock(ip);
    80004d10:	d26fe0ef          	jal	80003236 <ilock>
  if(ip->type == T_DIR){
    80004d14:	04449703          	lh	a4,68(s1)
    80004d18:	4785                	li	a5,1
    80004d1a:	06f70663          	beq	a4,a5,80004d86 <sys_link+0xb8>
    80004d1e:	ea4a                	sd	s2,272(sp)
  ip->nlink++;
    80004d20:	04a4d783          	lhu	a5,74(s1)
    80004d24:	2785                	addiw	a5,a5,1
    80004d26:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004d2a:	8526                	mv	a0,s1
    80004d2c:	c56fe0ef          	jal	80003182 <iupdate>
  iunlock(ip);
    80004d30:	8526                	mv	a0,s1
    80004d32:	db2fe0ef          	jal	800032e4 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    80004d36:	fd040593          	addi	a1,s0,-48
    80004d3a:	f5040513          	addi	a0,s0,-176
    80004d3e:	d29fe0ef          	jal	80003a66 <nameiparent>
    80004d42:	892a                	mv	s2,a0
    80004d44:	cd21                	beqz	a0,80004d9c <sys_link+0xce>
  ilock(dp);
    80004d46:	cf0fe0ef          	jal	80003236 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    80004d4a:	00092703          	lw	a4,0(s2)
    80004d4e:	409c                	lw	a5,0(s1)
    80004d50:	04f71363          	bne	a4,a5,80004d96 <sys_link+0xc8>
    80004d54:	40d0                	lw	a2,4(s1)
    80004d56:	fd040593          	addi	a1,s0,-48
    80004d5a:	854a                	mv	a0,s2
    80004d5c:	c57fe0ef          	jal	800039b2 <dirlink>
    80004d60:	02054b63          	bltz	a0,80004d96 <sys_link+0xc8>
  iunlockput(dp);
    80004d64:	854a                	mv	a0,s2
    80004d66:	edafe0ef          	jal	80003440 <iunlockput>
  iput(ip);
    80004d6a:	8526                	mv	a0,s1
    80004d6c:	e4cfe0ef          	jal	800033b8 <iput>
  end_op();
    80004d70:	f1bfe0ef          	jal	80003c8a <end_op>
  return 0;
    80004d74:	4781                	li	a5,0
    80004d76:	64f2                	ld	s1,280(sp)
    80004d78:	6952                	ld	s2,272(sp)
    80004d7a:	a0a1                	j	80004dc2 <sys_link+0xf4>
    end_op();
    80004d7c:	f0ffe0ef          	jal	80003c8a <end_op>
    return -1;
    80004d80:	57fd                	li	a5,-1
    80004d82:	64f2                	ld	s1,280(sp)
    80004d84:	a83d                	j	80004dc2 <sys_link+0xf4>
    iunlockput(ip);
    80004d86:	8526                	mv	a0,s1
    80004d88:	eb8fe0ef          	jal	80003440 <iunlockput>
    end_op();
    80004d8c:	efffe0ef          	jal	80003c8a <end_op>
    return -1;
    80004d90:	57fd                	li	a5,-1
    80004d92:	64f2                	ld	s1,280(sp)
    80004d94:	a03d                	j	80004dc2 <sys_link+0xf4>
    iunlockput(dp);
    80004d96:	854a                	mv	a0,s2
    80004d98:	ea8fe0ef          	jal	80003440 <iunlockput>
  ilock(ip);
    80004d9c:	8526                	mv	a0,s1
    80004d9e:	c98fe0ef          	jal	80003236 <ilock>
  ip->nlink--;
    80004da2:	04a4d783          	lhu	a5,74(s1)
    80004da6:	37fd                	addiw	a5,a5,-1
    80004da8:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004dac:	8526                	mv	a0,s1
    80004dae:	bd4fe0ef          	jal	80003182 <iupdate>
  iunlockput(ip);
    80004db2:	8526                	mv	a0,s1
    80004db4:	e8cfe0ef          	jal	80003440 <iunlockput>
  end_op();
    80004db8:	ed3fe0ef          	jal	80003c8a <end_op>
  return -1;
    80004dbc:	57fd                	li	a5,-1
    80004dbe:	64f2                	ld	s1,280(sp)
    80004dc0:	6952                	ld	s2,272(sp)
}
    80004dc2:	853e                	mv	a0,a5
    80004dc4:	70b2                	ld	ra,296(sp)
    80004dc6:	7412                	ld	s0,288(sp)
    80004dc8:	6155                	addi	sp,sp,304
    80004dca:	8082                	ret

0000000080004dcc <sys_unlink>:
{
    80004dcc:	7151                	addi	sp,sp,-240
    80004dce:	f586                	sd	ra,232(sp)
    80004dd0:	f1a2                	sd	s0,224(sp)
    80004dd2:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    80004dd4:	08000613          	li	a2,128
    80004dd8:	f3040593          	addi	a1,s0,-208
    80004ddc:	4501                	li	a0,0
    80004dde:	a8bfd0ef          	jal	80002868 <argstr>
    80004de2:	16054063          	bltz	a0,80004f42 <sys_unlink+0x176>
    80004de6:	eda6                	sd	s1,216(sp)
  begin_op();
    80004de8:	e39fe0ef          	jal	80003c20 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    80004dec:	fb040593          	addi	a1,s0,-80
    80004df0:	f3040513          	addi	a0,s0,-208
    80004df4:	c73fe0ef          	jal	80003a66 <nameiparent>
    80004df8:	84aa                	mv	s1,a0
    80004dfa:	c945                	beqz	a0,80004eaa <sys_unlink+0xde>
  ilock(dp);
    80004dfc:	c3afe0ef          	jal	80003236 <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80004e00:	00002597          	auipc	a1,0x2
    80004e04:	7c058593          	addi	a1,a1,1984 # 800075c0 <etext+0x5c0>
    80004e08:	fb040513          	addi	a0,s0,-80
    80004e0c:	9c5fe0ef          	jal	800037d0 <namecmp>
    80004e10:	10050e63          	beqz	a0,80004f2c <sys_unlink+0x160>
    80004e14:	00002597          	auipc	a1,0x2
    80004e18:	7b458593          	addi	a1,a1,1972 # 800075c8 <etext+0x5c8>
    80004e1c:	fb040513          	addi	a0,s0,-80
    80004e20:	9b1fe0ef          	jal	800037d0 <namecmp>
    80004e24:	10050463          	beqz	a0,80004f2c <sys_unlink+0x160>
    80004e28:	e9ca                	sd	s2,208(sp)
  if((ip = dirlookup(dp, name, &off)) == 0)
    80004e2a:	f2c40613          	addi	a2,s0,-212
    80004e2e:	fb040593          	addi	a1,s0,-80
    80004e32:	8526                	mv	a0,s1
    80004e34:	9b3fe0ef          	jal	800037e6 <dirlookup>
    80004e38:	892a                	mv	s2,a0
    80004e3a:	0e050863          	beqz	a0,80004f2a <sys_unlink+0x15e>
  ilock(ip);
    80004e3e:	bf8fe0ef          	jal	80003236 <ilock>
  if(ip->nlink < 1)
    80004e42:	04a91783          	lh	a5,74(s2)
    80004e46:	06f05763          	blez	a5,80004eb4 <sys_unlink+0xe8>
  if(ip->type == T_DIR && !isdirempty(ip)){
    80004e4a:	04491703          	lh	a4,68(s2)
    80004e4e:	4785                	li	a5,1
    80004e50:	06f70963          	beq	a4,a5,80004ec2 <sys_unlink+0xf6>
  memset(&de, 0, sizeof(de));
    80004e54:	4641                	li	a2,16
    80004e56:	4581                	li	a1,0
    80004e58:	fc040513          	addi	a0,s0,-64
    80004e5c:	e47fb0ef          	jal	80000ca2 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004e60:	4741                	li	a4,16
    80004e62:	f2c42683          	lw	a3,-212(s0)
    80004e66:	fc040613          	addi	a2,s0,-64
    80004e6a:	4581                	li	a1,0
    80004e6c:	8526                	mv	a0,s1
    80004e6e:	855fe0ef          	jal	800036c2 <writei>
    80004e72:	47c1                	li	a5,16
    80004e74:	08f51b63          	bne	a0,a5,80004f0a <sys_unlink+0x13e>
  if(ip->type == T_DIR){
    80004e78:	04491703          	lh	a4,68(s2)
    80004e7c:	4785                	li	a5,1
    80004e7e:	08f70d63          	beq	a4,a5,80004f18 <sys_unlink+0x14c>
  iunlockput(dp);
    80004e82:	8526                	mv	a0,s1
    80004e84:	dbcfe0ef          	jal	80003440 <iunlockput>
  ip->nlink--;
    80004e88:	04a95783          	lhu	a5,74(s2)
    80004e8c:	37fd                	addiw	a5,a5,-1
    80004e8e:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    80004e92:	854a                	mv	a0,s2
    80004e94:	aeefe0ef          	jal	80003182 <iupdate>
  iunlockput(ip);
    80004e98:	854a                	mv	a0,s2
    80004e9a:	da6fe0ef          	jal	80003440 <iunlockput>
  end_op();
    80004e9e:	dedfe0ef          	jal	80003c8a <end_op>
  return 0;
    80004ea2:	4501                	li	a0,0
    80004ea4:	64ee                	ld	s1,216(sp)
    80004ea6:	694e                	ld	s2,208(sp)
    80004ea8:	a849                	j	80004f3a <sys_unlink+0x16e>
    end_op();
    80004eaa:	de1fe0ef          	jal	80003c8a <end_op>
    return -1;
    80004eae:	557d                	li	a0,-1
    80004eb0:	64ee                	ld	s1,216(sp)
    80004eb2:	a061                	j	80004f3a <sys_unlink+0x16e>
    80004eb4:	e5ce                	sd	s3,200(sp)
    panic("unlink: nlink < 1");
    80004eb6:	00002517          	auipc	a0,0x2
    80004eba:	71a50513          	addi	a0,a0,1818 # 800075d0 <etext+0x5d0>
    80004ebe:	923fb0ef          	jal	800007e0 <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004ec2:	04c92703          	lw	a4,76(s2)
    80004ec6:	02000793          	li	a5,32
    80004eca:	f8e7f5e3          	bgeu	a5,a4,80004e54 <sys_unlink+0x88>
    80004ece:	e5ce                	sd	s3,200(sp)
    80004ed0:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004ed4:	4741                	li	a4,16
    80004ed6:	86ce                	mv	a3,s3
    80004ed8:	f1840613          	addi	a2,s0,-232
    80004edc:	4581                	li	a1,0
    80004ede:	854a                	mv	a0,s2
    80004ee0:	ee6fe0ef          	jal	800035c6 <readi>
    80004ee4:	47c1                	li	a5,16
    80004ee6:	00f51c63          	bne	a0,a5,80004efe <sys_unlink+0x132>
    if(de.inum != 0)
    80004eea:	f1845783          	lhu	a5,-232(s0)
    80004eee:	efa1                	bnez	a5,80004f46 <sys_unlink+0x17a>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004ef0:	29c1                	addiw	s3,s3,16
    80004ef2:	04c92783          	lw	a5,76(s2)
    80004ef6:	fcf9efe3          	bltu	s3,a5,80004ed4 <sys_unlink+0x108>
    80004efa:	69ae                	ld	s3,200(sp)
    80004efc:	bfa1                	j	80004e54 <sys_unlink+0x88>
      panic("isdirempty: readi");
    80004efe:	00002517          	auipc	a0,0x2
    80004f02:	6ea50513          	addi	a0,a0,1770 # 800075e8 <etext+0x5e8>
    80004f06:	8dbfb0ef          	jal	800007e0 <panic>
    80004f0a:	e5ce                	sd	s3,200(sp)
    panic("unlink: writei");
    80004f0c:	00002517          	auipc	a0,0x2
    80004f10:	6f450513          	addi	a0,a0,1780 # 80007600 <etext+0x600>
    80004f14:	8cdfb0ef          	jal	800007e0 <panic>
    dp->nlink--;
    80004f18:	04a4d783          	lhu	a5,74(s1)
    80004f1c:	37fd                	addiw	a5,a5,-1
    80004f1e:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004f22:	8526                	mv	a0,s1
    80004f24:	a5efe0ef          	jal	80003182 <iupdate>
    80004f28:	bfa9                	j	80004e82 <sys_unlink+0xb6>
    80004f2a:	694e                	ld	s2,208(sp)
  iunlockput(dp);
    80004f2c:	8526                	mv	a0,s1
    80004f2e:	d12fe0ef          	jal	80003440 <iunlockput>
  end_op();
    80004f32:	d59fe0ef          	jal	80003c8a <end_op>
  return -1;
    80004f36:	557d                	li	a0,-1
    80004f38:	64ee                	ld	s1,216(sp)
}
    80004f3a:	70ae                	ld	ra,232(sp)
    80004f3c:	740e                	ld	s0,224(sp)
    80004f3e:	616d                	addi	sp,sp,240
    80004f40:	8082                	ret
    return -1;
    80004f42:	557d                	li	a0,-1
    80004f44:	bfdd                	j	80004f3a <sys_unlink+0x16e>
    iunlockput(ip);
    80004f46:	854a                	mv	a0,s2
    80004f48:	cf8fe0ef          	jal	80003440 <iunlockput>
    goto bad;
    80004f4c:	694e                	ld	s2,208(sp)
    80004f4e:	69ae                	ld	s3,200(sp)
    80004f50:	bff1                	j	80004f2c <sys_unlink+0x160>

0000000080004f52 <sys_open>:

uint64
sys_open(void)
{
    80004f52:	7131                	addi	sp,sp,-192
    80004f54:	fd06                	sd	ra,184(sp)
    80004f56:	f922                	sd	s0,176(sp)
    80004f58:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  argint(1, &omode);
    80004f5a:	f4c40593          	addi	a1,s0,-180
    80004f5e:	4505                	li	a0,1
    80004f60:	8d1fd0ef          	jal	80002830 <argint>
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004f64:	08000613          	li	a2,128
    80004f68:	f5040593          	addi	a1,s0,-176
    80004f6c:	4501                	li	a0,0
    80004f6e:	8fbfd0ef          	jal	80002868 <argstr>
    80004f72:	87aa                	mv	a5,a0
    return -1;
    80004f74:	557d                	li	a0,-1
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004f76:	0a07c263          	bltz	a5,8000501a <sys_open+0xc8>
    80004f7a:	f526                	sd	s1,168(sp)

  begin_op();
    80004f7c:	ca5fe0ef          	jal	80003c20 <begin_op>

  if(omode & O_CREATE){
    80004f80:	f4c42783          	lw	a5,-180(s0)
    80004f84:	2007f793          	andi	a5,a5,512
    80004f88:	c3d5                	beqz	a5,8000502c <sys_open+0xda>
    ip = create(path, T_FILE, 0, 0);
    80004f8a:	4681                	li	a3,0
    80004f8c:	4601                	li	a2,0
    80004f8e:	4589                	li	a1,2
    80004f90:	f5040513          	addi	a0,s0,-176
    80004f94:	aa9ff0ef          	jal	80004a3c <create>
    80004f98:	84aa                	mv	s1,a0
    if(ip == 0){
    80004f9a:	c541                	beqz	a0,80005022 <sys_open+0xd0>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    80004f9c:	04449703          	lh	a4,68(s1)
    80004fa0:	478d                	li	a5,3
    80004fa2:	00f71763          	bne	a4,a5,80004fb0 <sys_open+0x5e>
    80004fa6:	0464d703          	lhu	a4,70(s1)
    80004faa:	47a5                	li	a5,9
    80004fac:	0ae7ed63          	bltu	a5,a4,80005066 <sys_open+0x114>
    80004fb0:	f14a                	sd	s2,160(sp)
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80004fb2:	fd7fe0ef          	jal	80003f88 <filealloc>
    80004fb6:	892a                	mv	s2,a0
    80004fb8:	c179                	beqz	a0,8000507e <sys_open+0x12c>
    80004fba:	ed4e                	sd	s3,152(sp)
    80004fbc:	a43ff0ef          	jal	800049fe <fdalloc>
    80004fc0:	89aa                	mv	s3,a0
    80004fc2:	0a054a63          	bltz	a0,80005076 <sys_open+0x124>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80004fc6:	04449703          	lh	a4,68(s1)
    80004fca:	478d                	li	a5,3
    80004fcc:	0cf70263          	beq	a4,a5,80005090 <sys_open+0x13e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80004fd0:	4789                	li	a5,2
    80004fd2:	00f92023          	sw	a5,0(s2)
    f->off = 0;
    80004fd6:	02092023          	sw	zero,32(s2)
  }
  f->ip = ip;
    80004fda:	00993c23          	sd	s1,24(s2)
  f->readable = !(omode & O_WRONLY);
    80004fde:	f4c42783          	lw	a5,-180(s0)
    80004fe2:	0017c713          	xori	a4,a5,1
    80004fe6:	8b05                	andi	a4,a4,1
    80004fe8:	00e90423          	sb	a4,8(s2)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    80004fec:	0037f713          	andi	a4,a5,3
    80004ff0:	00e03733          	snez	a4,a4
    80004ff4:	00e904a3          	sb	a4,9(s2)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80004ff8:	4007f793          	andi	a5,a5,1024
    80004ffc:	c791                	beqz	a5,80005008 <sys_open+0xb6>
    80004ffe:	04449703          	lh	a4,68(s1)
    80005002:	4789                	li	a5,2
    80005004:	08f70d63          	beq	a4,a5,8000509e <sys_open+0x14c>
    itrunc(ip);
  }

  iunlock(ip);
    80005008:	8526                	mv	a0,s1
    8000500a:	adafe0ef          	jal	800032e4 <iunlock>
  end_op();
    8000500e:	c7dfe0ef          	jal	80003c8a <end_op>

  return fd;
    80005012:	854e                	mv	a0,s3
    80005014:	74aa                	ld	s1,168(sp)
    80005016:	790a                	ld	s2,160(sp)
    80005018:	69ea                	ld	s3,152(sp)
}
    8000501a:	70ea                	ld	ra,184(sp)
    8000501c:	744a                	ld	s0,176(sp)
    8000501e:	6129                	addi	sp,sp,192
    80005020:	8082                	ret
      end_op();
    80005022:	c69fe0ef          	jal	80003c8a <end_op>
      return -1;
    80005026:	557d                	li	a0,-1
    80005028:	74aa                	ld	s1,168(sp)
    8000502a:	bfc5                	j	8000501a <sys_open+0xc8>
    if((ip = namei(path)) == 0){
    8000502c:	f5040513          	addi	a0,s0,-176
    80005030:	a1dfe0ef          	jal	80003a4c <namei>
    80005034:	84aa                	mv	s1,a0
    80005036:	c11d                	beqz	a0,8000505c <sys_open+0x10a>
    ilock(ip);
    80005038:	9fefe0ef          	jal	80003236 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    8000503c:	04449703          	lh	a4,68(s1)
    80005040:	4785                	li	a5,1
    80005042:	f4f71de3          	bne	a4,a5,80004f9c <sys_open+0x4a>
    80005046:	f4c42783          	lw	a5,-180(s0)
    8000504a:	d3bd                	beqz	a5,80004fb0 <sys_open+0x5e>
      iunlockput(ip);
    8000504c:	8526                	mv	a0,s1
    8000504e:	bf2fe0ef          	jal	80003440 <iunlockput>
      end_op();
    80005052:	c39fe0ef          	jal	80003c8a <end_op>
      return -1;
    80005056:	557d                	li	a0,-1
    80005058:	74aa                	ld	s1,168(sp)
    8000505a:	b7c1                	j	8000501a <sys_open+0xc8>
      end_op();
    8000505c:	c2ffe0ef          	jal	80003c8a <end_op>
      return -1;
    80005060:	557d                	li	a0,-1
    80005062:	74aa                	ld	s1,168(sp)
    80005064:	bf5d                	j	8000501a <sys_open+0xc8>
    iunlockput(ip);
    80005066:	8526                	mv	a0,s1
    80005068:	bd8fe0ef          	jal	80003440 <iunlockput>
    end_op();
    8000506c:	c1ffe0ef          	jal	80003c8a <end_op>
    return -1;
    80005070:	557d                	li	a0,-1
    80005072:	74aa                	ld	s1,168(sp)
    80005074:	b75d                	j	8000501a <sys_open+0xc8>
      fileclose(f);
    80005076:	854a                	mv	a0,s2
    80005078:	fb5fe0ef          	jal	8000402c <fileclose>
    8000507c:	69ea                	ld	s3,152(sp)
    iunlockput(ip);
    8000507e:	8526                	mv	a0,s1
    80005080:	bc0fe0ef          	jal	80003440 <iunlockput>
    end_op();
    80005084:	c07fe0ef          	jal	80003c8a <end_op>
    return -1;
    80005088:	557d                	li	a0,-1
    8000508a:	74aa                	ld	s1,168(sp)
    8000508c:	790a                	ld	s2,160(sp)
    8000508e:	b771                	j	8000501a <sys_open+0xc8>
    f->type = FD_DEVICE;
    80005090:	00f92023          	sw	a5,0(s2)
    f->major = ip->major;
    80005094:	04649783          	lh	a5,70(s1)
    80005098:	02f91223          	sh	a5,36(s2)
    8000509c:	bf3d                	j	80004fda <sys_open+0x88>
    itrunc(ip);
    8000509e:	8526                	mv	a0,s1
    800050a0:	a84fe0ef          	jal	80003324 <itrunc>
    800050a4:	b795                	j	80005008 <sys_open+0xb6>

00000000800050a6 <sys_mkdir>:

uint64
sys_mkdir(void)
{
    800050a6:	7175                	addi	sp,sp,-144
    800050a8:	e506                	sd	ra,136(sp)
    800050aa:	e122                	sd	s0,128(sp)
    800050ac:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    800050ae:	b73fe0ef          	jal	80003c20 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    800050b2:	08000613          	li	a2,128
    800050b6:	f7040593          	addi	a1,s0,-144
    800050ba:	4501                	li	a0,0
    800050bc:	facfd0ef          	jal	80002868 <argstr>
    800050c0:	02054363          	bltz	a0,800050e6 <sys_mkdir+0x40>
    800050c4:	4681                	li	a3,0
    800050c6:	4601                	li	a2,0
    800050c8:	4585                	li	a1,1
    800050ca:	f7040513          	addi	a0,s0,-144
    800050ce:	96fff0ef          	jal	80004a3c <create>
    800050d2:	c911                	beqz	a0,800050e6 <sys_mkdir+0x40>
    end_op();
    return -1;
  }
  iunlockput(ip);
    800050d4:	b6cfe0ef          	jal	80003440 <iunlockput>
  end_op();
    800050d8:	bb3fe0ef          	jal	80003c8a <end_op>
  return 0;
    800050dc:	4501                	li	a0,0
}
    800050de:	60aa                	ld	ra,136(sp)
    800050e0:	640a                	ld	s0,128(sp)
    800050e2:	6149                	addi	sp,sp,144
    800050e4:	8082                	ret
    end_op();
    800050e6:	ba5fe0ef          	jal	80003c8a <end_op>
    return -1;
    800050ea:	557d                	li	a0,-1
    800050ec:	bfcd                	j	800050de <sys_mkdir+0x38>

00000000800050ee <sys_mknod>:

uint64
sys_mknod(void)
{
    800050ee:	7135                	addi	sp,sp,-160
    800050f0:	ed06                	sd	ra,152(sp)
    800050f2:	e922                	sd	s0,144(sp)
    800050f4:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    800050f6:	b2bfe0ef          	jal	80003c20 <begin_op>
  argint(1, &major);
    800050fa:	f6c40593          	addi	a1,s0,-148
    800050fe:	4505                	li	a0,1
    80005100:	f30fd0ef          	jal	80002830 <argint>
  argint(2, &minor);
    80005104:	f6840593          	addi	a1,s0,-152
    80005108:	4509                	li	a0,2
    8000510a:	f26fd0ef          	jal	80002830 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    8000510e:	08000613          	li	a2,128
    80005112:	f7040593          	addi	a1,s0,-144
    80005116:	4501                	li	a0,0
    80005118:	f50fd0ef          	jal	80002868 <argstr>
    8000511c:	02054563          	bltz	a0,80005146 <sys_mknod+0x58>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005120:	f6841683          	lh	a3,-152(s0)
    80005124:	f6c41603          	lh	a2,-148(s0)
    80005128:	458d                	li	a1,3
    8000512a:	f7040513          	addi	a0,s0,-144
    8000512e:	90fff0ef          	jal	80004a3c <create>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005132:	c911                	beqz	a0,80005146 <sys_mknod+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005134:	b0cfe0ef          	jal	80003440 <iunlockput>
  end_op();
    80005138:	b53fe0ef          	jal	80003c8a <end_op>
  return 0;
    8000513c:	4501                	li	a0,0
}
    8000513e:	60ea                	ld	ra,152(sp)
    80005140:	644a                	ld	s0,144(sp)
    80005142:	610d                	addi	sp,sp,160
    80005144:	8082                	ret
    end_op();
    80005146:	b45fe0ef          	jal	80003c8a <end_op>
    return -1;
    8000514a:	557d                	li	a0,-1
    8000514c:	bfcd                	j	8000513e <sys_mknod+0x50>

000000008000514e <sys_chdir>:

uint64
sys_chdir(void)
{
    8000514e:	7135                	addi	sp,sp,-160
    80005150:	ed06                	sd	ra,152(sp)
    80005152:	e922                	sd	s0,144(sp)
    80005154:	e14a                	sd	s2,128(sp)
    80005156:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005158:	f76fc0ef          	jal	800018ce <myproc>
    8000515c:	892a                	mv	s2,a0
  
  begin_op();
    8000515e:	ac3fe0ef          	jal	80003c20 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    80005162:	08000613          	li	a2,128
    80005166:	f6040593          	addi	a1,s0,-160
    8000516a:	4501                	li	a0,0
    8000516c:	efcfd0ef          	jal	80002868 <argstr>
    80005170:	04054363          	bltz	a0,800051b6 <sys_chdir+0x68>
    80005174:	e526                	sd	s1,136(sp)
    80005176:	f6040513          	addi	a0,s0,-160
    8000517a:	8d3fe0ef          	jal	80003a4c <namei>
    8000517e:	84aa                	mv	s1,a0
    80005180:	c915                	beqz	a0,800051b4 <sys_chdir+0x66>
    end_op();
    return -1;
  }
  ilock(ip);
    80005182:	8b4fe0ef          	jal	80003236 <ilock>
  if(ip->type != T_DIR){
    80005186:	04449703          	lh	a4,68(s1)
    8000518a:	4785                	li	a5,1
    8000518c:	02f71963          	bne	a4,a5,800051be <sys_chdir+0x70>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    80005190:	8526                	mv	a0,s1
    80005192:	952fe0ef          	jal	800032e4 <iunlock>
  iput(p->cwd);
    80005196:	15093503          	ld	a0,336(s2)
    8000519a:	a1efe0ef          	jal	800033b8 <iput>
  end_op();
    8000519e:	aedfe0ef          	jal	80003c8a <end_op>
  p->cwd = ip;
    800051a2:	14993823          	sd	s1,336(s2)
  return 0;
    800051a6:	4501                	li	a0,0
    800051a8:	64aa                	ld	s1,136(sp)
}
    800051aa:	60ea                	ld	ra,152(sp)
    800051ac:	644a                	ld	s0,144(sp)
    800051ae:	690a                	ld	s2,128(sp)
    800051b0:	610d                	addi	sp,sp,160
    800051b2:	8082                	ret
    800051b4:	64aa                	ld	s1,136(sp)
    end_op();
    800051b6:	ad5fe0ef          	jal	80003c8a <end_op>
    return -1;
    800051ba:	557d                	li	a0,-1
    800051bc:	b7fd                	j	800051aa <sys_chdir+0x5c>
    iunlockput(ip);
    800051be:	8526                	mv	a0,s1
    800051c0:	a80fe0ef          	jal	80003440 <iunlockput>
    end_op();
    800051c4:	ac7fe0ef          	jal	80003c8a <end_op>
    return -1;
    800051c8:	557d                	li	a0,-1
    800051ca:	64aa                	ld	s1,136(sp)
    800051cc:	bff9                	j	800051aa <sys_chdir+0x5c>

00000000800051ce <sys_exec>:

uint64
sys_exec(void)
{
    800051ce:	7121                	addi	sp,sp,-448
    800051d0:	ff06                	sd	ra,440(sp)
    800051d2:	fb22                	sd	s0,432(sp)
    800051d4:	0380                	addi	s0,sp,448
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  argaddr(1, &uargv);
    800051d6:	e4840593          	addi	a1,s0,-440
    800051da:	4505                	li	a0,1
    800051dc:	e70fd0ef          	jal	8000284c <argaddr>
  if(argstr(0, path, MAXPATH) < 0) {
    800051e0:	08000613          	li	a2,128
    800051e4:	f5040593          	addi	a1,s0,-176
    800051e8:	4501                	li	a0,0
    800051ea:	e7efd0ef          	jal	80002868 <argstr>
    800051ee:	87aa                	mv	a5,a0
    return -1;
    800051f0:	557d                	li	a0,-1
  if(argstr(0, path, MAXPATH) < 0) {
    800051f2:	0c07c463          	bltz	a5,800052ba <sys_exec+0xec>
    800051f6:	f726                	sd	s1,424(sp)
    800051f8:	f34a                	sd	s2,416(sp)
    800051fa:	ef4e                	sd	s3,408(sp)
    800051fc:	eb52                	sd	s4,400(sp)
  }
  memset(argv, 0, sizeof(argv));
    800051fe:	10000613          	li	a2,256
    80005202:	4581                	li	a1,0
    80005204:	e5040513          	addi	a0,s0,-432
    80005208:	a9bfb0ef          	jal	80000ca2 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    8000520c:	e5040493          	addi	s1,s0,-432
  memset(argv, 0, sizeof(argv));
    80005210:	89a6                	mv	s3,s1
    80005212:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005214:	02000a13          	li	s4,32
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005218:	00391513          	slli	a0,s2,0x3
    8000521c:	e4040593          	addi	a1,s0,-448
    80005220:	e4843783          	ld	a5,-440(s0)
    80005224:	953e                	add	a0,a0,a5
    80005226:	d80fd0ef          	jal	800027a6 <fetchaddr>
    8000522a:	02054663          	bltz	a0,80005256 <sys_exec+0x88>
      goto bad;
    }
    if(uarg == 0){
    8000522e:	e4043783          	ld	a5,-448(s0)
    80005232:	c3a9                	beqz	a5,80005274 <sys_exec+0xa6>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005234:	8cbfb0ef          	jal	80000afe <kalloc>
    80005238:	85aa                	mv	a1,a0
    8000523a:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    8000523e:	cd01                	beqz	a0,80005256 <sys_exec+0x88>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005240:	6605                	lui	a2,0x1
    80005242:	e4043503          	ld	a0,-448(s0)
    80005246:	daafd0ef          	jal	800027f0 <fetchstr>
    8000524a:	00054663          	bltz	a0,80005256 <sys_exec+0x88>
    if(i >= NELEM(argv)){
    8000524e:	0905                	addi	s2,s2,1
    80005250:	09a1                	addi	s3,s3,8
    80005252:	fd4913e3          	bne	s2,s4,80005218 <sys_exec+0x4a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005256:	f5040913          	addi	s2,s0,-176
    8000525a:	6088                	ld	a0,0(s1)
    8000525c:	c931                	beqz	a0,800052b0 <sys_exec+0xe2>
    kfree(argv[i]);
    8000525e:	fbefb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005262:	04a1                	addi	s1,s1,8
    80005264:	ff249be3          	bne	s1,s2,8000525a <sys_exec+0x8c>
  return -1;
    80005268:	557d                	li	a0,-1
    8000526a:	74ba                	ld	s1,424(sp)
    8000526c:	791a                	ld	s2,416(sp)
    8000526e:	69fa                	ld	s3,408(sp)
    80005270:	6a5a                	ld	s4,400(sp)
    80005272:	a0a1                	j	800052ba <sys_exec+0xec>
      argv[i] = 0;
    80005274:	0009079b          	sext.w	a5,s2
    80005278:	078e                	slli	a5,a5,0x3
    8000527a:	fd078793          	addi	a5,a5,-48
    8000527e:	97a2                	add	a5,a5,s0
    80005280:	e807b023          	sd	zero,-384(a5)
  int ret = kexec(path, argv);
    80005284:	e5040593          	addi	a1,s0,-432
    80005288:	f5040513          	addi	a0,s0,-176
    8000528c:	ba8ff0ef          	jal	80004634 <kexec>
    80005290:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005292:	f5040993          	addi	s3,s0,-176
    80005296:	6088                	ld	a0,0(s1)
    80005298:	c511                	beqz	a0,800052a4 <sys_exec+0xd6>
    kfree(argv[i]);
    8000529a:	f82fb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000529e:	04a1                	addi	s1,s1,8
    800052a0:	ff349be3          	bne	s1,s3,80005296 <sys_exec+0xc8>
  return ret;
    800052a4:	854a                	mv	a0,s2
    800052a6:	74ba                	ld	s1,424(sp)
    800052a8:	791a                	ld	s2,416(sp)
    800052aa:	69fa                	ld	s3,408(sp)
    800052ac:	6a5a                	ld	s4,400(sp)
    800052ae:	a031                	j	800052ba <sys_exec+0xec>
  return -1;
    800052b0:	557d                	li	a0,-1
    800052b2:	74ba                	ld	s1,424(sp)
    800052b4:	791a                	ld	s2,416(sp)
    800052b6:	69fa                	ld	s3,408(sp)
    800052b8:	6a5a                	ld	s4,400(sp)
}
    800052ba:	70fa                	ld	ra,440(sp)
    800052bc:	745a                	ld	s0,432(sp)
    800052be:	6139                	addi	sp,sp,448
    800052c0:	8082                	ret

00000000800052c2 <sys_pipe>:

uint64
sys_pipe(void)
{
    800052c2:	7139                	addi	sp,sp,-64
    800052c4:	fc06                	sd	ra,56(sp)
    800052c6:	f822                	sd	s0,48(sp)
    800052c8:	f426                	sd	s1,40(sp)
    800052ca:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    800052cc:	e02fc0ef          	jal	800018ce <myproc>
    800052d0:	84aa                	mv	s1,a0

  argaddr(0, &fdarray);
    800052d2:	fd840593          	addi	a1,s0,-40
    800052d6:	4501                	li	a0,0
    800052d8:	d74fd0ef          	jal	8000284c <argaddr>
  if(pipealloc(&rf, &wf) < 0)
    800052dc:	fc840593          	addi	a1,s0,-56
    800052e0:	fd040513          	addi	a0,s0,-48
    800052e4:	852ff0ef          	jal	80004336 <pipealloc>
    return -1;
    800052e8:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    800052ea:	0a054463          	bltz	a0,80005392 <sys_pipe+0xd0>
  fd0 = -1;
    800052ee:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    800052f2:	fd043503          	ld	a0,-48(s0)
    800052f6:	f08ff0ef          	jal	800049fe <fdalloc>
    800052fa:	fca42223          	sw	a0,-60(s0)
    800052fe:	08054163          	bltz	a0,80005380 <sys_pipe+0xbe>
    80005302:	fc843503          	ld	a0,-56(s0)
    80005306:	ef8ff0ef          	jal	800049fe <fdalloc>
    8000530a:	fca42023          	sw	a0,-64(s0)
    8000530e:	06054063          	bltz	a0,8000536e <sys_pipe+0xac>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    80005312:	4691                	li	a3,4
    80005314:	fc440613          	addi	a2,s0,-60
    80005318:	fd843583          	ld	a1,-40(s0)
    8000531c:	68a8                	ld	a0,80(s1)
    8000531e:	ac4fc0ef          	jal	800015e2 <copyout>
    80005322:	00054e63          	bltz	a0,8000533e <sys_pipe+0x7c>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005326:	4691                	li	a3,4
    80005328:	fc040613          	addi	a2,s0,-64
    8000532c:	fd843583          	ld	a1,-40(s0)
    80005330:	0591                	addi	a1,a1,4
    80005332:	68a8                	ld	a0,80(s1)
    80005334:	aaefc0ef          	jal	800015e2 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    80005338:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    8000533a:	04055c63          	bgez	a0,80005392 <sys_pipe+0xd0>
    p->ofile[fd0] = 0;
    8000533e:	fc442783          	lw	a5,-60(s0)
    80005342:	07e9                	addi	a5,a5,26
    80005344:	078e                	slli	a5,a5,0x3
    80005346:	97a6                	add	a5,a5,s1
    80005348:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    8000534c:	fc042783          	lw	a5,-64(s0)
    80005350:	07e9                	addi	a5,a5,26
    80005352:	078e                	slli	a5,a5,0x3
    80005354:	94be                	add	s1,s1,a5
    80005356:	0004b023          	sd	zero,0(s1)
    fileclose(rf);
    8000535a:	fd043503          	ld	a0,-48(s0)
    8000535e:	ccffe0ef          	jal	8000402c <fileclose>
    fileclose(wf);
    80005362:	fc843503          	ld	a0,-56(s0)
    80005366:	cc7fe0ef          	jal	8000402c <fileclose>
    return -1;
    8000536a:	57fd                	li	a5,-1
    8000536c:	a01d                	j	80005392 <sys_pipe+0xd0>
    if(fd0 >= 0)
    8000536e:	fc442783          	lw	a5,-60(s0)
    80005372:	0007c763          	bltz	a5,80005380 <sys_pipe+0xbe>
      p->ofile[fd0] = 0;
    80005376:	07e9                	addi	a5,a5,26
    80005378:	078e                	slli	a5,a5,0x3
    8000537a:	97a6                	add	a5,a5,s1
    8000537c:	0007b023          	sd	zero,0(a5)
    fileclose(rf);
    80005380:	fd043503          	ld	a0,-48(s0)
    80005384:	ca9fe0ef          	jal	8000402c <fileclose>
    fileclose(wf);
    80005388:	fc843503          	ld	a0,-56(s0)
    8000538c:	ca1fe0ef          	jal	8000402c <fileclose>
    return -1;
    80005390:	57fd                	li	a5,-1
}
    80005392:	853e                	mv	a0,a5
    80005394:	70e2                	ld	ra,56(sp)
    80005396:	7442                	ld	s0,48(sp)
    80005398:	74a2                	ld	s1,40(sp)
    8000539a:	6121                	addi	sp,sp,64
    8000539c:	8082                	ret
	...

00000000800053a0 <kernelvec>:
.globl kerneltrap
.globl kernelvec
.align 4
kernelvec:
        # make room to save registers.
        addi sp, sp, -256
    800053a0:	7111                	addi	sp,sp,-256

        # save caller-saved registers.
        sd ra, 0(sp)
    800053a2:	e006                	sd	ra,0(sp)
        # sd sp, 8(sp)
        sd gp, 16(sp)
    800053a4:	e80e                	sd	gp,16(sp)
        sd tp, 24(sp)
    800053a6:	ec12                	sd	tp,24(sp)
        sd t0, 32(sp)
    800053a8:	f016                	sd	t0,32(sp)
        sd t1, 40(sp)
    800053aa:	f41a                	sd	t1,40(sp)
        sd t2, 48(sp)
    800053ac:	f81e                	sd	t2,48(sp)
        sd a0, 72(sp)
    800053ae:	e4aa                	sd	a0,72(sp)
        sd a1, 80(sp)
    800053b0:	e8ae                	sd	a1,80(sp)
        sd a2, 88(sp)
    800053b2:	ecb2                	sd	a2,88(sp)
        sd a3, 96(sp)
    800053b4:	f0b6                	sd	a3,96(sp)
        sd a4, 104(sp)
    800053b6:	f4ba                	sd	a4,104(sp)
        sd a5, 112(sp)
    800053b8:	f8be                	sd	a5,112(sp)
        sd a6, 120(sp)
    800053ba:	fcc2                	sd	a6,120(sp)
        sd a7, 128(sp)
    800053bc:	e146                	sd	a7,128(sp)
        sd t3, 216(sp)
    800053be:	edf2                	sd	t3,216(sp)
        sd t4, 224(sp)
    800053c0:	f1f6                	sd	t4,224(sp)
        sd t5, 232(sp)
    800053c2:	f5fa                	sd	t5,232(sp)
        sd t6, 240(sp)
    800053c4:	f9fe                	sd	t6,240(sp)

        # call the C trap handler in trap.c
        call kerneltrap
    800053c6:	af0fd0ef          	jal	800026b6 <kerneltrap>

        # restore registers.
        ld ra, 0(sp)
    800053ca:	6082                	ld	ra,0(sp)
        # ld sp, 8(sp)
        ld gp, 16(sp)
    800053cc:	61c2                	ld	gp,16(sp)
        # not tp (contains hartid), in case we moved CPUs
        ld t0, 32(sp)
    800053ce:	7282                	ld	t0,32(sp)
        ld t1, 40(sp)
    800053d0:	7322                	ld	t1,40(sp)
        ld t2, 48(sp)
    800053d2:	73c2                	ld	t2,48(sp)
        ld a0, 72(sp)
    800053d4:	6526                	ld	a0,72(sp)
        ld a1, 80(sp)
    800053d6:	65c6                	ld	a1,80(sp)
        ld a2, 88(sp)
    800053d8:	6666                	ld	a2,88(sp)
        ld a3, 96(sp)
    800053da:	7686                	ld	a3,96(sp)
        ld a4, 104(sp)
    800053dc:	7726                	ld	a4,104(sp)
        ld a5, 112(sp)
    800053de:	77c6                	ld	a5,112(sp)
        ld a6, 120(sp)
    800053e0:	7866                	ld	a6,120(sp)
        ld a7, 128(sp)
    800053e2:	688a                	ld	a7,128(sp)
        ld t3, 216(sp)
    800053e4:	6e6e                	ld	t3,216(sp)
        ld t4, 224(sp)
    800053e6:	7e8e                	ld	t4,224(sp)
        ld t5, 232(sp)
    800053e8:	7f2e                	ld	t5,232(sp)
        ld t6, 240(sp)
    800053ea:	7fce                	ld	t6,240(sp)

        addi sp, sp, 256
    800053ec:	6111                	addi	sp,sp,256

        # return to whatever we were doing in the kernel.
        sret
    800053ee:	10200073          	sret
	...

00000000800053fe <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    800053fe:	1141                	addi	sp,sp,-16
    80005400:	e422                	sd	s0,8(sp)
    80005402:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80005404:	0c0007b7          	lui	a5,0xc000
    80005408:	4705                	li	a4,1
    8000540a:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    8000540c:	0c0007b7          	lui	a5,0xc000
    80005410:	c3d8                	sw	a4,4(a5)
}
    80005412:	6422                	ld	s0,8(sp)
    80005414:	0141                	addi	sp,sp,16
    80005416:	8082                	ret

0000000080005418 <plicinithart>:

void
plicinithart(void)
{
    80005418:	1141                	addi	sp,sp,-16
    8000541a:	e406                	sd	ra,8(sp)
    8000541c:	e022                	sd	s0,0(sp)
    8000541e:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005420:	c82fc0ef          	jal	800018a2 <cpuid>
  
  // set enable bits for this hart's S-mode
  // for the uart and virtio disk.
  *(uint32*)PLIC_SENABLE(hart) = (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80005424:	0085171b          	slliw	a4,a0,0x8
    80005428:	0c0027b7          	lui	a5,0xc002
    8000542c:	97ba                	add	a5,a5,a4
    8000542e:	40200713          	li	a4,1026
    80005432:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80005436:	00d5151b          	slliw	a0,a0,0xd
    8000543a:	0c2017b7          	lui	a5,0xc201
    8000543e:	97aa                	add	a5,a5,a0
    80005440:	0007a023          	sw	zero,0(a5) # c201000 <_entry-0x73dff000>
}
    80005444:	60a2                	ld	ra,8(sp)
    80005446:	6402                	ld	s0,0(sp)
    80005448:	0141                	addi	sp,sp,16
    8000544a:	8082                	ret

000000008000544c <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    8000544c:	1141                	addi	sp,sp,-16
    8000544e:	e406                	sd	ra,8(sp)
    80005450:	e022                	sd	s0,0(sp)
    80005452:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005454:	c4efc0ef          	jal	800018a2 <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    80005458:	00d5151b          	slliw	a0,a0,0xd
    8000545c:	0c2017b7          	lui	a5,0xc201
    80005460:	97aa                	add	a5,a5,a0
  return irq;
}
    80005462:	43c8                	lw	a0,4(a5)
    80005464:	60a2                	ld	ra,8(sp)
    80005466:	6402                	ld	s0,0(sp)
    80005468:	0141                	addi	sp,sp,16
    8000546a:	8082                	ret

000000008000546c <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    8000546c:	1101                	addi	sp,sp,-32
    8000546e:	ec06                	sd	ra,24(sp)
    80005470:	e822                	sd	s0,16(sp)
    80005472:	e426                	sd	s1,8(sp)
    80005474:	1000                	addi	s0,sp,32
    80005476:	84aa                	mv	s1,a0
  int hart = cpuid();
    80005478:	c2afc0ef          	jal	800018a2 <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    8000547c:	00d5151b          	slliw	a0,a0,0xd
    80005480:	0c2017b7          	lui	a5,0xc201
    80005484:	97aa                	add	a5,a5,a0
    80005486:	c3c4                	sw	s1,4(a5)
}
    80005488:	60e2                	ld	ra,24(sp)
    8000548a:	6442                	ld	s0,16(sp)
    8000548c:	64a2                	ld	s1,8(sp)
    8000548e:	6105                	addi	sp,sp,32
    80005490:	8082                	ret

0000000080005492 <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    80005492:	1141                	addi	sp,sp,-16
    80005494:	e406                	sd	ra,8(sp)
    80005496:	e022                	sd	s0,0(sp)
    80005498:	0800                	addi	s0,sp,16
  if(i >= NUM)
    8000549a:	479d                	li	a5,7
    8000549c:	04a7ca63          	blt	a5,a0,800054f0 <free_desc+0x5e>
    panic("free_desc 1");
  if(disk.free[i])
    800054a0:	0001c797          	auipc	a5,0x1c
    800054a4:	98878793          	addi	a5,a5,-1656 # 80020e28 <disk>
    800054a8:	97aa                	add	a5,a5,a0
    800054aa:	0187c783          	lbu	a5,24(a5)
    800054ae:	e7b9                	bnez	a5,800054fc <free_desc+0x6a>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    800054b0:	00451693          	slli	a3,a0,0x4
    800054b4:	0001c797          	auipc	a5,0x1c
    800054b8:	97478793          	addi	a5,a5,-1676 # 80020e28 <disk>
    800054bc:	6398                	ld	a4,0(a5)
    800054be:	9736                	add	a4,a4,a3
    800054c0:	00073023          	sd	zero,0(a4)
  disk.desc[i].len = 0;
    800054c4:	6398                	ld	a4,0(a5)
    800054c6:	9736                	add	a4,a4,a3
    800054c8:	00072423          	sw	zero,8(a4)
  disk.desc[i].flags = 0;
    800054cc:	00071623          	sh	zero,12(a4)
  disk.desc[i].next = 0;
    800054d0:	00071723          	sh	zero,14(a4)
  disk.free[i] = 1;
    800054d4:	97aa                	add	a5,a5,a0
    800054d6:	4705                	li	a4,1
    800054d8:	00e78c23          	sb	a4,24(a5)
  wakeup(&disk.free[0]);
    800054dc:	0001c517          	auipc	a0,0x1c
    800054e0:	96450513          	addi	a0,a0,-1692 # 80020e40 <disk+0x18>
    800054e4:	a4dfc0ef          	jal	80001f30 <wakeup>
}
    800054e8:	60a2                	ld	ra,8(sp)
    800054ea:	6402                	ld	s0,0(sp)
    800054ec:	0141                	addi	sp,sp,16
    800054ee:	8082                	ret
    panic("free_desc 1");
    800054f0:	00002517          	auipc	a0,0x2
    800054f4:	12050513          	addi	a0,a0,288 # 80007610 <etext+0x610>
    800054f8:	ae8fb0ef          	jal	800007e0 <panic>
    panic("free_desc 2");
    800054fc:	00002517          	auipc	a0,0x2
    80005500:	12450513          	addi	a0,a0,292 # 80007620 <etext+0x620>
    80005504:	adcfb0ef          	jal	800007e0 <panic>

0000000080005508 <virtio_disk_init>:
{
    80005508:	1101                	addi	sp,sp,-32
    8000550a:	ec06                	sd	ra,24(sp)
    8000550c:	e822                	sd	s0,16(sp)
    8000550e:	e426                	sd	s1,8(sp)
    80005510:	e04a                	sd	s2,0(sp)
    80005512:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    80005514:	00002597          	auipc	a1,0x2
    80005518:	11c58593          	addi	a1,a1,284 # 80007630 <etext+0x630>
    8000551c:	0001c517          	auipc	a0,0x1c
    80005520:	a3450513          	addi	a0,a0,-1484 # 80020f50 <disk+0x128>
    80005524:	e2afb0ef          	jal	80000b4e <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005528:	100017b7          	lui	a5,0x10001
    8000552c:	4398                	lw	a4,0(a5)
    8000552e:	2701                	sext.w	a4,a4
    80005530:	747277b7          	lui	a5,0x74727
    80005534:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80005538:	18f71063          	bne	a4,a5,800056b8 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    8000553c:	100017b7          	lui	a5,0x10001
    80005540:	0791                	addi	a5,a5,4 # 10001004 <_entry-0x6fffeffc>
    80005542:	439c                	lw	a5,0(a5)
    80005544:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005546:	4709                	li	a4,2
    80005548:	16e79863          	bne	a5,a4,800056b8 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000554c:	100017b7          	lui	a5,0x10001
    80005550:	07a1                	addi	a5,a5,8 # 10001008 <_entry-0x6fffeff8>
    80005552:	439c                	lw	a5,0(a5)
    80005554:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    80005556:	16e79163          	bne	a5,a4,800056b8 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    8000555a:	100017b7          	lui	a5,0x10001
    8000555e:	47d8                	lw	a4,12(a5)
    80005560:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80005562:	554d47b7          	lui	a5,0x554d4
    80005566:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    8000556a:	14f71763          	bne	a4,a5,800056b8 <virtio_disk_init+0x1b0>
  *R(VIRTIO_MMIO_STATUS) = status;
    8000556e:	100017b7          	lui	a5,0x10001
    80005572:	0607a823          	sw	zero,112(a5) # 10001070 <_entry-0x6fffef90>
  *R(VIRTIO_MMIO_STATUS) = status;
    80005576:	4705                	li	a4,1
    80005578:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    8000557a:	470d                	li	a4,3
    8000557c:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    8000557e:	10001737          	lui	a4,0x10001
    80005582:	4b14                	lw	a3,16(a4)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    80005584:	c7ffe737          	lui	a4,0xc7ffe
    80005588:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fdd7f7>
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    8000558c:	8ef9                	and	a3,a3,a4
    8000558e:	10001737          	lui	a4,0x10001
    80005592:	d314                	sw	a3,32(a4)
  *R(VIRTIO_MMIO_STATUS) = status;
    80005594:	472d                	li	a4,11
    80005596:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80005598:	07078793          	addi	a5,a5,112
  status = *R(VIRTIO_MMIO_STATUS);
    8000559c:	439c                	lw	a5,0(a5)
    8000559e:	0007891b          	sext.w	s2,a5
  if(!(status & VIRTIO_CONFIG_S_FEATURES_OK))
    800055a2:	8ba1                	andi	a5,a5,8
    800055a4:	12078063          	beqz	a5,800056c4 <virtio_disk_init+0x1bc>
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    800055a8:	100017b7          	lui	a5,0x10001
    800055ac:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  if(*R(VIRTIO_MMIO_QUEUE_READY))
    800055b0:	100017b7          	lui	a5,0x10001
    800055b4:	04478793          	addi	a5,a5,68 # 10001044 <_entry-0x6fffefbc>
    800055b8:	439c                	lw	a5,0(a5)
    800055ba:	2781                	sext.w	a5,a5
    800055bc:	10079a63          	bnez	a5,800056d0 <virtio_disk_init+0x1c8>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    800055c0:	100017b7          	lui	a5,0x10001
    800055c4:	03478793          	addi	a5,a5,52 # 10001034 <_entry-0x6fffefcc>
    800055c8:	439c                	lw	a5,0(a5)
    800055ca:	2781                	sext.w	a5,a5
  if(max == 0)
    800055cc:	10078863          	beqz	a5,800056dc <virtio_disk_init+0x1d4>
  if(max < NUM)
    800055d0:	471d                	li	a4,7
    800055d2:	10f77b63          	bgeu	a4,a5,800056e8 <virtio_disk_init+0x1e0>
  disk.desc = kalloc();
    800055d6:	d28fb0ef          	jal	80000afe <kalloc>
    800055da:	0001c497          	auipc	s1,0x1c
    800055de:	84e48493          	addi	s1,s1,-1970 # 80020e28 <disk>
    800055e2:	e088                	sd	a0,0(s1)
  disk.avail = kalloc();
    800055e4:	d1afb0ef          	jal	80000afe <kalloc>
    800055e8:	e488                	sd	a0,8(s1)
  disk.used = kalloc();
    800055ea:	d14fb0ef          	jal	80000afe <kalloc>
    800055ee:	87aa                	mv	a5,a0
    800055f0:	e888                	sd	a0,16(s1)
  if(!disk.desc || !disk.avail || !disk.used)
    800055f2:	6088                	ld	a0,0(s1)
    800055f4:	10050063          	beqz	a0,800056f4 <virtio_disk_init+0x1ec>
    800055f8:	0001c717          	auipc	a4,0x1c
    800055fc:	83873703          	ld	a4,-1992(a4) # 80020e30 <disk+0x8>
    80005600:	0e070a63          	beqz	a4,800056f4 <virtio_disk_init+0x1ec>
    80005604:	0e078863          	beqz	a5,800056f4 <virtio_disk_init+0x1ec>
  memset(disk.desc, 0, PGSIZE);
    80005608:	6605                	lui	a2,0x1
    8000560a:	4581                	li	a1,0
    8000560c:	e96fb0ef          	jal	80000ca2 <memset>
  memset(disk.avail, 0, PGSIZE);
    80005610:	0001c497          	auipc	s1,0x1c
    80005614:	81848493          	addi	s1,s1,-2024 # 80020e28 <disk>
    80005618:	6605                	lui	a2,0x1
    8000561a:	4581                	li	a1,0
    8000561c:	6488                	ld	a0,8(s1)
    8000561e:	e84fb0ef          	jal	80000ca2 <memset>
  memset(disk.used, 0, PGSIZE);
    80005622:	6605                	lui	a2,0x1
    80005624:	4581                	li	a1,0
    80005626:	6888                	ld	a0,16(s1)
    80005628:	e7afb0ef          	jal	80000ca2 <memset>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    8000562c:	100017b7          	lui	a5,0x10001
    80005630:	4721                	li	a4,8
    80005632:	df98                	sw	a4,56(a5)
  *R(VIRTIO_MMIO_QUEUE_DESC_LOW) = (uint64)disk.desc;
    80005634:	4098                	lw	a4,0(s1)
    80005636:	100017b7          	lui	a5,0x10001
    8000563a:	08e7a023          	sw	a4,128(a5) # 10001080 <_entry-0x6fffef80>
  *R(VIRTIO_MMIO_QUEUE_DESC_HIGH) = (uint64)disk.desc >> 32;
    8000563e:	40d8                	lw	a4,4(s1)
    80005640:	100017b7          	lui	a5,0x10001
    80005644:	08e7a223          	sw	a4,132(a5) # 10001084 <_entry-0x6fffef7c>
  *R(VIRTIO_MMIO_DRIVER_DESC_LOW) = (uint64)disk.avail;
    80005648:	649c                	ld	a5,8(s1)
    8000564a:	0007869b          	sext.w	a3,a5
    8000564e:	10001737          	lui	a4,0x10001
    80005652:	08d72823          	sw	a3,144(a4) # 10001090 <_entry-0x6fffef70>
  *R(VIRTIO_MMIO_DRIVER_DESC_HIGH) = (uint64)disk.avail >> 32;
    80005656:	9781                	srai	a5,a5,0x20
    80005658:	10001737          	lui	a4,0x10001
    8000565c:	08f72a23          	sw	a5,148(a4) # 10001094 <_entry-0x6fffef6c>
  *R(VIRTIO_MMIO_DEVICE_DESC_LOW) = (uint64)disk.used;
    80005660:	689c                	ld	a5,16(s1)
    80005662:	0007869b          	sext.w	a3,a5
    80005666:	10001737          	lui	a4,0x10001
    8000566a:	0ad72023          	sw	a3,160(a4) # 100010a0 <_entry-0x6fffef60>
  *R(VIRTIO_MMIO_DEVICE_DESC_HIGH) = (uint64)disk.used >> 32;
    8000566e:	9781                	srai	a5,a5,0x20
    80005670:	10001737          	lui	a4,0x10001
    80005674:	0af72223          	sw	a5,164(a4) # 100010a4 <_entry-0x6fffef5c>
  *R(VIRTIO_MMIO_QUEUE_READY) = 0x1;
    80005678:	10001737          	lui	a4,0x10001
    8000567c:	4785                	li	a5,1
    8000567e:	c37c                	sw	a5,68(a4)
    disk.free[i] = 1;
    80005680:	00f48c23          	sb	a5,24(s1)
    80005684:	00f48ca3          	sb	a5,25(s1)
    80005688:	00f48d23          	sb	a5,26(s1)
    8000568c:	00f48da3          	sb	a5,27(s1)
    80005690:	00f48e23          	sb	a5,28(s1)
    80005694:	00f48ea3          	sb	a5,29(s1)
    80005698:	00f48f23          	sb	a5,30(s1)
    8000569c:	00f48fa3          	sb	a5,31(s1)
  status |= VIRTIO_CONFIG_S_DRIVER_OK;
    800056a0:	00496913          	ori	s2,s2,4
  *R(VIRTIO_MMIO_STATUS) = status;
    800056a4:	100017b7          	lui	a5,0x10001
    800056a8:	0727a823          	sw	s2,112(a5) # 10001070 <_entry-0x6fffef90>
}
    800056ac:	60e2                	ld	ra,24(sp)
    800056ae:	6442                	ld	s0,16(sp)
    800056b0:	64a2                	ld	s1,8(sp)
    800056b2:	6902                	ld	s2,0(sp)
    800056b4:	6105                	addi	sp,sp,32
    800056b6:	8082                	ret
    panic("could not find virtio disk");
    800056b8:	00002517          	auipc	a0,0x2
    800056bc:	f8850513          	addi	a0,a0,-120 # 80007640 <etext+0x640>
    800056c0:	920fb0ef          	jal	800007e0 <panic>
    panic("virtio disk FEATURES_OK unset");
    800056c4:	00002517          	auipc	a0,0x2
    800056c8:	f9c50513          	addi	a0,a0,-100 # 80007660 <etext+0x660>
    800056cc:	914fb0ef          	jal	800007e0 <panic>
    panic("virtio disk should not be ready");
    800056d0:	00002517          	auipc	a0,0x2
    800056d4:	fb050513          	addi	a0,a0,-80 # 80007680 <etext+0x680>
    800056d8:	908fb0ef          	jal	800007e0 <panic>
    panic("virtio disk has no queue 0");
    800056dc:	00002517          	auipc	a0,0x2
    800056e0:	fc450513          	addi	a0,a0,-60 # 800076a0 <etext+0x6a0>
    800056e4:	8fcfb0ef          	jal	800007e0 <panic>
    panic("virtio disk max queue too short");
    800056e8:	00002517          	auipc	a0,0x2
    800056ec:	fd850513          	addi	a0,a0,-40 # 800076c0 <etext+0x6c0>
    800056f0:	8f0fb0ef          	jal	800007e0 <panic>
    panic("virtio disk kalloc");
    800056f4:	00002517          	auipc	a0,0x2
    800056f8:	fec50513          	addi	a0,a0,-20 # 800076e0 <etext+0x6e0>
    800056fc:	8e4fb0ef          	jal	800007e0 <panic>

0000000080005700 <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    80005700:	7159                	addi	sp,sp,-112
    80005702:	f486                	sd	ra,104(sp)
    80005704:	f0a2                	sd	s0,96(sp)
    80005706:	eca6                	sd	s1,88(sp)
    80005708:	e8ca                	sd	s2,80(sp)
    8000570a:	e4ce                	sd	s3,72(sp)
    8000570c:	e0d2                	sd	s4,64(sp)
    8000570e:	fc56                	sd	s5,56(sp)
    80005710:	f85a                	sd	s6,48(sp)
    80005712:	f45e                	sd	s7,40(sp)
    80005714:	f062                	sd	s8,32(sp)
    80005716:	ec66                	sd	s9,24(sp)
    80005718:	1880                	addi	s0,sp,112
    8000571a:	8a2a                	mv	s4,a0
    8000571c:	8bae                	mv	s7,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    8000571e:	00c52c83          	lw	s9,12(a0)
    80005722:	001c9c9b          	slliw	s9,s9,0x1
    80005726:	1c82                	slli	s9,s9,0x20
    80005728:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    8000572c:	0001c517          	auipc	a0,0x1c
    80005730:	82450513          	addi	a0,a0,-2012 # 80020f50 <disk+0x128>
    80005734:	c9afb0ef          	jal	80000bce <acquire>
  for(int i = 0; i < 3; i++){
    80005738:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    8000573a:	44a1                	li	s1,8
      disk.free[i] = 0;
    8000573c:	0001bb17          	auipc	s6,0x1b
    80005740:	6ecb0b13          	addi	s6,s6,1772 # 80020e28 <disk>
  for(int i = 0; i < 3; i++){
    80005744:	4a8d                	li	s5,3
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    80005746:	0001cc17          	auipc	s8,0x1c
    8000574a:	80ac0c13          	addi	s8,s8,-2038 # 80020f50 <disk+0x128>
    8000574e:	a8b9                	j	800057ac <virtio_disk_rw+0xac>
      disk.free[i] = 0;
    80005750:	00fb0733          	add	a4,s6,a5
    80005754:	00070c23          	sb	zero,24(a4) # 10001018 <_entry-0x6fffefe8>
    idx[i] = alloc_desc();
    80005758:	c19c                	sw	a5,0(a1)
    if(idx[i] < 0){
    8000575a:	0207c563          	bltz	a5,80005784 <virtio_disk_rw+0x84>
  for(int i = 0; i < 3; i++){
    8000575e:	2905                	addiw	s2,s2,1
    80005760:	0611                	addi	a2,a2,4 # 1004 <_entry-0x7fffeffc>
    80005762:	05590963          	beq	s2,s5,800057b4 <virtio_disk_rw+0xb4>
    idx[i] = alloc_desc();
    80005766:	85b2                	mv	a1,a2
  for(int i = 0; i < NUM; i++){
    80005768:	0001b717          	auipc	a4,0x1b
    8000576c:	6c070713          	addi	a4,a4,1728 # 80020e28 <disk>
    80005770:	87ce                	mv	a5,s3
    if(disk.free[i]){
    80005772:	01874683          	lbu	a3,24(a4)
    80005776:	fee9                	bnez	a3,80005750 <virtio_disk_rw+0x50>
  for(int i = 0; i < NUM; i++){
    80005778:	2785                	addiw	a5,a5,1
    8000577a:	0705                	addi	a4,a4,1
    8000577c:	fe979be3          	bne	a5,s1,80005772 <virtio_disk_rw+0x72>
    idx[i] = alloc_desc();
    80005780:	57fd                	li	a5,-1
    80005782:	c19c                	sw	a5,0(a1)
      for(int j = 0; j < i; j++)
    80005784:	01205d63          	blez	s2,8000579e <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    80005788:	f9042503          	lw	a0,-112(s0)
    8000578c:	d07ff0ef          	jal	80005492 <free_desc>
      for(int j = 0; j < i; j++)
    80005790:	4785                	li	a5,1
    80005792:	0127d663          	bge	a5,s2,8000579e <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    80005796:	f9442503          	lw	a0,-108(s0)
    8000579a:	cf9ff0ef          	jal	80005492 <free_desc>
    sleep(&disk.free[0], &disk.vdisk_lock);
    8000579e:	85e2                	mv	a1,s8
    800057a0:	0001b517          	auipc	a0,0x1b
    800057a4:	6a050513          	addi	a0,a0,1696 # 80020e40 <disk+0x18>
    800057a8:	f3cfc0ef          	jal	80001ee4 <sleep>
  for(int i = 0; i < 3; i++){
    800057ac:	f9040613          	addi	a2,s0,-112
    800057b0:	894e                	mv	s2,s3
    800057b2:	bf55                	j	80005766 <virtio_disk_rw+0x66>
  }

  // format the three descriptors.
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    800057b4:	f9042503          	lw	a0,-112(s0)
    800057b8:	00451693          	slli	a3,a0,0x4

  if(write)
    800057bc:	0001b797          	auipc	a5,0x1b
    800057c0:	66c78793          	addi	a5,a5,1644 # 80020e28 <disk>
    800057c4:	00a50713          	addi	a4,a0,10
    800057c8:	0712                	slli	a4,a4,0x4
    800057ca:	973e                	add	a4,a4,a5
    800057cc:	01703633          	snez	a2,s7
    800057d0:	c710                	sw	a2,8(a4)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    800057d2:	00072623          	sw	zero,12(a4)
  buf0->sector = sector;
    800057d6:	01973823          	sd	s9,16(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    800057da:	6398                	ld	a4,0(a5)
    800057dc:	9736                	add	a4,a4,a3
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    800057de:	0a868613          	addi	a2,a3,168
    800057e2:	963e                	add	a2,a2,a5
  disk.desc[idx[0]].addr = (uint64) buf0;
    800057e4:	e310                	sd	a2,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    800057e6:	6390                	ld	a2,0(a5)
    800057e8:	00d605b3          	add	a1,a2,a3
    800057ec:	4741                	li	a4,16
    800057ee:	c598                	sw	a4,8(a1)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    800057f0:	4805                	li	a6,1
    800057f2:	01059623          	sh	a6,12(a1)
  disk.desc[idx[0]].next = idx[1];
    800057f6:	f9442703          	lw	a4,-108(s0)
    800057fa:	00e59723          	sh	a4,14(a1)

  disk.desc[idx[1]].addr = (uint64) b->data;
    800057fe:	0712                	slli	a4,a4,0x4
    80005800:	963a                	add	a2,a2,a4
    80005802:	058a0593          	addi	a1,s4,88
    80005806:	e20c                	sd	a1,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    80005808:	0007b883          	ld	a7,0(a5)
    8000580c:	9746                	add	a4,a4,a7
    8000580e:	40000613          	li	a2,1024
    80005812:	c710                	sw	a2,8(a4)
  if(write)
    80005814:	001bb613          	seqz	a2,s7
    80005818:	0016161b          	slliw	a2,a2,0x1
    disk.desc[idx[1]].flags = 0; // device reads b->data
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    8000581c:	00166613          	ori	a2,a2,1
    80005820:	00c71623          	sh	a2,12(a4)
  disk.desc[idx[1]].next = idx[2];
    80005824:	f9842583          	lw	a1,-104(s0)
    80005828:	00b71723          	sh	a1,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    8000582c:	00250613          	addi	a2,a0,2
    80005830:	0612                	slli	a2,a2,0x4
    80005832:	963e                	add	a2,a2,a5
    80005834:	577d                	li	a4,-1
    80005836:	00e60823          	sb	a4,16(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    8000583a:	0592                	slli	a1,a1,0x4
    8000583c:	98ae                	add	a7,a7,a1
    8000583e:	03068713          	addi	a4,a3,48
    80005842:	973e                	add	a4,a4,a5
    80005844:	00e8b023          	sd	a4,0(a7)
  disk.desc[idx[2]].len = 1;
    80005848:	6398                	ld	a4,0(a5)
    8000584a:	972e                	add	a4,a4,a1
    8000584c:	01072423          	sw	a6,8(a4)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    80005850:	4689                	li	a3,2
    80005852:	00d71623          	sh	a3,12(a4)
  disk.desc[idx[2]].next = 0;
    80005856:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    8000585a:	010a2223          	sw	a6,4(s4)
  disk.info[idx[0]].b = b;
    8000585e:	01463423          	sd	s4,8(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    80005862:	6794                	ld	a3,8(a5)
    80005864:	0026d703          	lhu	a4,2(a3)
    80005868:	8b1d                	andi	a4,a4,7
    8000586a:	0706                	slli	a4,a4,0x1
    8000586c:	96ba                	add	a3,a3,a4
    8000586e:	00a69223          	sh	a0,4(a3)

  __sync_synchronize();
    80005872:	0ff0000f          	fence

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    80005876:	6798                	ld	a4,8(a5)
    80005878:	00275783          	lhu	a5,2(a4)
    8000587c:	2785                	addiw	a5,a5,1
    8000587e:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    80005882:	0ff0000f          	fence

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    80005886:	100017b7          	lui	a5,0x10001
    8000588a:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    8000588e:	004a2783          	lw	a5,4(s4)
    sleep(b, &disk.vdisk_lock);
    80005892:	0001b917          	auipc	s2,0x1b
    80005896:	6be90913          	addi	s2,s2,1726 # 80020f50 <disk+0x128>
  while(b->disk == 1) {
    8000589a:	4485                	li	s1,1
    8000589c:	01079a63          	bne	a5,a6,800058b0 <virtio_disk_rw+0x1b0>
    sleep(b, &disk.vdisk_lock);
    800058a0:	85ca                	mv	a1,s2
    800058a2:	8552                	mv	a0,s4
    800058a4:	e40fc0ef          	jal	80001ee4 <sleep>
  while(b->disk == 1) {
    800058a8:	004a2783          	lw	a5,4(s4)
    800058ac:	fe978ae3          	beq	a5,s1,800058a0 <virtio_disk_rw+0x1a0>
  }

  disk.info[idx[0]].b = 0;
    800058b0:	f9042903          	lw	s2,-112(s0)
    800058b4:	00290713          	addi	a4,s2,2
    800058b8:	0712                	slli	a4,a4,0x4
    800058ba:	0001b797          	auipc	a5,0x1b
    800058be:	56e78793          	addi	a5,a5,1390 # 80020e28 <disk>
    800058c2:	97ba                	add	a5,a5,a4
    800058c4:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    800058c8:	0001b997          	auipc	s3,0x1b
    800058cc:	56098993          	addi	s3,s3,1376 # 80020e28 <disk>
    800058d0:	00491713          	slli	a4,s2,0x4
    800058d4:	0009b783          	ld	a5,0(s3)
    800058d8:	97ba                	add	a5,a5,a4
    800058da:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    800058de:	854a                	mv	a0,s2
    800058e0:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    800058e4:	bafff0ef          	jal	80005492 <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    800058e8:	8885                	andi	s1,s1,1
    800058ea:	f0fd                	bnez	s1,800058d0 <virtio_disk_rw+0x1d0>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    800058ec:	0001b517          	auipc	a0,0x1b
    800058f0:	66450513          	addi	a0,a0,1636 # 80020f50 <disk+0x128>
    800058f4:	b72fb0ef          	jal	80000c66 <release>
}
    800058f8:	70a6                	ld	ra,104(sp)
    800058fa:	7406                	ld	s0,96(sp)
    800058fc:	64e6                	ld	s1,88(sp)
    800058fe:	6946                	ld	s2,80(sp)
    80005900:	69a6                	ld	s3,72(sp)
    80005902:	6a06                	ld	s4,64(sp)
    80005904:	7ae2                	ld	s5,56(sp)
    80005906:	7b42                	ld	s6,48(sp)
    80005908:	7ba2                	ld	s7,40(sp)
    8000590a:	7c02                	ld	s8,32(sp)
    8000590c:	6ce2                	ld	s9,24(sp)
    8000590e:	6165                	addi	sp,sp,112
    80005910:	8082                	ret

0000000080005912 <virtio_disk_intr>:

void
virtio_disk_intr()
{
    80005912:	1101                	addi	sp,sp,-32
    80005914:	ec06                	sd	ra,24(sp)
    80005916:	e822                	sd	s0,16(sp)
    80005918:	e426                	sd	s1,8(sp)
    8000591a:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    8000591c:	0001b497          	auipc	s1,0x1b
    80005920:	50c48493          	addi	s1,s1,1292 # 80020e28 <disk>
    80005924:	0001b517          	auipc	a0,0x1b
    80005928:	62c50513          	addi	a0,a0,1580 # 80020f50 <disk+0x128>
    8000592c:	aa2fb0ef          	jal	80000bce <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80005930:	100017b7          	lui	a5,0x10001
    80005934:	53b8                	lw	a4,96(a5)
    80005936:	8b0d                	andi	a4,a4,3
    80005938:	100017b7          	lui	a5,0x10001
    8000593c:	d3f8                	sw	a4,100(a5)

  __sync_synchronize();
    8000593e:	0ff0000f          	fence

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80005942:	689c                	ld	a5,16(s1)
    80005944:	0204d703          	lhu	a4,32(s1)
    80005948:	0027d783          	lhu	a5,2(a5) # 10001002 <_entry-0x6fffeffe>
    8000594c:	04f70663          	beq	a4,a5,80005998 <virtio_disk_intr+0x86>
    __sync_synchronize();
    80005950:	0ff0000f          	fence
    int id = disk.used->ring[disk.used_idx % NUM].id;
    80005954:	6898                	ld	a4,16(s1)
    80005956:	0204d783          	lhu	a5,32(s1)
    8000595a:	8b9d                	andi	a5,a5,7
    8000595c:	078e                	slli	a5,a5,0x3
    8000595e:	97ba                	add	a5,a5,a4
    80005960:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    80005962:	00278713          	addi	a4,a5,2
    80005966:	0712                	slli	a4,a4,0x4
    80005968:	9726                	add	a4,a4,s1
    8000596a:	01074703          	lbu	a4,16(a4)
    8000596e:	e321                	bnez	a4,800059ae <virtio_disk_intr+0x9c>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    80005970:	0789                	addi	a5,a5,2
    80005972:	0792                	slli	a5,a5,0x4
    80005974:	97a6                	add	a5,a5,s1
    80005976:	6788                	ld	a0,8(a5)
    b->disk = 0;   // disk is done with buf
    80005978:	00052223          	sw	zero,4(a0)
    wakeup(b);
    8000597c:	db4fc0ef          	jal	80001f30 <wakeup>

    disk.used_idx += 1;
    80005980:	0204d783          	lhu	a5,32(s1)
    80005984:	2785                	addiw	a5,a5,1
    80005986:	17c2                	slli	a5,a5,0x30
    80005988:	93c1                	srli	a5,a5,0x30
    8000598a:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    8000598e:	6898                	ld	a4,16(s1)
    80005990:	00275703          	lhu	a4,2(a4)
    80005994:	faf71ee3          	bne	a4,a5,80005950 <virtio_disk_intr+0x3e>
  }

  release(&disk.vdisk_lock);
    80005998:	0001b517          	auipc	a0,0x1b
    8000599c:	5b850513          	addi	a0,a0,1464 # 80020f50 <disk+0x128>
    800059a0:	ac6fb0ef          	jal	80000c66 <release>
}
    800059a4:	60e2                	ld	ra,24(sp)
    800059a6:	6442                	ld	s0,16(sp)
    800059a8:	64a2                	ld	s1,8(sp)
    800059aa:	6105                	addi	sp,sp,32
    800059ac:	8082                	ret
      panic("virtio_disk_intr status");
    800059ae:	00002517          	auipc	a0,0x2
    800059b2:	d4a50513          	addi	a0,a0,-694 # 800076f8 <etext+0x6f8>
    800059b6:	e2bfa0ef          	jal	800007e0 <panic>
	...

0000000080006000 <_trampoline>:
    80006000:	14051073          	csrw	sscratch,a0
    80006004:	02000537          	lui	a0,0x2000
    80006008:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    8000600a:	0536                	slli	a0,a0,0xd
    8000600c:	02153423          	sd	ra,40(a0)
    80006010:	02253823          	sd	sp,48(a0)
    80006014:	02353c23          	sd	gp,56(a0)
    80006018:	04453023          	sd	tp,64(a0)
    8000601c:	04553423          	sd	t0,72(a0)
    80006020:	04653823          	sd	t1,80(a0)
    80006024:	04753c23          	sd	t2,88(a0)
    80006028:	f120                	sd	s0,96(a0)
    8000602a:	f524                	sd	s1,104(a0)
    8000602c:	fd2c                	sd	a1,120(a0)
    8000602e:	e150                	sd	a2,128(a0)
    80006030:	e554                	sd	a3,136(a0)
    80006032:	e958                	sd	a4,144(a0)
    80006034:	ed5c                	sd	a5,152(a0)
    80006036:	0b053023          	sd	a6,160(a0)
    8000603a:	0b153423          	sd	a7,168(a0)
    8000603e:	0b253823          	sd	s2,176(a0)
    80006042:	0b353c23          	sd	s3,184(a0)
    80006046:	0d453023          	sd	s4,192(a0)
    8000604a:	0d553423          	sd	s5,200(a0)
    8000604e:	0d653823          	sd	s6,208(a0)
    80006052:	0d753c23          	sd	s7,216(a0)
    80006056:	0f853023          	sd	s8,224(a0)
    8000605a:	0f953423          	sd	s9,232(a0)
    8000605e:	0fa53823          	sd	s10,240(a0)
    80006062:	0fb53c23          	sd	s11,248(a0)
    80006066:	11c53023          	sd	t3,256(a0)
    8000606a:	11d53423          	sd	t4,264(a0)
    8000606e:	11e53823          	sd	t5,272(a0)
    80006072:	11f53c23          	sd	t6,280(a0)
    80006076:	140022f3          	csrr	t0,sscratch
    8000607a:	06553823          	sd	t0,112(a0)
    8000607e:	00853103          	ld	sp,8(a0)
    80006082:	02053203          	ld	tp,32(a0)
    80006086:	01053283          	ld	t0,16(a0)
    8000608a:	00053303          	ld	t1,0(a0)
    8000608e:	12000073          	sfence.vma
    80006092:	18031073          	csrw	satp,t1
    80006096:	12000073          	sfence.vma
    8000609a:	9282                	jalr	t0

000000008000609c <userret>:
    8000609c:	12000073          	sfence.vma
    800060a0:	18051073          	csrw	satp,a0
    800060a4:	12000073          	sfence.vma
    800060a8:	02000537          	lui	a0,0x2000
    800060ac:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    800060ae:	0536                	slli	a0,a0,0xd
    800060b0:	02853083          	ld	ra,40(a0)
    800060b4:	03053103          	ld	sp,48(a0)
    800060b8:	03853183          	ld	gp,56(a0)
    800060bc:	04053203          	ld	tp,64(a0)
    800060c0:	04853283          	ld	t0,72(a0)
    800060c4:	05053303          	ld	t1,80(a0)
    800060c8:	05853383          	ld	t2,88(a0)
    800060cc:	7120                	ld	s0,96(a0)
    800060ce:	7524                	ld	s1,104(a0)
    800060d0:	7d2c                	ld	a1,120(a0)
    800060d2:	6150                	ld	a2,128(a0)
    800060d4:	6554                	ld	a3,136(a0)
    800060d6:	6958                	ld	a4,144(a0)
    800060d8:	6d5c                	ld	a5,152(a0)
    800060da:	0a053803          	ld	a6,160(a0)
    800060de:	0a853883          	ld	a7,168(a0)
    800060e2:	0b053903          	ld	s2,176(a0)
    800060e6:	0b853983          	ld	s3,184(a0)
    800060ea:	0c053a03          	ld	s4,192(a0)
    800060ee:	0c853a83          	ld	s5,200(a0)
    800060f2:	0d053b03          	ld	s6,208(a0)
    800060f6:	0d853b83          	ld	s7,216(a0)
    800060fa:	0e053c03          	ld	s8,224(a0)
    800060fe:	0e853c83          	ld	s9,232(a0)
    80006102:	0f053d03          	ld	s10,240(a0)
    80006106:	0f853d83          	ld	s11,248(a0)
    8000610a:	10053e03          	ld	t3,256(a0)
    8000610e:	10853e83          	ld	t4,264(a0)
    80006112:	11053f03          	ld	t5,272(a0)
    80006116:	11853f83          	ld	t6,280(a0)
    8000611a:	7928                	ld	a0,112(a0)
    8000611c:	10200073          	sret
	...
