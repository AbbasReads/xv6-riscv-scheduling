...
0000000080000060 <start>:
{
  ...
  x &= ~MSTATUS_MPP_MASK;
    ...
    8000006e:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffddb67>
...
consoleinit(void)
{
  ...
  devsw[CONSOLE].read = consoleread;
    ...
    80000448:	6bc78793          	addi	a5,a5,1724 # 8001fb00 <devsw>
...
kfree(void *pa)
{
  ...
  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    ...
    80000a34:	26878793          	addi	a5,a5,616 # 80020c98 <end>
...
0000000080000aca <kinit>:
{
  ...
  freerange(end, (void*)PHYSTOP);
    ...
    80000aee:	1ae50513          	addi	a0,a0,430 # 80020c98 <end>
    ...
    80000d0e:	00c587b3          	add	a5,a1,a2
{
    ...
    80000d16:	0705                	addi	a4,a4,1 # fffffffffffff001 <end+0xffffffff7ffde369>
...
main()
{
  ...
  scheduler();        
    ...
    80000ec2:	75c040ef          	jal	8000561e <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000ec6:	772040ef          	jal	80005638 <plicinithart>
    binit();         // buffer cache
    80000eca:	63b010ef          	jal	80002d04 <binit>
    iinit();         // inode table
    80000ece:	3c0020ef          	jal	8000328e <iinit>
    fileinit();      // file table
    80000ed2:	2b2030ef          	jal	80004184 <fileinit>
    virtio_disk_init(); // emulated hard disk
    80000ed6:	053040ef          	jal	80005728 <virtio_disk_init>
    userinit();      // first user process
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {
    80000f6a:	3a5d                	addiw	s4,s4,-9 # ffffffffffffeff7 <end+0xffffffff7ffde35f>
...
proc_mapstacks(pagetable_t kpgtbl)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    8000179a:	022a8a93          	addi	s5,s5,34 # 800157b8 <tickslock>
    char *pa = kalloc();
...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001860:	f5ca0a13          	addi	s4,s4,-164 # 800157b8 <tickslock>
      initlock(&p->lock, "proc");
    ...
    8000187a:	2785                	addiw	a5,a5,1 # fffffffffffff001 <end+0xffffffff7ffde369>
...
forkret(void)
{
  ...
  if (first) {
    ...
    8000191e:	62d010ef          	jal	8000374a <fsinit>
    ...
    80001942:	713020ef          	jal	80004854 <kexec>
...
0000000080001aee <allocproc>:
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001afe:	2be48493          	addi	s1,s1,702 # 8000fdb8 <proc>
    80001b02:	00014917          	auipc	s2,0x14
    80001b06:	cb690913          	addi	s2,s2,-842 # 800157b8 <tickslock>
    acquire(&p->lock);
...
0000000080001b94 <userinit>:
{
  ...
  initproc = p;
    ...
    80001ba8:	cca7be23          	sd	a0,-804(a5) # 80007880 <initproc>
  p->cwd = namei("/");
...
0000000080001c32 <kfork>:
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  acquire(&wait_lock);
    ...
    80001d02:	ca248493          	addi	s1,s1,-862 # 8000f9a0 <wait_lock>
...
0000000080001d40 <scheduler>:
{
  ...
  c->proc = 0;
    ...
    80001d64:	c2870713          	addi	a4,a4,-984 # 8000f988 <pid_lock>
    80001d68:	975a                	add	a4,a4,s6
    80001d6a:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001d6e:	0000e717          	auipc	a4,0xe
    80001d72:	c5270713          	addi	a4,a4,-942 # 8000f9c0 <cpus+0x8>
    ...
    80001d80:	c0ca0a13          	addi	s4,s4,-1012 # 8000f988 <pid_lock>
    80001d84:	9a3e                	add	s4,s4,a5
        found = 1;
    80001d86:	4b85                	li	s7,1
    for(p = proc; p < &proc[NPROC]; p++) {
    80001d88:	00014997          	auipc	s3,0x14
    80001d8c:	a3098993          	addi	s3,s3,-1488 # 800157b8 <tickslock>
  ...
  asm volatile("csrw sstatus, %0" : : "r" (x));
    ...
    80001dea:	fd248493          	addi	s1,s1,-46 # 8000fdb8 <proc>
      if(p->state == RUNNABLE) {
...
0000000080001df2 <sched>:
{
  ...
  if(mycpu()->noff != 1)
    ...
    80001e16:	b7670713          	addi	a4,a4,-1162 # 8000f988 <pid_lock>
  ...
  intena = mycpu()->intena;
    ...
    80001e3c:	b5090913          	addi	s2,s2,-1200 # 8000f988 <pid_lock>
  ...
  swtch(&p->context, &mycpu()->context);
    ...
    80001e54:	b7058593          	addi	a1,a1,-1168 # 8000f9c0 <cpus+0x8>
...
wakeup(void *chan)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f3c:	e8048493          	addi	s1,s1,-384 # 8000fdb8 <proc>
    if(p != myproc()){
      acquire(&p->lock);
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    80001f48:	87490913          	addi	s2,s2,-1932 # 800157b8 <tickslock>
...
0000000080001f8e <reparent>:
{
  ...
  for(pp = proc; pp < &proc[NPROC]; pp++){
    ...
    80001fa4:	e1848493          	addi	s1,s1,-488 # 8000fdb8 <proc>
      pp->parent = initproc;
    80001fa8:	00006a17          	auipc	s4,0x6
    80001fac:	8d8a0a13          	addi	s4,s4,-1832 # 80007880 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fb0:	00014997          	auipc	s3,0x14
    80001fb4:	80898993          	addi	s3,s3,-2040 # 800157b8 <tickslock>
...
0000000080001fe4 <kexit>:
{
  ...
  if(p == initproc)
    ...
    80002000:	8847b783          	ld	a5,-1916(a5) # 80007880 <initproc>
    ...
    80002018:	fc8fe0ef          	jal	800007e0 <panic>
      fileclose(f);
    8000201c:	230020ef          	jal	8000424c <fileclose>
      p->ofile[fd] = 0;
  ...
  begin_op();
    80002030:	611010ef          	jal	80003e40 <begin_op>
  iput(p->cwd);
    80002034:	1509b503          	ld	a0,336(s3)
    80002038:	5a0010ef          	jal	800035d8 <iput>
  end_op();
    8000203c:	66f010ef          	jal	80003eaa <end_op>
  p->cwd = 0;
    80002040:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    80002044:	0000e497          	auipc	s1,0xe
    80002048:	95c48493          	addi	s1,s1,-1700 # 8000f9a0 <wait_lock>
...
kkill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    8000209a:	d2248493          	addi	s1,s1,-734 # 8000fdb8 <proc>
...
000000008000213a <kwait>:
{
  ...
  acquire(&wait_lock);
    ...
    8000215e:	84650513          	addi	a0,a0,-1978 # 8000f9a0 <wait_lock>
    ...
    80002170:	64c98993          	addi	s3,s3,1612 # 800157b8 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002174:	0000ec17          	auipc	s8,0xe
    80002178:	82cc0c13          	addi	s8,s8,-2004 # 8000f9a0 <wait_lock>
    ...
    800021aa:	7fa50513          	addi	a0,a0,2042 # 8000f9a0 <wait_lock>
    ...
    800021d6:	7ce50513          	addi	a0,a0,1998 # 8000f9a0 <wait_lock>
    ...
    8000221e:	b9e48493          	addi	s1,s1,-1122 # 8000fdb8 <proc>
    80002222:	b7e1                	j	800021ea <kwait+0xb0>
      release(&wait_lock);
    80002224:	0000d517          	auipc	a0,0xd
    80002228:	77c50513          	addi	a0,a0,1916 # 8000f9a0 <wait_lock>
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    800022ee:	c2648493          	addi	s1,s1,-986 # 8000ff10 <proc+0x158>
    800022f2:	00013917          	auipc	s2,0x13
    800022f6:	61e90913          	addi	s2,s2,1566 # 80015910 <bcache+0x40>
    if(p->state == UNUSED)
      continue;
...
trapinit(void)
{
  ...
  initlock(&tickslock, "time");
    ...
    800023ea:	3d250513          	addi	a0,a0,978 # 800157b8 <tickslock>
...
trapinithart(void)
{
  ...
  asm volatile("csrw stvec, %0" : : "r" (x));
    ...
    80002404:	1c078793          	addi	a5,a5,448 # 800055c0 <kernelvec>
...
clockintr()
{
  ...
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    ...
    800024ba:	30248493          	addi	s1,s1,770 # 800157b8 <tickslock>
    800024be:	8526                	mv	a0,s1
    800024c0:	f0efe0ef          	jal	80000bce <acquire>
    ticks++;
    800024c4:	00005517          	auipc	a0,0x5
    800024c8:	3c450513          	addi	a0,a0,964 # 80007888 <ticks>
...
devintr()
{
  ...
  if(scause == 0x8000000000000009L){
    ...
    int irq = plic_claim();
    8000250c:	160030ef          	jal	8000566c <plic_claim>
    ...
    8000252a:	a819                	j	80002540 <devintr+0x60>
      virtio_disk_intr();
    8000252c:	606030ef          	jal	80005b32 <virtio_disk_intr>
    if(irq)
    ...
    80002542:	14a030ef          	jal	8000568c <plic_complete>
    return 1;
...
0000000080002554 <usertrap>:
{
  ...
  asm volatile("csrw stvec, %0" : : "r" (x));
    ...
    8000256e:	05678793          	addi	a5,a5,86 # 800055c0 <kernelvec>
...
sys_pause(void)
{
  ...
  acquire(&tickslock);
    ...
    800029b4:	e0850513          	addi	a0,a0,-504 # 800157b8 <tickslock>
    800029b8:	a16fe0ef          	jal	80000bce <acquire>
  ticks0 = ticks;
    800029bc:	00005917          	auipc	s2,0x5
    800029c0:	ecc92903          	lw	s2,-308(s2) # 80007888 <ticks>
  while(ticks - ticks0 < n){
    ...
    800029d2:	dea98993          	addi	s3,s3,-534 # 800157b8 <tickslock>
    800029d6:	00005497          	auipc	s1,0x5
    800029da:	eb248493          	addi	s1,s1,-334 # 80007888 <ticks>
    if(killed(myproc())){
  ...
  release(&tickslock);
    ...
    80002a06:	db650513          	addi	a0,a0,-586 # 800157b8 <tickslock>
  ...
  return 0;
    ...
    80002a24:	d9850513          	addi	a0,a0,-616 # 800157b8 <tickslock>
...
sys_uptime(void)
{
  ...
  acquire(&tickslock);
    ...
    80002a64:	d5850513          	addi	a0,a0,-680 # 800157b8 <tickslock>
    80002a68:	966fe0ef          	jal	80000bce <acquire>
  xticks = ticks;
    80002a6c:	00005497          	auipc	s1,0x5
    80002a70:	e1c4a483          	lw	s1,-484(s1) # 80007888 <ticks>
  release(&tickslock);
    80002a74:	00013517          	auipc	a0,0x13
    80002a78:	d4450513          	addi	a0,a0,-700 # 800157b8 <tickslock>
...
lockinit(int id)
{
  ...
  for(int i = 0; i < MAX_LOCKS; i++){
    80002a96:	00013797          	auipc	a5,0x13
    80002a9a:	d3a78793          	addi	a5,a5,-710 # 800157d0 <locktable>
    80002a9e:	00013697          	auipc	a3,0x13
    80002aa2:	e3268693          	addi	a3,a3,-462 # 800158d0 <bcache>
    80002aa6:	a021                	j	80002aae <lockinit+0x1e>
    80002aa8:	07c1                	addi	a5,a5,16
    80002aaa:	00d78863          	beq	a5,a3,80002aba <lockinit+0x2a>
    if(locktable[i].valid && locktable[i].id == id)
    80002aae:	47d8                	lw	a4,12(a5)
    80002ab0:	df65                	beqz	a4,80002aa8 <lockinit+0x18>
    80002ab2:	4398                	lw	a4,0(a5)
    80002ab4:	fea71ae3          	bne	a4,a0,80002aa8 <lockinit+0x18>
    80002ab8:	a831                	j	80002ad4 <lockinit+0x44>
    80002aba:	00013717          	auipc	a4,0x13
    80002abe:	d2270713          	addi	a4,a4,-734 # 800157dc <locktable+0xc>
      return id;
  }
  for(int i = 0; i < MAX_LOCKS; i++){
    80002ac2:	4781                	li	a5,0
    80002ac4:	4641                	li	a2,16
    if(!locktable[i].valid){
    80002ac6:	4314                	lw	a3,0(a4)
    80002ac8:	ca89                	beqz	a3,80002ada <lockinit+0x4a>
  for(int i = 0; i < MAX_LOCKS; i++){
    80002aca:	2785                	addiw	a5,a5,1
    80002acc:	0741                	addi	a4,a4,16
    80002ace:	fec79ce3          	bne	a5,a2,80002ac6 <lockinit+0x36>
      locktable[i].pid = -1;
      locktable[i].valid = 1;
      return id;
    }
  ...
  return -1;
    ...
    80002ad4:	6422                	ld	s0,8(sp)
    80002ad6:	0141                	addi	sp,sp,16
    80002ad8:	8082                	ret
      locktable[i].id = id;
    80002ada:	0792                	slli	a5,a5,0x4
    80002adc:	00013717          	auipc	a4,0x13
    80002ae0:	cf470713          	addi	a4,a4,-780 # 800157d0 <locktable>
    80002ae4:	97ba                	add	a5,a5,a4
    80002ae6:	c388                	sw	a0,0(a5)
      locktable[i].held = 0;
    80002ae8:	0007a223          	sw	zero,4(a5)
      locktable[i].pid = -1;
    80002aec:	577d                	li	a4,-1
    80002aee:	c798                	sw	a4,8(a5)
      locktable[i].valid = 1;
    80002af0:	4705                	li	a4,1
    80002af2:	c7d8                	sw	a4,12(a5)
      return id;
    80002af4:	b7c5                	j	80002ad4 <lockinit+0x44>

0000000080002af6 <lockacquire>:
...
lockacquire(int id)
{
  ...
  for(int i = 0; i < MAX_LOCKS; i++){
    80002b00:	00013797          	auipc	a5,0x13
    80002b04:	cd078793          	addi	a5,a5,-816 # 800157d0 <locktable>
    80002b08:	4701                	li	a4,0
    80002b0a:	4641                	li	a2,16
    80002b0c:	a029                	j	80002b16 <lockacquire+0x20>
    80002b0e:	2705                	addiw	a4,a4,1
    80002b10:	07c1                	addi	a5,a5,16
    80002b12:	02c70c63          	beq	a4,a2,80002b4a <lockacquire+0x54>
    if(locktable[i].valid && locktable[i].id == id){
    80002b16:	47d4                	lw	a3,12(a5)
    80002b18:	dafd                	beqz	a3,80002b0e <lockacquire+0x18>
    80002b1a:	4394                	lw	a3,0(a5)
    80002b1c:	fea699e3          	bne	a3,a0,80002b0e <lockacquire+0x18>
      if(locktable[i].held)
    80002b20:	00471693          	slli	a3,a4,0x4
    80002b24:	00013797          	auipc	a5,0x13
    80002b28:	cac78793          	addi	a5,a5,-852 # 800157d0 <locktable>
    80002b2c:	97b6                	add	a5,a5,a3
    80002b2e:	43c4                	lw	s1,4(a5)
    80002b30:	e485                	bnez	s1,80002b58 <lockacquire+0x62>
    80002b32:	e04a                	sd	s2,0(sp)
        return -1;
      locktable[i].held = 1;
    80002b34:	893e                	mv	s2,a5
    80002b36:	4785                	li	a5,1
    80002b38:	00f92223          	sw	a5,4(s2)
      locktable[i].pid = myproc()->pid;
    80002b3c:	d93fe0ef          	jal	800018ce <myproc>
    80002b40:	591c                	lw	a5,48(a0)
    80002b42:	00f92423          	sw	a5,8(s2)
      return 0;
    80002b46:	6902                	ld	s2,0(sp)
    80002b48:	a011                	j	80002b4c <lockacquire+0x56>
    }
  }
  return -1;
    80002b4a:	54fd                	li	s1,-1
}
    ...
    80002b56:	8082                	ret
        return -1;
    80002b58:	54fd                	li	s1,-1
    80002b5a:	bfcd                	j	80002b4c <lockacquire+0x56>

0000000080002b5c <lockrelease>:

int
lockrelease(int id)
{
    80002b5c:	1141                	addi	sp,sp,-16
    80002b5e:	e422                	sd	s0,8(sp)
    80002b60:	0800                	addi	s0,sp,16
  for(int i = 0; i < MAX_LOCKS; i++){
    80002b62:	00013797          	auipc	a5,0x13
    80002b66:	c6e78793          	addi	a5,a5,-914 # 800157d0 <locktable>
    80002b6a:	4701                	li	a4,0
    80002b6c:	4641                	li	a2,16
    80002b6e:	a029                	j	80002b78 <lockrelease+0x1c>
    80002b70:	2705                	addiw	a4,a4,1
    80002b72:	07c1                	addi	a5,a5,16
    80002b74:	02c70b63          	beq	a4,a2,80002baa <lockrelease+0x4e>
    if(locktable[i].valid && locktable[i].id == id){
    80002b78:	47d4                	lw	a3,12(a5)
    80002b7a:	dafd                	beqz	a3,80002b70 <lockrelease+0x14>
    80002b7c:	4394                	lw	a3,0(a5)
    80002b7e:	fea699e3          	bne	a3,a0,80002b70 <lockrelease+0x14>
      if(!locktable[i].held)
    80002b82:	00471693          	slli	a3,a4,0x4
    80002b86:	00013797          	auipc	a5,0x13
    80002b8a:	c4a78793          	addi	a5,a5,-950 # 800157d0 <locktable>
    80002b8e:	97b6                	add	a5,a5,a3
    80002b90:	43dc                	lw	a5,4(a5)
    80002b92:	c385                	beqz	a5,80002bb2 <lockrelease+0x56>
        return -1;
      locktable[i].held = 0;
    80002b94:	00013797          	auipc	a5,0x13
    80002b98:	c3c78793          	addi	a5,a5,-964 # 800157d0 <locktable>
    80002b9c:	97b6                	add	a5,a5,a3
    80002b9e:	0007a223          	sw	zero,4(a5)
      locktable[i].pid = -1;
    80002ba2:	577d                	li	a4,-1
    80002ba4:	c798                	sw	a4,8(a5)
      return 0;
    80002ba6:	4501                	li	a0,0
    80002ba8:	a011                	j	80002bac <lockrelease+0x50>
    }
  }
  return -1;
    ...
    80002bb4:	bfe5                	j	80002bac <lockrelease+0x50>

0000000080002bb6 <locktry>:

int
locktry(int id)
{
  for(int i = 0; i < MAX_LOCKS; i++){
    80002bb6:	00013797          	auipc	a5,0x13
    80002bba:	c1a78793          	addi	a5,a5,-998 # 800157d0 <locktable>
    80002bbe:	4701                	li	a4,0
    80002bc0:	4641                	li	a2,16
    80002bc2:	a81d                	j	80002bf8 <locktry+0x42>
{
    ...
    if(locktable[i].valid && locktable[i].id == id){
      if(locktable[i].held)
        return 0;
      locktable[i].held = 1;
    80002bce:	00013497          	auipc	s1,0x13
    80002bd2:	c0248493          	addi	s1,s1,-1022 # 800157d0 <locktable>
    80002bd6:	94b6                	add	s1,s1,a3
    80002bd8:	4785                	li	a5,1
    80002bda:	c0dc                	sw	a5,4(s1)
      locktable[i].pid = myproc()->pid;
    80002bdc:	cf3fe0ef          	jal	800018ce <myproc>
    80002be0:	591c                	lw	a5,48(a0)
  ...
  return -1;
    ...
    80002bee:	8082                	ret
  for(int i = 0; i < MAX_LOCKS; i++){
    80002bf0:	2705                	addiw	a4,a4,1
    80002bf2:	07c1                	addi	a5,a5,16
    80002bf4:	02c70263          	beq	a4,a2,80002c18 <locktry+0x62>
    if(locktable[i].valid && locktable[i].id == id){
    80002bf8:	47d4                	lw	a3,12(a5)
    80002bfa:	dafd                	beqz	a3,80002bf0 <locktry+0x3a>
    80002bfc:	4394                	lw	a3,0(a5)
    80002bfe:	fea699e3          	bne	a3,a0,80002bf0 <locktry+0x3a>
      if(locktable[i].held)
    80002c02:	00471693          	slli	a3,a4,0x4
    80002c06:	00013797          	auipc	a5,0x13
    80002c0a:	bca78793          	addi	a5,a5,-1078 # 800157d0 <locktable>
    80002c0e:	97b6                	add	a5,a5,a3
    80002c10:	43dc                	lw	a5,4(a5)
        return 0;
    80002c12:	4501                	li	a0,0
      if(locktable[i].held)
    80002c14:	dbc5                	beqz	a5,80002bc4 <locktry+0xe>
}
    80002c16:	8082                	ret
  return -1;
...
lockcheck(int id)
{
  ...
  for(int i = 0; i < MAX_LOCKS; i++){
    80002c22:	00013797          	auipc	a5,0x13
    80002c26:	bae78793          	addi	a5,a5,-1106 # 800157d0 <locktable>
    80002c2a:	4701                	li	a4,0
    80002c2c:	4641                	li	a2,16
    80002c2e:	a029                	j	80002c38 <lockcheck+0x1c>
    80002c30:	2705                	addiw	a4,a4,1
    80002c32:	07c1                	addi	a5,a5,16
    80002c34:	00c70f63          	beq	a4,a2,80002c52 <lockcheck+0x36>
    if(locktable[i].valid && locktable[i].id == id)
    80002c38:	47d4                	lw	a3,12(a5)
    80002c3a:	dafd                	beqz	a3,80002c30 <lockcheck+0x14>
    80002c3c:	4394                	lw	a3,0(a5)
    80002c3e:	fea699e3          	bne	a3,a0,80002c30 <lockcheck+0x14>
      return locktable[i].held;
    80002c42:	0712                	slli	a4,a4,0x4
    80002c44:	00013797          	auipc	a5,0x13
    80002c48:	b8c78793          	addi	a5,a5,-1140 # 800157d0 <locktable>
    80002c4c:	97ba                	add	a5,a5,a4
    80002c4e:	43c8                	lw	a0,4(a5)
    80002c50:	a011                	j	80002c54 <lockcheck+0x38>
  }
  return -1;
...
sys_lockinit(void)
{
  ...
  argint(0, &id);
    ...
    80002c68:	b75ff0ef          	jal	800027dc <argint>
  return lockinit(id);
    80002c6c:	fec42503          	lw	a0,-20(s0)
    80002c70:	e21ff0ef          	jal	80002a90 <lockinit>
}
...
0000000080002c7c <sys_lockacquire>:
...
sys_lockacquire(void)
{
  ...
  argint(0, &id);
    ...
    80002c8a:	b53ff0ef          	jal	800027dc <argint>
  return lockacquire(id);
    80002c8e:	fec42503          	lw	a0,-20(s0)
    80002c92:	e65ff0ef          	jal	80002af6 <lockacquire>
}
...
0000000080002c9e <sys_lockrelease>:
...
sys_lockrelease(void)
{
  ...
  int id;
  argint(0, &id);
    80002ca6:	fec40593          	addi	a1,s0,-20
    80002caa:	4501                	li	a0,0
    80002cac:	b31ff0ef          	jal	800027dc <argint>
  return lockrelease(id);
    80002cb0:	fec42503          	lw	a0,-20(s0)
    80002cb4:	ea9ff0ef          	jal	80002b5c <lockrelease>
}
...
sys_locktry(void)
{
  ...
  argint(0, &id);
    ...
    80002cce:	b0fff0ef          	jal	800027dc <argint>
  return locktry(id);
    80002cd2:	fec42503          	lw	a0,-20(s0)
    80002cd6:	ee1ff0ef          	jal	80002bb6 <locktry>
}
...
0000000080002ce2 <sys_lockcheck>:
...
sys_lockcheck(void)
{
  ...
  argint(0, &id);
    ...
    80002cf0:	aedff0ef          	jal	800027dc <argint>
  return lockcheck(id);
    80002cf4:	fec42503          	lw	a0,-20(s0)
    80002cf8:	f25ff0ef          	jal	80002c1c <lockcheck>
}
...
binit(void)
{
  ...
  initlock(&bcache.lock, "bcache");
    80002d14:	00004597          	auipc	a1,0x4
    80002d18:	67c58593          	addi	a1,a1,1660 # 80007390 <etext+0x390>
    80002d1c:	00013517          	auipc	a0,0x13
    80002d20:	bb450513          	addi	a0,a0,-1100 # 800158d0 <bcache>
    80002d24:	e2bfd0ef          	jal	80000b4e <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002d28:	0001b797          	auipc	a5,0x1b
    80002d2c:	ba878793          	addi	a5,a5,-1112 # 8001d8d0 <bcache+0x8000>
    80002d30:	0001b717          	auipc	a4,0x1b
    80002d34:	e0870713          	addi	a4,a4,-504 # 8001db38 <bcache+0x8268>
    80002d38:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002d3c:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002d40:	00013497          	auipc	s1,0x13
    80002d44:	ba848493          	addi	s1,s1,-1112 # 800158e8 <bcache+0x18>
    b->next = bcache.head.next;
    80002d48:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80002d4a:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80002d4c:	00004a17          	auipc	s4,0x4
    80002d50:	64ca0a13          	addi	s4,s4,1612 # 80007398 <etext+0x398>
    b->next = bcache.head.next;
    80002d54:	2b893783          	ld	a5,696(s2)
    80002d58:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80002d5a:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    80002d5e:	85d2                	mv	a1,s4
    80002d60:	01048513          	addi	a0,s1,16
    80002d64:	322010ef          	jal	80004086 <initsleeplock>
    bcache.head.next->prev = b;
    80002d68:	2b893783          	ld	a5,696(s2)
    80002d6c:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80002d6e:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002d72:	45848493          	addi	s1,s1,1112
    80002d76:	fd349fe3          	bne	s1,s3,80002d54 <binit+0x50>
  }
}
...
bread(uint dev, uint blockno)
{
    ...
    80002d9a:	89ae                	mv	s3,a1
  acquire(&bcache.lock);
    80002d9c:	00013517          	auipc	a0,0x13
    80002da0:	b3450513          	addi	a0,a0,-1228 # 800158d0 <bcache>
    80002da4:	e2bfd0ef          	jal	80000bce <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002da8:	0001b497          	auipc	s1,0x1b
    80002dac:	de04b483          	ld	s1,-544(s1) # 8001db88 <bcache+0x82b8>
    80002db0:	0001b797          	auipc	a5,0x1b
    80002db4:	d8878793          	addi	a5,a5,-632 # 8001db38 <bcache+0x8268>
    80002db8:	02f48b63          	beq	s1,a5,80002dee <bread+0x64>
    80002dbc:	873e                	mv	a4,a5
    80002dbe:	a021                	j	80002dc6 <bread+0x3c>
    80002dc0:	68a4                	ld	s1,80(s1)
    80002dc2:	02e48663          	beq	s1,a4,80002dee <bread+0x64>
    if(b->dev == dev && b->blockno == blockno){
    80002dc6:	449c                	lw	a5,8(s1)
    80002dc8:	ff279ce3          	bne	a5,s2,80002dc0 <bread+0x36>
    80002dcc:	44dc                	lw	a5,12(s1)
    80002dce:	ff3799e3          	bne	a5,s3,80002dc0 <bread+0x36>
      b->refcnt++;
    80002dd2:	40bc                	lw	a5,64(s1)
    80002dd4:	2785                	addiw	a5,a5,1
    80002dd6:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002dd8:	00013517          	auipc	a0,0x13
    80002ddc:	af850513          	addi	a0,a0,-1288 # 800158d0 <bcache>
    80002de0:	e87fd0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002de4:	01048513          	addi	a0,s1,16
    80002de8:	2d4010ef          	jal	800040bc <acquiresleep>
      return b;
    80002dec:	a889                	j	80002e3e <bread+0xb4>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002dee:	0001b497          	auipc	s1,0x1b
    80002df2:	d924b483          	ld	s1,-622(s1) # 8001db80 <bcache+0x82b0>
    80002df6:	0001b797          	auipc	a5,0x1b
    80002dfa:	d4278793          	addi	a5,a5,-702 # 8001db38 <bcache+0x8268>
    80002dfe:	00f48863          	beq	s1,a5,80002e0e <bread+0x84>
    80002e02:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    80002e04:	40bc                	lw	a5,64(s1)
    80002e06:	cb91                	beqz	a5,80002e1a <bread+0x90>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002e08:	64a4                	ld	s1,72(s1)
    80002e0a:	fee49de3          	bne	s1,a4,80002e04 <bread+0x7a>
  panic("bget: no buffers");
    80002e0e:	00004517          	auipc	a0,0x4
    80002e12:	59250513          	addi	a0,a0,1426 # 800073a0 <etext+0x3a0>
    80002e16:	9cbfd0ef          	jal	800007e0 <panic>
      b->dev = dev;
    80002e1a:	0124a423          	sw	s2,8(s1)
      b->blockno = blockno;
    80002e1e:	0134a623          	sw	s3,12(s1)
      b->valid = 0;
    80002e22:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    80002e26:	4785                	li	a5,1
    80002e28:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002e2a:	00013517          	auipc	a0,0x13
    80002e2e:	aa650513          	addi	a0,a0,-1370 # 800158d0 <bcache>
    80002e32:	e35fd0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002e36:	01048513          	addi	a0,s1,16
    80002e3a:	282010ef          	jal	800040bc <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80002e3e:	409c                	lw	a5,0(s1)
    80002e40:	cb89                	beqz	a5,80002e52 <bread+0xc8>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
    ...
    80002e50:	8082                	ret
    virtio_disk_rw(b, 0);
    80002e52:	4581                	li	a1,0
    80002e54:	8526                	mv	a0,s1
    80002e56:	2cb020ef          	jal	80005920 <virtio_disk_rw>
    b->valid = 1;
    80002e5a:	4785                	li	a5,1
    80002e5c:	c09c                	sw	a5,0(s1)
  return b;
    80002e5e:	b7d5                	j	80002e42 <bread+0xb8>

0000000080002e60 <bwrite>:
...
bwrite(struct buf *b)
{
  ...
  if(!holdingsleep(&b->lock))
    ...
    80002e6e:	2cc010ef          	jal	8000413a <holdingsleep>
    80002e72:	c911                	beqz	a0,80002e86 <bwrite+0x26>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80002e74:	4585                	li	a1,1
    80002e76:	8526                	mv	a0,s1
    80002e78:	2a9020ef          	jal	80005920 <virtio_disk_rw>
}
    ...
    80002e8a:	53250513          	addi	a0,a0,1330 # 800073b8 <etext+0x3b8>
    80002e8e:	953fd0ef          	jal	800007e0 <panic>

0000000080002e92 <brelse>:
...
brelse(struct buf *b)
{
  ...
  if(!holdingsleep(&b->lock))
    80002ea0:	01050913          	addi	s2,a0,16
    80002ea4:	854a                	mv	a0,s2
    80002ea6:	294010ef          	jal	8000413a <holdingsleep>
    80002eaa:	c135                	beqz	a0,80002f0e <brelse+0x7c>
    panic("brelse");

  releasesleep(&b->lock);
    80002eac:	854a                	mv	a0,s2
    80002eae:	254010ef          	jal	80004102 <releasesleep>

  acquire(&bcache.lock);
    80002eb2:	00013517          	auipc	a0,0x13
    80002eb6:	a1e50513          	addi	a0,a0,-1506 # 800158d0 <bcache>
    80002eba:	d15fd0ef          	jal	80000bce <acquire>
  b->refcnt--;
  ...
  if (b->refcnt == 0) {
    80002ec8:	e71d                	bnez	a4,80002ef6 <brelse+0x64>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80002eca:	68b8                	ld	a4,80(s1)
    80002ecc:	64bc                	ld	a5,72(s1)
    80002ece:	e73c                	sd	a5,72(a4)
    b->prev->next = b->next;
    80002ed0:	68b8                	ld	a4,80(s1)
    80002ed2:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80002ed4:	0001b797          	auipc	a5,0x1b
    80002ed8:	9fc78793          	addi	a5,a5,-1540 # 8001d8d0 <bcache+0x8000>
    80002edc:	2b87b703          	ld	a4,696(a5)
    80002ee0:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002ee2:	0001b717          	auipc	a4,0x1b
    80002ee6:	c5670713          	addi	a4,a4,-938 # 8001db38 <bcache+0x8268>
    80002eea:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80002eec:	2b87b703          	ld	a4,696(a5)
    80002ef0:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80002ef2:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    80002ef6:	00013517          	auipc	a0,0x13
    80002efa:	9da50513          	addi	a0,a0,-1574 # 800158d0 <bcache>
    80002efe:	d69fd0ef          	jal	80000c66 <release>
}
    ...
    80002f12:	4b250513          	addi	a0,a0,1202 # 800073c0 <etext+0x3c0>
    80002f16:	8cbfd0ef          	jal	800007e0 <panic>

0000000080002f1a <bpin>:
...
bpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    ...
    80002f2a:	9aa50513          	addi	a0,a0,-1622 # 800158d0 <bcache>
    80002f2e:	ca1fd0ef          	jal	80000bce <acquire>
  b->refcnt++;
    80002f32:	40bc                	lw	a5,64(s1)
    80002f34:	2785                	addiw	a5,a5,1
    80002f36:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002f38:	00013517          	auipc	a0,0x13
    80002f3c:	99850513          	addi	a0,a0,-1640 # 800158d0 <bcache>
...
bunpin(struct buf *b) {
  ...
  acquire(&bcache.lock);
    80002f5a:	00013517          	auipc	a0,0x13
    80002f5e:	97650513          	addi	a0,a0,-1674 # 800158d0 <bcache>
    80002f62:	c6dfd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002f66:	40bc                	lw	a5,64(s1)
    80002f68:	37fd                	addiw	a5,a5,-1
    80002f6a:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002f6c:	00013517          	auipc	a0,0x13
    80002f70:	96450513          	addi	a0,a0,-1692 # 800158d0 <bcache>
    80002f74:	cf3fd0ef          	jal	80000c66 <release>
}
...
bfree(int dev, uint b)
{
  ...
  bp = bread(dev, BBLOCK(b, sb));
    80002f90:	00d5d59b          	srliw	a1,a1,0xd
    80002f94:	0001b797          	auipc	a5,0x1b
    80002f98:	0187a783          	lw	a5,24(a5) # 8001dfac <sb+0x1c>
    80002f9c:	9dbd                	addw	a1,a1,a5
    80002f9e:	dedff0ef          	jal	80002d8a <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
  ...
  if((bp->data[bi/8] & m) == 0)
    ...
    80002fb4:	05874703          	lbu	a4,88(a4)
    80002fb8:	00e7f6b3          	and	a3,a5,a4
    80002fbc:	c29d                	beqz	a3,80002fe2 <bfree+0x60>
    80002fbe:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80002fc0:	94aa                	add	s1,s1,a0
    80002fc2:	fff7c793          	not	a5,a5
    80002fc6:	8f7d                	and	a4,a4,a5
    80002fc8:	04e48c23          	sb	a4,88(s1)
  log_write(bp);
    80002fcc:	7f9000ef          	jal	80003fc4 <log_write>
  brelse(bp);
    80002fd0:	854a                	mv	a0,s2
    80002fd2:	ec1ff0ef          	jal	80002e92 <brelse>
}
    ...
    80002fea:	ff6fd0ef          	jal	800007e0 <panic>

0000000080002fee <balloc>:
{
    80002fee:	711d                	addi	sp,sp,-96
    80002ff0:	ec86                	sd	ra,88(sp)
    80002ff2:	e8a2                	sd	s0,80(sp)
    80002ff4:	e4a6                	sd	s1,72(sp)
    80002ff6:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80002ff8:	0001b797          	auipc	a5,0x1b
    80002ffc:	f9c7a783          	lw	a5,-100(a5) # 8001df94 <sb+0x4>
    80003000:	0e078f63          	beqz	a5,800030fe <balloc+0x110>
    80003004:	e0ca                	sd	s2,64(sp)
    80003006:	fc4e                	sd	s3,56(sp)
    80003008:	f852                	sd	s4,48(sp)
    8000300a:	f456                	sd	s5,40(sp)
    8000300c:	f05a                	sd	s6,32(sp)
    8000300e:	ec5e                	sd	s7,24(sp)
    80003010:	e862                	sd	s8,16(sp)
    80003012:	e466                	sd	s9,8(sp)
    80003014:	8baa                	mv	s7,a0
    80003016:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80003018:	0001bb17          	auipc	s6,0x1b
    8000301c:	f78b0b13          	addi	s6,s6,-136 # 8001df90 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003020:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80003022:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80003024:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80003026:	6c89                	lui	s9,0x2
    80003028:	a0b5                	j	80003094 <balloc+0xa6>
        bp->data[bi/8] |= m;  // Mark block in use.
    8000302a:	97ca                	add	a5,a5,s2
    8000302c:	8e55                	or	a2,a2,a3
    8000302e:	04c78c23          	sb	a2,88(a5)
        log_write(bp);
    80003032:	854a                	mv	a0,s2
    80003034:	791000ef          	jal	80003fc4 <log_write>
        brelse(bp);
    80003038:	854a                	mv	a0,s2
    8000303a:	e59ff0ef          	jal	80002e92 <brelse>
  bp = bread(dev, bno);
    8000303e:	85a6                	mv	a1,s1
    80003040:	855e                	mv	a0,s7
    80003042:	d49ff0ef          	jal	80002d8a <bread>
    80003046:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    80003048:	40000613          	li	a2,1024
    8000304c:	4581                	li	a1,0
    8000304e:	05850513          	addi	a0,a0,88
    80003052:	c51fd0ef          	jal	80000ca2 <memset>
  log_write(bp);
    80003056:	854a                	mv	a0,s2
    80003058:	76d000ef          	jal	80003fc4 <log_write>
  brelse(bp);
    8000305c:	854a                	mv	a0,s2
    8000305e:	e35ff0ef          	jal	80002e92 <brelse>
}
    ...
    80003074:	60e6                	ld	ra,88(sp)
    80003076:	6446                	ld	s0,80(sp)
    80003078:	64a6                	ld	s1,72(sp)
    8000307a:	6125                	addi	sp,sp,96
    8000307c:	8082                	ret
    brelse(bp);
    8000307e:	854a                	mv	a0,s2
    80003080:	e13ff0ef          	jal	80002e92 <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80003084:	015c87bb          	addw	a5,s9,s5
    80003088:	00078a9b          	sext.w	s5,a5
    8000308c:	004b2703          	lw	a4,4(s6)
    80003090:	04eaff63          	bgeu	s5,a4,800030ee <balloc+0x100>
    bp = bread(dev, BBLOCK(b, sb));
    80003094:	41fad79b          	sraiw	a5,s5,0x1f
    80003098:	0137d79b          	srliw	a5,a5,0x13
    8000309c:	015787bb          	addw	a5,a5,s5
    800030a0:	40d7d79b          	sraiw	a5,a5,0xd
    800030a4:	01cb2583          	lw	a1,28(s6)
    800030a8:	9dbd                	addw	a1,a1,a5
    800030aa:	855e                	mv	a0,s7
    800030ac:	cdfff0ef          	jal	80002d8a <bread>
    800030b0:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    800030b2:	004b2503          	lw	a0,4(s6)
    800030b6:	000a849b          	sext.w	s1,s5
    800030ba:	8762                	mv	a4,s8
    800030bc:	fca4f1e3          	bgeu	s1,a0,8000307e <balloc+0x90>
      m = 1 << (bi % 8);
    ...
    800030e2:	d5a1                	beqz	a1,8000302a <balloc+0x3c>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    800030e4:	2705                	addiw	a4,a4,1
    800030e6:	2485                	addiw	s1,s1,1
    800030e8:	fd471ae3          	bne	a4,s4,800030bc <balloc+0xce>
    800030ec:	bf49                	j	8000307e <balloc+0x90>
  ...
  printf("balloc: out of blocks\n");
    ...
    80003102:	2e250513          	addi	a0,a0,738 # 800073e0 <etext+0x3e0>
    80003106:	bf4fd0ef          	jal	800004fa <printf>
  return 0;
    8000310a:	4481                	li	s1,0
    8000310c:	b79d                	j	80003072 <balloc+0x84>

000000008000310e <bmap>:
...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    8000311e:	47ad                	li	a5,11
    80003120:	02b7e663          	bltu	a5,a1,8000314c <bmap+0x3e>
    if((addr = ip->addrs[bn]) == 0){
    80003124:	02059793          	slli	a5,a1,0x20
    80003128:	01e7d593          	srli	a1,a5,0x1e
    8000312c:	00b504b3          	add	s1,a0,a1
    80003130:	0504a903          	lw	s2,80(s1)
    80003134:	06091a63          	bnez	s2,800031a8 <bmap+0x9a>
      addr = balloc(ip->dev);
    80003138:	4108                	lw	a0,0(a0)
    8000313a:	eb5ff0ef          	jal	80002fee <balloc>
    8000313e:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80003142:	06090363          	beqz	s2,800031a8 <bmap+0x9a>
        return 0;
      ip->addrs[bn] = addr;
    80003146:	0524a823          	sw	s2,80(s1)
    8000314a:	a8b9                	j	800031a8 <bmap+0x9a>
    }
    return addr;
  }
  bn -= NDIRECT;
    8000314c:	ff45849b          	addiw	s1,a1,-12
    80003150:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80003154:	0ff00793          	li	a5,255
    80003158:	06e7ee63          	bltu	a5,a4,800031d4 <bmap+0xc6>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0){
    8000315c:	08052903          	lw	s2,128(a0)
    80003160:	00091d63          	bnez	s2,8000317a <bmap+0x6c>
      addr = balloc(ip->dev);
    80003164:	4108                	lw	a0,0(a0)
    80003166:	e89ff0ef          	jal	80002fee <balloc>
    8000316a:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    8000316e:	02090d63          	beqz	s2,800031a8 <bmap+0x9a>
    80003172:	e052                	sd	s4,0(sp)
        return 0;
      ip->addrs[NDIRECT] = addr;
    80003174:	0929a023          	sw	s2,128(s3)
    80003178:	a011                	j	8000317c <bmap+0x6e>
    8000317a:	e052                	sd	s4,0(sp)
    }
    bp = bread(ip->dev, addr);
    8000317c:	85ca                	mv	a1,s2
    8000317e:	0009a503          	lw	a0,0(s3)
    80003182:	c09ff0ef          	jal	80002d8a <bread>
    80003186:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80003188:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    8000318c:	02049713          	slli	a4,s1,0x20
    80003190:	01e75593          	srli	a1,a4,0x1e
    80003194:	00b784b3          	add	s1,a5,a1
    80003198:	0004a903          	lw	s2,0(s1)
    8000319c:	00090e63          	beqz	s2,800031b8 <bmap+0xaa>
      if(addr){
        a[bn] = addr;
        log_write(bp);
      }
    ...
    800031a2:	cf1ff0ef          	jal	80002e92 <brelse>
    return addr;
  ...
  panic("bmap: out of range");
    ...
    800031b6:	8082                	ret
      addr = balloc(ip->dev);
    800031b8:	0009a503          	lw	a0,0(s3)
    800031bc:	e33ff0ef          	jal	80002fee <balloc>
    800031c0:	0005091b          	sext.w	s2,a0
      if(addr){
    800031c4:	fc090ee3          	beqz	s2,800031a0 <bmap+0x92>
        a[bn] = addr;
    800031c8:	0124a023          	sw	s2,0(s1)
        log_write(bp);
    800031cc:	8552                	mv	a0,s4
    800031ce:	5f7000ef          	jal	80003fc4 <log_write>
    800031d2:	b7f9                	j	800031a0 <bmap+0x92>
    800031d4:	e052                	sd	s4,0(sp)
  panic("bmap: out of range");
    800031d6:	00004517          	auipc	a0,0x4
    800031da:	22250513          	addi	a0,a0,546 # 800073f8 <etext+0x3f8>
    800031de:	e02fd0ef          	jal	800007e0 <panic>

00000000800031e2 <iget>:
{
  ...
  acquire(&itable.lock);
    ...
    800031fa:	dba50513          	addi	a0,a0,-582 # 8001dfb0 <itable>
    800031fe:	9d1fd0ef          	jal	80000bce <acquire>
  empty = 0;
    80003202:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80003204:	0001b497          	auipc	s1,0x1b
    80003208:	dc448493          	addi	s1,s1,-572 # 8001dfc8 <itable+0x18>
    8000320c:	0001d697          	auipc	a3,0x1d
    80003210:	84c68693          	addi	a3,a3,-1972 # 8001fa58 <log>
    80003214:	a039                	j	80003222 <iget+0x40>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003216:	02090963          	beqz	s2,80003248 <iget+0x66>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    8000321a:	08848493          	addi	s1,s1,136
    8000321e:	02d48863          	beq	s1,a3,8000324e <iget+0x6c>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    80003222:	449c                	lw	a5,8(s1)
    80003224:	fef059e3          	blez	a5,80003216 <iget+0x34>
    80003228:	4098                	lw	a4,0(s1)
    8000322a:	ff3716e3          	bne	a4,s3,80003216 <iget+0x34>
    8000322e:	40d8                	lw	a4,4(s1)
    80003230:	ff4713e3          	bne	a4,s4,80003216 <iget+0x34>
      ip->ref++;
    80003234:	2785                	addiw	a5,a5,1
    80003236:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    80003238:	0001b517          	auipc	a0,0x1b
    8000323c:	d7850513          	addi	a0,a0,-648 # 8001dfb0 <itable>
    80003240:	a27fd0ef          	jal	80000c66 <release>
      return ip;
    80003244:	8926                	mv	s2,s1
    80003246:	a02d                	j	80003270 <iget+0x8e>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80003248:	fbe9                	bnez	a5,8000321a <iget+0x38>
      empty = ip;
    8000324a:	8926                	mv	s2,s1
    8000324c:	b7f9                	j	8000321a <iget+0x38>
  if(empty == 0)
    8000324e:	02090a63          	beqz	s2,80003282 <iget+0xa0>
  ip->dev = dev;
    80003252:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    80003256:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    8000325a:	4785                	li	a5,1
    8000325c:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80003260:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    80003264:	0001b517          	auipc	a0,0x1b
    80003268:	d4c50513          	addi	a0,a0,-692 # 8001dfb0 <itable>
    8000326c:	9fbfd0ef          	jal	80000c66 <release>
}
    ...
    80003286:	18e50513          	addi	a0,a0,398 # 80007410 <etext+0x410>
    8000328a:	d56fd0ef          	jal	800007e0 <panic>

000000008000328e <iinit>:
{
  ...
  initlock(&itable.lock, "itable");
    ...
    800032a0:	18458593          	addi	a1,a1,388 # 80007420 <etext+0x420>
    800032a4:	0001b517          	auipc	a0,0x1b
    800032a8:	d0c50513          	addi	a0,a0,-756 # 8001dfb0 <itable>
    800032ac:	8a3fd0ef          	jal	80000b4e <initlock>
  for(i = 0; i < NINODE; i++) {
    800032b0:	0001b497          	auipc	s1,0x1b
    800032b4:	d2848493          	addi	s1,s1,-728 # 8001dfd8 <itable+0x28>
    800032b8:	0001c997          	auipc	s3,0x1c
    800032bc:	7b098993          	addi	s3,s3,1968 # 8001fa68 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    800032c0:	00004917          	auipc	s2,0x4
    800032c4:	16890913          	addi	s2,s2,360 # 80007428 <etext+0x428>
    800032c8:	85ca                	mv	a1,s2
    800032ca:	8526                	mv	a0,s1
    800032cc:	5bb000ef          	jal	80004086 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    800032d0:	08848493          	addi	s1,s1,136
    800032d4:	ff349ae3          	bne	s1,s3,800032c8 <iinit+0x3a>
}
...
00000000800032e6 <ialloc>:
{
  ...
  for(inum = 1; inum < sb.ninodes; inum++){
    800032ee:	0001b717          	auipc	a4,0x1b
    800032f2:	cae72703          	lw	a4,-850(a4) # 8001df9c <sb+0xc>
    800032f6:	4785                	li	a5,1
    800032f8:	06e7f063          	bgeu	a5,a4,80003358 <ialloc+0x72>
    ...
    80003306:	e05a                	sd	s6,0(sp)
    80003308:	8aaa                	mv	s5,a0
    8000330a:	8b2e                	mv	s6,a1
    8000330c:	4905                	li	s2,1
    bp = bread(dev, IBLOCK(inum, sb));
    8000330e:	0001ba17          	auipc	s4,0x1b
    80003312:	c82a0a13          	addi	s4,s4,-894 # 8001df90 <sb>
    80003316:	00495593          	srli	a1,s2,0x4
    8000331a:	018a2783          	lw	a5,24(s4)
    8000331e:	9dbd                	addw	a1,a1,a5
    80003320:	8556                	mv	a0,s5
    80003322:	a69ff0ef          	jal	80002d8a <bread>
    80003326:	84aa                	mv	s1,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    80003328:	05850993          	addi	s3,a0,88
    8000332c:	00f97793          	andi	a5,s2,15
    80003330:	079a                	slli	a5,a5,0x6
    80003332:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    80003334:	00099783          	lh	a5,0(s3)
    80003338:	cb9d                	beqz	a5,8000336e <ialloc+0x88>
    brelse(bp);
    8000333a:	b59ff0ef          	jal	80002e92 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    8000333e:	0905                	addi	s2,s2,1
    80003340:	00ca2703          	lw	a4,12(s4)
    80003344:	0009079b          	sext.w	a5,s2
    80003348:	fce7e7e3          	bltu	a5,a4,80003316 <ialloc+0x30>
  ...
  printf("ialloc: no inodes\n");
    ...
    8000335c:	0d850513          	addi	a0,a0,216 # 80007430 <etext+0x430>
    80003360:	99afd0ef          	jal	800004fa <printf>
  return 0;
    ...
    8000336e:	04000613          	li	a2,64
    80003372:	4581                	li	a1,0
    80003374:	854e                	mv	a0,s3
    80003376:	92dfd0ef          	jal	80000ca2 <memset>
      dip->type = type;
    8000337a:	01699023          	sh	s6,0(s3)
      log_write(bp);   // mark it allocated on the disk
    8000337e:	8526                	mv	a0,s1
    80003380:	445000ef          	jal	80003fc4 <log_write>
      brelse(bp);
    80003384:	8526                	mv	a0,s1
    80003386:	b0dff0ef          	jal	80002e92 <brelse>
      return iget(dev, inum);
    8000338a:	0009059b          	sext.w	a1,s2
    8000338e:	8556                	mv	a0,s5
    80003390:	e53ff0ef          	jal	800031e2 <iget>
    ...
    8000339e:	6b02                	ld	s6,0(sp)
    800033a0:	b7d9                	j	80003366 <ialloc+0x80>

00000000800033a2 <iupdate>:
{
  ...
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    800033b0:	415c                	lw	a5,4(a0)
    800033b2:	0047d79b          	srliw	a5,a5,0x4
    800033b6:	0001b597          	auipc	a1,0x1b
    800033ba:	bf25a583          	lw	a1,-1038(a1) # 8001dfa8 <sb+0x18>
    800033be:	9dbd                	addw	a1,a1,a5
    800033c0:	4108                	lw	a0,0(a0)
    800033c2:	9c9ff0ef          	jal	80002d8a <bread>
    800033c6:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    800033c8:	05850793          	addi	a5,a0,88
    800033cc:	40d8                	lw	a4,4(s1)
    800033ce:	8b3d                	andi	a4,a4,15
    800033d0:	071a                	slli	a4,a4,0x6
    800033d2:	97ba                	add	a5,a5,a4
  dip->type = ip->type;
    800033d4:	04449703          	lh	a4,68(s1)
    800033d8:	00e79023          	sh	a4,0(a5)
  dip->major = ip->major;
    800033dc:	04649703          	lh	a4,70(s1)
    800033e0:	00e79123          	sh	a4,2(a5)
  dip->minor = ip->minor;
    800033e4:	04849703          	lh	a4,72(s1)
    800033e8:	00e79223          	sh	a4,4(a5)
  dip->nlink = ip->nlink;
    800033ec:	04a49703          	lh	a4,74(s1)
    800033f0:	00e79323          	sh	a4,6(a5)
  dip->size = ip->size;
    800033f4:	44f8                	lw	a4,76(s1)
    800033f6:	c798                	sw	a4,8(a5)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    800033f8:	03400613          	li	a2,52
    800033fc:	05048593          	addi	a1,s1,80
    80003400:	00c78513          	addi	a0,a5,12
    80003404:	8fbfd0ef          	jal	80000cfe <memmove>
  log_write(bp);
    80003408:	854a                	mv	a0,s2
    8000340a:	3bb000ef          	jal	80003fc4 <log_write>
  brelse(bp);
    8000340e:	854a                	mv	a0,s2
    80003410:	a83ff0ef          	jal	80002e92 <brelse>
}
...
0000000080003420 <idup>:
{
  ...
  acquire(&itable.lock);
    ...
    80003430:	b8450513          	addi	a0,a0,-1148 # 8001dfb0 <itable>
    80003434:	f9afd0ef          	jal	80000bce <acquire>
  ip->ref++;
  ...
  release(&itable.lock);
    ...
    80003442:	b7250513          	addi	a0,a0,-1166 # 8001dfb0 <itable>
    80003446:	821fd0ef          	jal	80000c66 <release>
}
...
0000000080003456 <ilock>:
{
  ...
  if(ip == 0 || ip->ref < 1)
    80003460:	cd19                	beqz	a0,8000347e <ilock+0x28>
    80003462:	84aa                	mv	s1,a0
    80003464:	451c                	lw	a5,8(a0)
    80003466:	00f05c63          	blez	a5,8000347e <ilock+0x28>
  acquiresleep(&ip->lock);
    8000346a:	0541                	addi	a0,a0,16
    8000346c:	451000ef          	jal	800040bc <acquiresleep>
  if(ip->valid == 0){
    80003470:	40bc                	lw	a5,64(s1)
    80003472:	cf89                	beqz	a5,8000348c <ilock+0x36>
}
    ...
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    8000348e:	40dc                	lw	a5,4(s1)
    80003490:	0047d79b          	srliw	a5,a5,0x4
    80003494:	0001b597          	auipc	a1,0x1b
    80003498:	b145a583          	lw	a1,-1260(a1) # 8001dfa8 <sb+0x18>
    8000349c:	9dbd                	addw	a1,a1,a5
    8000349e:	4088                	lw	a0,0(s1)
    800034a0:	8ebff0ef          	jal	80002d8a <bread>
    800034a4:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    800034a6:	05850593          	addi	a1,a0,88
    800034aa:	40dc                	lw	a5,4(s1)
    800034ac:	8bbd                	andi	a5,a5,15
    800034ae:	079a                	slli	a5,a5,0x6
    800034b0:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    800034b2:	00059783          	lh	a5,0(a1)
    800034b6:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    800034ba:	00259783          	lh	a5,2(a1)
    800034be:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    800034c2:	00459783          	lh	a5,4(a1)
    800034c6:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    800034ca:	00659783          	lh	a5,6(a1)
    800034ce:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    800034d2:	459c                	lw	a5,8(a1)
    800034d4:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    800034d6:	03400613          	li	a2,52
    800034da:	05b1                	addi	a1,a1,12
    800034dc:	05048513          	addi	a0,s1,80
    800034e0:	81ffd0ef          	jal	80000cfe <memmove>
    brelse(bp);
    800034e4:	854a                	mv	a0,s2
    800034e6:	9adff0ef          	jal	80002e92 <brelse>
    ip->valid = 1;
    800034ea:	4785                	li	a5,1
    800034ec:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    800034ee:	04449783          	lh	a5,68(s1)
    800034f2:	c399                	beqz	a5,800034f8 <ilock+0xa2>
    800034f4:	6902                	ld	s2,0(sp)
    800034f6:	bfbd                	j	80003474 <ilock+0x1e>
      panic("ilock: no type");
    800034f8:	00004517          	auipc	a0,0x4
    800034fc:	f5850513          	addi	a0,a0,-168 # 80007450 <etext+0x450>
    80003500:	ae0fd0ef          	jal	800007e0 <panic>

0000000080003504 <iunlock>:
{
  ...
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    80003510:	c505                	beqz	a0,80003538 <iunlock+0x34>
    80003512:	84aa                	mv	s1,a0
    80003514:	01050913          	addi	s2,a0,16
    80003518:	854a                	mv	a0,s2
    8000351a:	421000ef          	jal	8000413a <holdingsleep>
    8000351e:	cd09                	beqz	a0,80003538 <iunlock+0x34>
    80003520:	449c                	lw	a5,8(s1)
    80003522:	00f05b63          	blez	a5,80003538 <iunlock+0x34>
  releasesleep(&ip->lock);
    80003526:	854a                	mv	a0,s2
    80003528:	3db000ef          	jal	80004102 <releasesleep>
}
    ...
    8000353c:	f2850513          	addi	a0,a0,-216 # 80007460 <etext+0x460>
    80003540:	aa0fd0ef          	jal	800007e0 <panic>

0000000080003544 <itrunc>:
...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    ...
    8000355c:	a021                	j	80003564 <itrunc+0x20>
    8000355e:	0491                	addi	s1,s1,4
    80003560:	01248b63          	beq	s1,s2,80003576 <itrunc+0x32>
    if(ip->addrs[i]){
    80003564:	408c                	lw	a1,0(s1)
    80003566:	dde5                	beqz	a1,8000355e <itrunc+0x1a>
      bfree(ip->dev, ip->addrs[i]);
    80003568:	0009a503          	lw	a0,0(s3)
    8000356c:	a17ff0ef          	jal	80002f82 <bfree>
      ip->addrs[i] = 0;
    80003570:	0004a023          	sw	zero,0(s1)
    80003574:	b7ed                	j	8000355e <itrunc+0x1a>
    }
  }

  if(ip->addrs[NDIRECT]){
    80003576:	0809a583          	lw	a1,128(s3)
    8000357a:	ed89                	bnez	a1,80003594 <itrunc+0x50>
    brelse(bp);
  ...
  iupdate(ip);
    ...
    80003582:	e21ff0ef          	jal	800033a2 <iupdate>
}
    ...
    8000359a:	ff0ff0ef          	jal	80002d8a <bread>
    8000359e:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    800035a0:	05850493          	addi	s1,a0,88
    800035a4:	45850913          	addi	s2,a0,1112
    800035a8:	a021                	j	800035b0 <itrunc+0x6c>
    800035aa:	0491                	addi	s1,s1,4
    800035ac:	01248963          	beq	s1,s2,800035be <itrunc+0x7a>
      if(a[j])
    800035b0:	408c                	lw	a1,0(s1)
    800035b2:	dde5                	beqz	a1,800035aa <itrunc+0x66>
        bfree(ip->dev, a[j]);
    800035b4:	0009a503          	lw	a0,0(s3)
    800035b8:	9cbff0ef          	jal	80002f82 <bfree>
    800035bc:	b7fd                	j	800035aa <itrunc+0x66>
    brelse(bp);
    800035be:	8552                	mv	a0,s4
    800035c0:	8d3ff0ef          	jal	80002e92 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    800035c4:	0809a583          	lw	a1,128(s3)
    800035c8:	0009a503          	lw	a0,0(s3)
    800035cc:	9b7ff0ef          	jal	80002f82 <bfree>
    ip->addrs[NDIRECT] = 0;
    800035d0:	0809a023          	sw	zero,128(s3)
    800035d4:	6a02                	ld	s4,0(sp)
    800035d6:	b75d                	j	8000357c <itrunc+0x38>

00000000800035d8 <iput>:
{
  ...
  acquire(&itable.lock);
    ...
    800035e8:	9cc50513          	addi	a0,a0,-1588 # 8001dfb0 <itable>
    800035ec:	de2fd0ef          	jal	80000bce <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    800035f0:	4498                	lw	a4,8(s1)
    800035f2:	4785                	li	a5,1
    800035f4:	02f70063          	beq	a4,a5,80003614 <iput+0x3c>
  ip->ref--;
  ...
  release(&itable.lock);
    ...
    80003602:	9b250513          	addi	a0,a0,-1614 # 8001dfb0 <itable>
  ...
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    ...
    80003616:	d3ed                	beqz	a5,800035f8 <iput+0x20>
    80003618:	04a49783          	lh	a5,74(s1)
    8000361c:	fff1                	bnez	a5,800035f8 <iput+0x20>
    8000361e:	e04a                	sd	s2,0(sp)
    acquiresleep(&ip->lock);
    80003620:	01048913          	addi	s2,s1,16
    80003624:	854a                	mv	a0,s2
    80003626:	297000ef          	jal	800040bc <acquiresleep>
    release(&itable.lock);
    8000362a:	0001b517          	auipc	a0,0x1b
    8000362e:	98650513          	addi	a0,a0,-1658 # 8001dfb0 <itable>
    80003632:	e34fd0ef          	jal	80000c66 <release>
    itrunc(ip);
    80003636:	8526                	mv	a0,s1
    80003638:	f0dff0ef          	jal	80003544 <itrunc>
    ip->type = 0;
    8000363c:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    80003640:	8526                	mv	a0,s1
    80003642:	d61ff0ef          	jal	800033a2 <iupdate>
    ip->valid = 0;
    80003646:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    8000364a:	854a                	mv	a0,s2
    8000364c:	2b7000ef          	jal	80004102 <releasesleep>
    acquire(&itable.lock);
    80003650:	0001b517          	auipc	a0,0x1b
    80003654:	96050513          	addi	a0,a0,-1696 # 8001dfb0 <itable>
    80003658:	d76fd0ef          	jal	80000bce <acquire>
    8000365c:	6902                	ld	s2,0(sp)
    8000365e:	bf69                	j	800035f8 <iput+0x20>

0000000080003660 <iunlockput>:
{
  ...
  iunlock(ip);
    8000366c:	e99ff0ef          	jal	80003504 <iunlock>
  iput(ip);
    80003670:	8526                	mv	a0,s1
    80003672:	f67ff0ef          	jal	800035d8 <iput>
}
...
0000000080003680 <ireclaim>:
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003680:	0001b717          	auipc	a4,0x1b
    80003684:	91c72703          	lw	a4,-1764(a4) # 8001df9c <sb+0xc>
    80003688:	4785                	li	a5,1
    8000368a:	0ae7ff63          	bgeu	a5,a4,80003748 <ireclaim+0xc8>
{
    ...
    800036a0:	0080                	addi	s0,sp,64
  for (int inum = 1; inum < sb.ninodes; inum++) {
    800036a2:	4485                	li	s1,1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    800036a4:	00050a1b          	sext.w	s4,a0
    800036a8:	0001ba97          	auipc	s5,0x1b
    800036ac:	8e8a8a93          	addi	s5,s5,-1816 # 8001df90 <sb>
      printf("ireclaim: orphaned inode %d\n", inum);
    800036b0:	00004b17          	auipc	s6,0x4
    800036b4:	db8b0b13          	addi	s6,s6,-584 # 80007468 <etext+0x468>
    800036b8:	a099                	j	800036fe <ireclaim+0x7e>
    800036ba:	85ce                	mv	a1,s3
    800036bc:	855a                	mv	a0,s6
    800036be:	e3dfc0ef          	jal	800004fa <printf>
      ip = iget(dev, inum);
    800036c2:	85ce                	mv	a1,s3
    800036c4:	8552                	mv	a0,s4
    800036c6:	b1dff0ef          	jal	800031e2 <iget>
    800036ca:	89aa                	mv	s3,a0
    brelse(bp);
    800036cc:	854a                	mv	a0,s2
    800036ce:	fc4ff0ef          	jal	80002e92 <brelse>
    if (ip) {
    800036d2:	00098f63          	beqz	s3,800036f0 <ireclaim+0x70>
      begin_op();
    800036d6:	76a000ef          	jal	80003e40 <begin_op>
      ilock(ip);
    800036da:	854e                	mv	a0,s3
    800036dc:	d7bff0ef          	jal	80003456 <ilock>
      iunlock(ip);
    800036e0:	854e                	mv	a0,s3
    800036e2:	e23ff0ef          	jal	80003504 <iunlock>
      iput(ip);
    800036e6:	854e                	mv	a0,s3
    800036e8:	ef1ff0ef          	jal	800035d8 <iput>
      end_op();
    800036ec:	7be000ef          	jal	80003eaa <end_op>
  for (int inum = 1; inum < sb.ninodes; inum++) {
    800036f0:	0485                	addi	s1,s1,1
    800036f2:	00caa703          	lw	a4,12(s5)
    800036f6:	0004879b          	sext.w	a5,s1
    800036fa:	02e7fd63          	bgeu	a5,a4,80003734 <ireclaim+0xb4>
    800036fe:	0004899b          	sext.w	s3,s1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    80003702:	0044d593          	srli	a1,s1,0x4
    80003706:	018aa783          	lw	a5,24(s5)
    8000370a:	9dbd                	addw	a1,a1,a5
    8000370c:	8552                	mv	a0,s4
    8000370e:	e7cff0ef          	jal	80002d8a <bread>
    80003712:	892a                	mv	s2,a0
    struct dinode *dip = (struct dinode *)bp->data + inum % IPB;
    80003714:	05850793          	addi	a5,a0,88
    80003718:	00f9f713          	andi	a4,s3,15
    8000371c:	071a                	slli	a4,a4,0x6
    8000371e:	97ba                	add	a5,a5,a4
    if (dip->type != 0 && dip->nlink == 0) {  // is an orphaned inode
    80003720:	00079703          	lh	a4,0(a5)
    80003724:	c701                	beqz	a4,8000372c <ireclaim+0xac>
    80003726:	00679783          	lh	a5,6(a5)
    8000372a:	dbc1                	beqz	a5,800036ba <ireclaim+0x3a>
    brelse(bp);
    8000372c:	854a                	mv	a0,s2
    8000372e:	f64ff0ef          	jal	80002e92 <brelse>
    if (ip) {
    80003732:	bf7d                	j	800036f0 <ireclaim+0x70>
}
...
fsinit(int dev) {
  ...
  bp = bread(dev, 1);
    ...
    8000375c:	e2eff0ef          	jal	80002d8a <bread>
    80003760:	892a                	mv	s2,a0
  memmove(sb, bp->data, sizeof(*sb));
    80003762:	0001b997          	auipc	s3,0x1b
    80003766:	82e98993          	addi	s3,s3,-2002 # 8001df90 <sb>
    8000376a:	02000613          	li	a2,32
    8000376e:	05850593          	addi	a1,a0,88
    80003772:	854e                	mv	a0,s3
    80003774:	d8afd0ef          	jal	80000cfe <memmove>
  brelse(bp);
    80003778:	854a                	mv	a0,s2
    8000377a:	f18ff0ef          	jal	80002e92 <brelse>
  if(sb.magic != FSMAGIC)
    8000377e:	0009a703          	lw	a4,0(s3)
    80003782:	102037b7          	lui	a5,0x10203
    80003786:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    8000378a:	02f71363          	bne	a4,a5,800037b0 <fsinit+0x66>
  initlog(dev, &sb);
    8000378e:	0001b597          	auipc	a1,0x1b
    80003792:	80258593          	addi	a1,a1,-2046 # 8001df90 <sb>
    80003796:	8526                	mv	a0,s1
    80003798:	62a000ef          	jal	80003dc2 <initlog>
  ireclaim(dev);
    8000379c:	8526                	mv	a0,s1
    8000379e:	ee3ff0ef          	jal	80003680 <ireclaim>
}
    ...
    800037b4:	cd850513          	addi	a0,a0,-808 # 80007488 <etext+0x488>
    800037b8:	828fd0ef          	jal	800007e0 <panic>
    ...
    800037e8:	0ed7eb63          	bltu	a5,a3,800038de <readi+0xf8>
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    80003846:	9effe0ef          	jal	80002234 <either_copyout>
    ...
    80003850:	e42ff0ef          	jal	80002e92 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    8000389a:	df8ff0ef          	jal	80002e92 <brelse>
      tot = -1;
    ...
    800038e4:	10d7e063          	bltu	a5,a3,800039e4 <writei+0x102>
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    80003942:	93dfe0ef          	jal	8000227e <either_copyin>
    ...
    8000394c:	678000ef          	jal	80003fc4 <log_write>
    brelse(bp);
    80003950:	8526                	mv	a0,s1
    80003952:	d40ff0ef          	jal	80002e92 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    8000399c:	cf6ff0ef          	jal	80002e92 <brelse>
  }
  ...
  iupdate(ip);
    ...
    800039b8:	9ebff0ef          	jal	800033a2 <iupdate>
...
namecmp(const char *s, const char *t)
{
  ...
  return strncmp(s, t, DIRSIZ);
    ...
    800039fa:	b74fd0ef          	jal	80000d6e <strncmp>
}
...
dirlookup(struct inode *dp, char *name, uint *poff)
{
  ...
  for(off = 0; off < dp->size; off += sizeof(de)){
    ...
    80003a34:	a7050513          	addi	a0,a0,-1424 # 800074a0 <etext+0x4a0>
    80003a38:	da9fc0ef          	jal	800007e0 <panic>
      panic("dirlookup read");
    80003a3c:	00004517          	auipc	a0,0x4
    80003a40:	a7c50513          	addi	a0,a0,-1412 # 800074b8 <etext+0x4b8>
    80003a44:	d9dfc0ef          	jal	800007e0 <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    ...
    80003a8a:	f58ff0ef          	jal	800031e2 <iget>
...
namex(char *path, int nameiparent, char *name)
{
  ...
  if(*path == '/')
    ...
    ip = idup(myproc()->cwd);
    80003ace:	e01fd0ef          	jal	800018ce <myproc>
    80003ad2:	15053503          	ld	a0,336(a0)
    80003ad6:	94bff0ef          	jal	80003420 <idup>
  ...
  return ip;
    ...
    80003b18:	9edff0ef          	jal	80003504 <iunlock>
      return ip;
    80003b1c:	bff9                	j	80003afa <namex+0x58>
      iunlockput(ip);
    80003b1e:	8552                	mv	a0,s4
    80003b20:	b41ff0ef          	jal	80003660 <iunlockput>
      return 0;
  ...
  if(len >= DIRSIZ)
    ...
    80003b3a:	9c4fd0ef          	jal	80000cfe <memmove>
  ...
  while(*path == '/')
    ...
    80003b54:	903ff0ef          	jal	80003456 <ilock>
    if(ip->type != T_DIR){
    ...
    80003b7a:	ae7ff0ef          	jal	80003660 <iunlockput>
    ip = next;
  ...
  if(nameiparent){
    ...
    80003bca:	a0fff0ef          	jal	800035d8 <iput>
    return 0;
...
0000000080003bd2 <dirlink>:
{
  ...
  strncpy(de.name, name, DIRSIZ);
    ...
    80003c26:	97efd0ef          	jal	80000da4 <strncpy>
  de.inum = inum;
  ...
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    ...
    80003c56:	8082                	ret
    iput(ip);
    80003c58:	981ff0ef          	jal	800035d8 <iput>
    return -1;
    80003c5c:	557d                	li	a0,-1
    80003c5e:	b7f5                	j	80003c4a <dirlink+0x78>
      panic("dirlink read");
    80003c60:	00004517          	auipc	a0,0x4
    80003c64:	86850513          	addi	a0,a0,-1944 # 800074c8 <etext+0x4c8>
    80003c68:	b79fc0ef          	jal	800007e0 <panic>
...
write_head(void)
{
  ...
  struct buf *buf = bread(log.dev, log.start);
    ...
    80003cae:	dae90913          	addi	s2,s2,-594 # 8001fa58 <log>
    80003cb2:	01892583          	lw	a1,24(s2)
    80003cb6:	02492503          	lw	a0,36(s2)
    80003cba:	8d0ff0ef          	jal	80002d8a <bread>
  ...
  for (i = 0; i < log.lh.n; i++) {
    ...
    80003cce:	dba70713          	addi	a4,a4,-582 # 8001fa84 <log+0x2c>
  ...
  bwrite(buf);
    ...
    80003ce6:	97aff0ef          	jal	80002e60 <bwrite>
  brelse(buf);
    80003cea:	8526                	mv	a0,s1
    80003cec:	9a6ff0ef          	jal	80002e92 <brelse>
}
...
0000000080003cfc <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    ...
    80003d00:	d847a783          	lw	a5,-636(a5) # 8001fa80 <log+0x28>
    80003d04:	0af05e63          	blez	a5,80003dc0 <install_trans+0xc4>
{
    ...
    80003d1e:	8b2a                	mv	s6,a0
    80003d20:	0001ca97          	auipc	s5,0x1c
    80003d24:	d64a8a93          	addi	s5,s5,-668 # 8001fa84 <log+0x2c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d28:	4981                	li	s3,0
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003d2a:	00003b97          	auipc	s7,0x3
    80003d2e:	7aeb8b93          	addi	s7,s7,1966 # 800074d8 <etext+0x4d8>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003d32:	0001ca17          	auipc	s4,0x1c
    80003d36:	d26a0a13          	addi	s4,s4,-730 # 8001fa58 <log>
    80003d3a:	a025                	j	80003d62 <install_trans+0x66>
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003d3c:	000aa603          	lw	a2,0(s5)
    80003d40:	85ce                	mv	a1,s3
    80003d42:	855e                	mv	a0,s7
    80003d44:	fb6fc0ef          	jal	800004fa <printf>
    80003d48:	a839                	j	80003d66 <install_trans+0x6a>
    brelse(lbuf);
    80003d4a:	854a                	mv	a0,s2
    80003d4c:	946ff0ef          	jal	80002e92 <brelse>
    brelse(dbuf);
    80003d50:	8526                	mv	a0,s1
    80003d52:	940ff0ef          	jal	80002e92 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d56:	2985                	addiw	s3,s3,1
    80003d58:	0a91                	addi	s5,s5,4
    80003d5a:	028a2783          	lw	a5,40(s4)
    80003d5e:	04f9d663          	bge	s3,a5,80003daa <install_trans+0xae>
    if(recovering) {
    80003d62:	fc0b1de3          	bnez	s6,80003d3c <install_trans+0x40>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003d66:	018a2583          	lw	a1,24(s4)
    80003d6a:	013585bb          	addw	a1,a1,s3
    80003d6e:	2585                	addiw	a1,a1,1
    80003d70:	024a2503          	lw	a0,36(s4)
    80003d74:	816ff0ef          	jal	80002d8a <bread>
    80003d78:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80003d7a:	000aa583          	lw	a1,0(s5)
    80003d7e:	024a2503          	lw	a0,36(s4)
    80003d82:	808ff0ef          	jal	80002d8a <bread>
    80003d86:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80003d88:	40000613          	li	a2,1024
    80003d8c:	05890593          	addi	a1,s2,88
    80003d90:	05850513          	addi	a0,a0,88
    80003d94:	f6bfc0ef          	jal	80000cfe <memmove>
    bwrite(dbuf);  // write dst to disk
    80003d98:	8526                	mv	a0,s1
    80003d9a:	8c6ff0ef          	jal	80002e60 <bwrite>
    if(recovering == 0)
    80003d9e:	fa0b16e3          	bnez	s6,80003d4a <install_trans+0x4e>
      bunpin(dbuf);
    80003da2:	8526                	mv	a0,s1
    80003da4:	9aaff0ef          	jal	80002f4e <bunpin>
    80003da8:	b74d                	j	80003d4a <install_trans+0x4e>
}
...
0000000080003dc2 <initlog>:
{
  ...
  initlock(&log.lock, "log");
    80003dd4:	0001c497          	auipc	s1,0x1c
    80003dd8:	c8448493          	addi	s1,s1,-892 # 8001fa58 <log>
    80003ddc:	00003597          	auipc	a1,0x3
    80003de0:	71c58593          	addi	a1,a1,1820 # 800074f8 <etext+0x4f8>
    80003de4:	8526                	mv	a0,s1
    80003de6:	d69fc0ef          	jal	80000b4e <initlock>
  log.start = sb->logstart;
    80003dea:	0149a583          	lw	a1,20(s3)
    80003dee:	cc8c                	sw	a1,24(s1)
  log.dev = dev;
    80003df0:	0324a223          	sw	s2,36(s1)
  struct buf *buf = bread(log.dev, log.start);
    80003df4:	854a                	mv	a0,s2
    80003df6:	f95fe0ef          	jal	80002d8a <bread>
  log.lh.n = lh->n;
    80003dfa:	4d30                	lw	a2,88(a0)
    80003dfc:	d490                	sw	a2,40(s1)
  for (i = 0; i < log.lh.n; i++) {
    80003dfe:	00c05f63          	blez	a2,80003e1c <initlog+0x5a>
    80003e02:	87aa                	mv	a5,a0
    80003e04:	0001c717          	auipc	a4,0x1c
    80003e08:	c8070713          	addi	a4,a4,-896 # 8001fa84 <log+0x2c>
    80003e0c:	060a                	slli	a2,a2,0x2
    80003e0e:	962a                	add	a2,a2,a0
    log.lh.block[i] = lh->block[i];
    80003e10:	4ff4                	lw	a3,92(a5)
    80003e12:	c314                	sw	a3,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80003e14:	0791                	addi	a5,a5,4
    80003e16:	0711                	addi	a4,a4,4
    80003e18:	fec79ce3          	bne	a5,a2,80003e10 <initlog+0x4e>
  brelse(buf);
    80003e1c:	876ff0ef          	jal	80002e92 <brelse>
...
recover_from_log(void)
{
  ...
  install_trans(1); // if committed, copy from log to disk
    ...
    80003e22:	edbff0ef          	jal	80003cfc <install_trans>
  log.lh.n = 0;
    80003e26:	0001c797          	auipc	a5,0x1c
    80003e2a:	c407ad23          	sw	zero,-934(a5) # 8001fa80 <log+0x28>
  write_head(); // clear the log
    80003e2e:	e71ff0ef          	jal	80003c9e <write_head>
}
...
begin_op(void)
{
  ...
  acquire(&log.lock);
    ...
    80003e50:	c0c50513          	addi	a0,a0,-1012 # 8001fa58 <log>
    80003e54:	d7bfc0ef          	jal	80000bce <acquire>
  while(1){
    if(log.committing){
    80003e58:	0001c497          	auipc	s1,0x1c
    80003e5c:	c0048493          	addi	s1,s1,-1024 # 8001fa58 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003e60:	4979                	li	s2,30
    80003e62:	a029                	j	80003e6c <begin_op+0x2c>
      sleep(&log, &log.lock);
    80003e64:	85a6                	mv	a1,s1
    80003e66:	8526                	mv	a0,s1
    80003e68:	870fe0ef          	jal	80001ed8 <sleep>
    if(log.committing){
    80003e6c:	509c                	lw	a5,32(s1)
    80003e6e:	fbfd                	bnez	a5,80003e64 <begin_op+0x24>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003e70:	4cd8                	lw	a4,28(s1)
    80003e72:	2705                	addiw	a4,a4,1
    80003e74:	0027179b          	slliw	a5,a4,0x2
    80003e78:	9fb9                	addw	a5,a5,a4
    80003e7a:	0017979b          	slliw	a5,a5,0x1
    80003e7e:	5494                	lw	a3,40(s1)
    80003e80:	9fb5                	addw	a5,a5,a3
    80003e82:	00f95763          	bge	s2,a5,80003e90 <begin_op+0x50>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80003e86:	85a6                	mv	a1,s1
    80003e88:	8526                	mv	a0,s1
    80003e8a:	84efe0ef          	jal	80001ed8 <sleep>
    80003e8e:	bff9                	j	80003e6c <begin_op+0x2c>
    } else {
      log.outstanding += 1;
    80003e90:	0001c517          	auipc	a0,0x1c
    80003e94:	bc850513          	addi	a0,a0,-1080 # 8001fa58 <log>
    80003e98:	cd58                	sw	a4,28(a0)
      release(&log.lock);
    80003e9a:	dcdfc0ef          	jal	80000c66 <release>
      break;
    }
  }
}
...
end_op(void)
{
  ...
  acquire(&log.lock);
    80003eb6:	0001c497          	auipc	s1,0x1c
    80003eba:	ba248493          	addi	s1,s1,-1118 # 8001fa58 <log>
    80003ebe:	8526                	mv	a0,s1
    80003ec0:	d0ffc0ef          	jal	80000bce <acquire>
  log.outstanding -= 1;
    80003ec4:	4cdc                	lw	a5,28(s1)
    80003ec6:	37fd                	addiw	a5,a5,-1
    80003ec8:	0007891b          	sext.w	s2,a5
    80003ecc:	ccdc                	sw	a5,28(s1)
  if(log.committing)
    80003ece:	509c                	lw	a5,32(s1)
    80003ed0:	ef9d                	bnez	a5,80003f0e <end_op+0x64>
    panic("log.committing");
  if(log.outstanding == 0){
    80003ed2:	04091763          	bnez	s2,80003f20 <end_op+0x76>
    do_commit = 1;
    log.committing = 1;
    80003ed6:	0001c497          	auipc	s1,0x1c
    80003eda:	b8248493          	addi	s1,s1,-1150 # 8001fa58 <log>
    80003ede:	4785                	li	a5,1
    80003ee0:	d09c                	sw	a5,32(s1)
  ...
  release(&log.lock);
    ...
    80003ee4:	d83fc0ef          	jal	80000c66 <release>
}
...
{
  if (log.lh.n > 0) {
    80003ee8:	549c                	lw	a5,40(s1)
    80003eea:	04f04b63          	bgtz	a5,80003f40 <end_op+0x96>
    acquire(&log.lock);
    80003eee:	0001c497          	auipc	s1,0x1c
    80003ef2:	b6a48493          	addi	s1,s1,-1174 # 8001fa58 <log>
    80003ef6:	8526                	mv	a0,s1
    80003ef8:	cd7fc0ef          	jal	80000bce <acquire>
    log.committing = 0;
    80003efc:	0204a023          	sw	zero,32(s1)
    wakeup(&log);
    80003f00:	8526                	mv	a0,s1
    80003f02:	822fe0ef          	jal	80001f24 <wakeup>
    release(&log.lock);
    80003f06:	8526                	mv	a0,s1
    80003f08:	d5ffc0ef          	jal	80000c66 <release>
}
    80003f0c:	a025                	j	80003f34 <end_op+0x8a>
    80003f0e:	ec4e                	sd	s3,24(sp)
    80003f10:	e852                	sd	s4,16(sp)
    80003f12:	e456                	sd	s5,8(sp)
    panic("log.committing");
    80003f14:	00003517          	auipc	a0,0x3
    80003f18:	5ec50513          	addi	a0,a0,1516 # 80007500 <etext+0x500>
    80003f1c:	8c5fc0ef          	jal	800007e0 <panic>
    wakeup(&log);
    80003f20:	0001c497          	auipc	s1,0x1c
    80003f24:	b3848493          	addi	s1,s1,-1224 # 8001fa58 <log>
    80003f28:	8526                	mv	a0,s1
    80003f2a:	ffbfd0ef          	jal	80001f24 <wakeup>
  release(&log.lock);
    80003f2e:	8526                	mv	a0,s1
    80003f30:	d37fc0ef          	jal	80000c66 <release>
}
  ...
  for (tail = 0; tail < log.lh.n; tail++) {
    80003f46:	0001ca97          	auipc	s5,0x1c
    80003f4a:	b3ea8a93          	addi	s5,s5,-1218 # 8001fa84 <log+0x2c>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003f4e:	0001ca17          	auipc	s4,0x1c
    80003f52:	b0aa0a13          	addi	s4,s4,-1270 # 8001fa58 <log>
    80003f56:	018a2583          	lw	a1,24(s4)
    80003f5a:	012585bb          	addw	a1,a1,s2
    80003f5e:	2585                	addiw	a1,a1,1
    80003f60:	024a2503          	lw	a0,36(s4)
    80003f64:	e27fe0ef          	jal	80002d8a <bread>
    80003f68:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80003f6a:	000aa583          	lw	a1,0(s5)
    80003f6e:	024a2503          	lw	a0,36(s4)
    80003f72:	e19fe0ef          	jal	80002d8a <bread>
    80003f76:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80003f78:	40000613          	li	a2,1024
    80003f7c:	05850593          	addi	a1,a0,88
    80003f80:	05848513          	addi	a0,s1,88
    80003f84:	d7bfc0ef          	jal	80000cfe <memmove>
    bwrite(to);  // write the log
    80003f88:	8526                	mv	a0,s1
    80003f8a:	ed7fe0ef          	jal	80002e60 <bwrite>
    brelse(from);
    80003f8e:	854e                	mv	a0,s3
    80003f90:	f03fe0ef          	jal	80002e92 <brelse>
    brelse(to);
    80003f94:	8526                	mv	a0,s1
    80003f96:	efdfe0ef          	jal	80002e92 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003f9a:	2905                	addiw	s2,s2,1
    80003f9c:	0a91                	addi	s5,s5,4
    80003f9e:	028a2783          	lw	a5,40(s4)
    80003fa2:	faf94ae3          	blt	s2,a5,80003f56 <end_op+0xac>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80003fa6:	cf9ff0ef          	jal	80003c9e <write_head>
    install_trans(0); // Now install writes to home locations
    80003faa:	4501                	li	a0,0
    80003fac:	d51ff0ef          	jal	80003cfc <install_trans>
    log.lh.n = 0;
    80003fb0:	0001c797          	auipc	a5,0x1c
    80003fb4:	ac07a823          	sw	zero,-1328(a5) # 8001fa80 <log+0x28>
    write_head();    // Erase the transaction from the log
    80003fb8:	ce7ff0ef          	jal	80003c9e <write_head>
    80003fbc:	69e2                	ld	s3,24(sp)
    80003fbe:	6a42                	ld	s4,16(sp)
    80003fc0:	6aa2                	ld	s5,8(sp)
    80003fc2:	b735                	j	80003eee <end_op+0x44>

0000000080003fc4 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
  ...
  acquire(&log.lock);
    80003fd2:	0001c917          	auipc	s2,0x1c
    80003fd6:	a8690913          	addi	s2,s2,-1402 # 8001fa58 <log>
    80003fda:	854a                	mv	a0,s2
    80003fdc:	bf3fc0ef          	jal	80000bce <acquire>
  if (log.lh.n >= LOGBLOCKS)
    80003fe0:	02892603          	lw	a2,40(s2)
    80003fe4:	47f5                	li	a5,29
    80003fe6:	04c7cc63          	blt	a5,a2,8000403e <log_write+0x7a>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80003fea:	0001c797          	auipc	a5,0x1c
    80003fee:	a8a7a783          	lw	a5,-1398(a5) # 8001fa74 <log+0x1c>
    80003ff2:	04f05c63          	blez	a5,8000404a <log_write+0x86>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80003ff6:	4781                	li	a5,0
    80003ff8:	04c05f63          	blez	a2,80004056 <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003ffc:	44cc                	lw	a1,12(s1)
    80003ffe:	0001c717          	auipc	a4,0x1c
    80004002:	a8670713          	addi	a4,a4,-1402 # 8001fa84 <log+0x2c>
  for (i = 0; i < log.lh.n; i++) {
    80004006:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80004008:	4314                	lw	a3,0(a4)
    8000400a:	04b68663          	beq	a3,a1,80004056 <log_write+0x92>
  for (i = 0; i < log.lh.n; i++) {
    8000400e:	2785                	addiw	a5,a5,1
    80004010:	0711                	addi	a4,a4,4
    80004012:	fef61be3          	bne	a2,a5,80004008 <log_write+0x44>
      break;
  }
  log.lh.block[i] = b->blockno;
    80004016:	0621                	addi	a2,a2,8
    80004018:	060a                	slli	a2,a2,0x2
    8000401a:	0001c797          	auipc	a5,0x1c
    8000401e:	a3e78793          	addi	a5,a5,-1474 # 8001fa58 <log>
    80004022:	97b2                	add	a5,a5,a2
    80004024:	44d8                	lw	a4,12(s1)
    80004026:	c7d8                	sw	a4,12(a5)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80004028:	8526                	mv	a0,s1
    8000402a:	ef1fe0ef          	jal	80002f1a <bpin>
    log.lh.n++;
    8000402e:	0001c717          	auipc	a4,0x1c
    80004032:	a2a70713          	addi	a4,a4,-1494 # 8001fa58 <log>
    80004036:	571c                	lw	a5,40(a4)
    80004038:	2785                	addiw	a5,a5,1
    8000403a:	d71c                	sw	a5,40(a4)
    8000403c:	a80d                	j	8000406e <log_write+0xaa>
    panic("too big a transaction");
    8000403e:	00003517          	auipc	a0,0x3
    80004042:	4d250513          	addi	a0,a0,1234 # 80007510 <etext+0x510>
    80004046:	f9afc0ef          	jal	800007e0 <panic>
    panic("log_write outside of trans");
    8000404a:	00003517          	auipc	a0,0x3
    8000404e:	4de50513          	addi	a0,a0,1246 # 80007528 <etext+0x528>
    80004052:	f8efc0ef          	jal	800007e0 <panic>
  log.lh.block[i] = b->blockno;
    80004056:	00878693          	addi	a3,a5,8
    8000405a:	068a                	slli	a3,a3,0x2
    8000405c:	0001c717          	auipc	a4,0x1c
    80004060:	9fc70713          	addi	a4,a4,-1540 # 8001fa58 <log>
    80004064:	9736                	add	a4,a4,a3
    80004066:	44d4                	lw	a3,12(s1)
    80004068:	c754                	sw	a3,12(a4)
  if (i == log.lh.n) {  // Add new block to log?
    8000406a:	faf60fe3          	beq	a2,a5,80004028 <log_write+0x64>
  }
  release(&log.lock);
    8000406e:	0001c517          	auipc	a0,0x1c
    80004072:	9ea50513          	addi	a0,a0,-1558 # 8001fa58 <log>
    80004076:	bf1fc0ef          	jal	80000c66 <release>
}
...
initsleeplock(struct sleeplock *lk, char *name)
{
  ...
  initlock(&lk->lk, "sleep lock");
    80004096:	00003597          	auipc	a1,0x3
    8000409a:	4b258593          	addi	a1,a1,1202 # 80007548 <etext+0x548>
...
acquiresleep(struct sleeplock *lk)
{
  ...
  acquire(&lk->lk);
    ...
    800040d0:	afffc0ef          	jal	80000bce <acquire>
  while (lk->locked) {
    ...
    800040dc:	dfdfd0ef          	jal	80001ed8 <sleep>
  while (lk->locked) {
  ...
  lk->pid = myproc()->pid;
    800040e8:	fe6fd0ef          	jal	800018ce <myproc>
...
releasesleep(struct sleeplock *lk)
{
  ...
  wakeup(lk);
    ...
    80004124:	e01fd0ef          	jal	80001f24 <wakeup>
  release(&lk->lk);
...
holdingsleep(struct sleeplock *lk)
{
  ...
  acquire(&lk->lk);
    ...
    8000414e:	a81fc0ef          	jal	80000bce <acquire>
  ...
  release(&lk->lk);
    ...
    8000415a:	b0dfc0ef          	jal	80000c66 <release>
  return r;
}
  ...
  r = lk->locked && (lk->pid == myproc()->pid);
    ...
    80004172:	f5cfd0ef          	jal	800018ce <myproc>
...
fileinit(void)
{
  ...
  initlock(&ftable.lock, "ftable");
    ...
    80004190:	3cc58593          	addi	a1,a1,972 # 80007558 <etext+0x558>
    80004194:	0001c517          	auipc	a0,0x1c
    80004198:	a0c50513          	addi	a0,a0,-1524 # 8001fba0 <ftable>
    8000419c:	9b3fc0ef          	jal	80000b4e <initlock>
}
...
filealloc(void)
{
  ...
  acquire(&ftable.lock);
    800041b2:	0001c517          	auipc	a0,0x1c
    800041b6:	9ee50513          	addi	a0,a0,-1554 # 8001fba0 <ftable>
    800041ba:	a15fc0ef          	jal	80000bce <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    800041be:	0001c497          	auipc	s1,0x1c
    800041c2:	9fa48493          	addi	s1,s1,-1542 # 8001fbb8 <ftable+0x18>
    800041c6:	0001d717          	auipc	a4,0x1d
    800041ca:	99270713          	addi	a4,a4,-1646 # 80020b58 <disk>
    if(f->ref == 0){
  ...
  release(&ftable.lock);
    ...
    800041de:	9c650513          	addi	a0,a0,-1594 # 8001fba0 <ftable>
    800041e2:	a85fc0ef          	jal	80000c66 <release>
  return 0;
    ...
    800041f2:	9b250513          	addi	a0,a0,-1614 # 8001fba0 <ftable>
    800041f6:	a71fc0ef          	jal	80000c66 <release>
}
...
filedup(struct file *f)
{
  ...
  acquire(&ftable.lock);
    80004212:	0001c517          	auipc	a0,0x1c
    80004216:	98e50513          	addi	a0,a0,-1650 # 8001fba0 <ftable>
    8000421a:	9b5fc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
  ...
  release(&ftable.lock);
    80004228:	0001c517          	auipc	a0,0x1c
    8000422c:	97850513          	addi	a0,a0,-1672 # 8001fba0 <ftable>
  ...
  return f;
    ...
    80004244:	32050513          	addi	a0,a0,800 # 80007560 <etext+0x560>
    80004248:	d98fc0ef          	jal	800007e0 <panic>
...
fileclose(struct file *f)
{
  ...
  acquire(&ftable.lock);
    ...
    8000425c:	94850513          	addi	a0,a0,-1720 # 8001fba0 <ftable>
    80004260:	96ffc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
  ...
  release(&ftable.lock);
    ...
    8000429a:	90a50513          	addi	a0,a0,-1782 # 8001fba0 <ftable>
    8000429e:	9c9fc0ef          	jal	80000c66 <release>
  ...
  if(ff.type == FD_PIPE){
    ...
    800042c6:	2a650513          	addi	a0,a0,678 # 80007568 <etext+0x568>
    800042ca:	d16fc0ef          	jal	800007e0 <panic>
    release(&ftable.lock);
    800042ce:	0001c517          	auipc	a0,0x1c
    800042d2:	8d250513          	addi	a0,a0,-1838 # 8001fba0 <ftable>
    ...
    begin_op();
    800042f6:	b4bff0ef          	jal	80003e40 <begin_op>
    iput(ff.ip);
    800042fa:	854e                	mv	a0,s3
    800042fc:	adcff0ef          	jal	800035d8 <iput>
    end_op();
    80004300:	babff0ef          	jal	80003eaa <end_op>
...
filestat(struct file *f, uint64 addr)
{
  ...
  struct proc *p = myproc();
    8000431e:	db0fd0ef          	jal	800018ce <myproc>
  struct stat st;
...
fileread(struct file *f, uint64 addr, int n)
{
  ...
  if(f->type == FD_PIPE){
    ...
    8000439c:	8baff0ef          	jal	80003456 <ilock>
    ...
    800043aa:	c3cff0ef          	jal	800037e6 <readi>
    ...
    800043bc:	948ff0ef          	jal	80003504 <iunlock>
  ...
  return r;
    ...
    800043f0:	0001b717          	auipc	a4,0x1b
    800043f4:	71070713          	addi	a4,a4,1808 # 8001fb00 <devsw>
    ...
    8000440e:	16e50513          	addi	a0,a0,366 # 80007578 <etext+0x578>
    ...
    8000442e:	10078b63          	beqz	a5,80004544 <filewrite+0x11a>
{
  ...
  if(f->type == FD_PIPE){
    ...
    80004492:	0001b717          	auipc	a4,0x1b
    80004496:	66e70713          	addi	a4,a4,1646 # 8001fb00 <devsw>
    ...
    800044a6:	00048a9b          	sext.w	s5,s1
      ...
      begin_op();
    800044aa:	997ff0ef          	jal	80003e40 <begin_op>
      ilock(f->ip);
    800044ae:	01893503          	ld	a0,24(s2)
    800044b2:	fa5fe0ef          	jal	80003456 <ilock>
    ...
    800044c6:	c1cff0ef          	jal	800038e2 <writei>
    ...
    800044de:	826ff0ef          	jal	80003504 <iunlock>
      end_op();
    800044e2:	9c9ff0ef          	jal	80003eaa <end_op>
  ...
  return ret;
    ...
    8000453c:	05050513          	addi	a0,a0,80 # 80007588 <etext+0x588>
    80004540:	aa0fc0ef          	jal	800007e0 <panic>
    return -1;
...
pipealloc(struct file **f0, struct file **f1)
{
  ...
  *f0 = *f1 = 0;
  ...
  if((pi = (struct pipe*)kalloc()) == 0)
    80004582:	d7cfc0ef          	jal	80000afe <kalloc>
  ...
  initlock(&pi->lock, "pipe");
    ...
    800045a2:	ffa58593          	addi	a1,a1,-6 # 80007598 <etext+0x598>
    800045a6:	da8fc0ef          	jal	80000b4e <initlock>
  (*f0)->type = FD_PIPE;
...
pipeclose(struct pipe *pi, int writable)
{
  ...
  acquire(&pi->lock);
    8000462e:	da0fc0ef          	jal	80000bce <acquire>
  if(writable){
    ...
    8000463e:	8e7fd0ef          	jal	80001f24 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    ...
    8000464a:	e1cfc0ef          	jal	80000c66 <release>
    kfree((char*)pi);
    8000464e:	8526                	mv	a0,s1
    80004650:	bccfc0ef          	jal	80000a1c <kfree>
  } else
    release(&pi->lock);
}
    ...
    80004668:	8bdfd0ef          	jal	80001f24 <wakeup>
    8000466c:	bfd9                	j	80004642 <pipeclose+0x24>
    release(&pi->lock);
    8000466e:	8526                	mv	a0,s1
    80004670:	df6fc0ef          	jal	80000c66 <release>
}
...
pipewrite(struct pipe *pi, uint64 addr, int n)
{
  ...
  struct proc *pr = myproc();
    8000468e:	a40fd0ef          	jal	800018ce <myproc>
    80004692:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    80004694:	8526                	mv	a0,s1
    80004696:	d38fc0ef          	jal	80000bce <acquire>
  while(i < n){
  ...
  return i;
    ...
    800046d6:	84ffd0ef          	jal	80001f24 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    800046da:	85a6                	mv	a1,s1
    800046dc:	855e                	mv	a0,s7
    800046de:	ffafd0ef          	jal	80001ed8 <sleep>
  while(i < n){
    ...
    800046ee:	a23fd0ef          	jal	80002110 <killed>
    ...
    80004712:	fb5fc0ef          	jal	800016c6 <copyin>
  ...
  wakeup(&pi->nread);
    ...
    80004742:	fe2fd0ef          	jal	80001f24 <wakeup>
  release(&pi->lock);
    80004746:	8526                	mv	a0,s1
    80004748:	d1efc0ef          	jal	80000c66 <release>
  return i;
...
piperead(struct pipe *pi, uint64 addr, int n)
{
  ...
  int i;
  struct proc *pr = myproc();
    80004772:	95cfd0ef          	jal	800018ce <myproc>
    80004776:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004778:	8526                	mv	a0,s1
    8000477a:	c54fc0ef          	jal	80000bce <acquire>
  ...
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    ...
    800047ba:	cacfc0ef          	jal	80000c66 <release>
      return -1;
    800047be:	59fd                	li	s3,-1
    800047c0:	a8b9                	j	8000481e <piperead+0xc4>
  ...
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    800047c8:	05505363          	blez	s5,8000480e <piperead+0xb4>
    if(pi->nread == pi->nwrite)
    800047cc:	2184a783          	lw	a5,536(s1)
    800047d0:	21c4a703          	lw	a4,540(s1)
    800047d4:	02f70d63          	beq	a4,a5,8000480e <piperead+0xb4>
    ch = pi->data[pi->nread % PIPESIZE];
    800047d8:	1ff7f793          	andi	a5,a5,511
    800047dc:	97a6                	add	a5,a5,s1
    800047de:	0187c783          	lbu	a5,24(a5)
    800047e2:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    800047e6:	4685                	li	a3,1
    800047e8:	fbf40613          	addi	a2,s0,-65
    800047ec:	85ca                	mv	a1,s2
    800047ee:	050a3503          	ld	a0,80(s4)
    800047f2:	df1fc0ef          	jal	800015e2 <copyout>
    800047f6:	03650e63          	beq	a0,s6,80004832 <piperead+0xd8>
      if(i == 0)
        i = -1;
      break;
    }
    ...
    80004800:	20f4ac23          	sw	a5,536(s1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004804:	2985                	addiw	s3,s3,1
    80004806:	0905                	addi	s2,s2,1
    80004808:	fd3a92e3          	bne	s5,s3,800047cc <piperead+0x72>
    8000480c:	89d6                	mv	s3,s5
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    8000480e:	21c48513          	addi	a0,s1,540
    80004812:	f12fd0ef          	jal	80001f24 <wakeup>
  release(&pi->lock);
    80004816:	8526                	mv	a0,s1
    80004818:	c4efc0ef          	jal	80000c66 <release>
  ...
  return i;
    ...
    80004830:	8082                	ret
      if(i == 0)
    80004832:	fc099ee3          	bnez	s3,8000480e <piperead+0xb4>
        i = -1;
    80004836:	89aa                	mv	s3,a0
    80004838:	bfd9                	j	8000480e <piperead+0xb4>
...
kexec(char *path, char **argv)
{
  ...
  struct proc *p = myproc();
    80004870:	85efd0ef          	jal	800018ce <myproc>
    80004874:	84aa                	mv	s1,a0

  begin_op();
    80004876:	dcaff0ef          	jal	80003e40 <begin_op>

  // Open the executable file.
  if((ip = namei(path)) == 0){
    8000487a:	854a                	mv	a0,s2
    8000487c:	bf0ff0ef          	jal	80003c6c <namei>
    80004880:	c931                	beqz	a0,800048d4 <kexec+0x80>
  ...
  ilock(ip);
    80004886:	bd1fe0ef          	jal	80003456 <ilock>
  ...
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    ...
    80004898:	f4ffe0ef          	jal	800037e6 <readi>
    8000489c:	04000793          	li	a5,64
    800048a0:	00f51a63          	bne	a0,a5,800048b4 <kexec+0x60>
    goto bad;
  ...
  if(elf.magic != ELF_MAGIC)
    ...
    800048b0:	02f70663          	beq	a4,a5,800048dc <kexec+0x88>
 ...
 bad:
  ...
  if(ip){
    ...
    800048b6:	dabfe0ef          	jal	80003660 <iunlockput>
    end_op();
    800048ba:	df0ff0ef          	jal	80003eaa <end_op>
  }
  return -1;
    ...
    800048d2:	8082                	ret
    end_op();
    800048d4:	dd6ff0ef          	jal	80003eaa <end_op>
    return -1;
    800048d8:	557d                	li	a0,-1
    800048da:	b7e5                	j	800048c2 <kexec+0x6e>
    800048dc:	ebda                	sd	s6,464(sp)
  if((pagetable = proc_pagetable(p)) == 0)
    800048de:	8526                	mv	a0,s1
    800048e0:	8f4fd0ef          	jal	800019d4 <proc_pagetable>
    800048e4:	8b2a                	mv	s6,a0
    800048e6:	2c050b63          	beqz	a0,80004bbc <kexec+0x368>
  ...
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    ...
    800048fe:	12078963          	beqz	a5,80004a30 <kexec+0x1dc>
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    80004914:	a085                	j	80004974 <kexec+0x120>
      panic("loadseg: address should exist");
    80004916:	00003517          	auipc	a0,0x3
    8000491a:	c8a50513          	addi	a0,a0,-886 # 800075a0 <etext+0x5a0>
    8000491e:	ec3fb0ef          	jal	800007e0 <panic>
    if(sz - i < PGSIZE)
    ...
    8000492e:	eb9fe0ef          	jal	800037e6 <readi>
    80004932:	2501                	sext.w	a0,a0
    80004934:	24a49a63          	bne	s1,a0,80004b88 <kexec+0x334>
  for(i = 0; i < sz; i += PGSIZE){
    80004938:	012a893b          	addw	s2,s5,s2
    8000493c:	03397363          	bgeu	s2,s3,80004962 <kexec+0x10e>
    pa = walkaddr(pagetable, va + i);
    80004940:	02091593          	slli	a1,s2,0x20
    80004944:	9181                	srli	a1,a1,0x20
    80004946:	95de                	add	a1,a1,s7
    80004948:	855a                	mv	a0,s6
    8000494a:	e66fc0ef          	jal	80000fb0 <walkaddr>
    8000494e:	862a                	mv	a2,a0
    if(pa == 0)
    80004950:	d179                	beqz	a0,80004916 <kexec+0xc2>
    if(sz - i < PGSIZE)
    80004952:	412984bb          	subw	s1,s3,s2
    80004956:	0004879b          	sext.w	a5,s1
    8000495a:	fcfcf4e3          	bgeu	s9,a5,80004922 <kexec+0xce>
    8000495e:	84d6                	mv	s1,s5
    80004960:	b7c9                	j	80004922 <kexec+0xce>
    sz = sz1;
    80004962:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    80004966:	2d85                	addiw	s11,s11,1
    80004968:	038d0d1b          	addiw	s10,s10,56 # 1038 <_entry-0x7fffefc8>
    8000496c:	e8845783          	lhu	a5,-376(s0)
    80004970:	08fdd063          	bge	s11,a5,800049f0 <kexec+0x19c>
    ...
    80004984:	e63fe0ef          	jal	800037e6 <readi>
    80004988:	03800793          	li	a5,56
    8000498c:	1cf51663          	bne	a0,a5,80004b58 <kexec+0x304>
    if(ph.type != ELF_PROG_LOAD)
    80004990:	e1842783          	lw	a5,-488(s0)
    80004994:	4705                	li	a4,1
    80004996:	fce798e3          	bne	a5,a4,80004966 <kexec+0x112>
    if(ph.memsz < ph.filesz)
    8000499a:	e4043483          	ld	s1,-448(s0)
    8000499e:	e3843783          	ld	a5,-456(s0)
    800049a2:	1af4ef63          	bltu	s1,a5,80004b60 <kexec+0x30c>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    800049a6:	e2843783          	ld	a5,-472(s0)
    800049aa:	94be                	add	s1,s1,a5
    800049ac:	1af4ee63          	bltu	s1,a5,80004b68 <kexec+0x314>
    if(ph.vaddr % PGSIZE != 0)
    800049b0:	df043703          	ld	a4,-528(s0)
    800049b4:	8ff9                	and	a5,a5,a4
    800049b6:	1a079d63          	bnez	a5,80004b70 <kexec+0x31c>
    ...
    800049ca:	8bffc0ef          	jal	80001288 <uvmalloc>
    800049ce:	e0a43423          	sd	a0,-504(s0)
    800049d2:	1a050363          	beqz	a0,80004b78 <kexec+0x324>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    800049d6:	e2843b83          	ld	s7,-472(s0)
    800049da:	e2042c03          	lw	s8,-480(s0)
    800049de:	e3842983          	lw	s3,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    800049e2:	00098463          	beqz	s3,800049ea <kexec+0x196>
    800049e6:	4901                	li	s2,0
    800049e8:	bfa1                	j	80004940 <kexec+0xec>
    sz = sz1;
    800049ea:	e0843903          	ld	s2,-504(s0)
    800049ee:	bfa5                	j	80004966 <kexec+0x112>
    800049f0:	7dba                	ld	s11,424(sp)
  iunlockput(ip);
    800049f2:	8552                	mv	a0,s4
    800049f4:	c6dfe0ef          	jal	80003660 <iunlockput>
  end_op();
    800049f8:	cb2ff0ef          	jal	80003eaa <end_op>
  p = myproc();
  ...
  for(argc = 0; argv[argc]; argc++) {
    ...
    80004a9a:	c909                	beqz	a0,80004aac <kexec+0x258>
    if(argc >= MAXARG)
    80004a9c:	09a1                	addi	s3,s3,8
    80004a9e:	fb899be3          	bne	s3,s8,80004a54 <kexec+0x200>
  ip = 0;
    80004aa2:	4a01                	li	s4,0
    80004aa4:	a0dd                	j	80004b8a <kexec+0x336>
  sp = sz;
  ...
  ustack[argc] = 0;
    ...
    80004ab0:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffde2f8>
  ...
  if(sp < stackbase)
    80004acc:	f5796ee3          	bltu	s2,s7,80004a28 <kexec+0x1d4>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    80004ad0:	e9040613          	addi	a2,s0,-368
    80004ad4:	85ca                	mv	a1,s2
    80004ad6:	855a                	mv	a0,s6
    80004ad8:	b0bfc0ef          	jal	800015e2 <copyout>
    80004adc:	0e054263          	bltz	a0,80004bc0 <kexec+0x36c>
  p->trapframe->a1 = sp;
  ...
  for(last=s=path; *s; s++)
    ...
    80004af0:	cf11                	beqz	a4,80004b0c <kexec+0x2b8>
    80004af2:	0785                	addi	a5,a5,1
    if(*s == '/')
    80004af4:	02f00693          	li	a3,47
    80004af8:	a039                	j	80004b06 <kexec+0x2b2>
      last = s+1;
    80004afa:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    80004afe:	0785                	addi	a5,a5,1
    80004b00:	fff7c703          	lbu	a4,-1(a5)
    80004b04:	c701                	beqz	a4,80004b0c <kexec+0x2b8>
    if(*s == '/')
    80004b06:	fed71ce3          	bne	a4,a3,80004afe <kexec+0x2aa>
    80004b0a:	bfc5                	j	80004afa <kexec+0x2a6>
  safestrcpy(p->name, last, sizeof(p->name));
    80004b0c:	4641                	li	a2,16
    80004b0e:	df843583          	ld	a1,-520(s0)
    80004b12:	158a8513          	addi	a0,s5,344
    80004b16:	acafc0ef          	jal	80000de0 <safestrcpy>
  ...
  proc_freepagetable(oldpagetable, oldsz);
    ...
    80004b3e:	f1bfc0ef          	jal	80001a58 <proc_freepagetable>
  ...
  if(ip){
    ...
    80004baa:	bb21                	j	800048c2 <kexec+0x6e>
    ...
    80004bba:	b9ed                	j	800048b4 <kexec+0x60>
    80004bbc:	6b5e                	ld	s6,464(sp)
    80004bbe:	b9dd                	j	800048b4 <kexec+0x60>
  sz = sz1;
    80004bc0:	e0843983          	ld	s3,-504(s0)
    80004bc4:	b595                	j	80004a28 <kexec+0x1d4>
...
fdalloc(struct file *f)
{
  ...
  int fd;
  struct proc *p = myproc();
    80004c2a:	ca5fc0ef          	jal	800018ce <myproc>
...
create(char *path, short type, short major, short minor)
{
  ...
  if((dp = nameiparent(path, name)) == 0)
    ...
    80004c78:	80eff0ef          	jal	80003c86 <nameiparent>
    80004c7c:	84aa                	mv	s1,a0
    80004c7e:	10050a63          	beqz	a0,80004d92 <create+0x136>
    return 0;

  ilock(dp);
    80004c82:	fd4fe0ef          	jal	80003456 <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    80004c86:	4601                	li	a2,0
    80004c88:	fb040593          	addi	a1,s0,-80
    80004c8c:	8526                	mv	a0,s1
    80004c8e:	d79fe0ef          	jal	80003a06 <dirlookup>
    80004c92:	8aaa                	mv	s5,a0
    80004c94:	c129                	beqz	a0,80004cd6 <create+0x7a>
    iunlockput(dp);
    80004c96:	8526                	mv	a0,s1
    80004c98:	9c9fe0ef          	jal	80003660 <iunlockput>
    ilock(ip);
    80004c9c:	8556                	mv	a0,s5
    80004c9e:	fb8fe0ef          	jal	80003456 <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    80004ca2:	4789                	li	a5,2
    80004ca4:	02fb1463          	bne	s6,a5,80004ccc <create+0x70>
    80004ca8:	044ad783          	lhu	a5,68(s5)
    80004cac:	37f9                	addiw	a5,a5,-2
    80004cae:	17c2                	slli	a5,a5,0x30
    80004cb0:	93c1                	srli	a5,a5,0x30
    80004cb2:	4705                	li	a4,1
    80004cb4:	00f76c63          	bltu	a4,a5,80004ccc <create+0x70>
  ip->nlink = 0;
  iupdate(ip);
  iunlockput(ip);
  iunlockput(dp);
  return 0;
    ...
    80004cce:	993fe0ef          	jal	80003660 <iunlockput>
    return 0;
    80004cd2:	4a81                	li	s5,0
    80004cd4:	b7d5                	j	80004cb8 <create+0x5c>
    80004cd6:	f052                	sd	s4,32(sp)
  if((ip = ialloc(dp->dev, type)) == 0){
    80004cd8:	85da                	mv	a1,s6
    80004cda:	4088                	lw	a0,0(s1)
    80004cdc:	e0afe0ef          	jal	800032e6 <ialloc>
    80004ce0:	8a2a                	mv	s4,a0
    80004ce2:	cd15                	beqz	a0,80004d1e <create+0xc2>
  ilock(ip);
    80004ce4:	f72fe0ef          	jal	80003456 <ilock>
  ip->major = major;
    80004ce8:	053a1323          	sh	s3,70(s4)
  ip->minor = minor;
    80004cec:	052a1423          	sh	s2,72(s4)
  ip->nlink = 1;
    80004cf0:	4905                	li	s2,1
    80004cf2:	052a1523          	sh	s2,74(s4)
  iupdate(ip);
    80004cf6:	8552                	mv	a0,s4
    80004cf8:	eaafe0ef          	jal	800033a2 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80004cfc:	032b0763          	beq	s6,s2,80004d2a <create+0xce>
  if(dirlink(dp, name, ip->inum) < 0)
    80004d00:	004a2603          	lw	a2,4(s4)
    80004d04:	fb040593          	addi	a1,s0,-80
    80004d08:	8526                	mv	a0,s1
    80004d0a:	ec9fe0ef          	jal	80003bd2 <dirlink>
    80004d0e:	06054563          	bltz	a0,80004d78 <create+0x11c>
  iunlockput(dp);
    80004d12:	8526                	mv	a0,s1
    80004d14:	94dfe0ef          	jal	80003660 <iunlockput>
  return ip;
    80004d18:	8ad2                	mv	s5,s4
    80004d1a:	7a02                	ld	s4,32(sp)
    80004d1c:	bf71                	j	80004cb8 <create+0x5c>
    iunlockput(dp);
    80004d1e:	8526                	mv	a0,s1
    80004d20:	941fe0ef          	jal	80003660 <iunlockput>
    return 0;
    80004d24:	8ad2                	mv	s5,s4
    80004d26:	7a02                	ld	s4,32(sp)
    80004d28:	bf41                	j	80004cb8 <create+0x5c>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80004d2a:	004a2603          	lw	a2,4(s4)
    80004d2e:	00003597          	auipc	a1,0x3
    80004d32:	89258593          	addi	a1,a1,-1902 # 800075c0 <etext+0x5c0>
    80004d36:	8552                	mv	a0,s4
    80004d38:	e9bfe0ef          	jal	80003bd2 <dirlink>
    80004d3c:	02054e63          	bltz	a0,80004d78 <create+0x11c>
    80004d40:	40d0                	lw	a2,4(s1)
    80004d42:	00003597          	auipc	a1,0x3
    80004d46:	88658593          	addi	a1,a1,-1914 # 800075c8 <etext+0x5c8>
    80004d4a:	8552                	mv	a0,s4
    80004d4c:	e87fe0ef          	jal	80003bd2 <dirlink>
    80004d50:	02054463          	bltz	a0,80004d78 <create+0x11c>
  if(dirlink(dp, name, ip->inum) < 0)
    80004d54:	004a2603          	lw	a2,4(s4)
    80004d58:	fb040593          	addi	a1,s0,-80
    80004d5c:	8526                	mv	a0,s1
    80004d5e:	e75fe0ef          	jal	80003bd2 <dirlink>
    80004d62:	00054b63          	bltz	a0,80004d78 <create+0x11c>
    dp->nlink++;  // for ".."
    80004d66:	04a4d783          	lhu	a5,74(s1)
    80004d6a:	2785                	addiw	a5,a5,1
    80004d6c:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004d70:	8526                	mv	a0,s1
    80004d72:	e30fe0ef          	jal	800033a2 <iupdate>
    80004d76:	bf71                	j	80004d12 <create+0xb6>
  ip->nlink = 0;
    80004d78:	040a1523          	sh	zero,74(s4)
  iupdate(ip);
    80004d7c:	8552                	mv	a0,s4
    80004d7e:	e24fe0ef          	jal	800033a2 <iupdate>
  iunlockput(ip);
    80004d82:	8552                	mv	a0,s4
    80004d84:	8ddfe0ef          	jal	80003660 <iunlockput>
  iunlockput(dp);
    80004d88:	8526                	mv	a0,s1
    80004d8a:	8d7fe0ef          	jal	80003660 <iunlockput>
  return 0;
    80004d8e:	7a02                	ld	s4,32(sp)
    80004d90:	b725                	j	80004cb8 <create+0x5c>
    return 0;
    80004d92:	8aaa                	mv	s5,a0
    80004d94:	b715                	j	80004cb8 <create+0x5c>

0000000080004d96 <sys_dup>:
{
  ...
  if(argfd(0, 0, &f) < 0)
    80004d9e:	fd840613          	addi	a2,s0,-40
    80004da2:	4581                	li	a1,0
    80004da4:	4501                	li	a0,0
    80004da6:	e21ff0ef          	jal	80004bc6 <argfd>
    return -1;
    80004daa:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80004dac:	02054363          	bltz	a0,80004dd2 <sys_dup+0x3c>
    80004db0:	ec26                	sd	s1,24(sp)
    80004db2:	e84a                	sd	s2,16(sp)
  if((fd=fdalloc(f)) < 0)
    80004db4:	fd843903          	ld	s2,-40(s0)
    80004db8:	854a                	mv	a0,s2
    80004dba:	e65ff0ef          	jal	80004c1e <fdalloc>
    80004dbe:	84aa                	mv	s1,a0
    return -1;
    80004dc0:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80004dc2:	00054d63          	bltz	a0,80004ddc <sys_dup+0x46>
  filedup(f);
    80004dc6:	854a                	mv	a0,s2
    80004dc8:	c3eff0ef          	jal	80004206 <filedup>
  return fd;
    80004dcc:	87a6                	mv	a5,s1
    80004dce:	64e2                	ld	s1,24(sp)
    80004dd0:	6942                	ld	s2,16(sp)
}
    80004dd2:	853e                	mv	a0,a5
    80004dd4:	70a2                	ld	ra,40(sp)
    80004dd6:	7402                	ld	s0,32(sp)
    80004dd8:	6145                	addi	sp,sp,48
    80004dda:	8082                	ret
    80004ddc:	64e2                	ld	s1,24(sp)
    80004dde:	6942                	ld	s2,16(sp)
    80004de0:	bfcd                	j	80004dd2 <sys_dup+0x3c>

0000000080004de2 <sys_read>:
{
  ...
  argaddr(1, &p);
    80004dea:	fd840593          	addi	a1,s0,-40
    80004dee:	4505                	li	a0,1
    80004df0:	a09fd0ef          	jal	800027f8 <argaddr>
  argint(2, &n);
    80004df4:	fe440593          	addi	a1,s0,-28
    80004df8:	4509                	li	a0,2
    80004dfa:	9e3fd0ef          	jal	800027dc <argint>
  if(argfd(0, 0, &f) < 0)
    80004dfe:	fe840613          	addi	a2,s0,-24
    80004e02:	4581                	li	a1,0
    80004e04:	4501                	li	a0,0
    80004e06:	dc1ff0ef          	jal	80004bc6 <argfd>
    80004e0a:	87aa                	mv	a5,a0
    return -1;
    80004e0c:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004e0e:	0007ca63          	bltz	a5,80004e22 <sys_read+0x40>
  return fileread(f, p, n);
    80004e12:	fe442603          	lw	a2,-28(s0)
    80004e16:	fd843583          	ld	a1,-40(s0)
    80004e1a:	fe843503          	ld	a0,-24(s0)
    80004e1e:	d4eff0ef          	jal	8000436c <fileread>
}
    80004e22:	70a2                	ld	ra,40(sp)
    80004e24:	7402                	ld	s0,32(sp)
    80004e26:	6145                	addi	sp,sp,48
    80004e28:	8082                	ret

0000000080004e2a <sys_write>:
{
...
0000000080004e72 <sys_close>:
{
    80004e72:	1101                	addi	sp,sp,-32
    80004e74:	ec06                	sd	ra,24(sp)
    80004e76:	e822                	sd	s0,16(sp)
    80004e78:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    80004e7a:	fe040613          	addi	a2,s0,-32
    80004e7e:	fec40593          	addi	a1,s0,-20
    80004e82:	4501                	li	a0,0
    80004e84:	d43ff0ef          	jal	80004bc6 <argfd>
    return -1;
    80004e88:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    80004e8a:	02054063          	bltz	a0,80004eaa <sys_close+0x38>
  myproc()->ofile[fd] = 0;
    80004e8e:	a41fc0ef          	jal	800018ce <myproc>
    80004e92:	fec42783          	lw	a5,-20(s0)
    80004e96:	07e9                	addi	a5,a5,26
    80004e98:	078e                	slli	a5,a5,0x3
    80004e9a:	953e                	add	a0,a0,a5
    80004e9c:	00053023          	sd	zero,0(a0)
  fileclose(f);
    80004ea0:	fe043503          	ld	a0,-32(s0)
    80004ea4:	ba8ff0ef          	jal	8000424c <fileclose>
  return 0;
...
0000000080004eb4 <sys_fstat>:
{
  ...
  argaddr(1, &st);
    80004ebc:	fe040593          	addi	a1,s0,-32
    80004ec0:	4505                	li	a0,1
    80004ec2:	937fd0ef          	jal	800027f8 <argaddr>
  if(argfd(0, 0, &f) < 0)
    80004ec6:	fe840613          	addi	a2,s0,-24
    80004eca:	4581                	li	a1,0
    80004ecc:	4501                	li	a0,0
    80004ece:	cf9ff0ef          	jal	80004bc6 <argfd>
    80004ed2:	87aa                	mv	a5,a0
    return -1;
    80004ed4:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004ed6:	0007c863          	bltz	a5,80004ee6 <sys_fstat+0x32>
  return filestat(f, st);
    80004eda:	fe043583          	ld	a1,-32(s0)
    80004ede:	fe843503          	ld	a0,-24(s0)
    80004ee2:	c2cff0ef          	jal	8000430e <filestat>
}
...
0000000080004eee <sys_link>:
{
    80004eee:	7169                	addi	sp,sp,-304
    80004ef0:	f606                	sd	ra,296(sp)
    80004ef2:	f222                	sd	s0,288(sp)
    80004ef4:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004ef6:	08000613          	li	a2,128
    80004efa:	ed040593          	addi	a1,s0,-304
    80004efe:	4501                	li	a0,0
    80004f00:	915fd0ef          	jal	80002814 <argstr>
    return -1;
    80004f04:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004f06:	0c054e63          	bltz	a0,80004fe2 <sys_link+0xf4>
    80004f0a:	08000613          	li	a2,128
    80004f0e:	f5040593          	addi	a1,s0,-176
    80004f12:	4505                	li	a0,1
    80004f14:	901fd0ef          	jal	80002814 <argstr>
    return -1;
    80004f18:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004f1a:	0c054463          	bltz	a0,80004fe2 <sys_link+0xf4>
    80004f1e:	ee26                	sd	s1,280(sp)
  begin_op();
    80004f20:	f21fe0ef          	jal	80003e40 <begin_op>
  if((ip = namei(old)) == 0){
    80004f24:	ed040513          	addi	a0,s0,-304
    80004f28:	d45fe0ef          	jal	80003c6c <namei>
    80004f2c:	84aa                	mv	s1,a0
    80004f2e:	c53d                	beqz	a0,80004f9c <sys_link+0xae>
  ilock(ip);
    80004f30:	d26fe0ef          	jal	80003456 <ilock>
  if(ip->type == T_DIR){
    80004f34:	04449703          	lh	a4,68(s1)
    80004f38:	4785                	li	a5,1
    80004f3a:	06f70663          	beq	a4,a5,80004fa6 <sys_link+0xb8>
    80004f3e:	ea4a                	sd	s2,272(sp)
  ip->nlink++;
    80004f40:	04a4d783          	lhu	a5,74(s1)
    80004f44:	2785                	addiw	a5,a5,1
    80004f46:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004f4a:	8526                	mv	a0,s1
    80004f4c:	c56fe0ef          	jal	800033a2 <iupdate>
  iunlock(ip);
    80004f50:	8526                	mv	a0,s1
    80004f52:	db2fe0ef          	jal	80003504 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    80004f56:	fd040593          	addi	a1,s0,-48
    80004f5a:	f5040513          	addi	a0,s0,-176
    80004f5e:	d29fe0ef          	jal	80003c86 <nameiparent>
    80004f62:	892a                	mv	s2,a0
    80004f64:	cd21                	beqz	a0,80004fbc <sys_link+0xce>
  ilock(dp);
    80004f66:	cf0fe0ef          	jal	80003456 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    80004f6a:	00092703          	lw	a4,0(s2)
    80004f6e:	409c                	lw	a5,0(s1)
    80004f70:	04f71363          	bne	a4,a5,80004fb6 <sys_link+0xc8>
    80004f74:	40d0                	lw	a2,4(s1)
    80004f76:	fd040593          	addi	a1,s0,-48
    80004f7a:	854a                	mv	a0,s2
    80004f7c:	c57fe0ef          	jal	80003bd2 <dirlink>
    80004f80:	02054b63          	bltz	a0,80004fb6 <sys_link+0xc8>
  iunlockput(dp);
    80004f84:	854a                	mv	a0,s2
    80004f86:	edafe0ef          	jal	80003660 <iunlockput>
  iput(ip);
    80004f8a:	8526                	mv	a0,s1
    80004f8c:	e4cfe0ef          	jal	800035d8 <iput>
  end_op();
    80004f90:	f1bfe0ef          	jal	80003eaa <end_op>
  return 0;
    80004f94:	4781                	li	a5,0
    80004f96:	64f2                	ld	s1,280(sp)
    80004f98:	6952                	ld	s2,272(sp)
    80004f9a:	a0a1                	j	80004fe2 <sys_link+0xf4>
    end_op();
    80004f9c:	f0ffe0ef          	jal	80003eaa <end_op>
    return -1;
    80004fa0:	57fd                	li	a5,-1
    80004fa2:	64f2                	ld	s1,280(sp)
    80004fa4:	a83d                	j	80004fe2 <sys_link+0xf4>
    iunlockput(ip);
    80004fa6:	8526                	mv	a0,s1
    80004fa8:	eb8fe0ef          	jal	80003660 <iunlockput>
    end_op();
    80004fac:	efffe0ef          	jal	80003eaa <end_op>
    return -1;
    80004fb0:	57fd                	li	a5,-1
    80004fb2:	64f2                	ld	s1,280(sp)
    80004fb4:	a03d                	j	80004fe2 <sys_link+0xf4>
    iunlockput(dp);
    80004fb6:	854a                	mv	a0,s2
    80004fb8:	ea8fe0ef          	jal	80003660 <iunlockput>
  ilock(ip);
    80004fbc:	8526                	mv	a0,s1
    80004fbe:	c98fe0ef          	jal	80003456 <ilock>
  ip->nlink--;
  ...
  iupdate(ip);
    ...
    80004fce:	bd4fe0ef          	jal	800033a2 <iupdate>
  iunlockput(ip);
    80004fd2:	8526                	mv	a0,s1
    80004fd4:	e8cfe0ef          	jal	80003660 <iunlockput>
  end_op();
    80004fd8:	ed3fe0ef          	jal	80003eaa <end_op>
  return -1;
    80004fdc:	57fd                	li	a5,-1
    80004fde:	64f2                	ld	s1,280(sp)
    80004fe0:	6952                	ld	s2,272(sp)
}
    80004fe2:	853e                	mv	a0,a5
    80004fe4:	70b2                	ld	ra,296(sp)
    80004fe6:	7412                	ld	s0,288(sp)
    80004fe8:	6155                	addi	sp,sp,304
    80004fea:	8082                	ret

0000000080004fec <sys_unlink>:
{
    80004fec:	7151                	addi	sp,sp,-240
    80004fee:	f586                	sd	ra,232(sp)
    80004ff0:	f1a2                	sd	s0,224(sp)
    80004ff2:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    80004ff4:	08000613          	li	a2,128
    80004ff8:	f3040593          	addi	a1,s0,-208
    80004ffc:	4501                	li	a0,0
    80004ffe:	817fd0ef          	jal	80002814 <argstr>
    80005002:	16054063          	bltz	a0,80005162 <sys_unlink+0x176>
    80005006:	eda6                	sd	s1,216(sp)
  begin_op();
    80005008:	e39fe0ef          	jal	80003e40 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    8000500c:	fb040593          	addi	a1,s0,-80
    80005010:	f3040513          	addi	a0,s0,-208
    80005014:	c73fe0ef          	jal	80003c86 <nameiparent>
    80005018:	84aa                	mv	s1,a0
    8000501a:	c945                	beqz	a0,800050ca <sys_unlink+0xde>
  ilock(dp);
    8000501c:	c3afe0ef          	jal	80003456 <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80005020:	00002597          	auipc	a1,0x2
    80005024:	5a058593          	addi	a1,a1,1440 # 800075c0 <etext+0x5c0>
    80005028:	fb040513          	addi	a0,s0,-80
    8000502c:	9c5fe0ef          	jal	800039f0 <namecmp>
    80005030:	10050e63          	beqz	a0,8000514c <sys_unlink+0x160>
    80005034:	00002597          	auipc	a1,0x2
    80005038:	59458593          	addi	a1,a1,1428 # 800075c8 <etext+0x5c8>
    8000503c:	fb040513          	addi	a0,s0,-80
    80005040:	9b1fe0ef          	jal	800039f0 <namecmp>
    80005044:	10050463          	beqz	a0,8000514c <sys_unlink+0x160>
    80005048:	e9ca                	sd	s2,208(sp)
  if((ip = dirlookup(dp, name, &off)) == 0)
    8000504a:	f2c40613          	addi	a2,s0,-212
    8000504e:	fb040593          	addi	a1,s0,-80
    80005052:	8526                	mv	a0,s1
    80005054:	9b3fe0ef          	jal	80003a06 <dirlookup>
    80005058:	892a                	mv	s2,a0
    8000505a:	0e050863          	beqz	a0,8000514a <sys_unlink+0x15e>
  ilock(ip);
    8000505e:	bf8fe0ef          	jal	80003456 <ilock>
  if(ip->nlink < 1)
    80005062:	04a91783          	lh	a5,74(s2)
    80005066:	06f05763          	blez	a5,800050d4 <sys_unlink+0xe8>
  if(ip->type == T_DIR && !isdirempty(ip)){
    8000506a:	04491703          	lh	a4,68(s2)
    8000506e:	4785                	li	a5,1
    80005070:	06f70963          	beq	a4,a5,800050e2 <sys_unlink+0xf6>
  memset(&de, 0, sizeof(de));
    80005074:	4641                	li	a2,16
    80005076:	4581                	li	a1,0
    80005078:	fc040513          	addi	a0,s0,-64
    8000507c:	c27fb0ef          	jal	80000ca2 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80005080:	4741                	li	a4,16
    80005082:	f2c42683          	lw	a3,-212(s0)
    80005086:	fc040613          	addi	a2,s0,-64
    8000508a:	4581                	li	a1,0
    8000508c:	8526                	mv	a0,s1
    8000508e:	855fe0ef          	jal	800038e2 <writei>
    80005092:	47c1                	li	a5,16
    80005094:	08f51b63          	bne	a0,a5,8000512a <sys_unlink+0x13e>
  if(ip->type == T_DIR){
    80005098:	04491703          	lh	a4,68(s2)
    8000509c:	4785                	li	a5,1
    8000509e:	08f70d63          	beq	a4,a5,80005138 <sys_unlink+0x14c>
  iunlockput(dp);
    800050a2:	8526                	mv	a0,s1
    800050a4:	dbcfe0ef          	jal	80003660 <iunlockput>
  ip->nlink--;
    800050a8:	04a95783          	lhu	a5,74(s2)
    800050ac:	37fd                	addiw	a5,a5,-1
    800050ae:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    800050b2:	854a                	mv	a0,s2
    800050b4:	aeefe0ef          	jal	800033a2 <iupdate>
  iunlockput(ip);
    800050b8:	854a                	mv	a0,s2
    800050ba:	da6fe0ef          	jal	80003660 <iunlockput>
  end_op();
    800050be:	dedfe0ef          	jal	80003eaa <end_op>
  return 0;
    800050c2:	4501                	li	a0,0
    800050c4:	64ee                	ld	s1,216(sp)
    800050c6:	694e                	ld	s2,208(sp)
    800050c8:	a849                	j	8000515a <sys_unlink+0x16e>
    end_op();
    800050ca:	de1fe0ef          	jal	80003eaa <end_op>
    return -1;
    800050ce:	557d                	li	a0,-1
    800050d0:	64ee                	ld	s1,216(sp)
    800050d2:	a061                	j	8000515a <sys_unlink+0x16e>
    800050d4:	e5ce                	sd	s3,200(sp)
    panic("unlink: nlink < 1");
    800050d6:	00002517          	auipc	a0,0x2
    800050da:	4fa50513          	addi	a0,a0,1274 # 800075d0 <etext+0x5d0>
    800050de:	f02fb0ef          	jal	800007e0 <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    800050e2:	04c92703          	lw	a4,76(s2)
    800050e6:	02000793          	li	a5,32
    800050ea:	f8e7f5e3          	bgeu	a5,a4,80005074 <sys_unlink+0x88>
    800050ee:	e5ce                	sd	s3,200(sp)
    800050f0:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800050f4:	4741                	li	a4,16
    800050f6:	86ce                	mv	a3,s3
    800050f8:	f1840613          	addi	a2,s0,-232
    800050fc:	4581                	li	a1,0
    800050fe:	854a                	mv	a0,s2
    80005100:	ee6fe0ef          	jal	800037e6 <readi>
    80005104:	47c1                	li	a5,16
    80005106:	00f51c63          	bne	a0,a5,8000511e <sys_unlink+0x132>
    if(de.inum != 0)
    8000510a:	f1845783          	lhu	a5,-232(s0)
    8000510e:	efa1                	bnez	a5,80005166 <sys_unlink+0x17a>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80005110:	29c1                	addiw	s3,s3,16
    80005112:	04c92783          	lw	a5,76(s2)
    80005116:	fcf9efe3          	bltu	s3,a5,800050f4 <sys_unlink+0x108>
    8000511a:	69ae                	ld	s3,200(sp)
    8000511c:	bfa1                	j	80005074 <sys_unlink+0x88>
      panic("isdirempty: readi");
    8000511e:	00002517          	auipc	a0,0x2
    80005122:	4ca50513          	addi	a0,a0,1226 # 800075e8 <etext+0x5e8>
    80005126:	ebafb0ef          	jal	800007e0 <panic>
    8000512a:	e5ce                	sd	s3,200(sp)
    panic("unlink: writei");
    8000512c:	00002517          	auipc	a0,0x2
    80005130:	4d450513          	addi	a0,a0,1236 # 80007600 <etext+0x600>
    80005134:	eacfb0ef          	jal	800007e0 <panic>
    dp->nlink--;
    80005138:	04a4d783          	lhu	a5,74(s1)
    8000513c:	37fd                	addiw	a5,a5,-1
    8000513e:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80005142:	8526                	mv	a0,s1
    80005144:	a5efe0ef          	jal	800033a2 <iupdate>
    80005148:	bfa9                	j	800050a2 <sys_unlink+0xb6>
    8000514a:	694e                	ld	s2,208(sp)
  iunlockput(dp);
    8000514c:	8526                	mv	a0,s1
    8000514e:	d12fe0ef          	jal	80003660 <iunlockput>
  end_op();
    80005152:	d59fe0ef          	jal	80003eaa <end_op>
  return -1;
    80005156:	557d                	li	a0,-1
    80005158:	64ee                	ld	s1,216(sp)
}
    8000515a:	70ae                	ld	ra,232(sp)
    8000515c:	740e                	ld	s0,224(sp)
    8000515e:	616d                	addi	sp,sp,240
    80005160:	8082                	ret
    return -1;
    80005162:	557d                	li	a0,-1
    80005164:	bfdd                	j	8000515a <sys_unlink+0x16e>
    iunlockput(ip);
    80005166:	854a                	mv	a0,s2
    80005168:	cf8fe0ef          	jal	80003660 <iunlockput>
    goto bad;
    8000516c:	694e                	ld	s2,208(sp)
    8000516e:	69ae                	ld	s3,200(sp)
    80005170:	bff1                	j	8000514c <sys_unlink+0x160>

0000000080005172 <sys_open>:

uint64
sys_open(void)
{
    80005172:	7131                	addi	sp,sp,-192
    80005174:	fd06                	sd	ra,184(sp)
    80005176:	f922                	sd	s0,176(sp)
    80005178:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;
  ...
  argint(1, &omode);
    8000517a:	f4c40593          	addi	a1,s0,-180
    8000517e:	4505                	li	a0,1
    80005180:	e5cfd0ef          	jal	800027dc <argint>
  if((n = argstr(0, path, MAXPATH)) < 0)
    80005184:	08000613          	li	a2,128
    80005188:	f5040593          	addi	a1,s0,-176
    8000518c:	4501                	li	a0,0
    8000518e:	e86fd0ef          	jal	80002814 <argstr>
    80005192:	87aa                	mv	a5,a0
    return -1;
    80005194:	557d                	li	a0,-1
  if((n = argstr(0, path, MAXPATH)) < 0)
    80005196:	0a07c263          	bltz	a5,8000523a <sys_open+0xc8>
    8000519a:	f526                	sd	s1,168(sp)

  begin_op();
    8000519c:	ca5fe0ef          	jal	80003e40 <begin_op>

  if(omode & O_CREATE){
    800051a0:	f4c42783          	lw	a5,-180(s0)
    800051a4:	2007f793          	andi	a5,a5,512
    800051a8:	c3d5                	beqz	a5,8000524c <sys_open+0xda>
    ip = create(path, T_FILE, 0, 0);
    800051aa:	4681                	li	a3,0
    800051ac:	4601                	li	a2,0
    800051ae:	4589                	li	a1,2
    800051b0:	f5040513          	addi	a0,s0,-176
    800051b4:	aa9ff0ef          	jal	80004c5c <create>
    800051b8:	84aa                	mv	s1,a0
    if(ip == 0){
    800051ba:	c541                	beqz	a0,80005242 <sys_open+0xd0>
      end_op();
      return -1;
    }
  }
  ...
  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    ...
    800051c2:	00f71763          	bne	a4,a5,800051d0 <sys_open+0x5e>
    800051c6:	0464d703          	lhu	a4,70(s1)
    800051ca:	47a5                	li	a5,9
    800051cc:	0ae7ed63          	bltu	a5,a4,80005286 <sys_open+0x114>
    800051d0:	f14a                	sd	s2,160(sp)
    iunlockput(ip);
    end_op();
    return -1;
  }
  ...
  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    800051d2:	fd7fe0ef          	jal	800041a8 <filealloc>
    800051d6:	892a                	mv	s2,a0
    800051d8:	c179                	beqz	a0,8000529e <sys_open+0x12c>
    800051da:	ed4e                	sd	s3,152(sp)
    800051dc:	a43ff0ef          	jal	80004c1e <fdalloc>
    800051e0:	89aa                	mv	s3,a0
    800051e2:	0a054a63          	bltz	a0,80005296 <sys_open+0x124>
    iunlockput(ip);
    end_op();
    return -1;
  }
  ...
  if(ip->type == T_DEVICE){
    800051e6:	04449703          	lh	a4,68(s1)
    800051ea:	478d                	li	a5,3
    800051ec:	0cf70263          	beq	a4,a5,800052b0 <sys_open+0x13e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    800051f0:	4789                	li	a5,2
    800051f2:	00f92023          	sw	a5,0(s2)
    f->off = 0;
    800051f6:	02092023          	sw	zero,32(s2)
  }
  f->ip = ip;
    800051fa:	00993c23          	sd	s1,24(s2)
  f->readable = !(omode & O_WRONLY);
    800051fe:	f4c42783          	lw	a5,-180(s0)
    80005202:	0017c713          	xori	a4,a5,1
    80005206:	8b05                	andi	a4,a4,1
    80005208:	00e90423          	sb	a4,8(s2)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    8000520c:	0037f713          	andi	a4,a5,3
    80005210:	00e03733          	snez	a4,a4
    80005214:	00e904a3          	sb	a4,9(s2)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80005218:	4007f793          	andi	a5,a5,1024
    8000521c:	c791                	beqz	a5,80005228 <sys_open+0xb6>
    8000521e:	04449703          	lh	a4,68(s1)
    80005222:	4789                	li	a5,2
    80005224:	08f70d63          	beq	a4,a5,800052be <sys_open+0x14c>
    itrunc(ip);
  }

  iunlock(ip);
    80005228:	8526                	mv	a0,s1
    8000522a:	adafe0ef          	jal	80003504 <iunlock>
  end_op();
    8000522e:	c7dfe0ef          	jal	80003eaa <end_op>
  ...
  return fd;
    ...
    8000523a:	70ea                	ld	ra,184(sp)
    8000523c:	744a                	ld	s0,176(sp)
    8000523e:	6129                	addi	sp,sp,192
    80005240:	8082                	ret
      end_op();
    80005242:	c69fe0ef          	jal	80003eaa <end_op>
      return -1;
    80005246:	557d                	li	a0,-1
    80005248:	74aa                	ld	s1,168(sp)
    8000524a:	bfc5                	j	8000523a <sys_open+0xc8>
    if((ip = namei(path)) == 0){
    8000524c:	f5040513          	addi	a0,s0,-176
    80005250:	a1dfe0ef          	jal	80003c6c <namei>
    80005254:	84aa                	mv	s1,a0
    80005256:	c11d                	beqz	a0,8000527c <sys_open+0x10a>
    ilock(ip);
    80005258:	9fefe0ef          	jal	80003456 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    8000525c:	04449703          	lh	a4,68(s1)
    80005260:	4785                	li	a5,1
    80005262:	f4f71de3          	bne	a4,a5,800051bc <sys_open+0x4a>
    80005266:	f4c42783          	lw	a5,-180(s0)
    8000526a:	d3bd                	beqz	a5,800051d0 <sys_open+0x5e>
      iunlockput(ip);
    8000526c:	8526                	mv	a0,s1
    8000526e:	bf2fe0ef          	jal	80003660 <iunlockput>
      end_op();
    80005272:	c39fe0ef          	jal	80003eaa <end_op>
      return -1;
    80005276:	557d                	li	a0,-1
    80005278:	74aa                	ld	s1,168(sp)
    8000527a:	b7c1                	j	8000523a <sys_open+0xc8>
      end_op();
    8000527c:	c2ffe0ef          	jal	80003eaa <end_op>
      return -1;
    80005280:	557d                	li	a0,-1
    80005282:	74aa                	ld	s1,168(sp)
    80005284:	bf5d                	j	8000523a <sys_open+0xc8>
    iunlockput(ip);
    80005286:	8526                	mv	a0,s1
    80005288:	bd8fe0ef          	jal	80003660 <iunlockput>
    end_op();
    8000528c:	c1ffe0ef          	jal	80003eaa <end_op>
    return -1;
    80005290:	557d                	li	a0,-1
    80005292:	74aa                	ld	s1,168(sp)
    80005294:	b75d                	j	8000523a <sys_open+0xc8>
      fileclose(f);
    80005296:	854a                	mv	a0,s2
    80005298:	fb5fe0ef          	jal	8000424c <fileclose>
    8000529c:	69ea                	ld	s3,152(sp)
    iunlockput(ip);
    8000529e:	8526                	mv	a0,s1
    800052a0:	bc0fe0ef          	jal	80003660 <iunlockput>
    end_op();
    800052a4:	c07fe0ef          	jal	80003eaa <end_op>
    return -1;
    800052a8:	557d                	li	a0,-1
    800052aa:	74aa                	ld	s1,168(sp)
    800052ac:	790a                	ld	s2,160(sp)
    800052ae:	b771                	j	8000523a <sys_open+0xc8>
    f->type = FD_DEVICE;
    800052b0:	00f92023          	sw	a5,0(s2)
    f->major = ip->major;
    800052b4:	04649783          	lh	a5,70(s1)
    800052b8:	02f91223          	sh	a5,36(s2)
    800052bc:	bf3d                	j	800051fa <sys_open+0x88>
    itrunc(ip);
    800052be:	8526                	mv	a0,s1
    800052c0:	a84fe0ef          	jal	80003544 <itrunc>
    800052c4:	b795                	j	80005228 <sys_open+0xb6>

00000000800052c6 <sys_mkdir>:

uint64
sys_mkdir(void)
{
    800052c6:	7175                	addi	sp,sp,-144
    800052c8:	e506                	sd	ra,136(sp)
    800052ca:	e122                	sd	s0,128(sp)
    800052cc:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    800052ce:	b73fe0ef          	jal	80003e40 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    800052d2:	08000613          	li	a2,128
    800052d6:	f7040593          	addi	a1,s0,-144
    800052da:	4501                	li	a0,0
    800052dc:	d38fd0ef          	jal	80002814 <argstr>
    800052e0:	02054363          	bltz	a0,80005306 <sys_mkdir+0x40>
    800052e4:	4681                	li	a3,0
    800052e6:	4601                	li	a2,0
    800052e8:	4585                	li	a1,1
    800052ea:	f7040513          	addi	a0,s0,-144
    800052ee:	96fff0ef          	jal	80004c5c <create>
    800052f2:	c911                	beqz	a0,80005306 <sys_mkdir+0x40>
    end_op();
    return -1;
  }
  iunlockput(ip);
    800052f4:	b6cfe0ef          	jal	80003660 <iunlockput>
  end_op();
    800052f8:	bb3fe0ef          	jal	80003eaa <end_op>
  return 0;
    800052fc:	4501                	li	a0,0
}
    800052fe:	60aa                	ld	ra,136(sp)
    80005300:	640a                	ld	s0,128(sp)
    80005302:	6149                	addi	sp,sp,144
    80005304:	8082                	ret
    end_op();
    80005306:	ba5fe0ef          	jal	80003eaa <end_op>
    return -1;
    8000530a:	557d                	li	a0,-1
    8000530c:	bfcd                	j	800052fe <sys_mkdir+0x38>

000000008000530e <sys_mknod>:

uint64
sys_mknod(void)
{
    8000530e:	7135                	addi	sp,sp,-160
    80005310:	ed06                	sd	ra,152(sp)
    80005312:	e922                	sd	s0,144(sp)
    80005314:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    80005316:	b2bfe0ef          	jal	80003e40 <begin_op>
  argint(1, &major);
    8000531a:	f6c40593          	addi	a1,s0,-148
    8000531e:	4505                	li	a0,1
    80005320:	cbcfd0ef          	jal	800027dc <argint>
  argint(2, &minor);
    80005324:	f6840593          	addi	a1,s0,-152
    80005328:	4509                	li	a0,2
    8000532a:	cb2fd0ef          	jal	800027dc <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    8000532e:	08000613          	li	a2,128
    80005332:	f7040593          	addi	a1,s0,-144
    80005336:	4501                	li	a0,0
    80005338:	cdcfd0ef          	jal	80002814 <argstr>
    8000533c:	02054563          	bltz	a0,80005366 <sys_mknod+0x58>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    80005340:	f6841683          	lh	a3,-152(s0)
    80005344:	f6c41603          	lh	a2,-148(s0)
    80005348:	458d                	li	a1,3
    8000534a:	f7040513          	addi	a0,s0,-144
    8000534e:	90fff0ef          	jal	80004c5c <create>
  if((argstr(0, path, MAXPATH)) < 0 ||
    80005352:	c911                	beqz	a0,80005366 <sys_mknod+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
    80005354:	b0cfe0ef          	jal	80003660 <iunlockput>
  end_op();
    80005358:	b53fe0ef          	jal	80003eaa <end_op>
  return 0;
    8000535c:	4501                	li	a0,0
}
    8000535e:	60ea                	ld	ra,152(sp)
    80005360:	644a                	ld	s0,144(sp)
    80005362:	610d                	addi	sp,sp,160
    80005364:	8082                	ret
    end_op();
    80005366:	b45fe0ef          	jal	80003eaa <end_op>
    return -1;
    8000536a:	557d                	li	a0,-1
    8000536c:	bfcd                	j	8000535e <sys_mknod+0x50>

000000008000536e <sys_chdir>:

uint64
sys_chdir(void)
{
    8000536e:	7135                	addi	sp,sp,-160
    80005370:	ed06                	sd	ra,152(sp)
    80005372:	e922                	sd	s0,144(sp)
    80005374:	e14a                	sd	s2,128(sp)
    80005376:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005378:	d56fc0ef          	jal	800018ce <myproc>
    8000537c:	892a                	mv	s2,a0
  
  begin_op();
    8000537e:	ac3fe0ef          	jal	80003e40 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    80005382:	08000613          	li	a2,128
    80005386:	f6040593          	addi	a1,s0,-160
    8000538a:	4501                	li	a0,0
    8000538c:	c88fd0ef          	jal	80002814 <argstr>
    80005390:	04054363          	bltz	a0,800053d6 <sys_chdir+0x68>
    80005394:	e526                	sd	s1,136(sp)
    80005396:	f6040513          	addi	a0,s0,-160
    8000539a:	8d3fe0ef          	jal	80003c6c <namei>
    8000539e:	84aa                	mv	s1,a0
    800053a0:	c915                	beqz	a0,800053d4 <sys_chdir+0x66>
    end_op();
    return -1;
  }
  ilock(ip);
    800053a2:	8b4fe0ef          	jal	80003456 <ilock>
  if(ip->type != T_DIR){
    800053a6:	04449703          	lh	a4,68(s1)
    800053aa:	4785                	li	a5,1
    800053ac:	02f71963          	bne	a4,a5,800053de <sys_chdir+0x70>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    ...
    800053b2:	952fe0ef          	jal	80003504 <iunlock>
  iput(p->cwd);
    800053b6:	15093503          	ld	a0,336(s2)
    800053ba:	a1efe0ef          	jal	800035d8 <iput>
  end_op();
    800053be:	aedfe0ef          	jal	80003eaa <end_op>
  p->cwd = ip;
    800053c2:	14993823          	sd	s1,336(s2)
  return 0;
    800053c6:	4501                	li	a0,0
    800053c8:	64aa                	ld	s1,136(sp)
}
    800053ca:	60ea                	ld	ra,152(sp)
    800053cc:	644a                	ld	s0,144(sp)
    800053ce:	690a                	ld	s2,128(sp)
    800053d0:	610d                	addi	sp,sp,160
    800053d2:	8082                	ret
    800053d4:	64aa                	ld	s1,136(sp)
    end_op();
    800053d6:	ad5fe0ef          	jal	80003eaa <end_op>
    return -1;
    800053da:	557d                	li	a0,-1
    800053dc:	b7fd                	j	800053ca <sys_chdir+0x5c>
    iunlockput(ip);
    800053de:	8526                	mv	a0,s1
    800053e0:	a80fe0ef          	jal	80003660 <iunlockput>
    end_op();
    800053e4:	ac7fe0ef          	jal	80003eaa <end_op>
    return -1;
    800053e8:	557d                	li	a0,-1
    800053ea:	64aa                	ld	s1,136(sp)
    800053ec:	bff9                	j	800053ca <sys_chdir+0x5c>
...
sys_exec(void)
{
    800053ee:	7121                	addi	sp,sp,-448
    800053f0:	ff06                	sd	ra,440(sp)
    800053f2:	fb22                	sd	s0,432(sp)
    800053f4:	0380                	addi	s0,sp,448
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  argaddr(1, &uargv);
    800053f6:	e4840593          	addi	a1,s0,-440
    800053fa:	4505                	li	a0,1
    800053fc:	bfcfd0ef          	jal	800027f8 <argaddr>
  if(argstr(0, path, MAXPATH) < 0) {
    80005400:	08000613          	li	a2,128
    80005404:	f5040593          	addi	a1,s0,-176
    80005408:	4501                	li	a0,0
    8000540a:	c0afd0ef          	jal	80002814 <argstr>
    8000540e:	87aa                	mv	a5,a0
    return -1;
    80005410:	557d                	li	a0,-1
  if(argstr(0, path, MAXPATH) < 0) {
    80005412:	0c07c463          	bltz	a5,800054da <sys_exec+0xec>
    80005416:	f726                	sd	s1,424(sp)
    80005418:	f34a                	sd	s2,416(sp)
    8000541a:	ef4e                	sd	s3,408(sp)
    8000541c:	eb52                	sd	s4,400(sp)
  }
  memset(argv, 0, sizeof(argv));
    8000541e:	10000613          	li	a2,256
    80005422:	4581                	li	a1,0
    80005424:	e5040513          	addi	a0,s0,-432
    80005428:	87bfb0ef          	jal	80000ca2 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    8000542c:	e5040493          	addi	s1,s0,-432
  memset(argv, 0, sizeof(argv));
    80005430:	89a6                	mv	s3,s1
    80005432:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    80005434:	02000a13          	li	s4,32
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    80005438:	00391513          	slli	a0,s2,0x3
    8000543c:	e4040593          	addi	a1,s0,-448
    80005440:	e4843783          	ld	a5,-440(s0)
    80005444:	953e                	add	a0,a0,a5
    80005446:	b0cfd0ef          	jal	80002752 <fetchaddr>
    8000544a:	02054663          	bltz	a0,80005476 <sys_exec+0x88>
      goto bad;
    }
    if(uarg == 0){
    8000544e:	e4043783          	ld	a5,-448(s0)
    80005452:	c3a9                	beqz	a5,80005494 <sys_exec+0xa6>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    80005454:	eaafb0ef          	jal	80000afe <kalloc>
    80005458:	85aa                	mv	a1,a0
    8000545a:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    8000545e:	cd01                	beqz	a0,80005476 <sys_exec+0x88>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    80005460:	6605                	lui	a2,0x1
    80005462:	e4043503          	ld	a0,-448(s0)
    80005466:	b36fd0ef          	jal	8000279c <fetchstr>
    8000546a:	00054663          	bltz	a0,80005476 <sys_exec+0x88>
    if(i >= NELEM(argv)){
    8000546e:	0905                	addi	s2,s2,1
    80005470:	09a1                	addi	s3,s3,8
    80005472:	fd4913e3          	bne	s2,s4,80005438 <sys_exec+0x4a>
    kfree(argv[i]);
 ...
 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005476:	f5040913          	addi	s2,s0,-176
    8000547a:	6088                	ld	a0,0(s1)
    8000547c:	c931                	beqz	a0,800054d0 <sys_exec+0xe2>
    kfree(argv[i]);
    8000547e:	d9efb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005482:	04a1                	addi	s1,s1,8
    80005484:	ff249be3          	bne	s1,s2,8000547a <sys_exec+0x8c>
  return -1;
    80005488:	557d                	li	a0,-1
    8000548a:	74ba                	ld	s1,424(sp)
    8000548c:	791a                	ld	s2,416(sp)
    8000548e:	69fa                	ld	s3,408(sp)
    80005490:	6a5a                	ld	s4,400(sp)
    80005492:	a0a1                	j	800054da <sys_exec+0xec>
      argv[i] = 0;
    80005494:	0009079b          	sext.w	a5,s2
    80005498:	078e                	slli	a5,a5,0x3
    8000549a:	fd078793          	addi	a5,a5,-48
    8000549e:	97a2                	add	a5,a5,s0
    800054a0:	e807b023          	sd	zero,-384(a5)
  int ret = kexec(path, argv);
    800054a4:	e5040593          	addi	a1,s0,-432
    800054a8:	f5040513          	addi	a0,s0,-176
    800054ac:	ba8ff0ef          	jal	80004854 <kexec>
    800054b0:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800054b2:	f5040993          	addi	s3,s0,-176
    800054b6:	6088                	ld	a0,0(s1)
    800054b8:	c511                	beqz	a0,800054c4 <sys_exec+0xd6>
    kfree(argv[i]);
    800054ba:	d62fb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    800054be:	04a1                	addi	s1,s1,8
    800054c0:	ff349be3          	bne	s1,s3,800054b6 <sys_exec+0xc8>
  return ret;
    800054c4:	854a                	mv	a0,s2
    800054c6:	74ba                	ld	s1,424(sp)
    800054c8:	791a                	ld	s2,416(sp)
    800054ca:	69fa                	ld	s3,408(sp)
    800054cc:	6a5a                	ld	s4,400(sp)
    800054ce:	a031                	j	800054da <sys_exec+0xec>
  return -1;
    800054d0:	557d                	li	a0,-1
    800054d2:	74ba                	ld	s1,424(sp)
    800054d4:	791a                	ld	s2,416(sp)
    800054d6:	69fa                	ld	s3,408(sp)
    800054d8:	6a5a                	ld	s4,400(sp)
}
    800054da:	70fa                	ld	ra,440(sp)
    800054dc:	745a                	ld	s0,432(sp)
    800054de:	6139                	addi	sp,sp,448
    800054e0:	8082                	ret

00000000800054e2 <sys_pipe>:
...
sys_pipe(void)
{
    ...
    800054ea:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    800054ec:	be2fc0ef          	jal	800018ce <myproc>
    800054f0:	84aa                	mv	s1,a0

  argaddr(0, &fdarray);
    800054f2:	fd840593          	addi	a1,s0,-40
    800054f6:	4501                	li	a0,0
    800054f8:	b00fd0ef          	jal	800027f8 <argaddr>
  if(pipealloc(&rf, &wf) < 0)
    800054fc:	fc840593          	addi	a1,s0,-56
    80005500:	fd040513          	addi	a0,s0,-48
    80005504:	852ff0ef          	jal	80004556 <pipealloc>
    return -1;
    80005508:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    8000550a:	0a054463          	bltz	a0,800055b2 <sys_pipe+0xd0>
  fd0 = -1;
    8000550e:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    80005512:	fd043503          	ld	a0,-48(s0)
    80005516:	f08ff0ef          	jal	80004c1e <fdalloc>
    8000551a:	fca42223          	sw	a0,-60(s0)
    8000551e:	08054163          	bltz	a0,800055a0 <sys_pipe+0xbe>
    80005522:	fc843503          	ld	a0,-56(s0)
    80005526:	ef8ff0ef          	jal	80004c1e <fdalloc>
    8000552a:	fca42023          	sw	a0,-64(s0)
    8000552e:	06054063          	bltz	a0,8000558e <sys_pipe+0xac>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  ...
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    ...
    80005534:	fc440613          	addi	a2,s0,-60
    80005538:	fd843583          	ld	a1,-40(s0)
    8000553c:	68a8                	ld	a0,80(s1)
    8000553e:	8a4fc0ef          	jal	800015e2 <copyout>
    80005542:	00054e63          	bltz	a0,8000555e <sys_pipe+0x7c>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    80005546:	4691                	li	a3,4
    80005548:	fc040613          	addi	a2,s0,-64
    8000554c:	fd843583          	ld	a1,-40(s0)
    80005550:	0591                	addi	a1,a1,4
    80005552:	68a8                	ld	a0,80(s1)
    80005554:	88efc0ef          	jal	800015e2 <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  ...
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    8000555a:	04055c63          	bgez	a0,800055b2 <sys_pipe+0xd0>
    p->ofile[fd0] = 0;
    8000555e:	fc442783          	lw	a5,-60(s0)
    80005562:	07e9                	addi	a5,a5,26
    80005564:	078e                	slli	a5,a5,0x3
    80005566:	97a6                	add	a5,a5,s1
    80005568:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    8000556c:	fc042783          	lw	a5,-64(s0)
    80005570:	07e9                	addi	a5,a5,26
    80005572:	078e                	slli	a5,a5,0x3
    80005574:	94be                	add	s1,s1,a5
    80005576:	0004b023          	sd	zero,0(s1)
    fileclose(rf);
    8000557a:	fd043503          	ld	a0,-48(s0)
    8000557e:	ccffe0ef          	jal	8000424c <fileclose>
    fileclose(wf);
    80005582:	fc843503          	ld	a0,-56(s0)
    80005586:	cc7fe0ef          	jal	8000424c <fileclose>
    return -1;
    8000558a:	57fd                	li	a5,-1
    8000558c:	a01d                	j	800055b2 <sys_pipe+0xd0>
    if(fd0 >= 0)
    8000558e:	fc442783          	lw	a5,-60(s0)
    80005592:	0007c763          	bltz	a5,800055a0 <sys_pipe+0xbe>
      p->ofile[fd0] = 0;
    80005596:	07e9                	addi	a5,a5,26
    80005598:	078e                	slli	a5,a5,0x3
    8000559a:	97a6                	add	a5,a5,s1
    8000559c:	0007b023          	sd	zero,0(a5)
    fileclose(rf);
    800055a0:	fd043503          	ld	a0,-48(s0)
    800055a4:	ca9fe0ef          	jal	8000424c <fileclose>
    fileclose(wf);
    800055a8:	fc843503          	ld	a0,-56(s0)
    800055ac:	ca1fe0ef          	jal	8000424c <fileclose>
    return -1;
    800055b0:	57fd                	li	a5,-1
}
    800055b2:	853e                	mv	a0,a5
    800055b4:	70e2                	ld	ra,56(sp)
    800055b6:	7442                	ld	s0,48(sp)
    800055b8:	74a2                	ld	s1,40(sp)
    800055ba:	6121                	addi	sp,sp,64
    800055bc:	8082                	ret
	...
...
kernelvec:
    ...
    800055e4:	f9fe                	sd	t6,240(sp)
        ...
        call kerneltrap
    800055e6:	87cfd0ef          	jal	80002662 <kerneltrap>
...
plicinithart(void)
{
  ...
  int hart = cpuid();
    80005640:	a62fc0ef          	jal	800018a2 <cpuid>
...
plic_claim(void)
{
  ...
  int hart = cpuid();
    80005674:	a2efc0ef          	jal	800018a2 <cpuid>
...
plic_complete(int irq)
{
  ...
  int hart = cpuid();
    80005698:	a0afc0ef          	jal	800018a2 <cpuid>
...
free_desc(int i)
{
  ...
  if(disk.free[i])
    ...
    800056c4:	49878793          	addi	a5,a5,1176 # 80020b58 <disk>
  ...
  disk.desc[i].addr = 0;
    ...
    800056d8:	48478793          	addi	a5,a5,1156 # 80020b58 <disk>
  ...
  wakeup(&disk.free[0]);
    ...
    80005700:	47450513          	addi	a0,a0,1140 # 80020b70 <disk+0x18>
    80005704:	821fc0ef          	jal	80001f24 <wakeup>
}
    ...
    80005714:	f0050513          	addi	a0,a0,-256 # 80007610 <etext+0x610>
    80005718:	8c8fb0ef          	jal	800007e0 <panic>
    panic("free_desc 2");
    8000571c:	00002517          	auipc	a0,0x2
    80005720:	f0450513          	addi	a0,a0,-252 # 80007620 <etext+0x620>
...
0000000080005728 <virtio_disk_init>:
{
  ...
  initlock(&disk.vdisk_lock, "virtio_disk");
    ...
    80005738:	efc58593          	addi	a1,a1,-260 # 80007630 <etext+0x630>
    8000573c:	0001b517          	auipc	a0,0x1b
    80005740:	54450513          	addi	a0,a0,1348 # 80020c80 <disk+0x128>
  ...
  *R(VIRTIO_MMIO_STATUS) = status;
  ...
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    ...
    800057a8:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fddac7>
  ...
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
  ...
  disk.desc = kalloc();
    ...
    800057fe:	35e48493          	addi	s1,s1,862 # 80020b58 <disk>
  ...
  if(!disk.desc || !disk.avail || !disk.used)
    ...
    8000581c:	34873703          	ld	a4,840(a4) # 80020b60 <disk+0x8>
  ...
  memset(disk.avail, 0, PGSIZE);
    ...
    80005834:	32848493          	addi	s1,s1,808 # 80020b58 <disk>
...
virtio_disk_rw(struct buf *b, int write)
{
  ...
  acquire(&disk.vdisk_lock);
    ...
    80005950:	33450513          	addi	a0,a0,820 # 80020c80 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    80005960:	1fcb0b13          	addi	s6,s6,508 # 80020b58 <disk>
  for(int i = 0; i < 3; i++){
  ...
  while(1){
    ...
    8000596a:	31ac0c13          	addi	s8,s8,794 # 80020c80 <disk+0x128>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    8000598c:	1d070713          	addi	a4,a4,464 # 80020b58 <disk>
  ...
  for(int i = 0; i < NUM; i++){
    ...
    800059c4:	1b050513          	addi	a0,a0,432 # 80020b70 <disk+0x18>
    800059c8:	d10fc0ef          	jal	80001ed8 <sleep>
  for(int i = 0; i < 3; i++){
  ...
  if(write)
    ...
    800059e0:	17c78793          	addi	a5,a5,380 # 80020b58 <disk>
  ...
  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
  ...
  while(b->disk == 1) {
    ...
    80005ab6:	1ce90913          	addi	s2,s2,462 # 80020c80 <disk+0x128>
  while(b->disk == 1) {
    ...
    80005ac4:	c14fc0ef          	jal	80001ed8 <sleep>
  while(b->disk == 1) {
  ...
  disk.info[idx[0]].b = 0;
    ...
    80005ade:	07e78793          	addi	a5,a5,126 # 80020b58 <disk>
    80005ae2:	97ba                	add	a5,a5,a4
    80005ae4:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80005ae8:	0001b997          	auipc	s3,0x1b
    80005aec:	07098993          	addi	s3,s3,112 # 80020b58 <disk>
  ...
  release(&disk.vdisk_lock);
    ...
    80005b10:	17450513          	addi	a0,a0,372 # 80020c80 <disk+0x128>
...
virtio_disk_intr()
{
  ...
  acquire(&disk.vdisk_lock);
    ...
    80005b40:	01c48493          	addi	s1,s1,28 # 80020b58 <disk>
    80005b44:	0001b517          	auipc	a0,0x1b
    80005b48:	13c50513          	addi	a0,a0,316 # 80020c80 <disk+0x128>
  ...
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
  ...
  while(disk.used_idx != disk.used->idx){
    ...
    wakeup(b);
    80005b9c:	b88fc0ef          	jal	80001f24 <wakeup>
  ...
  release(&disk.vdisk_lock);
    ...
    80005bbc:	0c850513          	addi	a0,a0,200 # 80020c80 <disk+0x128>
    ...
    80005bd2:	b2a50513          	addi	a0,a0,-1238 # 800076f8 <etext+0x6f8>
